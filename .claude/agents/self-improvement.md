...
description: Self-Improvement Agent - Skill mining from recurring tasks and boilerplate improvement proposals
...
This agent NEVER schedules itself, never polls for epic completion, and has no standalone epic-close mechanism — **epic-completion detection belongs to the PO-Agent** (`.claude/agents/po-agent.md`). It runs only when invoked by:
...
Before proposing, check `.claude/skills/` and `.claude/commands/` — if an existing skill already covers the task, propose extending it (named file, named section) instead of a new one.
...
- **Belongs at**: `.claude/skills/<skill-name>/SKILL.md`
- **Draft SKILL.md**:
