...
If AITBC number provided ($1):

- Fetch ticket: `mcp__linear-mcp__get_issue $1`
- Verify it's a bug fix
- Create branch: `git checkout -b AITBC-$1-fix-{description}`

If no argument:

- Ask for AITBC number
- Proceed with setup
...
```bash
git add .
git commit -m "fix(scope): resolve {issue} [AITBC-XXX]"
```
