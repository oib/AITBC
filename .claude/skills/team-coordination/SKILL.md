...
description: Agent Teams orchestration patterns for multi-agent SAFe workflows. Use when spawning agent teams, coordinating teammates, enforcing quality gates via task dependencies, or orchestrating the 17-agent SAFe pipeline. Covers TeamCreate, SendMessage, shared TaskList, and SAFe gate enforcement.
...
The TDM (Technical Delivery Manager) is the natural team lead in the 17-agent model:

```
Create an agent team for AITBC-XXX implementation.
...
- Task: Implement endpoints per spec at specs/AITBC-XXX-spec.md
