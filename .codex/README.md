...
### Agents: .codex/agents/ (11 core-delivery roles — intentional subset)

`.codex/agents/` carries **11 of the harness's 17 roles by design** (ADR-A-0015). Codex CLI is
provisioned for the **core delivery** seats only:

`be-developer, bsa, data-engineer, data-provisioning-eng, fe-developer, qas, rte,
security-engineer, system-architect, tdm, tech-writer`.

The six harness roles deliberately **not** mirrored here are harness-maintenance, design, and
orchestration seats that are not driven from a Codex CLI session:

`boilerplate-migration, issue-enrichment, po-agent, qas-design, self-improvement, ui-ux-design`.

This subset is not drift — it is a recorded decision. `scripts/pre-release-check.sh` enforces
roster-parity against the list above, so adding or removing a Codex agent requires updating this
README and ADR-A-0015 together. To onboard one of the six omitted roles, adapt its prompt from
`harness/claude/agents/<role>.md` into a `.toml` here and extend the roster in both places.
...
└── agents/             # SAFe agent role definitions (TOML)
...
.agents/
└── skills/             # 21 shared skills (discovered by Codex and other agents)
    ├── api-patterns/
    │   └── SKILL.md
    ├── safe-workflow/
    │   └── SKILL.md
...
Since Codex uses natural language instead of slash commands, describe the SAFe workflow step you want to execute conversationally (e.g., "Start work on AITBC-42" instead of `/start-work AITBC-42`).
