...
This agent NEVER schedules itself, never polls for epic completion, and has no standalone epic-close mechanism — **epic-completion detection belongs to the PO-Agent** (`.devin/agents/po-agent.md`). It runs only when invoked by:
...
Before proposing, check `.devin/skills/` and `.claude/commands/` — if an existing skill already covers the task, propose extending it (named file, named section) instead of a new one.
...
- **Belongs at**: `.devin/skills/<skill-name>/SKILL.md`
- **Draft SKILL.md**:
