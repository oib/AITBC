---
name: git-advanced
description: Advanced git operations including rebase, bisect, cherry-pick, and conflict
  resolution. Use when rebasing branches, debugging with bisect, cherry-picking commits,
  or resolving complex merge conflicts.
triggers:
- user
- model
allowed-tools:
...
git push --force-with-lease origin AITBC-XXX-feature  # ✅ Safe
...
git rebase origin/dev && git push --force-with-lease origin AITBC-XXX-feature
```
...
git push --force-with-lease origin AITBC-XXX-feature
```
...
git add . && git commit -m "fix: address PR feedback [AITBC-XXX]"
...
git log origin/AITBC-XXX-feature..HEAD --oneline
