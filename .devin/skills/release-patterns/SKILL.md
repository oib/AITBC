...
description: PR creation, CI/CD validation, and release coordination patterns. Use
  when creating pull requests, running pre-PR validation, checking CI status, or coordinating
  merges.
triggers:
- user
- model
...
gh pr create --title "feat: add feature"  # Missing [AITBC-XXX]
...
gh pr create --title "feat(scope): description [AITBC-XXX]"
...
git push --force-with-lease origin AITBC-XXX-description
```
...
````bash
gh pr create --title "feat(scope): description [AITBC-XXX]" --body "$(cat <<'EOF'
## Summary

Implements [feature/fix] as specified in Linear ticket AITBC-XXX.

**Linear Ticket**: https://linear.app/{{LINEAR_WORKSPACE}}/issue/AITBC-XXX
...
Prompt: "Review PR #XXX for AITBC-YYY. Validate commit format, CI status, patterns."
```
