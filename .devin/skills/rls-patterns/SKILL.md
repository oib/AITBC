---
name: rls-patterns
description: Row Level Security patterns for database operations. Use when writing
  Prisma/database code, creating API routes that access data, or implementing webhooks.
  Enforces withUserContext, withAdminContext, or withSystemContext helpers. NEVER
  use direct prisma calls.
triggers:
- user
- model
allowed-tools:
...
cat scripts/rls-phase4-final-validation.sql | \
  docker exec -i AITBC-postgres-1 psql -U {{PROJECT}}_app_user -d {{PROJECT}}_dev
```
