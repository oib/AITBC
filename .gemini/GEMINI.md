...
You are working in a **SAFe multi-agent development environment** using the AITBC (Words to Film By) Agentic Workflow methodology.
...
Skills auto-load when context matches. All 21 skills:
...
| `duplicate-detection` | Before creating any ticket (dedup gate) |
| `issue-enrichment` | Ticket formatting after dedup verdict |
| `jira-sop` | Jira ticket create/search/transition |
| `team-coordination` | Spawning agent teams, multi-agent orchestration |
...
### Branch Naming
```
AITBC-{number}-{short-description}
```
Example: `AITBC-123-add-user-profile`
...
Example: `feat(user): add profile editing [AITBC-123]`
...
> **Tip**: If your team uses GitHub-Linear auto-sync, tickets referenced in commit messages (e.g., `[AITBC-123]`) auto-move to Done when the PR merges. Manually close child stories not referenced in commits.
