...
description: Mandatory dedup gate before any ticket creation. Use BEFORE creating any ticket in the tracker. Searches the tracker full-text for identical/similar tickets and returns a reject/append/create verdict with matched references and reasoning.
...
scripts/mock-tracker.sh comment DEMO-30 --kind decision --actor issue-enrichment \
  --body "Dedup gate: appended scope — add 'resend verification email' button (AC added)."
```
