...
**Owner**: Issue Enrichment Agent (`.claude/agents/issue-enrichment.md`) — this
...
by writing it to a temp file and passing it via `--body-file` — without that
flag the adapter seeds the `_TBD_` template and the enrichment output is dropped.
Never write `work/tickets/*.md` directly; ticket bodies are persisted only
through the adapter (adapter-only boundary, ADR-A-0007):
...
- **Owning agent**: `.claude/agents/issue-enrichment.md` (ABS-8)
- **Upstream gate**: `.claude/skills/duplicate-detection/SKILL.md` (runs FIRST, always)
