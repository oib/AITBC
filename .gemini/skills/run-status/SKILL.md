---
name: run-status
description: Answer the operator's recurring "Status?" question in one compact reply. Use when the operator (or the Ops-Sweep seat before it acts) asks "status?", "where are we?", "what's running / stuck / waiting on me?", or wants a run situation report. Runs as an isolated fork — only the short answer returns, the raw dumps stay in the fork.
context: fork
agent: Explore
