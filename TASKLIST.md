...
- [x] **FK violations resolved 2026-08-24.** `PRAGMA foreign_key_check` now passes on both the hub `coordinator.db` and `chain.db`. No data semantics were changed.
  ...
  - Detail: `LIVE_VALIDATION_SUMMARY.md`, section on the 2026-08-23 coordinator migration recovery.
- [x] **aitbc3 `coordinator.db` converged to hub 2026-08-23.** 197 tables down to 166; the only remaining difference is the eight multi-tenancy tables, which is a product decision rather than drift. Both databases are at head `d38eb9f3a80b` and both live coordinators resolve `settings.database.effective_url` to `sqlite:////var/lib/aitbc/data/coordinator.db`, so this was live schema, not a stale artifact.
  ...
  - **Still open:** the eight multi-tenancy tables (`tenants`, `tenant_users`, `tenant_api_keys`, `tenant_quotas`, `tenant_metrics`, `tenant_audit_logs`, `usage_records`, `invoices`) exist on aitbc3 and not on hub. They are current coordinator models in `models/multitenant.py`, consumed only by `QuotaEnforcementService`, which no router or middleware wires up. Harmless today; breaks the day quota enforcement is enabled on hub. Either create them on hub or delete `multitenant.py` and `quota_enforcement.py`.
