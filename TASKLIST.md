...
- [ ] **hub has 39 foreign-key violations on the reputation tables, and aitbc3 cannot be checked at all.** Pre-existing, unchanged by the 2026-08-23 migrations (identical count before and after).
  ...
  - aitbc3 `coordinator.db`: `PRAGMA foreign_key_check` cannot run at all — it aborts with `foreign key mismatch - "escrow" referencing "account"`, i.e. a declared FK whose target columns do not exist. That is a schema defect, not orphaned data, and it blanks out FK checking for the whole database, so aitbc3's real violation count is unknown.
  - **Decision needed:** two separate calls. (1) Fix or drop the `escrow` → `account` constraint on aitbc3 so the database can be checked; that has to come first. (2) Decide whether the 39 hub orphans get deleted, back-filled, or accepted — and whether reputation reads currently return wrong numbers because of them. Nobody has looked at what the orphaned rows actually mean yet.
  - Detail: `LIVE_VALIDATION_SUMMARY.md`, section on the 2026-08-23 coordinator migration recovery.
