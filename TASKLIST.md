...
- [ ] **hub has 39 foreign-key violations on the reputation tables, and aitbc3 cannot be checked at all.** Pre-existing, unchanged by the 2026-08-23 migrations (identical count before and after).
  ...
  - ~~The `escrow` → `account` constraint blocks `PRAGMA foreign_key_check` entirely~~ — fixed in the chain database by `fc7a0ee64` / migration `c9a4f1e2b73d`. `escrow` referenced `account.address` while account's key is `(chain_id, address)`; escrow now carries a `chain_id` and the references are composite. **aitbc3's chain.db is migrated and reports zero violations.** Two pieces are still open:
    - **hub's chain.db is not migrated.** Same defect, 75 escrow rows, and the fix is already on `main` — a `git pull` plus a restart there would put the new model in front of a table with no `chain_id` column. Migrating it surfaces 4 real violations: four escrows whose buyer `ait135daba990a37177398e0e0c1670baa316a032417` has no account row (verified on a snapshot).
    - **aitbc3's `coordinator.db` still cannot be checked.** Eight chain tables (`escrow`, `account`, `stake`, `mempool`, `consensus_state`, `block`, `receipt`, `transaction`) leaked into it before V23-74 gave the chain its own metadata, when a single `create_all` created both schemas. All eight are empty, no coordinator model declares them, and hub — at the same head `a3e7c15b8d94` — has none of them. Dropping them needs a permission the agent session did not have; backup taken at `coordinator.db.bak-2026-08-23-pre-chain-leftover-drop`.
  - **Decision needed:** whether the 39 hub orphans get deleted, back-filled, or accepted — and whether reputation reads currently return wrong numbers because of them. Nobody has looked at what the orphaned rows actually mean yet. Same question for the 4 escrow orphans once hub's chain.db is migrated.
  - Detail: `LIVE_VALIDATION_SUMMARY.md`, section on the 2026-08-23 coordinator migration recovery.
