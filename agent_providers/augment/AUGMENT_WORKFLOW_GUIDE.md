# Augment Starter Kit for AITBC SAFe-Agentic-Workflow

## Overview

This directory contains the starter kit for using the AITBC SAFe-Agentic-Workflow with Augment. Unlike the Claude Code "golden path" which is fully automated, this path requires some manual setup and adaptation.
...
> **Provider-parity note (ABS-206, decision: keep direct read).** Claude Code's
> agent defs redirect pattern discovery to the `pattern-discovery` skill — an
> isolated Explore fork invoked via the Skill tool (ABS-168). Augment has no
> Skill-tool/fork equivalent, so the direct `find`/`cat patterns_library/`
> commands above are the correct provider-native instruction and are
> **deliberately not** mirrored to the skill redirect. This divergence is
> sanctioned by ADR-A-0015 (Augment is a hand-adapted provider surface, not a
> generated/guarded mirror).
