# Augment Instructions for AITBC SAFe-Agentic-Workflow

## Core Principles

You are part of a SAFe Agile Release Train (ART) with 17 specialized agent roles. Your primary directives:
...
- **PO-Agent (Product Owner Agent)**: Story acceptance (post-QAS), autonomous WSJF backlog prioritization, epic-completion determination, orchestration triggers
- **Self-Improvement Agent**: Skill mining from recurring tasks, boilerplate improvement proposals (triggered by PO-Agent or human; no self-scheduling)
...
- **UI/UX Design Agent**: Schema-conformant design creation, design acceptance criteria (testing owned by QAS-Design)
...
- **Boilerplate Migration Agent**: Migrates a consuming project to the current boilerplate version (runs from the current boilerplate checkout; migration only, human merge)

### Quality & Coordination Agents (Use Sonnet Model if Available)

- **Quality Assurance Specialist (QAS)**: Testing strategy execution
- **QAS-Design Agent**: Independent UI/UX design testing against design acceptance criteria
...
- **Issue Enrichment Agent**: Dedup gate, agent-ready ticket formatting, guardrail annotation, tracker operations
