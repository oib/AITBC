...
class QueryMonitor:
    ...
    def record_query(self, query: str, execution_time_ms: float, success: bool=True, error_message: str | None=None, row_count: int=0) -> None:
        ...
        """
        query_metrics = QueryMetrics(query=query, execution_time_ms=execution_time_ms, timestamp=datetime.now(), success=success, error_message=error_message, row_count=row_count)
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        """
        return {'total_queries': self.metrics.total_queries, 'total_errors': self.metrics.total_errors, 'avg_execution_time_ms': self.metrics.avg_execution_time_ms, 'slow_query_count': len(self.metrics.slow_queries), 'error_rate': self.metrics.total_errors / self.metrics.total_queries if self.metrics.total_queries > 0 else 0, 'unique_queries': len(self.query_counts)}
