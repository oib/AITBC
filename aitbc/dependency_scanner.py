...
class DependencyScanner:
    ...
    def scan_with_bandit(self, target_dir: Path | None=None) -> list[dict[str, Any]]:
        ...
        try:
            result = subprocess.run(['bandit', '-r', str(target_dir), '-f', 'json'], capture_output=True, text=True, timeout=300)
            try:
    ...
    def _parse_pip_audit_output(self, audit_data: dict[str, Any]) -> list[VulnerabilityReport]:
        ...
        for dep in audit_data.get('dependencies', []):
            for vuln in dep.get('vulnerabilities', []):
                report = VulnerabilityReport(package=dep.get('name', ''), version=dep.get('version', ''), vulnerability_id=vuln.get('id', ''), severity=vuln.get('severity', 'UNKNOWN'), description=vuln.get('description', ''), fix_available=vuln.get('fix_versions', []) != [], fixed_version=vuln.get('fix_versions', [None])[0] if vuln.get('fix_versions') else None)
    ...
    def generate_report(self) -> dict[str, Any]:
        ...
        return {'timestamp': datetime.now().isoformat(), 'dependency_vulnerabilities': len(pip_audit_results), 'security_issues': len(bandit_results), 'severity_breakdown': severity_counts, 'vulnerabilities': [{'package': v.package, 'version': v.version, 'id': v.vulnerability_id, 'severity': v.severity, 'description': v.description, 'fix_available': v.fix_available, 'fixed_version': v.fixed_version} for v in pip_audit_results], 'bandit_issues': bandit_results}
...
def check_vulnerability_thresholds(report: dict[str, Any], max_critical: int=0, max_high: int=0, max_medium: int=10, max_low: int=50) -> bool:
    """
