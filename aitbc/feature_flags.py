...
class FeatureFlagManager:
    ...
    def _load_flags(self) -> None:
        ...
        try:
            ...
            for name, config in data.items():
                self._flags[name] = FeatureFlag(name=name, enabled=config.get('enabled', False), description=config.get('description', ''), rollout_percentage=config.get('rollout_percentage', 100.0), whitelisted_users=set(config.get('whitelisted_users', [])), blacklisted_users=set(config.get('blacklisted_users', [])), enabled_since=datetime.fromisoformat(config['enabled_since']) if config.get('enabled_since') else None)
    ...
    def enable_feature(self, feature_name: str, rollout_percentage: float=100.0) -> None:
        ...
        if feature_name not in self._flags:
            self._flags[feature_name] = FeatureFlag(name=feature_name, enabled=True, description='', rollout_percentage=rollout_percentage, enabled_since=datetime.now())
        else:
