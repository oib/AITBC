...
dataclasses so the coordinator, market, CLI and operator tooling can build
and evaluate quotes with provenance.

The reader is intentionally thin: all arithmetic, validation and quote logic
lives in ``aitbc.market.energy_pricing``. This module only fetches the
...
from aitbc.market.energy_pricing import EnergyPricingError, EnergyProfile, EnergyRate
