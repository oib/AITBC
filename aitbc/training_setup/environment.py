...
class TrainingEnvironment:
    ...
    def __init__(self, aitbc_dir: str='/opt/aitbc', log_dir: str='/var/log/aitbc/training-setup', faucet_amount: int=1000, genesis_allocation: int=10000, wallet_prefix: str='training-w', genesis_password_path: str='/var/lib/aitbc/keystore/.genesis_password'):
    ...
    def check_prerequisites(self) -> bool:
        ...
        try:
            result = subprocess.run([str(aitbc_cli), 'blockchain', 'info'], cwd=self.aitbc_dir, capture_output=True, text=True, timeout=10)
    ...
    def create_genesis_allocation(self) -> dict[str, Any]:
        ...
        aitbc_cli = self.aitbc_dir / 'aitbc-cli'
        try:
            result = subprocess.run([str(aitbc_cli), 'wallet', 'list'], cwd=self.aitbc_dir, capture_output=True, text=True, timeout=10)
        ...
        try:
            result = subprocess.run([str(aitbc_cli), 'wallet', 'balance', 'genesis'], cwd=self.aitbc_dir, capture_output=True, text=True, timeout=10)
            if result.returncode == 0:
    ...
    def fund_training_wallet(self, wallet_name: str, password: str='training123') -> dict[str, Any]:
        ...
        try:
            ...
            if wallet_name not in result.stdout:
                create_result = subprocess.run([str(aitbc_cli), 'wallet', 'create', wallet_name, password], cwd=self.aitbc_dir, capture_output=True, text=True, timeout=30)
        ...
        try:
            result = subprocess.run([str(aitbc_cli), 'wallet', 'send', 'genesis', wallet_name, str(self.faucet_amount), self.genesis_password], cwd=self.aitbc_dir, capture_output=True, text=True, timeout=30)
            if result.returncode == 0:
            ...
            raise
        try:
            result = subprocess.run([str(aitbc_cli), 'wallet', 'balance', wallet_name], cwd=self.aitbc_dir, capture_output=True, text=True, timeout=10)
            if result.returncode == 0:
    ...
    def configure_messaging_auth(self, wallet_name: str, password: str='training123') -> dict[str, Any]:
        ...
        aitbc_cli = self.aitbc_dir / 'aitbc-cli'
        try:
            msg_cmd = [str(aitbc_cli), 'agent', 'message', '--wallet', wallet_name, '--password', password, '--auth-token', token]
    ...
    def test_messaging_connectivity(self) -> bool:
        ...
        aitbc_cli = self.aitbc_dir / 'aitbc-cli'
        try:
            result = subprocess.run([str(aitbc_cli), 'agent', 'message', '--topic', 'test-topic', '--message', 'test-message'], cwd=self.aitbc_dir, capture_output=True, text=True, timeout=30)
            if result.returncode == 0:
