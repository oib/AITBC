...
class HermesRestPollingDaemon:
    ...
    async def poll_messages(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            self.logger.warning(f"HTTP error polling messages: {e.response.status_code}")
            return []
        except httpx.ConnectError as e:
            self.logger.warning(f"Connection error polling messages: {e}")
            return []
        except Exception as e:
            self.logger.error(f"Unexpected error polling messages: {e}")
            return []
    ...
    async def forward_to_hermes(self, message: dict[str, Any]) -> bool:
        ...
        # Normalize message format for Hermes
        payload = {
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                ...
                response.raise_for_status()
                self.logger.info(
                    f"Forwarded message {payload['id']} from {payload['sender']} to Hermes"
                )
                return True
        except httpx.HTTPStatusError as e:
            self.logger.error(
                f"Hermes returned {e.response.status_code} for message {payload['id']}"
            )
            return False
        except httpx.ConnectError as e:
            self.logger.error(f"Cannot connect to Hermes at {self.hermes_url}: {e}")
            return False
        except Exception as e:
            self.logger.error(f"Failed to forward message to Hermes: {e}")
            return False
    ...
    async def run_once(self) -> int:
        ...
        # Prune seen set if it grows too large to prevent unbounded growth
    ...
    async def run(self):
        """Main polling loop."""
        self.logger.info(f"Hermes REST polling daemon started for agent: {self.agent_id}")
        self.logger.info(f"Coordinator URL: {self.coordinator_url}")
        self.logger.info(f"Hermes URL: {self.hermes_url}")
        ...
        while self.running:
            try:
                ...
                if forwarded > 0:
                    self.logger.info(f"Forwarded {forwarded} message(s) in this cycle")
            except Exception as e:
