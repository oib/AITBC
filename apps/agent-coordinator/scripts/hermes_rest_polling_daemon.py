...
class HermesRestPollingDaemon:
    ...
    async def forward_to_hermes(self, message: dict[str, Any]) -> bool:
        ...
        payload = {'id': message.get('id', self._message_hash(message)), 'sender': message.get('sender', message.get('sender_id', 'unknown')), 'recipient': message.get('recipient', message.get('receiver_id', self.agent_id)), 'content': message.get('content', ''), 'timestamp': message.get('timestamp', ''), 'message_type': message.get('message_type', 'direct')}
        try:
            async with httpx.AsyncClient(timeout=10) as client:
...
def main():
    ...
    parser.add_argument('--coordinator-url', default=os.getenv('HERMES_COORDINATOR_URL', DEFAULT_COORDINATOR_URL), help=f'Agent Coordinator endpoint (default: {DEFAULT_COORDINATOR_URL})')
    parser.add_argument('--hermes-url', default=os.getenv('HERMES_SERVICE_URL', DEFAULT_HERMES_URL), help=f'Hermes service endpoint (default: {DEFAULT_HERMES_URL})')
    parser.add_argument('--agent-id', required=True, help='Agent ID to poll messages for')
    parser.add_argument('--poll-interval', type=float, default=float(os.getenv('HERMES_POLL_INTERVAL', str(DEFAULT_POLL_INTERVAL))), help=f'Polling interval in seconds (default: {DEFAULT_POLL_INTERVAL})')
    parser.add_argument('--log-level', default=DEFAULT_LOG_LEVEL, choices=['DEBUG', 'INFO', 'WARNING', 'ERROR'], help=f'Logging level (default: {DEFAULT_LOG_LEVEL})')
    args = parser.parse_args()
    daemon = HermesRestPollingDaemon(coordinator_url=args.coordinator_url, hermes_url=args.hermes_url, agent_id=args.agent_id, poll_interval=args.poll_interval, log_level=args.log_level)
    try:
