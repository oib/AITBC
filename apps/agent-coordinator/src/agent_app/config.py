...
__all__ = ["validated_cors_origins", "Settings", "settings", "Environment", "LogLevel", "ConfigConstants", "EnvironmentConfig"]
...
def validated_cors_origins(origins: list[str]) -> list[str]:
    """Validate CORS origins - wildcard not allowed when credentials enabled."""
    if "*" in origins:
