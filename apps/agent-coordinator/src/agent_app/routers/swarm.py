...
if settings.debug:
    ...
    async def join_swarm(http_request: Request, body: JoinRequest) -> dict[str, Any]:
    ...
    @rate_limit(rate=50, per=60)
    async def coordinate_swarm(http_request: Request, body: CoordinateRequest) -> dict[str, Any]:
    ...
    @rate_limit(rate=200, per=60)
    async def get_task_status(request: Request, task_id: str) -> dict[str, Any]:
        """Get swarm task status."""
        return {"task_id": task_id, "status": "pending", "progress": 0, "active_collaborators": 0, "total_collaborators": 0}

    @router.post("/{swarm_id}/leave", response_model=dict)
    @rate_limit(rate=50, per=60)
    async def leave_swarm(request: Request, swarm_id: str) -> dict[str, Any]:
        """Leave swarm."""
        return {"swarm_id": swarm_id, "status": "left", "message": "Successfully left swarm"}

    @router.post("/tasks/{task_id}/consensus", response_model=dict)
    @rate_limit(rate=50, per=60)
    async def achieve_consensus(request: Request, task_id: str, body: ConsensusRequest) -> dict[str, Any]:
