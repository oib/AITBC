...
class JWTHandler:
    ...
    def refresh_access_token(self, refresh_token: str) -> dict[str, Any]:
        ...
        try:
            ...
            user_payload = {'user_id': payload.get('user_id'), 'username': payload.get('username'), 'role': payload.get('role'), 'permissions': payload.get('permissions', [])}
...
class APIKeyManager:
    ...
    def generate_api_key(self, user_id: str, permissions: list[str] | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            key_data = {'user_id': user_id, 'permissions': permissions or [], 'created_at': datetime.now(UTC).isoformat(), 'last_used': None, 'usage_count': 0}
            self.api_keys[api_key] = key_data
            self._save_keys()
            return {'status': 'success', 'api_key': api_key, 'permissions': permissions or [], 'created_at': key_data['created_at']}
        except Exception as e:
