...
class EncryptedMessage:
    ...
    ciphertext: bytes
    session_key: bytes  # Encrypted session key
    nonce: bytes
    signature: bytes
    sender_id: str
...
class AgentKeyPair:
    ...
    agent_id: str
    public_key: bytes
    private_key: bytes | None = None  # Only stored for own agent
    key_id: str = ""
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        # Only include private key if present (for own agent)
        if self.private_key:
...
class MessageEncryptor:
    ...
    def encrypt_message(self, message: dict[str, Any], sender_id: str, recipient_id: str) -> EncryptedMessage | None:
        ...
        try:
            ...
            if recipient_id not in self.key_pairs:
                logger.error(f"No public key for recipient {recipient_id}")
                return None
            ...
            if sender_id not in self.key_pairs or not self.key_pairs[sender_id].private_key:
                logger.error(f"No private key for sender {sender_id}")
                return None

            sender_private_key = serialization.load_pem_private_key(
                self.key_pairs[sender_id].private_key,
                password=None,
                backend=default_backend()
            )
            ...
            logger.info(f"Encrypted message from {sender_id} to {recipient_id}")
    ...
    def decrypt_message(self, encrypted_msg: EncryptedMessage, recipient_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            if recipient_id not in self.key_pairs or not self.key_pairs[recipient_id].private_key:
                logger.error(f"No private key for recipient {recipient_id}")
                return None

            recipient_private_key = serialization.load_pem_private_key(
                self.key_pairs[recipient_id].private_key,
                password=None,
                backend=default_backend()
            )
            ...
            if encrypted_msg.sender_id in self.key_pairs:
                ...
                try:
                    ...
                    logger.info(f"Signature verified for message from {encrypted_msg.sender_id}")
                except Exception as e:
                    logger.warning(f"Signature verification failed: {e}")
                    # Continue decryption even if signature fails

            # Parse message
            message = json.loads(message_json.decode('utf-8'))

            logger.info(f"Decrypted message from {encrypted_msg.sender_id} to {recipient_id}")
            return message
    ...
    def verify_signature(self, encrypted_msg: EncryptedMessage, sender_id: str) -> bool:
        ...
        try:
            if sender_id not in self.key_pairs:
                logger.error(f"No public key for sender {sender_id}")
                return False
            ...
            logger.info(f"Signature verified for message from {sender_id}")
            return True
    ...
    def rotate_key_pair(self, agent_id: str) -> AgentKeyPair | None:
        ...
        if agent_id in self.key_pairs:
            ...
            logger.info(f"Rotated key pair for agent {agent_id}")
            return new_key
    ...
    def _load_keys(self):
        ...
        try:
            ...
            logger.info(f"Loaded {len(self.key_pairs)} key pairs")
