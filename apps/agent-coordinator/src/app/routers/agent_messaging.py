...
async def send_message(request: Request, req: SendMessageRequest) -> dict[str, Any]:
    ...
    message = {'id': message_id, 'sender': req.sender, 'recipient': req.recipient, 'content': req.content, 'message_type': req.message_type, 'timestamp': datetime.now(UTC).isoformat()}
    if req.recipient not in _mock_messages:
        _mock_messages[req.recipient] = []
    _mock_messages[req.recipient].append(message)
    logger.info('Message sent from %s to %s: %s', req.sender, req.recipient, req.content)
...
async def get_messages(request: Request, agent_id: str) -> dict[str, Any]:
    ...
    if agent_id not in _mock_messages:
        return {'agent_id': agent_id, 'count': 0, 'messages': []}
...
async def broadcast_message(request: Request, req: BroadcastRequest) -> dict[str, Any]:
    ...
    for agent_id in _mock_agents:
        ...
        _message_counter += 1
        message_id = f'msg-{_message_counter:03d}'
        message = {'id': message_id, 'sender': req.sender, 'recipient': agent_id, 'content': req.content, 'message_type': 'broadcast', 'timestamp': datetime.now(UTC).isoformat()}
        if agent_id not in _mock_messages:
            _mock_messages[agent_id] = []
        _mock_messages[agent_id].append(message)
        recipients.append(agent_id)
    logger.info('Broadcast from %s to %s agents', req.sender, len(recipients))
    return {'success': True, 'sent_count': len(recipients), 'recipients': recipients}
...
async def mark_read(request: Request, req: MarkReadRequest) -> dict[str, Any]:
    ...
    for message in _mock_messages[req.agent_id]:
        if message['id'] == req.message_id:
            ...
            return {'status': 'read', 'message_id': req.message_id}
    raise HTTPException(status_code=404, detail='Message not found')

@router.get('/agents/list', summary='List all agents')
async def list_agents(request: Request) -> dict[str, Any]:
    """List all registered agents"""
    return {'agents': list(_mock_agents.values()), 'count': len(_mock_agents)}