...
async def update_agent_status(request: Request, agent_id: str, request_status: AgentStatusUpdate) -> dict[str, Any]:
    ...
    try:
        ...
        success = await state.agent_registry.update_agent_status(agent_id, AgentStatus(request_status.status), request_status.load_metrics)
        if success:
            return {'status': 'success', 'message': f'Agent {agent_id} status updated', 'agent_id': agent_id, 'new_status': request_status.status, 'updated_at': datetime.now(UTC).isoformat()}
        else:
...
async def agent_heartbeat(request: Request, agent_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        if success:
            return {'status': 'success', 'message': f'Heartbeat received from {agent_id}', 'agent_id': agent_id, 'heartbeat_at': datetime.now(UTC).isoformat()}
        else:
            raise HTTPException(status_code=404, detail='Agent not found')
    except HTTPException:
        raise
