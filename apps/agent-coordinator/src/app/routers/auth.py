...
async def login(request: Request, login_data: dict[str, str]) -> dict[str, Any]:
    ...
    try:
        ...
        import os
        demo_users = {'admin': os.getenv('TEST_ADMIN_PASSWORD') or os.getenv('DEMO_ADMIN_PASSWORD'), 'operator': os.getenv('TEST_OPERATOR_PASSWORD') or os.getenv('DEMO_OPERATOR_PASSWORD'), 'user': os.getenv('TEST_USER_PASSWORD') or os.getenv('DEMO_USER_PASSWORD')}
        ...
        token_result = jwt_handler.generate_token({'user_id': user_id, 'username': username, 'role': role.value, 'permissions': [perm.value for perm in permission_manager.user_permissions.get(user_id, set())]})
        refresh_result = jwt_handler.generate_refresh_token({'user_id': user_id, 'username': username, 'role': role.value})
        return {'status': 'success', 'user_id': user_id, 'username': username, 'role': role.value, 'access_token': token_result['token'], 'refresh_token': refresh_result['refresh_token'], 'expires_at': token_result['expires_at'], 'token_type': token_result['token_type']}
