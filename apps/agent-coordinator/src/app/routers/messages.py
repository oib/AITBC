...
async def send_encrypted_message(request: Request, req: SendMessageRequest) -> Any:
    ...
    try:
        ...
        message_content = {'content': req.content, 'message_type': req.message_type, 'timestamp': datetime.now(UTC).isoformat()}
        if req.encrypt:
            encrypted_msg = encryptor.encrypt_message(message=message_content, sender_id=req.sender, recipient_id=req.recipient)
...
async def discover_agents(request: Request, capability: str | None=Query(None, description='Filter by capability'), agent_type: str | None=Query(None, description='Filter by agent type'), min_health_score: float=Query(0.0, description='Minimum health score'), limit: int=Query(50, description='Maximum results')) -> Any:
    ...
    try:
        ...
        return {'agents': [agent.to_dict() for agent in agents], 'count': len(agents), 'query': query, 'timestamp': datetime.now(UTC).isoformat()}
    except HTTPException:
        raise
    except Exception as e:
...
async def subscribe_to_topic(request: Request, req: SubscribeRequest) -> Any:
    ...
    try:
        if state.message_storage:
            subscription_data = {'agent_id': req.agent_id, 'topic': req.topic, 'filter': req.filter, 'subscribed_at': datetime.now(UTC).isoformat()}
...
async def broadcast_message(request_http: Request, request: BroadcastRequest) -> Any:
    ...
    try:
        ...
        agents = await state.agent_registry.discover_agents(query)
        if not agents:
            return {'status': 'success', 'message': 'No matching agents found', 'recipients': [], 'count': 0, 'broadcast_at': datetime.now(UTC).isoformat()}
        recipients = []
        for agent in agents:
            message = AgentMessage(sender_id='agent-coordinator', receiver_id=agent.agent_id, message_type=message_type, priority=priority, payload=request.payload)
            message_data = {'sender_id': message.sender_id, 'receiver_id': message.receiver_id, 'message_type': message.message_type.value, 'priority': message.priority.value, 'payload': json.dumps(message.payload), 'protocol': 'broadcast', 'timestamp': datetime.now(UTC).isoformat()}
            if state.message_storage:
            ...
            if state.communication_manager:
                try:
                    ...
                    pass
        return {'status': 'success', 'message': f'Broadcast sent to {len(recipients)} agents', 'recipients': recipients, 'count': len(recipients), 'broadcast_at': datetime.now(UTC).isoformat()}
    except HTTPException:
        raise
    except Exception as e:
...
async def add_peer(request: Request, agent_id: str=Query(..., description='Agent ID'), peer_id: str=Query(..., description='Peer agent ID')) -> Any:
    ...
    try:
        ...
        if success:
            return {'status': 'success', 'message': f'Peer {peer_id} added for agent {agent_id}', 'agent_id': agent_id, 'peer_id': peer_id, 'connected_at': datetime.now(UTC).isoformat()}
        else:
...
async def remove_peer(request: Request, agent_id: str=Query(..., description='Agent ID'), peer_id: str=Query(..., description='Peer agent ID')) -> Any:
    ...
    try:
        ...
        if success:
            return {'status': 'success', 'message': f'Peer {peer_id} removed for agent {agent_id}', 'agent_id': agent_id, 'peer_id': peer_id, 'removed_at': datetime.now(UTC).isoformat()}
        else:
...
async def get_agent_peers(request: Request, agent_id: str) -> Any:
    ...
    try:
        ...
        return {'status': 'success', 'agent_id': agent_id, 'peers': peers, 'count': len(peers), 'timestamp': datetime.now(UTC).isoformat()}
    except HTTPException:
        raise
    except Exception as e:
