...
async def submit_task(request_http: Request, request: TaskSubmission, background_tasks: BackgroundTasks) -> dict[str, Any]:
    ...
    try:
        ...
        return {'status': 'success', 'message': 'Task submitted successfully', 'task_id': request.task_data.get('task_id', str(uuid.uuid4())), 'priority': request.priority, 'submitted_at': datetime.now(UTC).isoformat()}
...
async def clear_queue(request: Request, priority: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {'status': 'success', 'message': f'Cleared {cleared_count} tasks from {priority} queue', 'priority': priority, 'cleared_count': cleared_count, 'timestamp': datetime.now(UTC).isoformat()}
    except HTTPException:
        raise
    except Exception as e:
...
async def get_queue_stats(request: Request) -> dict[str, Any]:
    ...
    try:
        ...
        return {'status': 'success', 'queue_sizes': queue_sizes, 'distribution_stats': distribution_stats, 'timestamp': datetime.now(UTC).isoformat()}
    except HTTPException:
        raise
    except Exception as e:
