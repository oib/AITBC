...
async def health_check():
    return {
        ...
        "downloads_today": len(
            [
                d
                for downloads_list in downloads.values()
...
async def list_plugins(
    category: str | None = None, tag: str | None = None, search: str | None = None, sort_by: str = "created_at"
):
    ...
    return {
        ...
        "filters": {"category": category, "tag": tag, "search": search, "sort_by": sort_by},
    }
...
async def get_plugin(plugin_id: str):
    ...
    plugin["analytics"] = {
        ...
        "average_rating": sum(plugin_analytics.get("ratings", [])) / len(plugin_analytics.get("ratings", []))
        if plugin_analytics.get("ratings")
        else 0.0,
...
async def get_plugin_security(plugin_id: str):
    ...
    if plugin_id not in plugins:
        raise HTTPException(status_code=404, detail="Plugin not found")
    plugin_scans = []
    for _scan_id, scan in security_scans.items():
...
async def process_analytics():
    ...
    while True:
        ...
        for _plugin_id, plugin_analytics in analytics.items():
            daily_key = current_date.isoformat()
            if daily_key not in plugin_analytics["daily_stats"]:
                plugin_analytics["daily_stats"][daily_key] = {
                    "downloads": len(
                        [d for d in plugin_analytics.get("downloads", []) if datetime.fromtimestamp(d).date() == current_date]
                    ),
                    "views": len(
                        [v for v in plugin_analytics.get("views", []) if datetime.fromtimestamp(v).date() == current_date]
                    ),
                    "ratings": len(
                        [r for r in plugin_analytics.get("ratings", []) if datetime.fromtimestamp(r).date() == current_date]
                    ),
                }
