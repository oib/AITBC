    ...
    try:
        async with httpx.AsyncClient(timeout=8.0) as client:
            ...
            if r.status_code == 200:
                txs = r.json()
                # Walk newest-first (list is oldest-first, reverse it)
        ...
        logger.warning(f"Hub lookup failed: {e}")
    return plugin.get("offer_id")
...
async def register_plugin(request: dict[str, Any]) -> dict[str, Any]:
    ...
    if not plugin_id:
        # Auto-generate from service_type + model
