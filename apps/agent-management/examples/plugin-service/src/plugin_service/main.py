...
app = FastAPI(title='AITBC Plugin Service', description='Plugin registry: discover Ollama, Whisper, PeerTube and other software marketplace services', version='2.0.0')
        ...
        async with httpx.AsyncClient(timeout=8.0) as client:
            ...
            if r.status_code == 200:
                ...
                for tx in reversed(txs):
                    p = tx.get('payload', {})
                    if p.get('action') == 'software_offer' and p.get('service_type') == service_type and (not model or p.get('model') == model) and (not provider_address or p.get('provider_address') == provider_address):
...
async def register_plugin(request: dict[str, Any]) -> dict[str, Any]:
    ...
    entry = {**registry.get(plugin_id, {}), **{k: v for k, v in request.items() if v is not None}, 'plugin_id': plugin_id, 'registered_at': registry.get(plugin_id, {}).get('registered_at', now), 'updated_at': now, 'status': request.get('status', 'active')}
...
async def get_plugin_offer(plugin_id: str) -> dict[str, Any]:
    ...
    return {'plugin_id': plugin_id, 'offer_id': offer_id, 'service_type': plugin.get('service_type'), 'model': plugin.get('model'), 'price': plugin.get('price'), 'price_unit': plugin.get('price_unit')}
