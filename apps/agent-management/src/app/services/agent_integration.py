    ...
    async def integrate_with_zk_system(
        ...
        try:
            ...
            for step_execution in step_executions:
                if step_execution.requires_proof:
                    try:
                        ...
                        proof_result = await self._shared_service.generate_verification_proof(
                            execution_id, circuit_name, proof_inputs
                        )
                        zk_proofs.append(
                            {
                                "step_id": step_execution.step_id,
            ...
            try:
                ...
                workflow_proof = await self._shared_service.generate_verification_proof(
                    execution_id, circuit_name, workflow_inputs
                )
            ...
            for instance in current_instances:
                try:
                    ...
                    rollback_result["rollback_errors"].append(
                        {"instance_id": instance.instance_id, "error": "Previous version tracking not implemented"}
                    )
                except Exception as e:
                ...
                try:
                    ...
                    async with httpx.AsyncClient(timeout=5.0) as client:
                        ...
                        if response.status_code == 200:
                            ...
                            metrics_data.update(
                                {
                                    "cpu_usage": str(agent_metrics.get("cpu_usage", instance.cpu_usage)),
                                    "memory_usage": str(agent_metrics.get("memory_usage", instance.memory_usage)),
                                    "request_count": str(agent_metrics.get("request_count", instance.request_count)),
                                    "error_count": str(agent_metrics.get("error_count", instance.error_count)),
                                    "average_response_time": str(
                                        agent_metrics.get("average_response_time", instance.average_response_time)
                                    ),
                                    "uptime_percentage": str(
                                        agent_metrics.get("uptime_percentage", instance.uptime_percentage)
                                    ),
                                }
                            )
                        else:
                            metrics_data.update(
                                {
                                    ...
                                    "request_count": str(instance.request_count),
                                    "error_count": str(instance.error_count),
                                    "average_response_time": str(instance.average_response_time),
                                    "uptime_percentage": str(instance.uptime_percentage),
                                }
                            )
                except Exception as http_error:
