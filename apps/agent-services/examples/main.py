...
async def list_agents(region: str | None = None,
...
async def create_collaboration(session: CollaborationSession):
    ...
    for participant_id in session.participants:
        message_record = {
            "message_id": f"collab_{int(datetime.now(UTC).timestamp())}",
            "sender_id": "system",
...
async def send_collaboration_message(session_id: str, sender_id: str, content: dict[str, Any]):
    ...
    message_record = {
        "message_id": f"collab_msg_{int(datetime.now(UTC).timestamp())}",
        "sender_id": sender_id,
    ...
    for participant_id in collaboration_sessions[session_id]["participants"]:
        if participant_id != sender_id:
            ...
            msg_record = {
                "message_id": f"notif_{int(datetime.now(UTC).timestamp())}",
                "sender_id": "system",
                "recipient_id": participant_id,
                "message_type": "notification",
                "content": notification,
...
async def network_monitoring_task():
    ...
    while True:
        ...
        global_network_stats["last_update"] = datetime.now(UTC).isoformat()
