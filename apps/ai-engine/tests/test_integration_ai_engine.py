...
# No `sys.modules["numpy"] = MagicMock()` here. numpy is installed and always has been, and
# that assignment runs at *collection* -- before any test executes -- so it replaced numpy for
# every suite in the run, not just this file. tests/unit/test_property_crypto.py then failed
# all 14 of its tests on "No module named 'numpy.random'; 'numpy' is not a package" (V23-69).
# The tests below patch `ai_service.np.*` individually, which is what they actually need.
...
def test_get_ai_dashboard_endpoint():
    ...
    with (
        patch.object(ai_engine, "analyze_market") as mock_analyze,
