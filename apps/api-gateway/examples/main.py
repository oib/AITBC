...
app = FastAPI(
    title="AITBC Multi-Region Load Balancer", description="Intelligent load balancing across global regions", version="1.0.0"
)
...
async def get_all_region_health():
    ...
    return {
        "region_health": {region_id: health.dict() for region_id, health in region_health_status.items()},
...
def select_region_geographically(client_region: str) -> str | None:
    ...
    applicable_rules = [
        rule for rule in geographic_rules.values() if client_region in rule["source_regions"] and rule["status"] == "active"
    ]
...
def get_recent_activity() -> list[dict]:
    ...
    for rule_id, rule in load_balancing_rules.items():
        if (datetime.now(UTC) - datetime.fromisoformat(rule["last_updated"])).total_seconds() < 3600:
            activity.append(
                {"type": "rule_update", "rule_id": rule_id, "name": rule["name"], "timestamp": rule["last_updated"]}
            )
