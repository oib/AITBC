...
@app.get('/')
async def root():
    return {'service': 'AITBC Multi-Region Load Balancer', 'status': 'running', 'timestamp': datetime.now(UTC).isoformat(), 'version': '1.0.0'}

@app.get('/health')
async def health_check():
    return {'status': 'healthy', 'total_rules': len(load_balancing_rules), 'active_rules': len([r for r in load_balancing_rules.values() if r['status'] == 'active']), 'monitored_regions': len(region_health_status), 'healthy_regions': len([r for r in region_health_status.values() if r.status == 'healthy'])}
...
async def create_load_balancing_rule(rule: LoadBalancingRule):
    ...
    rule_record = {'rule_id': rule.rule_id, 'name': rule.name, 'algorithm': rule.algorithm, 'target_regions': rule.target_regions, 'weights': rule.weights, 'health_check_path': rule.health_check_path, 'failover_enabled': rule.failover_enabled, 'session_affinity': rule.session_affinity, 'status': 'active', 'created_at': datetime.now(UTC).isoformat(), 'total_requests': 0, 'failed_requests': 0, 'last_updated': datetime.now(UTC).isoformat()}
    load_balancing_rules[rule.rule_id] = rule_record
    asyncio.create_task(start_health_monitoring(rule.rule_id))
    logger.info('Load balancing rule created: %s (%s)', rule.name, rule.rule_id)
    return {'rule_id': rule.rule_id, 'status': 'created', 'name': rule.name, 'algorithm': rule.algorithm, 'created_at': rule_record['created_at']}

@app.get('/api/v1/rules')
async def list_load_balancing_rules():
    """List all load balancing rules"""
    return {'rules': list(load_balancing_rules.values()), 'total_rules': len(load_balancing_rules), 'active_rules': len([r for r in load_balancing_rules.values() if r['status'] == 'active'])}
...
async def get_load_balancing_rule(rule_id: str):
    ...
    rule['region_health'] = {region_id: region_health_status.get(region_id) for region_id in rule['target_regions'] if region_id in region_health_status}
...
async def update_rule_weights(rule_id: str, weights: dict[str, float]):
    ...
    logger.info('Weights updated for rule %s: %s', rule_id, normalized_weights)
    return {'rule_id': rule_id, 'new_weights': normalized_weights, 'updated_at': rule['last_updated']}
    ...
    for rule_id, rule in load_balancing_rules.items():
        if health.region_id in rule['target_regions']:
            if health.status == 'unhealthy' and rule['failover_enabled']:
                logger.warning('Region %s unhealthy, enabling failover for rule %s', health.region_id, rule_id)
                enable_failover(rule_id, health.region_id)
    return {'region_id': health.region_id, 'status': health.status, 'registered_at': datetime.now(UTC).isoformat()}

@app.get('/api/v1/health')
async def get_all_region_health():
    """Get health status for all monitored regions"""
    return {'region_health': {region_id: health.dict() for region_id, health in region_health_status.items()}, 'total_regions': len(region_health_status), 'healthy_regions': len([r for r in region_health_status.values() if r.status == 'healthy']), 'unhealthy_regions': len([r for r in region_health_status.values() if r.status == 'unhealthy']), 'degraded_regions': len([r for r in region_health_status.values() if r.status == 'degraded'])}
...
async def create_geographic_rule(rule: GeographicRule):
    ...
    rule_record = {'rule_id': rule.rule_id, 'source_regions': rule.source_regions, 'target_regions': rule.target_regions, 'priority': rule.priority, 'latency_threshold_ms': rule.latency_threshold_ms, 'status': 'active', 'created_at': datetime.now(UTC).isoformat(), 'usage_count': 0}
    geographic_rules[rule.rule_id] = rule_record
    logger.info('Geographic rule created: %s', rule.rule_id)
    return {'rule_id': rule.rule_id, 'status': 'created', 'priority': rule.priority, 'created_at': rule_record['created_at']}

@app.get('/api/v1/route/{client_region}')
async def get_optimal_region(client_region: str, rule_id: str | None=None):
    ...
    return {'client_region': client_region, 'optimal_region': optimal_region, 'rule_id': rule_id, 'selection_reason': get_selection_reason(optimal_region, client_region, rule_id), 'timestamp': datetime.now(UTC).isoformat()}
...
async def get_load_balancing_dashboard():
    ...
    health_summary = {'total_regions': len(region_health_status), 'healthy': len([r for r in region_health_status.values() if r.status == 'healthy']), 'unhealthy': len([r for r in region_health_status.values() if r.status == 'unhealthy']), 'degraded': len([r for r in region_health_status.values() if r.status == 'degraded'])}
    performance_summary = {}
    for rule_id, metrics_list in balancing_metrics.items():
        if metrics_list:
            latest_metrics = metrics_list[-1]
            performance_summary[rule_id] = {'total_requests': latest_metrics['total_requests'], 'average_response_time': latest_metrics['average_response_time'], 'error_rate': latest_metrics['error_rate'], 'throughput': latest_metrics['throughput']}
    algorithm_distribution = {}
    for rule in load_balancing_rules.values():
        algorithm = rule['algorithm']
        algorithm_distribution[algorithm] = algorithm_distribution.get(algorithm, 0) + 1
    return {'dashboard': {'overview': {'total_rules': total_rules, 'active_rules': active_rules, 'geographic_rules': len(geographic_rules), 'algorithm_distribution': algorithm_distribution}, 'region_health': health_summary, 'performance': performance_summary, 'recent_activity': get_recent_activity()}, 'generated_at': datetime.now(UTC).isoformat()}

def select_region_by_algorithm(rule_id: str, client_region: str) -> str | None:
    ...
    healthy_regions = [region for region in target_regions if region in region_health_status and region_health_status[region].status == 'healthy']
...
def select_geographic_optimal(client_region: str, target_regions: list[str]) -> str:
    ...
    geographic_proximity = {'us-east': ['us-east-1', 'us-west-1'], 'us-west': ['us-west-1', 'us-east-1'], 'europe': ['eu-west-1', 'eu-central-1'], 'asia': ['ap-southeast-1', 'ap-northeast-1']}
...
def select_region_geographically(client_region: str) -> str | None:
    """Select region based on geographic rules"""
    applicable_rules = [rule for rule in geographic_rules.values() if client_region in rule['source_regions'] and rule['status'] == 'active']
    ...
    for rule in applicable_rules:
        ...
        if best_target:
            rule['usage_count'] += 1
            return best_target
    healthy_regions = [region for region, health in region_health_status.items() if health.status == 'healthy']
    return healthy_regions[0] if healthy_regions else None

def get_selection_reason(region: str, client_region: str, rule_id: str | None) -> str:
...
def enable_failover(rule_id: str, unhealthy_region: str):
    ...
    if unhealthy_region in rule['target_regions']:
        ...
        logger.warning('Region %s removed from load balancing rule %s', unhealthy_region, rule_id)

def get_recent_activity() -> list[dict]:
    """Get recent load balancing activity"""
    activity = []
    for region_id, health in region_health_status.items():
        if (datetime.now(UTC) - health.last_check).total_seconds() < 3600:
            activity.append({'type': 'health_check', 'region': region_id, 'status': health.status, 'timestamp': health.last_check.isoformat()})
    for rule_id, rule in load_balancing_rules.items():
        if (datetime.now(UTC) - datetime.fromisoformat(rule['last_updated'])).total_seconds() < 3600:
            activity.append({'type': 'rule_update', 'rule_id': rule_id, 'name': rule['name'], 'timestamp': rule['last_updated']})
...
async def start_health_monitoring(rule_id: str):
    ...
    while rule['status'] == 'active':
        try:
            ...
            logger.error('Health monitoring error for rule %s: %s', rule_id, str(e))
...
async def check_region_health(region_id: str):
    ...
    if response_time < 100 and success_rate > 0.99:
        ...
        status = 'unhealthy'
    health = RegionHealth(region_id=region_id, status=status, response_time_ms=response_time, success_rate=success_rate, active_connections=active_connections, last_check=datetime.now(UTC))
    region_health_status[region_id] = health

@app.on_event('startup')
async def startup_event():
    logger.info('Starting AITBC Multi-Region Load Balancer')
    default_rules = [{'rule_id': 'global-web-rule', 'name': 'Global Web Load Balancer', 'algorithm': 'weighted_round_robin', 'target_regions': ['us-east-1', 'eu-west-1', 'ap-southeast-1'], 'weights': {'us-east-1': 0.4, 'eu-west-1': 0.35, 'ap-southeast-1': 0.25}, 'health_check_path': '/health', 'failover_enabled': True, 'session_affinity': False}, {'rule_id': 'api-performance-rule', 'name': 'API Performance Optimizer', 'algorithm': 'performance_based', 'target_regions': ['us-east-1', 'eu-west-1'], 'weights': {'us-east-1': 0.5, 'eu-west-1': 0.5}, 'health_check_path': '/api/health', 'failover_enabled': True, 'session_affinity': True}]
    for rule_data in default_rules:
        rule = LoadBalancingRule(**rule_data)
        rule_record = {'rule_id': rule.rule_id, 'name': rule.name, 'algorithm': rule.algorithm, 'target_regions': rule.target_regions, 'weights': rule.weights, 'health_check_path': rule.health_check_path, 'failover_enabled': rule.failover_enabled, 'session_affinity': rule.session_affinity, 'status': 'active', 'created_at': datetime.now(UTC).isoformat(), 'total_requests': 0, 'failed_requests': 0, 'last_updated': datetime.now(UTC).isoformat()}
        load_balancing_rules[rule.rule_id] = rule_record
        asyncio.create_task(start_health_monitoring(rule.rule_id))
    default_geo_rules = [{'rule_id': 'us-to-us', 'source_regions': ['us-east', 'us-west', 'north-america'], 'target_regions': ['us-east-1', 'us-west-1'], 'priority': 1, 'latency_threshold_ms': 50}, {'rule_id': 'eu-to-eu', 'source_regions': ['europe', 'eu-west', 'eu-central'], 'target_regions': ['eu-west-1', 'eu-central-1'], 'priority': 1, 'latency_threshold_ms': 30}]
    for geo_rule_data in default_geo_rules:
        geo_rule = GeographicRule(**geo_rule_data)
        geo_rule_record = {'rule_id': geo_rule.rule_id, 'source_regions': geo_rule.source_regions, 'target_regions': geo_rule.target_regions, 'priority': geo_rule.priority, 'latency_threshold_ms': geo_rule.latency_threshold_ms, 'status': 'active', 'created_at': datetime.now(UTC).isoformat(), 'usage_count': 0}
