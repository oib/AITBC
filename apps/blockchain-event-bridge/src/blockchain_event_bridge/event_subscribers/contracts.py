...
class ContractEventSubscriber:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        self.contract_addresses: dict[str, str] = {'AgentStaking': settings.agent_staking_address or '', 'PerformanceVerifier': settings.performance_verifier_address or '', 'AgentServiceMarketplace': settings.marketplace_address or '', 'BountyIntegration': settings.bounty_address or '', 'CrossChainBridge': settings.bridge_address or ''}
        self.event_topics: dict[str, list[str]] = {'AgentStaking': ['StakeCreated', 'RewardsDistributed', 'AgentTierUpdated'], 'PerformanceVerifier': ['PerformanceVerified', 'PenaltyApplied', 'RewardIssued'], 'AgentServiceMarketplace': ['ServiceListed', 'ServicePurchased'], 'BountyIntegration': ['BountyCreated', 'BountyCompleted'], 'CrossChainBridge': ['BridgeInitiated', 'BridgeCompleted']}
    ...
    async def _poll_contract_events(self) -> None:
        ...
        for contract_name, contract_address in self.contract_addresses.items():
            ...
            try:
                ...
                logs_data = await client.async_post('/eth_getLogs', json={'address': contract_address, 'from_block': last_height + 1, 'to_block': current_height, 'topics': self.event_topics.get(contract_name, [])})
