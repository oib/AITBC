...
def generate_test_transaction(i: int) -> dict[str, Any]:
    ...
    return {'from': f'0xtest_sender_{i % 100:040x}', 'to': f'0xtest_receiver_{i % 50:040x}', 'value': str((i + 1) * 1000), 'nonce': i, 'data': f'0x{hash(i) % 1000000:06x}', 'gas_limit': 21000, 'gas_price': '1000000000'}

async def worker_task(benchmark: BlockchainBenchmark, worker_id: int, transactions_per_worker: int, results: list[dict[str, Any]]) -> None:
...
async def run_benchmark(base_url: str, concurrent_clients: int, duration: int, target_tps: int=None) -> BenchmarkResult:
    ...
    async with BlockchainBenchmark(base_url) as benchmark:
        ...
        return BenchmarkResult(total_transactions=len(successful_tx), duration=actual_duration, tps=tps, latency_p50=latency_p50, latency_p95=latency_p95, latency_p99=latency_p99, cpu_usage=avg_cpu, memory_usage=avg_memory, errors=errors)
...
async def main():
    ...
    result = await run_benchmark(base_url=args.target_url, concurrent_clients=args.concurrent_clients, duration=args.duration, target_tps=args.target_tps)
    if args.output:
        with open(args.output, 'w') as f:
            json.dump({'total_transactions': result.total_transactions, 'duration': result.duration, 'tps': result.tps, 'latency_p50': result.latency_p50, 'latency_p95': result.latency_p95, 'latency_p99': result.latency_p99, 'cpu_usage': result.cpu_usage, 'memory_usage': result.memory_usage, 'errors': result.errors}, f, indent=2)
