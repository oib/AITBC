        ...
        while not self._stop_event.is_set():
            try:
                ...
                if retry_count <= max_retries:
                    ...
                    logger.warning('RPC connection failed (attempt %s/%s), retrying in %ss: %s', retry_count, max_retries, delay, e)
                    await asyncio.sleep(delay)
                    continue
                else:
    ...
    async def _import_block(self, block_data: dict[str, Any]) -> None:
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.post(f'http://{target_host}:{target_port}/rpc/importBlock', json=block_data) as resp:
                            if resp.status == 200:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.warning('Import failed (attempt %s/%s), retrying in %ss: %s', attempt + 1, max_retries, delay, e)
                        await asyncio.sleep(delay)
                    else:
                        logger.error('Failed to import block %s after %s attempts: %s', block_data.get('height'), max_retries, e)
                        return
        except Exception as e:
...
def main() -> None:
    ...
    try:
        asyncio.run(run_chain_sync(args.redis, args.node_id, args.rpc_port, args.leader_host, args.source_host, args.source_port, args.import_host, args.import_port, args.chain_id))
