...
class AgentWalletSecurity:
    ...
    def register_agent(
        ...
        try:
            ...
            if agent_address in self.agent_profiles:
                return {"status": "error", "reason": "Agent already registered"}

            # Validate security level
            if security_level not in self.configurations:
                return {"status": "error", "reason": f"Invalid security level: {security_level}"}
            ...
            guardian_contract = create_guardian_contract(agent_address=agent_address, guardians=guardian_addresses, **config)
            ...
            return {"status": "error", "reason": f"Registration failed: {str(e)}"}

    def protect_transaction(self, agent_address: str, to_address: str, amount: int, data: str = "") -> dict[str, Any]:
        """
        ...
        try:
            ...
            if not profile.enabled:
                return {"status": "unprotected", "reason": "Security protection disabled for this agent"}
            ...
            return {"status": "error", "reason": f"Transaction protection failed: {str(e)}"}

    def execute_protected_transaction(self, agent_address: str, operation_id: str, signature: str) -> dict[str, Any]:
        """
        ...
        try:
            ...
            return {"status": "error", "reason": f"Transaction execution failed: {str(e)}"}
    ...
    def emergency_pause_agent(self, agent_address: str, guardian_address: str) -> dict[str, Any]:
        ...
        try:
            ...
            if result["status"] == "paused":
                self._log_security_event(
                    event_type="emergency_pause", agent_address=agent_address, guardian_address=guardian_address
                )

            return result

        except Exception as e:
            return {"status": "error", "reason": f"Emergency pause failed: {str(e)}"}

    def update_agent_security(self, agent_address: str, new_limits: dict[str, Any], guardian_address: str) -> dict[str, Any]:
        """
        ...
        try:
            ...
            return {"status": "error", "reason": f"Security update failed: {str(e)}"}
    ...
    def get_agent_security_status(self, agent_address: str) -> dict[str, Any]:
        ...
        try:
            ...
            if agent_address not in self.agent_profiles:
                return {"status": "not_registered", "message": "Agent not registered for security protection"}
    ...
    def _log_security_event(self, **kwargs: Any) -> None:
        ...
        event = {"timestamp": datetime.now(UTC).isoformat(), **kwargs}
    ...
    def disable_agent_protection(self, agent_address: str, guardian_address: str) -> dict[str, Any]:
        ...
        try:
            ...
            if guardian_address not in profile.guardian_addresses:
                return {"status": "error", "reason": "Not authorized: not a guardian"}

            profile.enabled = False

            # Log security event
            self._log_security_event(
                event_type="protection_disabled", agent_address=agent_address, guardian_address=guardian_address
            )
            ...
            return {"status": "error", "reason": f"Disable protection failed: {str(e)}"}
...
def register_agent_for_protection(
    agent_address: str, security_level: str = "conservative", guardians: list[str] | None = None
) -> dict[str, Any]:
    """Register an agent for security protection"""
    return agent_wallet_security.register_agent(
        agent_address=agent_address, security_level=security_level, guardian_addresses=guardians
    )


def protect_agent_transaction(agent_address: str, to_address: str, amount: int, data: str = "") -> dict[str, Any]:
    """Protect a transaction for an agent"""
    return agent_wallet_security.protect_transaction(
        agent_address=agent_address, to_address=to_address, amount=amount, data=data
    )
...
def detect_suspicious_activity(agent_address: str, hours: int = 24) -> dict[str, Any]:
    ...
    if status["status"] != "protected":
        return {"status": "not_protected", "suspicious_activity": False}
