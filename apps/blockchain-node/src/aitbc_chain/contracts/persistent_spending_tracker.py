...
class PersistentSpendingTracker:
    ...
    def check_spending_limits(
        ...
        if amount > per_transaction:
            return SpendingCheckResult(
                allowed=False,
                reason=f"Amount {amount} exceeds per-transaction limit {per_transaction}",
        ...
        if spent_hour + amount > per_hour:
            return SpendingCheckResult(
                allowed=False,
                reason=f"Hourly spending {spent_hour + amount} would exceed limit {per_hour}",
                current_spent=current_spent,
                remaining=remaining,
                requires_time_lock=False,
            )
        ...
        if spent_day + amount > per_day:
            return SpendingCheckResult(
                allowed=False,
                reason=f"Daily spending {spent_day + amount} would exceed limit {per_day}",
                current_spent=current_spent,
                remaining=remaining,
                requires_time_lock=False,
            )
        ...
        if spent_week + amount > per_week:
            return SpendingCheckResult(
                allowed=False,
                reason=f"Weekly spending {spent_week + amount} would exceed limit {per_week}",
                current_spent=current_spent,
                remaining=remaining,
                requires_time_lock=False,
            )
        ...
        if requires_time_lock:
            time_lock_until = timestamp + timedelta(
                hours=float(limits.time_lock_delay_hours if limits.time_lock_delay_hours is not None else 0)
            )
        return SpendingCheckResult(
            allowed=True,
    ...
    def get_spending_summary(self, agent_address: str) -> dict[str, Any]:
        ...
        remaining = {
            ...
            "week": per_week_limit - current_spent["week"],
        }
        with self.get_session() as session:
            guardians = (
                session.query(GuardianAuthorization)
        ...
        return {
            ...
            "time_lock": {"threshold": limits.time_lock_threshold, "delay_hours": limits.time_lock_delay_hours},
