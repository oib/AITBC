...
class EconomicSecurityMonitor:
    ...
    async def _update_blacklist(self) -> None:
        ...
        for detection in self.attack_detections:
            if detection.threat_level in [ThreatLevel.HIGH, ThreatLevel.CRITICAL] and detection.confidence > 0.8 and (detection.attacker_address not in self.blacklisted_addresses):
    ...
    def get_attack_summary(self) -> dict[str, Any]:
        ...
        return {'total_detections': len(recent_detections), 'attack_types': attack_counts, 'threat_levels': threat_counts, 'blacklisted_addresses': len(self.blacklisted_addresses), 'security_metrics': {name: {'value': metric.current_value, 'threshold': metric.threshold, 'status': metric.status} for name, metric in self.security_metrics.items()}}
...
def create_security_monitor(staking_manager: StakingManager, reward_distributor: RewardDistributor, gas_manager: GasManager) -> EconomicSecurityMonitor:
