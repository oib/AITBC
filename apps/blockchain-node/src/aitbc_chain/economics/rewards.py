...
class RewardCalculator:
    ...
    def calculate_block_reward(
        ...
        block_height: int,
        is_proposer: bool,
        participated_validators: list[str],
        uptime_scores: dict[str, float],
    ) -> Decimal:
...
class RewardDistributor:
    ...
    def calculate_validator_rewards(
        self, validator_address: str, period_start: float, period_end: float
    ) -> dict[str, Decimal]:
        """Calculate rewards for validator over a period"""
        period_events = [
            event
            for event in self.reward_events
            if event.validator_address == validator_address and period_start <= event.timestamp <= period_end
        ]
        ...
        return {
            ...
            "block_proposal_rewards": sum(  # type: ignore
                event.amount for event in period_events if event.reward_type == RewardType.BLOCK_PROPOSAL
            ),
            "block_validation_rewards": sum(  # type: ignore
                event.amount for event in period_events if event.reward_type == RewardType.BLOCK_VALIDATION
            ),
            "consensus_rewards": sum(  # type: ignore
                event.amount for event in period_events if event.reward_type == RewardType.CONSENSUS_PARTICIPATION
            ),
            "uptime_rewards": sum(  # type: ignore
                event.amount for event in period_events if event.reward_type == RewardType.UPTIME
            ),
        }

    def distribute_rewards(self, block_height: int) -> tuple[bool, str, str | None]:
        ...
        try:
            ...
            for validator_address, validator_reward in self.pending_rewards.items():
                ...
                validator_positions = [
                    pos
                    for pos in self.staking_manager.stake_positions.values()
                    if pos.validator_address == validator_address and pos.status == StakingStatus.ACTIVE
                ]
    ...
    def get_reward_history(self, validator_address: str | None = None, limit: int = 100) -> list[RewardEvent]:
        """Get reward history"""
    ...
    def get_distribution_history(self, validator_address: str | None = None, limit: int = 50) -> list[RewardDistribution]:
        ...
        if validator_address:
            distributions = [
                ...
                if validator_address in d.validator_rewards
                or any(validator_address in key for key in d.delegator_rewards.keys())
            ]
...
reward_distributor: RewardDistributor | None = None


def get_reward_distributor() -> RewardDistributor | None:
