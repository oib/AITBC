        ...
        try:
            ...
            if os.path.exists(keystore_path):
                with open(keystore_path) as f:
                    keys = json.load(f)
                    for key_id, _key_data in keys.items():
            ...
            if rpc_host in {"0.0.0.0", "127.0.0.1", "localhost", ""}:  # nosec B104 - code default only; the effective bind is pinned per host in the systemd unit. AITBC runs no firewall, so this default is an accepted deviation tracked in docs/deployment/NETWORK_POLICY.md, not a safe fallback
    ...
    async def handle_join_request(self, join_request: dict[str, Any]) -> dict[str, Any] | None:
        ...
        try:
            ...
            if requested_island_id != self.island_id:
                ...
                return None
            members = []
            for _node_id, peer_info in self.peer_registry.items():
            ...
            members.append(
                {
                    ...
                    "is_hub": True,
                    "public_address": local_hub_info.public_address if local_hub_info else None,
                    "public_port": local_hub_info.public_port if local_hub_info else None,
                }
            )
