    ...
    for exporter_name in exporters:
        if exporter_name not in REGISTERED_EXPORTERS:
            try:
                ...
                logger.info(f"Registered exporter: {exporter_name}")
            except Exception as e:
                logger.warning(f"Failed to initialize exporter {exporter_name}: {e}")
...
def _initialize_exporter(exporter_name: str) -> None:
    ...
    if exporter_name == "prometheus":
        ...
        logger.debug(f"Unknown exporter type: {exporter_name}, skipping initialization")
...
def _initialize_log_shipper() -> None:
    ...
    try:
        ...
        if log_endpoint:
            logger.info(f"Log shipper configured for endpoint: {log_endpoint}")
            # In production, would configure log handlers to ship to endpoint
        else:
            logger.debug("LOG_SHIPPER_ENDPOINT not configured, skipping log shipper")
    except Exception as e:
        logger.warning(f"Failed to initialize log shipper: {e}")
...
def _initialize_tracing() -> None:
    ...
    try:
        ...
        logger.info(f"Jaeger tracing exporter configured: {jaeger_host}:{jaeger_port}")
    except ImportError:
        logger.warning("opentelemetry packages not installed, skipping tracing exporter")
    except Exception as e:
        logger.warning(f"Failed to initialize tracing exporter: {e}")
