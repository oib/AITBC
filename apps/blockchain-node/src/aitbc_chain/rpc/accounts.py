...
async def get_account_details(request: Request, address: str, chain_id: str | None = None) -> dict[str, Any]:
    ...
    with session_scope() as session:
        ...
        result = {'success': True, 'address': account.address, 'chain_id': account.chain_id, 'balance': account.balance, 'nonce': account.nonce, 'updated_at': account.updated_at.isoformat() if account.updated_at else None}
        _cache.set(cache_key, result, ttl=ACCOUNT_CACHE_TTL)
        return result
...
async def create_account(request: Request, account_data: dict) -> dict[str, Any]:
    ...
    with session_scope() as session:
        ...
        if existing_account:
            return {'success': True, 'address': address, 'chain_id': chain_id, 'balance': existing_account.balance, 'nonce': existing_account.nonce, 'created': False, 'message': 'Account already exists'}
...
async def faucet_request(request: Request, faucet_data: dict) -> dict[str, Any]:
    ...
    with session_scope() as session:
        ...
        session.add(account)
        faucet_tx = Transaction(chain_id=chain_id, tx_hash=tx_hash, sender='faucet', recipient=address, payload={'type': 'FAUCET', 'amount': amount, 'reason': 'test_funding'}, value=amount, fee=0, nonce=0, timestamp=timestamp, block_height=None, status='confirmed', type='FAUCET')
