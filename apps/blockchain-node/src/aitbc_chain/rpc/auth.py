...
def get_authenticated_address(request: Request, credentials: HTTPAuthorizationCredentials | None=None) -> str:
    ...
    if wallet_address:
        ...
        if os.getenv('TRUST_X_WALLET_ADDRESS', 'false').lower() != 'true':
            ...
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='X-Wallet-Address header is not trusted without explicit server configuration')
    ...
    if credentials and credentials.scheme == 'Bearer':
        ...
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='JWT authentication is not supported. Use X-Wallet-Address header with TRUST_X_WALLET_ADDRESS=true for trusted internal requests.')
    if os.getenv('DEV_MODE', 'false').lower() == 'true':
        _logger.warning('Development mode enabled but authentication still required')
    raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Authentication required. Provide X-Wallet-Address header with TRUST_X_WALLET_ADDRESS=true for trusted internal requests.', headers={'WWW-Authenticate': 'Bearer'})
