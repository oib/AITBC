...
async def bridge_lock(request: Request, lock_data: dict) -> dict[str, Any]:
    ...
    try:
        ...
        transfer = bridge.initiate_transfer(source_chain=source_chain, target_chain=cast(str, target_chain), sender=cast(str, sender).lower(), recipient=cast(str, recipient).lower(), amount=amount, asset=asset)
