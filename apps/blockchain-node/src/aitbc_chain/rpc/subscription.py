...
async def register_subscription(request: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        expiry = await lease_tracker.register_subscriber(node_id=node_id, transport=transport, chain_id=chain_id, duration=duration)
        return {'node_id': node_id, 'transport': transport, 'chain_id': chain_id, 'expiry': expiry, 'lease_duration': duration or settings.lease_duration}
...
async def get_subscribers(chain_id: str | None=None) -> dict[str, Any]:
    ...
    try:
        ...
        return {'subscribers': [{'node_id': s.node_id, 'transport': s.transport, 'chain_id': s.chain_id, 'expiry': s.expiry} for s in subscribers], 'count': len(subscribers)}
    except Exception as e:
