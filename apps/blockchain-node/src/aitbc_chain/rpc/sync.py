...
def _dedupe_import_blocks(blocks: list[dict[str, Any]], chain_id: str) -> list[dict[str, Any]]:
    ...
    for block_data in blocks:
        ...
        if block_chain_id and block_chain_id != chain_id:
            raise HTTPException(status_code=400, detail=f"Mismatched block chain_id '{block_chain_id}' for import chain '{chain_id}'")
        ...
        try:
            ...
            with session_scope() as session:
                ...
                for block_data in unique_blocks:
                    ...
                    block = Block(chain_id=chain_id, height=block_data['height'], hash=block_data['hash'], parent_hash=block_data['parent_hash'], proposer=block_data['proposer'], timestamp=block_timestamp, state_root=block_data.get('state_root'), tx_count=block_data.get('tx_count', 0), block_metadata=block_data.get('block_metadata'))
                    session.add(block)
                for account_data in accounts:
                    account_chain_id = account_data.get('chain_id', chain_id)
                    if account_chain_id != chain_id:
                        raise HTTPException(status_code=400, detail=f"Mismatched account chain_id '{account_chain_id}' for import chain '{chain_id}'")
                    account = Account(chain_id=account_chain_id, address=account_data['address'], balance=account_data['balance'], nonce=account_data['nonce'])
                    session.add(account)
                for tx_data in transactions:
                    tx_chain_id = tx_data.get('chain_id', chain_id)
                    if tx_chain_id != chain_id:
                        raise HTTPException(status_code=400, detail=f"Mismatched transaction chain_id '{tx_chain_id}' for import chain '{chain_id}'")
                    tx = Transaction(id=tx_data.get('id'), chain_id=tx_chain_id, tx_hash=str(tx_data.get('tx_hash') or tx_data.get('id') or ''), block_height=tx_data.get('block_height'), sender=tx_data['sender'], recipient=tx_data['recipient'], payload=tx_data.get('payload', {}), value=tx_data.get('value', 0), fee=tx_data.get('fee', 0), nonce=tx_data.get('nonce', 0), timestamp=_serialize_optional_timestamp(tx_data.get('timestamp')), status=tx_data.get('status', 'pending'), tx_metadata=tx_data.get('tx_metadata'))
                    created_at = _parse_datetime_value(tx_data.get('created_at'), 'transaction created_at')
                    if created_at is not None:
                        tx.created_at = created_at
                    session.add(tx)
                session.commit()
                return {'success': True, 'imported_blocks': len(unique_blocks), 'imported_accounts': len(accounts), 'imported_transactions': len(transactions), 'chain_id': chain_id, 'message': f'Successfully imported {len(unique_blocks)} blocks'}
        except HTTPException:
            raise
        except Exception as e:
...
async def force_sync(request: Request, peer_data: dict) -> dict[str, Any]:
    ...
    try:
        ...
        return {'success': True, 'synced_from': peer_url, 'synced_blocks': import_result['imported_blocks'], 'target_height': target_height or import_result['imported_blocks'], 'message': f'Successfully synced with peer {peer_url}'}
    except HTTPException:
        raise
    except Exception as e:
