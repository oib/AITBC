...
class BalanceTracker:
    ...
    def record_transaction(self, session: Session, sender: str, recipient: str, amount: int, fee: int, tx_hash: str, chain_id: str, tx_type: str='transfer') -> tuple[BalanceChange, BalanceChange]:
        """
        ...
        sender_change = BalanceChange(address=sender, chain_id=chain_id, change_type=BalanceChangeType.transaction_send, amount=-amount, fee=-fee, balance_before=sender_balance_before, balance_after=sender_account.balance, tx_hash=tx_hash, timestamp=datetime.now(UTC), details={'recipient': recipient, 'tx_type': tx_type})
        ...
        session.add(recipient_account)
        recipient_change = BalanceChange(address=recipient, chain_id=chain_id, change_type=BalanceChangeType.transaction_receive, amount=amount, fee=0, balance_before=recipient_balance_before, balance_after=recipient_account.balance, tx_hash=tx_hash, timestamp=datetime.now(UTC), details={'sender': sender, 'tx_type': tx_type})
    ...
    def record_stake(self, session: Session, address: str, amount: int, chain_id: str, stake_id: int) -> BalanceChange:
        ...
        session.add(account)
        change = BalanceChange(address=address, chain_id=chain_id, change_type=BalanceChangeType.staking_lock, amount=-amount, fee=0, balance_before=balance_before, balance_after=account.balance, tx_hash=None, timestamp=datetime.now(UTC), details={'stake_id': stake_id, 'operation': 'stake'})
    ...
    def record_unstake(self, session: Session, address: str, amount: int, chain_id: str, stake_id: int) -> BalanceChange:
        ...
        account.balance += amount
        session.add(account)
        change = BalanceChange(address=address, chain_id=chain_id, change_type=BalanceChangeType.staking_unlock, amount=amount, fee=0, balance_before=balance_before, balance_after=account.balance, tx_hash=None, timestamp=datetime.now(UTC), details={'stake_id': stake_id, 'operation': 'unstake'})
        logger.info('Unstake recorded: %s... returned %s', address[:16], amount)
        return change

    def record_bridge_lock(self, session: Session, address: str, amount: int, fee: int, chain_id: str, transfer_id: str) -> BalanceChange:
        ...
        session.add(account)
        change = BalanceChange(address=address, chain_id=chain_id, change_type=BalanceChangeType.bridge_lock, amount=-amount, fee=-fee, balance_before=balance_before, balance_after=account.balance, tx_hash=transfer_id, timestamp=datetime.now(UTC), details={'transfer_id': transfer_id, 'operation': 'lock'})
        logger.info('Bridge lock recorded: %s... locked %s (fee: %s)', address[:16], amount, fee)
        return change

    def record_bridge_release(self, session: Session, address: str, amount: int, chain_id: str, transfer_id: str) -> BalanceChange:
        ...
        account.balance += amount
        session.add(account)
        change = BalanceChange(address=address, chain_id=chain_id, change_type=BalanceChangeType.bridge_release, amount=amount, fee=0, balance_before=balance_before, balance_after=account.balance, tx_hash=transfer_id, timestamp=datetime.now(UTC), details={'transfer_id': transfer_id, 'operation': 'release'})
    ...
    def get_balance_breakdown(self, address: str, chain_id: str) -> dict[str, Any]:
        ...
        with self._session_factory() as session:
            ...
            statement = select(sql_func.sum(Stake.amount)).where(Stake.chain_id == chain_id, Stake.address == address, Stake.status == 'active')
            staked = session.exec(statement).one() or 0
            statement = select(sql_func.sum(CrossChainTransfer.amount)).where(CrossChainTransfer.source_chain == chain_id, CrossChainTransfer.sender == address, CrossChainTransfer.status == 'pending')
            bridge_locked = session.exec(statement).one() or 0
            total = available + staked + bridge_locked
            return {'address': address, 'chain_id': chain_id, 'available_balance': available, 'staked': staked, 'bridge_locked': bridge_locked, 'total_balance': total, 'timestamp': datetime.now(UTC).isoformat()}
    ...
    def reconcile_balance(self, address: str, chain_id: str) -> dict[str, Any]:
        ...
        with self._session_factory() as session:
            ...
            initial = 0
            received_stmt = select(sql_func.sum(Transaction.value)).where(Transaction.chain_id == chain_id, Transaction.recipient == address)
            total_received = session.exec(received_stmt).one() or 0
            sent_stmt = select(sql_func.sum(Transaction.value + Transaction.fee)).where(Transaction.chain_id == chain_id, Transaction.sender == address)
            total_sent = session.exec(sent_stmt).one() or 0
            staked_stmt = select(sql_func.sum(Stake.amount)).where(Stake.chain_id == chain_id, Stake.address == address, Stake.status == 'active')
            ...
            result = {'address': address, 'chain_id': chain_id, 'current_balance': current_balance, 'expected_balance': expected_balance, 'mismatch': mismatch, 'components': {'initial': initial, 'total_received': total_received, 'total_sent': total_sent, 'total_fees_paid': 0, 'total_staked': total_staked}}
            if mismatch:
                logger.warning('Balance mismatch for %s...: current=%s, expected=%s', address[:16], current_balance, expected_balance)
            return result
