...
class StateTransition:
    ...
    def validate_transaction(self, session: Session, chain_id: str, tx_data: dict, tx_hash: str) -> tuple[bool, str]:
        ...
        if tx_chain_id and tx_chain_id != chain_id:
            logger.warning('Chain isolation violation: Transaction %s has chain_id=%s but node is configured for chain_id=%s. Rejecting cross-chain transaction.', tx_hash, tx_chain_id, chain_id)
        ...
        tx_record = session.exec(select(Transaction).where(Transaction.chain_id == chain_id, Transaction.tx_hash == tx_hash)).first()
    ...
    def apply_transaction(self, session: Session, chain_id: str, tx_data: dict, tx_hash: str) -> tuple[bool, str]:
        ...
        session.execute(text('UPDATE account SET balance = balance - :total_cost, nonce = nonce + 1 WHERE chain_id = :chain_id AND address = :sender_addr'), {'total_cost': total_cost, 'chain_id': chain_id, 'sender_addr': sender_addr})
        if tx_type != 'MESSAGE':
            logger.info('Updating recipient balance: %s += %s', recipient_addr, value)
            session.execute(text('UPDATE account SET balance = balance + :value WHERE chain_id = :chain_id AND address = :recipient_addr'), {'value': value, 'chain_id': chain_id, 'recipient_addr': recipient_addr})
        session.flush()
        if tx_type == 'RECEIPT_CLAIM':
            receipt_id = tx_data.get('payload', {}).get('receipt_id')
            receipt = session.exec(select(Receipt).where(Receipt.chain_id == chain_id, Receipt.receipt_id == receipt_id)).first()
        ...
        logger.info('Applied transaction %s: %s -> %s, value=%s, fee=%s, type=%s', tx_hash, sender_addr, recipient_addr, value, fee, tx_type)
        return (True, 'Transaction applied successfully')

    def validate_state_transition(self, session: Session, chain_id: str, old_accounts: dict[str, Account], new_accounts: dict[str, Account]) -> tuple[bool, str]:
        """
        ...
        for address, old_acc in old_accounts.items():
            ...
            if old_acc.balance != new_acc.balance:
                logger.warning('Balance change detected for %s: %s -> %s (validated through transaction processing)', address, old_acc.balance, new_acc.balance)
    ...
    def handle_gpu_registration(self, session: Session, chain_id: str, gpu_data: dict) -> tuple[bool, str]:
        ...
        try:
            ...
            existing = session.exec(select(GPURegistration).where(GPURegistration.chain_id == chain_id, GPURegistration.gpu_id == gpu_id)).first()
            if existing:
                ...
                registration = GPURegistration(chain_id=chain_id, gpu_id=gpu_id, miner_id=gpu_data.get('miner_id', ''), model=gpu_data.get('model', ''), memory_gb=gpu_data.get('memory_gb', 0), cuda_version=gpu_data.get('cuda_version', ''), region=gpu_data.get('region', ''), capabilities=gpu_data.get('capabilities', []), price_per_hour=gpu_data.get('price_per_hour', 0.0), registered_by=gpu_data.get('registered_by', ''), registered_at=datetime.now(UTC), status='active')
    ...
    def handle_gpu_allocation(self, session: Session, chain_id: str, allocation_data: dict) -> tuple[bool, str]:
        ...
        try:
            ...
            if not gpu_id:
                return (False, 'GPU ID is required')
            gpu = session.exec(select(GPURegistration).where(GPURegistration.chain_id == chain_id, GPURegistration.gpu_id == gpu_id)).first()
            if not gpu:
                return (False, f'GPU not found: {gpu_id}')
            allocation_id = allocation_data.get('allocation_id', f'alloc_{uuid4().hex[:12]}')
            allocation = GPUAllocation(chain_id=chain_id, allocation_id=allocation_id, gpu_id=gpu_id, client_id=allocation_data.get('client_id', ''), duration_hours=allocation_data.get('duration_hours', 0.0), total_cost=allocation_data.get('total_cost', 0.0), status='active', allocated_by=allocation_data.get('allocated_by', ''), allocated_at=datetime.now(UTC))
