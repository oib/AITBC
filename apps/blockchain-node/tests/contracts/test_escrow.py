...
import pytest
from aitbc_chain.contracts.escrow import DisputeReason, EscrowManager, EscrowState
...
class TestEscrowManager:
    ...
    def test_verify_milestone(self):
        """Test verifying a completed milestone."""
        milestones = [
            {"milestone_id": "milestone_1", "description": "Setup", "amount": Decimal("50.0")},
            {"milestone_id": "milestone_2", "description": "Delivery", "amount": Decimal("50.0")},
        ]
        ...
        # Verify milestone
        success, message = asyncio.run(self.escrow_manager.verify_milestone(contract_id, "milestone_1"))
        assert success, f"Milestone verification failed: {message}"

        contract = asyncio.run(self.escrow_manager.get_contract_info(contract_id))
        assert contract.milestones[0]["verified"]

        # Double-verify should fail
        success, message = asyncio.run(self.escrow_manager.verify_milestone(contract_id, "milestone_1"))
        assert not success
