...
@pytest.mark.asyncio
async def test_a_follower_can_build_block_zero_from_its_genesis_file(proposer, session_factory) -> None:
    await proposer._ensure_genesis_block()

    with session_factory() as session:
        genesis = session.exec(select(Block).where(Block.height == 0)).first()

    assert genesis is not None, "the follower still cannot leave an empty database"
    assert genesis.proposer == "genesis"


@pytest.mark.asyncio
async def test_the_hash_comes_from_the_file_rather_than_being_recomputed(proposer, session_factory) -> None:
    """This is what makes local bootstrap safe: the node cannot invent a genesis of its own."""
    await proposer._ensure_genesis_block()

    with session_factory() as session:
        genesis = session.exec(select(Block).where(Block.height == 0)).first()

    assert genesis.hash == GENESIS_FILE["block"]["hash"]
    assert genesis.state_root == GENESIS_FILE["block"]["state_root"]
    assert genesis.timestamp.replace(tzinfo=UTC) == datetime.fromisoformat(GENESIS_FILE["block"]["timestamp"])


@pytest.mark.asyncio
async def test_bootstrapping_twice_does_not_duplicate_block_zero(proposer, session_factory) -> None:
    """Start-up runs on every restart, and a follower restarts far more often than a hub."""
    await proposer._ensure_genesis_block()
    await proposer._ensure_genesis_block()

    with session_factory() as session:
        assert len(session.exec(select(Block).where(Block.height == 0)).all()) == 1
...
@pytest.mark.asyncio
async def test_a_follower_without_a_genesis_file_still_starts(session_factory) -> None:
    """Backward compatibility: followers already syncing with TRUSTED_PROPOSERS must not break.

    `_ensure_genesis_block` raises when no file is found. On a hub that is correct — it has
    nothing to fall back on. On a follower the old sync path still exists, so a failure here
    must be logged and stepped over, never fatal.
    """
    ...
    with patch.object(
        BlockchainNode, "_ensure_genesis_for_chains", AsyncMock(side_effect=RuntimeError("Genesis file required"))
    ):
        await node._bootstrap_genesis_for_follower()  # must not raise
