"""Regression tests: GPU_MARKETPLACE must never carry value.

The /rpc/transactions/marketplace endpoint exempts ``offer``/``software_offer``
payloads from signature verification (the marketplace CLI has no wallet keys).
The exemption rested on offers being value-zero listings, but nothing enforced
it: consensus has no GPU_MARKETPLACE branch, so a nonzero amount fell through
...
Covers both layers of the fix:
  ...
  * consensus level — validate_transaction and compute_state_delta both refuse
    a nonzero-value GPU_MARKETPLACE regardless of how it arrived.
"""
...
async def test_unsigned_offer_with_amount_rejected(action: str):
    ...
    with pytest.raises(HTTPException) as exc_info:
        await tx_mod.submit_marketplace_transaction(MagicMock(), _offer_tx(10**9, action))
...
async def test_unsigned_zero_amount_offer_still_admitted():
    ...
    with (
        ...
        result = await tx_mod.submit_marketplace_transaction(MagicMock(), _offer_tx(0))
...
def test_validate_transaction_rejects_nonzero_marketplace(engine):
    st = StateTransition()
    tx = _offer_tx(10**9)
    tx["value"] = tx["amount"]
    with Session(engine) as session:
...
def test_validate_transaction_accepts_zero_value_marketplace(engine):
    st = StateTransition()
    tx = _offer_tx(0)
    tx["value"] = 0
    with Session(engine) as session:
...
def test_pure_state_transition_rejects_nonzero_marketplace():
