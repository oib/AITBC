...
`0xDb5247d0…` with a key controlling `0xFe2d63FE…`, then appended block 105,627 unsigned.
...
def test_address_of_returns_what_the_key_controls() -> None:
    ...
    assert address_of(account.key.hex()) == account.address


def test_a_matching_key_passes() -> None:
    account = Account.create()
    assert_can_sign(account.address, account.key.hex())
    # A lower-case 0x spelling must also verify against the EIP-55 checksummed address.
    assert_can_sign(account.address.lower(), account.key.hex())
...
def test_the_keystore_rejects_a_mislabelled_file(tmp_path) -> None:
    ...
    declared, actual = Account.create(), Account.create()
    _write_keystore(tmp_path, declared.address, actual.key.hex())

    loaded = _load_private_key_from_keystore(tmp_path, "unused", target_address=declared.address)
...
def test_the_keystore_accepts_a_correctly_labelled_file(tmp_path) -> None:
    account = Account.create()
    _write_keystore(tmp_path, account.address, account.key.hex())

    loaded = _load_private_key_from_keystore(tmp_path, "unused", target_address=account.address)

    assert loaded is not None
    assert address_of(loaded.hex()) == account.address


def test_the_keystore_matches_across_address_spellings(tmp_path) -> None:
    """PROPOSER_ID and the keystore may spell the same 0x address differently."""
    account = Account.create()
    _write_keystore(tmp_path, account.address.lower(), account.key.hex())
