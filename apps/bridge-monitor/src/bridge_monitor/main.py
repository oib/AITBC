...
class BridgeMonitor:
    ...
    def submit_ait_transfer(self, to_address: str, amount: Decimal) -> str | None:
        ...
        try:
            ...
            tx_payload = {'from': self.genesis_wallet_address, 'to': to_address, 'value': str(int(amount)), 'nonce': nonce, 'gas_limit': 21000, 'gas_price': '1', 'type': 'TRANSFER'}
    ...
    def process_deposit(self, tx_hash: str, from_address: str, eth_amount: Decimal, tx_data: str) -> None:
        ...
        update_deposit(tx_hash, ait_amount=str(ait_amount), eth_usd_price=str(eth_usd) if eth_usd else None, ait_usd_price=str(ait_usd) if ait_usd else None, status=BridgeDepositStatus.PROCESSING)
