...
regions = {
    ...
    "us-west": {"url": "http://127.0.0.1:8203", "weight": 2, "healthy": True, "edge_node": "aitbc1-edge-secondary"},
    ...
    "ap-southeast": {"url": "http://127.0.0.1:8203", "weight": 2, "healthy": True, "edge_node": "aitbc1-edge-secondary"},
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.request(
                    method=request.method, url=target_url, headers=headers, data=await request.read()
                ) as response:
                    # Read response
                    body = await response.read()

                    # Create response
                    resp = web.Response(body=body, status=response.status, headers=dict(response.headers))
            ...
            return web.json_response({"error": "Proxy error", "message": str(e), "region": region_name}, status=502)
...
async def health_check_handler(request):
    ...
    return web.json_response(
        {"status": "healthy", "load_balancer": "geographic", "regions": regions, "timestamp": datetime.now().isoformat()}
    )


async def status_handler(request):
    request.app["balancer"]
    ...
    return web.json_response(
        {
            ...
            "regions": {
                name: {"healthy": config["healthy"], "weight": config["weight"], "last_check": config.get("last_check")}
