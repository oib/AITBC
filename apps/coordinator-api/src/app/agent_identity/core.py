...
class AgentIdentityCore:
    ...
    async def create_identity(self, request: AgentIdentityCreate) -> AgentIdentity:
        ...
        identity = AgentIdentity(agent_id=request.agent_id, owner_address=request.owner_address.lower(), display_name=request.display_name, description=request.description, avatar_url=request.avatar_url, supported_chains=request.supported_chains, primary_chain=request.primary_chain, identity_data=request.meta_data, tags=request.tags)
    ...
    async def register_cross_chain_identity(self, identity_id: str, chain_id: int, chain_address: str, chain_type: ChainType=ChainType.ETHEREUM, wallet_address: str | None=None) -> CrossChainMapping:
        ...
        mapping = CrossChainMapping(agent_id=identity.agent_id, chain_id=chain_id, chain_type=chain_type, chain_address=chain_address.lower(), wallet_address=wallet_address.lower() if wallet_address else None)
    ...
    async def get_cross_chain_mapping(self, identity_id: str, chain_id: int) -> CrossChainMapping | None:
        ...
        if not identity:
            return None
        stmt = select(CrossChainMapping).where(CrossChainMapping.agent_id == identity.agent_id, CrossChainMapping.chain_id == chain_id)
        result = self.session.execute(stmt)
        return result.scalars().first()
    ...
    async def verify_cross_chain_identity(self, identity_id: str, chain_id: int, verifier_address: str, proof_hash: str, proof_data: dict[str, Any], verification_type: VerificationType=VerificationType.BASIC) -> IdentityVerification:
        ...
        verification = IdentityVerification(agent_id=mapping.agent_id, chain_id=chain_id, verification_type=verification_type, verifier_address=verifier_address.lower(), proof_hash=proof_hash, proof_data=proof_data)
    ...
    async def update_cross_chain_mapping(self, identity_id: str, chain_id: int, request: CrossChainMappingUpdate) -> CrossChainMapping:
    ...
    async def generate_identity_proof(self, identity_id: str, chain_id: int) -> dict[str, Any]:
        ...
        if not mapping:
            raise ValueError(f'Cross-chain mapping not found for chain {chain_id}')
        proof_data = {'identity_id': identity.id, 'agent_id': identity.agent_id, 'owner_address': identity.owner_address, 'chain_id': chain_id, 'chain_address': mapping.chain_address, 'timestamp': datetime.now(UTC).isoformat(), 'nonce': str(uuid4())}
        proof_string = json.dumps(proof_data, sort_keys=True)
        proof_hash = hashlib.sha256(proof_string.encode()).hexdigest()
        return {'proof_data': proof_data, 'proof_hash': proof_hash, 'expires_at': (datetime.now(UTC) + timedelta(hours=24)).isoformat()}