...
class CrossChainRegistry:
    ...
    async def register_cross_chain_identity(self, agent_id: str, chain_mappings: dict[int, str], verifier_address: str | None=None, verification_type: VerificationType=VerificationType.BASIC) -> dict[str, Any]:
        ...
        for chain_id, chain_address in chain_mappings.items():
            try:
                ...
                if existing:
                    ...
                    continue
                mapping = CrossChainMapping(agent_id=agent_id, chain_id=chain_id, chain_type=self._get_chain_type(chain_id), chain_address=chain_address.lower())
                self.session.add(mapping)
                self.session.commit()
                self.session.refresh(mapping)
                if verifier_address:
                    await self.verify_cross_chain_identity(identity.id, chain_id, verifier_address, self._generate_proof_hash(mapping), {'auto_verification': True}, verification_type)
                registration_results.append({'chain_id': chain_id, 'chain_address': chain_address, 'mapping_id': mapping.id, 'verified': verifier_address is not None})
        ...
        self.session.commit()
        return {'agent_id': agent_id, 'identity_id': identity.id, 'registration_results': registration_results, 'total_mappings': len([r for r in registration_results if 'error' not in r]), 'failed_mappings': len([r for r in registration_results if 'error' in r])}
    ...
    async def resolve_agent_identity_by_address(self, chain_address: str, chain_id: int) -> str | None:
        ...
        stmt = select(CrossChainMapping).where(CrossChainMapping.chain_address == chain_address.lower(), CrossChainMapping.chain_id == chain_id)
        result = self.session.execute(stmt)
        mapping = result.scalars().first()
        if not mapping:
            return None
    ...
    async def update_identity_mapping(self, agent_id: str, chain_id: int, new_address: str, verifier_address: str | None=None) -> bool:
        ...
        self.session.commit()
        if verifier_address:
            await self.verify_cross_chain_identity(await self._get_identity_id(agent_id), chain_id, verifier_address, self._generate_proof_hash(mapping), {'address_update': True, 'old_address': old_address})
        logger.info('Updated identity mapping: %s on chain %s: %s -> %s', agent_id, chain_id, old_address, new_address)
        return True

    async def verify_cross_chain_identity(self, identity_id: str, chain_id: int, verifier_address: str, proof_hash: str, proof_data: dict[str, Any], verification_type: VerificationType=VerificationType.BASIC) -> IdentityVerification:
        ...
        verification = IdentityVerification(agent_id=identity.agent_id, chain_id=chain_id, verification_type=verification_type, verifier_address=verifier_address.lower(), proof_hash=proof_hash, proof_data=proof_data, verification_result='approved', expires_at=datetime.now(UTC) + timedelta(days=30))
    ...
    async def migrate_agent_identity(self, agent_id: str, from_chain: int, to_chain: int, new_address: str, verifier_address: str | None=None) -> dict[str, Any]:
        ...
        migration_result = {'agent_id': agent_id, 'from_chain': from_chain, 'to_chain': to_chain, 'source_address': source_mapping.chain_address, 'target_address': new_address, 'migration_successful': False}
        try:
            if target_mapping:
    ...
    async def batch_verify_identities(self, verifications: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for verification_data in verifications:
            try:
                result = await self.verify_cross_chain_identity(verification_data['identity_id'], verification_data['chain_id'], verification_data['verifier_address'], verification_data['proof_hash'], verification_data.get('proof_data', {}), verification_data.get('verification_type', VerificationType.BASIC))
                results.append({'identity_id': verification_data['identity_id'], 'chain_id': verification_data['chain_id'], 'success': True, 'verification_id': result.id})
            except Exception as e:
                results.append({'identity_id': verification_data['identity_id'], 'chain_id': verification_data['chain_id'], 'success': False, 'error': str(e)})
        return results
    ...
    def _get_chain_type(self, chain_id: int) -> ChainType:
        ...
        chain_type_map = {1: ChainType.ETHEREUM, 3: ChainType.ETHEREUM, 4: ChainType.ETHEREUM, 5: ChainType.ETHEREUM, 137: ChainType.POLYGON, 80001: ChainType.POLYGON, 56: ChainType.BSC, 97: ChainType.BSC, 42161: ChainType.ARBITRUM, 421611: ChainType.ARBITRUM, 10: ChainType.OPTIMISM, 69: ChainType.OPTIMISM, 43114: ChainType.AVALANCHE, 43113: ChainType.AVALANCHE}
        return chain_type_map.get(chain_id, ChainType.CUSTOM)

    def _get_chain_name(self, chain_id: int) -> str:
        """Get chain name by chain ID"""
        chain_name_map = {1: 'Ethereum Mainnet', 3: 'Ethereum Ropsten', 4: 'Ethereum Rinkeby', 5: 'Ethereum Goerli', 137: 'Polygon Mainnet', 80001: 'Polygon Mumbai', 56: 'BSC Mainnet', 97: 'BSC Testnet', 42161: 'Arbitrum One', 421611: 'Arbitrum Testnet', 10: 'Optimism', 69: 'Optimism Testnet', 43114: 'Avalanche C-Chain', 43113: 'Avalanche Testnet'}
    ...
    def _generate_proof_hash(self, mapping: CrossChainMapping) -> str:
        ...
        proof_data = {'agent_id': mapping.agent_id, 'chain_id': mapping.chain_id, 'chain_address': mapping.chain_address, 'created_at': mapping.created_at.isoformat(), 'nonce': str(uuid4())}
    ...
    def _is_higher_verification_level(self, new_level: VerificationType, current_level: VerificationType) -> bool:
        ...
        level_hierarchy = {VerificationType.BASIC: 1, VerificationType.ADVANCED: 2, VerificationType.ZERO_KNOWLEDGE: 3, VerificationType.MULTI_SIGNATURE: 4}
