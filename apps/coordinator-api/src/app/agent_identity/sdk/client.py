...
class AgentIdentityClient:
    ...
    def __init__(self, base_url: str='http://localhost:8000/v1', api_key: str | None=None, timeout: int=30, max_retries: int=3):
        """
    ...
    async def create_identity(self, owner_address: str, chains: list[int], display_name: str='', description: str='', metadata: dict[str, Any] | None=None, tags: list[str] | None=None) -> CreateIdentityResponse:
        ...
        request_data = {'owner_address': owner_address, 'chains': chains, 'display_name': display_name, 'description': description, 'metadata': metadata or {}, 'tags': tags or []}
        response = await self._request('POST', '/agent-identity/identities', request_data)
        return CreateIdentityResponse(identity_id=response['identity_id'], agent_id=response['agent_id'], owner_address=response['owner_address'], display_name=response['display_name'], supported_chains=response['supported_chains'], primary_chain=response['primary_chain'], registration_result=response['registration_result'], wallet_results=response['wallet_results'], created_at=response['created_at'])
    ...
    async def update_identity(self, agent_id: str, updates: dict[str, Any]) -> UpdateIdentityResponse:
        ...
        return UpdateIdentityResponse(agent_id=response['agent_id'], identity_id=response['identity_id'], updated_fields=response['updated_fields'], updated_at=response['updated_at'])
    ...
    async def register_cross_chain_mappings(self, agent_id: str, chain_mappings: dict[int, str], verifier_address: str | None=None, verification_type: VerificationType=VerificationType.BASIC) -> dict[str, Any]:
        """Register cross-chain identity mappings"""
        request_data = {'chain_mappings': chain_mappings, 'verifier_address': verifier_address, 'verification_type': verification_type.value}
    ...
    async def verify_identity(self, agent_id: str, chain_id: int, verifier_address: str, proof_hash: str, proof_data: dict[str, Any], verification_type: VerificationType=VerificationType.BASIC) -> VerifyIdentityResponse:
        ...
        request_data = {'verifier_address': verifier_address, 'proof_hash': proof_hash, 'proof_data': proof_data, 'verification_type': verification_type.value}
        response = await self._request('POST', f'/agent-identity/identities/{agent_id}/cross-chain/{chain_id}/verify', request_data)
        return VerifyIdentityResponse(verification_id=response['verification_id'], agent_id=response['agent_id'], chain_id=response['chain_id'], verification_type=VerificationType(response['verification_type']), verified=response['verified'], timestamp=response['timestamp'])

    async def migrate_identity(self, agent_id: str, from_chain: int, to_chain: int, new_address: str, verifier_address: str | None=None) -> MigrationResponse:
        """Migrate agent identity from one chain to another"""
        request_data = {'from_chain': from_chain, 'to_chain': to_chain, 'new_address': new_address, 'verifier_address': verifier_address}
        response = await self._request('POST', f'/agent-identity/identities/{agent_id}/migrate', request_data)
        return MigrationResponse(agent_id=response['agent_id'], from_chain=response['from_chain'], to_chain=response['to_chain'], source_address=response['source_address'], target_address=response['target_address'], migration_successful=response['migration_successful'], action=response.get('action'), verification_copied=response.get('verification_copied'), wallet_created=response.get('wallet_created'), wallet_id=response.get('wallet_id'), wallet_address=response.get('wallet_address'), error=response.get('error'))
    ...
    async def create_wallet(self, agent_id: str, chain_id: int, owner_address: str | None=None) -> AgentWallet:
        ...
        return AgentWallet(id=response['wallet_id'], agent_id=response['agent_id'], chain_id=response['chain_id'], chain_address=response['chain_address'], wallet_type=response['wallet_type'], contract_address=response['contract_address'], balance=0.0, spending_limit=0.0, total_spent=0.0, is_active=True, permissions=[], requires_multisig=False, multisig_threshold=1, multisig_signers=[], last_transaction=None, transaction_count=0, created_at=datetime.fromisoformat(response['created_at']), updated_at=datetime.fromisoformat(response['created_at']))
    ...
    async def execute_transaction(self, agent_id: str, chain_id: int, to_address: str, amount: float, data: dict[str, Any] | None=None) -> TransactionResponse:
        """Execute a transaction from agent wallet"""
        request_data = {'to_address': to_address, 'amount': amount, 'data': data}
        response = await self._request('POST', f'/agent-identity/identities/{agent_id}/wallets/{chain_id}/transactions', request_data)
        return TransactionResponse(transaction_hash=response['transaction_hash'], from_address=response['from_address'], to_address=response['to_address'], amount=response['amount'], gas_used=response['gas_used'], gas_price=response['gas_price'], status=response['status'], block_number=response['block_number'], timestamp=response['timestamp'])

    async def get_transaction_history(self, agent_id: str, chain_id: int, limit: int=50, offset: int=0) -> list[Transaction]:
    ...
    async def search_identities(self, query: str='', chains: list[int] | None=None, status: IdentityStatus | None=None, verification_level: VerificationType | None=None, min_reputation: float | None=None, limit: int=50, offset: int=0) -> SearchResponse:
        ...
        return SearchResponse(results=response['results'], total_count=response['total_count'], query=response['query'], filters=response['filters'], pagination=response['pagination'])
    ...
    async def sync_reputation(self, agent_id: str) -> SyncReputationResponse:
        ...
        return SyncReputationResponse(agent_id=response['agent_id'], aggregated_reputation=response['aggregated_reputation'], chain_reputations=response['chain_reputations'], verified_chains=response['verified_chains'], sync_timestamp=response['sync_timestamp'])
    ...
    async def get_registry_health(self) -> RegistryHealth:
        ...
        return RegistryHealth(status=response['status'], registry_statistics=IdentityStatistics(**response['registry_statistics']), supported_chains=[ChainConfig(**chain) for chain in response['supported_chains']], cleaned_verifications=response['cleaned_verifications'], issues=response['issues'], timestamp=datetime.fromisoformat(response['timestamp']))
...
async def create_identity_with_wallets(client: AgentIdentityClient, owner_address: str, chains: list[int], display_name: str='', description: str='') -> CreateIdentityResponse:
    ...
    identity_response = await client.create_identity(owner_address=owner_address, chains=chains, display_name=display_name, description=description)
    wallet_results = identity_response.wallet_results
    failed_wallets = [w for w in wallet_results if not w.get('success', False)]
    if failed_wallets:
        logger.warning('%s wallets failed to create', len(failed_wallets))
        for wallet in failed_wallets:
            logger.warning('Chain %s: %s', wallet['chain_id'], wallet.get('error', 'Unknown error'))
    return identity_response

async def verify_identity_on_all_chains(client: AgentIdentityClient, agent_id: str, verifier_address: str, proof_data_template: dict[str, Any]) -> list[VerifyIdentityResponse]:
    """Verify identity on all supported chains"""
    mappings = await client.get_cross_chain_mappings(agent_id)
    verification_results = []
    for mapping in mappings:
        try:
            proof_data = {**proof_data_template, 'chain_id': mapping.chain_id, 'chain_address': mapping.chain_address, 'chain_type': mapping.chain_type.value}
            import hashlib
            proof_string = json.dumps(proof_data, sort_keys=True)
            proof_hash = hashlib.sha256(proof_string.encode()).hexdigest()
            result = await client.verify_identity(agent_id=agent_id, chain_id=mapping.chain_id, verifier_address=verifier_address, proof_hash=proof_hash, proof_data=proof_data)
            verification_results.append(result)
        except Exception as e:
            logger.error('Failed to verify on chain %s: %s', mapping.chain_id, e)
...
async def get_identity_summary(client: AgentIdentityClient, agent_id: str) -> dict[str, Any]:
    ...
    return {'identity': identity['identity'], 'cross_chain': identity['cross_chain'], 'wallets': wallets, 'metrics': {'total_balance': total_balance, 'total_wallets': total_wallets, 'active_wallets': active_wallets, 'wallet_activity_rate': active_wallets / max(total_wallets, 1), 'verification_rate': identity['cross_chain']['verification_rate'], 'chain_diversification': len(identity['cross_chain']['mappings'])}}