...
class EnhancedWalletAdapter(ABC):
    ...
    @abstractmethod
    async def execute_transaction(self, from_address: str, to_address: str, amount: Decimal | float | str, token_address: str | None=None, data: dict[str, Any] | None=None, gas_limit: int | None=None, gas_price: int | None=None) -> dict[str, Any]:
    ...
    @abstractmethod
    async def estimate_gas(self, from_address: str, to_address: str, amount: Decimal | float | str, token_address: str | None=None, data: dict[str, Any] | None=None) -> dict[str, Any]:
    ...
    @abstractmethod
    async def get_transaction_history(self, wallet_address: str, limit: int=100, offset: int=0, from_block: int | None=None, to_block: int | None=None) -> list[dict[str, Any]]:
...
class EthereumWalletAdapter(EnhancedWalletAdapter):
    ...
    async def get_balance(self, wallet_address: str, token_address: str | None=None) -> dict[str, Any]:
        ...
        try:
            ...
            result = {'address': wallet_address, 'chain_id': self.chain_id, 'eth_balance': eth_balance, 'token_balances': {}, 'last_updated': datetime.now(UTC).isoformat()}
            if token_address:
    ...
    async def execute_transaction(self, from_address: str, to_address: str, amount: Decimal | float | str, token_address: str | None=None, data: dict[str, Any] | None=None, gas_limit: int | None=None, gas_price: int | None=None) -> dict[str, Any]:
        ...
        try:
            ...
            transaction_data.update({'gas': hex(gas_limit), 'gasPrice': hex(gas_price), 'nonce': await self._get_nonce(from_address), 'chainId': self.chain_id})
            signed_tx = await self._sign_transaction(transaction_data, from_address)
            tx_hash = await self._send_raw_transaction(signed_tx)
            result = {'transaction_hash': tx_hash, 'from': from_address, 'to': to_address, 'amount': str(amount), 'token_address': token_address, 'gas_limit': gas_limit, 'gas_price': gas_price, 'status': TransactionStatus.PENDING.value, 'created_at': datetime.now(UTC).isoformat()}
    ...
    async def get_transaction_status(self, transaction_hash: str) -> dict[str, Any]:
        ...
        try:
            ...
            if not receipt:
                ...
                return {'transaction_hash': transaction_hash, 'status': TransactionStatus.PENDING.value, 'block_number': None, 'block_hash': None, 'gas_used': None, 'effective_gas_price': None, 'logs': [], 'created_at': datetime.now(UTC).isoformat()}
            tx_data = await self._get_transaction_by_hash(transaction_hash)
            result = {'transaction_hash': transaction_hash, 'status': TransactionStatus.COMPLETED.value if receipt['status'] == 1 else TransactionStatus.FAILED.value, 'block_number': receipt.get('blockNumber'), 'block_hash': receipt.get('blockHash'), 'gas_used': int(receipt.get('gasUsed', 0), 16), 'effective_gas_price': int(receipt.get('effectiveGasPrice', 0), 16), 'logs': receipt.get('logs', []), 'from': tx_data.get('from'), 'to': tx_data.get('to'), 'value': int(tx_data.get('value', '0x0'), 16), 'created_at': datetime.now(UTC).isoformat()}
            return result
        except Exception as e:
    ...
    async def estimate_gas(self, from_address: str, to_address: str, amount: Decimal | float | str, token_address: str | None=None, data: dict[str, Any] | None=None) -> dict[str, Any]:
        ...
        try:
            if token_address:
                ...
                amount_wei = int(float(amount) * 10 ** 18)
                call_data = {'from': from_address, 'to': to_address, 'value': hex(amount_wei), 'data': data.get('hex', '0x') if data else '0x'}
            gas_estimate = await self._estimate_gas_call(call_data)
            return {'gas_limit': int(gas_estimate, 16), 'gas_price_gwei': await self._get_gas_price_gwei(), 'estimated_cost_eth': float(int(gas_estimate, 16) * await self._get_gas_price()) / 10 ** 18, 'estimated_cost_usd': 0.0}
        except Exception as e:
    ...
    async def get_transaction_history(self, wallet_address: str, limit: int=100, offset: int=0, from_block: int | None=None, to_block: int | None=None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for tx in transactions:
                formatted_tx = {'hash': tx.get('hash'), 'from': tx.get('from'), 'to': tx.get('to'), 'value': int(tx.get('value', '0x0'), 16), 'block_number': tx.get('blockNumber'), 'timestamp': tx.get('timestamp'), 'gas_used': int(tx.get('gasUsed', '0x0'), 16), 'status': TransactionStatus.COMPLETED.value}
    ...
    async def _sign_transaction(self, transaction_data: dict[str, Any], from_address: str) -> str:
        ...
        try:
            ...
            tx_dict = {'nonce': int(transaction_data.get('nonce', 0), 16), 'gasPrice': int(transaction_data.get('gasPrice', 0), 16), 'gas': int(transaction_data.get('gas', 0), 16), 'to': transaction_data.get('to'), 'value': int(transaction_data.get('value', '0x0'), 16), 'data': transaction_data.get('data', '0x'), 'chainId': transaction_data.get('chainId', 1)}
    ...
    async def _get_wallet_transactions(self, address: str, limit: int, offset: int, from_block: int | None, to_block: int | None) -> list[dict[str, Any]]:
...
class AITBCWalletAdapter(EnhancedWalletAdapter):
    ...
    async def create_wallet(self, owner_address: str, security_config: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            wallet_data = {'address': address, 'private_key': private_key, 'chain_id': self.chain_id, 'chain_type': self.chain_type.value, 'aitbc_chain_id': self.aitbc_chain_id, 'owner_address': owner_address, 'security_level': self.security_level.value, 'created_at': datetime.now(UTC).isoformat(), 'status': WalletStatus.ACTIVE.value, 'security_config': security_config, 'nonce': 0, 'transaction_count': 0}
            encrypted_private_key = await self._encrypt_private_key(private_key, security_config)
            wallet_data['encrypted_private_key'] = encrypted_private_key
    ...
    async def get_balance(self, wallet_address: str, token_address: str | None=None) -> dict[str, Any]:
        ...
        try:
            ...
            return {'address': wallet_address, 'chain_id': self.chain_id, 'aitbc_chain_id': self.aitbc_chain_id, 'balance': balance, 'nonce': nonce, 'token_balances': {}, 'last_updated': datetime.now(UTC).isoformat()}
        except Exception as e:
            logger.error('Error getting balance for %s: %s', wallet_address, e)
            raise
    ...
    async def execute_transaction(self, from_address: str, to_address: str, amount: Decimal | float | str, token_address: str | None=None, data: dict[str, Any] | None=None, gas_limit: int | None=None, gas_price: int | None=None) -> dict[str, Any]:
        ...
        try:
            ...
            transaction_data = {'from': from_address, 'to': to_address, 'amount': amount_int, 'fee': gas_price or 10, 'nonce': await self._get_nonce(from_address), 'payload': data.get('payload', '') if data else '', 'type': data.get('type', 'transfer') if data else 'transfer', 'signature': data.get('signature', '') if data else ''}
            response = self._http_client.post('transaction', json=transaction_data)
            result = {'transaction_hash': response.get('transaction_hash', ''), 'from': from_address, 'to': to_address, 'amount': str(amount), 'fee': transaction_data['fee'], 'nonce': transaction_data['nonce'], 'status': TransactionStatus.PENDING.value, 'created_at': datetime.now(UTC).isoformat()}
    ...
    async def get_transaction_status(self, transaction_hash: str) -> dict[str, Any]:
        ...
        try:
            ...
            tx = transactions[0]
            return {'transaction_hash': transaction_hash, 'status': tx.get('status', TransactionStatus.UNKNOWN.value), 'from': tx.get('from', ''), 'to': tx.get('to', ''), 'amount': str(tx.get('amount', 0)), 'fee': tx.get('fee', 0), 'block_height': tx.get('block_height'), 'found': True}  # type: ignore[attr-defined]
        except Exception as e:
    ...
    async def get_transaction_history(self, wallet_address: str, limit: int=100, offset: int=0, from_block: int | None=None, to_block: int | None=None) -> list[dict[str, Any]]:
        try:
            ...
            formatted = []
            for tx in transactions:
                formatted.append({'hash': tx.get('hash', ''), 'from': tx.get('from', ''), 'to': tx.get('to', ''), 'value': str(tx.get('amount', 0)), 'block_number': tx.get('block_height'), 'timestamp': tx.get('timestamp'), 'fee': tx.get('fee', 0), 'status': tx.get('status', TransactionStatus.UNKNOWN.value)})  # type: ignore[attr-defined]
            return formatted
        except Exception as e:
...
class WalletAdapterFactory:
    ...
    def create_adapter(chain_id: int, rpc_url: str, security_level: SecurityLevel=SecurityLevel.MEDIUM) -> EnhancedWalletAdapter:
        ...
        chain_adapters = {1: EthereumWalletAdapter, 137: PolygonWalletAdapter, 56: BSCWalletAdapter, 42161: ArbitrumWalletAdapter, 10: OptimismWalletAdapter, 43114: AvalancheWalletAdapter, 1000: AITBCMainnetWalletAdapter, 1001: AITBCTestnetWalletAdapter}
    ...
    def get_chain_info(chain_id: int) -> dict[str, Any]:
        ...
        chain_info = {1: {'name': 'Ethereum', 'symbol': 'ETH', 'decimals': 18}, 137: {'name': 'Polygon', 'symbol': 'MATIC', 'decimals': 18}, 56: {'name': 'BSC', 'symbol': 'BNB', 'decimals': 18}, 42161: {'name': 'Arbitrum', 'symbol': 'ETH', 'decimals': 18}, 10: {'name': 'Optimism', 'symbol': 'ETH', 'decimals': 18}, 43114: {'name': 'Avalanche', 'symbol': 'AVAX', 'decimals': 18}, 1000: {'name': 'AITBC Mainnet', 'symbol': 'AIT', 'decimals': 0}, 1001: {'name': 'AITBC Testnet', 'symbol': 'AIT', 'decimals': 0}}
