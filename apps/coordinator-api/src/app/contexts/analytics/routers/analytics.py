...
@rate_limit(rate=200, per=60)
async def get_market_metrics(request: Request, period_type: AnalyticsPeriod=Query(default=AnalyticsPeriod.DAILY, description='Period type'), metric_name: str | None=Query(default=None, description='Filter by metric name'), category: str | None=Query(default=None, description='Filter by category'), geographic_region: str | None=Query(default=None, description='Filter by region'), limit: int=Query(default=50, ge=1, le=100, description='Number of results'), session: Session=Depends(get_session)) -> list[MetricResponse]:
...
@rate_limit(rate=200, per=60)
async def get_dashboard(request: Request, dashboard_id: str, session: Session=Depends(get_session)) -> DashboardResponse:
    """Get dashboard configuration"""
    try:
        dashboard = session.execute(select(DashboardConfig).where(DashboardConfig.dashboard_id == dashboard_id)).first()
        if not dashboard:
...
@rate_limit(rate=200, per=60)
async def list_dashboards(request: Request, owner_id: str | None=Query(default=None, description='Filter by owner ID'), dashboard_type: str | None=Query(default=None, description='Filter by dashboard type'), status: str | None=Query(default=None, description='Filter by status'), limit: int=Query(default=50, ge=1, le=100, description='Number of results'), session: Session=Depends(get_session)) -> list[DashboardResponse]:
...
@rate_limit(rate=20, per=60)
async def generate_report(request: Request, report_request: ReportRequest, session: Session=Depends(get_session)) -> dict[str, Any]:
    """Generate analytics report"""
    try:
        start_date = datetime.fromisoformat(report_request.start_date)
        end_date = datetime.fromisoformat(report_request.end_date)
        report = AnalyticsReport(report_id=f'report_{uuid4().hex[:8]}', report_type=report_request.report_type, title=f'{report_request.report_type.value.title()} Report', description=f'Analytics report for {report_request.period_type.value} period', period_type=report_request.period_type, start_date=start_date, end_date=end_date, filters=report_request.filters, generated_by='api', status='generated')
        session.add(report)
        session.commit()
        session.refresh(report)
        if report_request.report_type == ReportType.MARKET_OVERVIEW:
            content = await generate_market_overview_report(session, report_request.period_type, start_date, end_date, report_request.filters)
        elif report_request.report_type == ReportType.AGENT_PERFORMANCE:
            content = await generate_agent_performance_report(session, report_request.period_type, start_date, end_date, report_request.filters)
        elif report_request.report_type == ReportType.ECONOMIC_ANALYSIS:
            content = await generate_economic_analysis_report(session, report_request.period_type, start_date, end_date, report_request.filters)
        else:
        ...
        session.commit()
        return {'report_id': report.report_id, 'report_type': report.report_type.value, 'title': report.title, 'period': f'{report_request.period_type.value} from {report_request.start_date} to {report_request.end_date}', 'summary': report.summary, 'key_findings': report.key_findings, 'recommendations': report.recommendations, 'generated_at': report.generated_at.isoformat(), 'format': report_request.format}
    except Exception as e:
...
@rate_limit(rate=200, per=60)
async def get_report(request: Request, report_id: str, format: str=Query(default='json', description='Response format: json, csv, pdf'), session: Session=Depends(get_session)) -> dict[str, Any]:
    """Get generated analytics report"""
    try:
        report = session.execute(select(AnalyticsReport).where(AnalyticsReport.report_id == report_id)).first()
        if not report:
            raise HTTPException(status_code=404, detail='Report not found')
        response_data = {'report_id': report.report_id, 'report_type': report.report_type.value, 'title': report.title, 'description': report.description, 'period_type': report.period_type.value, 'start_date': report.start_date.isoformat(), 'end_date': report.end_date.isoformat(), 'summary': report.summary, 'key_findings': report.key_findings, 'recommendations': report.recommendations, 'data_sections': report.data_sections, 'charts': report.charts, 'tables': report.tables, 'generated_at': report.generated_at.isoformat(), 'status': report.status}
        if format == 'json':
...
async def get_analytics_alerts(request: Request, severity: str | None=Query(default=None, description='Filter by severity level'), status: str | None=Query(default='active', description='Filter by status'), limit: int=Query(default=20, ge=1, le=100, description='Number of results'), session: Session=Depends(get_session)) -> list[dict[str, Any]]:
    ...
    try:
        ...
        alerts = session.execute(query.order_by(AnalyticsAlert.created_at.desc()).limit(limit)).all()
        return [{'alert_id': alert.alert_id, 'rule_id': alert.rule_id, 'alert_type': alert.alert_type, 'title': alert.title, 'message': alert.message, 'severity': alert.severity, 'confidence': alert.confidence, 'trigger_value': alert.trigger_value, 'threshold_value': alert.threshold_value, 'affected_metrics': alert.affected_metrics, 'status': alert.status, 'created_at': alert.created_at.isoformat(), 'expires_at': alert.expires_at.isoformat() if alert.expires_at else None} for alert in alerts]
    except Exception as e:
...
@router.get('/kpi')
@rate_limit(rate=200, per=60)
async def get_key_performance_indicators(request: Request, period_type: AnalyticsPeriod=Query(default=AnalyticsPeriod.DAILY, description='Period type'), session: Session=Depends(get_session)) -> dict[str, Any]:
    ...
    try:
        ...
        for metric in metrics:
            if metric.metric_name in ['transaction_volume', 'active_agents', 'average_price', 'success_rate']:
                kpis[metric.metric_name] = {'value': metric.value, 'unit': metric.unit, 'change_percentage': metric.change_percentage, 'trend': 'up' if metric.change_percentage and metric.change_percentage > 0 else 'down', 'status': get_kpi_status(metric.metric_name, metric.value, metric.change_percentage)}
        return {'period_type': period_type.value, 'start_time': start_time.isoformat(), 'end_time': end_time.isoformat(), 'kpis': kpis, 'overall_health': calculate_overall_health(kpis)}
    except Exception as e:
...
async def generate_agent_performance_report(session: Session, period_type: AnalyticsPeriod, start_date: datetime, end_date: datetime, filters: dict[str, Any]) -> dict[str, Any]:
    ...
    return {'summary': f'Agent performance report for {period_type.value} period', 'key_findings': ['Top performing agents show 20% higher success rates', 'Agent retention rate improved by 5%', 'Average agent earnings increased by 10%'], 'recommendations': ['Provide additional training for lower-performing agents', 'Implement recognition programs for top performers', 'Optimize agent matching algorithms'], 'data_sections': [{'title': 'Performance Metrics', 'data': {'top_performers': 25, 'average_success_rate': 87.5, 'retention_rate': 92.0}}]}

async def generate_economic_analysis_report(session: Session, period_type: AnalyticsPeriod, start_date: datetime, end_date: datetime, filters: dict[str, Any]) -> dict[str, Any]:
    """Generate economic analysis report content"""
    return {'summary': f'Economic analysis for {period_type.value} period', 'key_findings': ['Market showed 15% growth in transaction volume', 'Price stability maintained across all regions', 'Supply/demand balance improved by 10%'], 'recommendations': ['Continue current pricing strategies', 'Focus on market expansion in high-growth regions', 'Monitor supply/demand ratios for optimization'], 'data_sections': [{'title': 'Economic Indicators', 'data': {'market_growth': 15.0, 'price_stability': 95.0, 'supply_demand_balance': 1.1}}]}
...
def convert_to_csv(data: dict[str, Any]) -> str:
    ...
    if 'kpis' in data:
        for metric_name, kpi_data in data['kpis'].items():
            csv_lines.append(f"{metric_name},{kpi_data.get('value', '')},{kpi_data.get('unit', '')},{kpi_data.get('change_percentage', '')}%,{kpi_data.get('trend', '')},{kpi_data.get('status', '')}")
    return '\n'.join(csv_lines)
