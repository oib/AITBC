...
async def blockchain_status() -> dict[str, Any]:
    ...
    try:
        ...
        return {'status': 'connected', 'height': response.get('height', 0), 'hash': response.get('hash', ''), 'timestamp': response.get('timestamp', ''), 'tx_count': response.get('tx_count', 0)}
    except NetworkError:
...
async def blockchain_sync_status() -> dict[str, Any]:
    ...
    try:
        ...
        if response.get('syncing', False):
            return {'status': 'syncing', 'current_block': response.get('current_block', 0), 'highest_block': response.get('highest_block', 0)}
        else:
