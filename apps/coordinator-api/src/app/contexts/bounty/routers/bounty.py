...
class BountyCreateRequest(BaseModel):
    ...
    def validate_reward_amount(self) -> 'BountyCreateRequest':
        tier_minimums = {BountyTier.BRONZE: 100.0, BountyTier.SILVER: 500.0, BountyTier.GOLD: 1000.0, BountyTier.PLATINUM: 5000.0}
...
async def get_bounties(request: Request, session: Session=Depends(get_session), filters: BountyFilterRequest=Depends(), bounty_service: BountyService=Depends(get_bounty_service)) -> list[BountyResponse]:
...
@rate_limit(rate=200, per=60)
async def get_bounty(request: Request, bounty_id: str, session: Session=Depends(get_session), bounty_service: BountyService=Depends(get_bounty_service)) -> BountyResponse:
    """Get bounty details"""
    try:
...
@rate_limit(rate=20, per=60)
async def submit_bounty_solution(request: Request, bounty_id: str, submission_request: BountySubmissionRequest, background_tasks: BackgroundTasks, session: Session=Depends(get_session), bounty_service: BountyService=Depends(get_bounty_service), blockchain_service: BlockchainService=Depends(get_blockchain_service), current_user: dict=Depends(get_current_user)) -> BountySubmissionResponse:
...
@rate_limit(rate=200, per=60)
async def get_bounty_submissions(request: Request, bounty_id: str, session: Session=Depends(get_session), bounty_service: BountyService=Depends(get_bounty_service), current_user: dict=Depends(get_current_user)) -> list[BountySubmissionResponse]:
...
@rate_limit(rate=20, per=60)
async def verify_bounty_submission(request: Request, bounty_id: str, verification_request: BountyVerificationRequest, background_tasks: BackgroundTasks, session: Session=Depends(get_session), bounty_service: BountyService=Depends(get_bounty_service), blockchain_service: BlockchainService=Depends(get_blockchain_service), current_user: dict=Depends(get_current_user)) -> dict[str, str]:
...
@rate_limit(rate=200, per=60)
async def get_my_created_bounties(request: Request, status: BountyStatus | None=None, page: int=Query(default=1, ge=1), limit: int=Query(default=20, ge=1, le=100), session: Session=Depends(get_session), bounty_service: BountyService=Depends(get_bounty_service), current_user: dict=Depends(get_current_user)) -> list[BountyResponse]:
...
@rate_limit(rate=200, per=60)
async def get_my_submissions(request: Request, status: SubmissionStatus | None=None, page: int=Query(default=1, ge=1), limit: int=Query(default=20, ge=1, le=100), session: Session=Depends(get_session), bounty_service: BountyService=Depends(get_bounty_service), current_user: dict=Depends(get_current_user)) -> list[BountySubmissionResponse]:
...
@rate_limit(rate=200, per=60)
async def get_bounty_leaderboard(request: Request, period: str=Query(default='weekly', pattern='^(daily|weekly|monthly)$'), limit: int=Query(default=50, ge=1, le=100), session: Session=Depends(get_session), bounty_service: BountyService=Depends(get_bounty_service)) -> dict[str, Any]:
...
@rate_limit(rate=200, per=60)
async def get_bounty_stats(request: Request, period: str=Query(default='monthly', pattern='^(daily|weekly|monthly)$'), session: Session=Depends(get_session), bounty_service: BountyService=Depends(get_bounty_service)) -> BountyStatsResponse:
...
async def get_bounty_categories(request: Request, session: Session=Depends(get_session), bounty_service: BountyService=Depends(get_bounty_service)) -> dict[str, Any]:
...
@rate_limit(rate=500, per=60)
async def get_bounty_tags(request: Request, limit: int=Query(default=100, ge=1, le=500), session: Session=Depends(get_session), bounty_service: BountyService=Depends(get_bounty_service)) -> dict[str, Any]:
...
@rate_limit(rate=200, per=60)
async def search_bounties(request: Request, query: str=Query(..., min_length=1, max_length=100), page: int=Query(default=1, ge=1), limit: int=Query(default=20, ge=1, le=100), session: Session=Depends(get_session), bounty_service: BountyService=Depends(get_bounty_service)) -> list[BountyResponse]:
