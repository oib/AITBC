...
async def get_agent_certifications(request: Request, agent_id: str, status: str | None=Query(default=None, description='Filter by status'), session: Session=Depends(get_session)) -> list[CertificationResponse]:
...
@rate_limit(rate=20, per=60)
async def create_partnership_program(request: Request, program_request: PartnershipProgramRequest, session: Session=Depends(get_session)) -> dict[str, Any]:
...
@rate_limit(rate=20, per=60)
async def apply_for_partnership(request: Request, application: PartnershipApplicationRequest, session: Session=Depends(get_session)) -> PartnershipResponse:
...
@rate_limit(rate=200, per=60)
async def get_agent_partnerships(request: Request, agent_id: str, status: str | None=Query(default=None, description='Filter by status'), partnership_type: str | None=Query(default=None, description='Filter by partnership type'), session: Session=Depends(get_session)) -> list[PartnershipResponse]:
...
@rate_limit(rate=200, per=60)
async def list_partnership_programs(request: Request, partnership_type: str | None=Query(default=None, description='Filter by partnership type'), status: str | None=Query(default='active', description='Filter by status'), limit: int=Query(default=50, ge=1, le=100, description='Number of results'), session: Session=Depends(get_session)) -> list[dict[str, Any]]:
...
@router.post('/badges')
@rate_limit(rate=20, per=60)
async def create_badge(request: Request, badge_request: BadgeCreationRequest, session: Session=Depends(get_session)) -> dict[str, Any]:
...
@rate_limit(rate=20, per=60)
async def award_badge(request: Request, badge_request: BadgeAwardRequest, session: Session=Depends(get_session)) -> BadgeResponse:
...
@rate_limit(rate=200, per=60)
async def get_agent_badges(request: Request, agent_id: str, badge_type: str | None=Query(default=None, description='Filter by badge type'), category: str | None=Query(default=None, description='Filter by category'), featured_only: bool=Query(default=False, description='Only featured badges'), limit: int=Query(default=50, ge=1, le=100, description='Number of results'), session: Session=Depends(get_session)) -> list[BadgeResponse]:
...
async def list_available_badges(request: Request, badge_type: str | None=Query(default=None, description='Filter by badge type'), category: str | None=Query(default=None, description='Filter by category'), rarity: str | None=Query(default=None, description='Filter by rarity'), active_only: bool=Query(default=True, description='Only active badges'), limit: int=Query(default=50, ge=1, le=100, description='Number of results'), session: Session=Depends(get_session)) -> list[dict[str, Any]]:
...
@rate_limit(rate=200, per=60)
async def get_verification_records(request: Request, agent_id: str, verification_type: str | None=Query(default=None, description='Filter by verification type'), status: str | None=Query(default=None, description='Filter by status'), limit: int=Query(default=20, ge=1, le=100, description='Number of results'), session: Session=Depends(get_session)) -> list[dict[str, Any]]:
...
async def get_certification_levels(request: Request, session: Session=Depends(get_session)) -> list[dict[str, Any]]:
    ...
    try:
        ...
        for level, config in certification_system.certification_levels.items():
            levels.append({'level': level.value, 'requirements': config['requirements'], 'privileges': config['privileges'], 'validity_days': config['validity_days'], 'renewal_requirements': config['renewal_requirements']})
...
@rate_limit(rate=500, per=60)
async def get_certification_requirements(request: Request, level: str | None=Query(default=None, description='Filter by certification level'), verification_type: str | None=Query(default=None, description='Filter by verification type'), session: Session=Depends(get_session)) -> list[dict[str, Any]]:
...
@rate_limit(rate=200, per=60)
async def get_certification_leaderboard(request: Request, category: str=Query(default='highest_level', description='Leaderboard category'), limit: int=Query(default=50, ge=1, le=100, description='Number of results'), session: Session=Depends(get_session)) -> list[dict[str, Any]]:
    ...
    try:
        ...
        for cert in certifications:
            if cert.agent_id not in agent_scores:
                agent_scores[cert.agent_id] = {'agent_id': cert.agent_id, 'highest_level': cert.certification_level.value, 'certification_count': 0, 'total_privileges': 0, 'latest_certification': cert.issued_at}
        ...
        if category == 'highest_level':
            sorted_agents = sorted(agent_scores.values(), key=lambda x: ['basic', 'intermediate', 'advanced', 'enterprise', 'premium'].index(x['highest_level']), reverse=True)
        elif category == 'most_certifications':
        ...
        return [{'rank': rank + 1, 'agent_id': agent['agent_id'], 'highest_level': agent['highest_level'], 'certification_count': agent['certification_count'], 'total_privileges': agent['total_privileges'], 'latest_certification': agent['latest_certification'].isoformat()} for rank, agent in enumerate(sorted_agents[:limit])]
    except Exception as e:
