...
class CrossChainBridgeService:
    ...
    async def initialize_bridge(self, chain_configs: dict[int, dict[str, Any]]) -> None:
        ...
        try:
            for chain_id, config in chain_configs.items():
                ...
                self.bridge_protocols[str(chain_id)] = {'protocol': protocol, 'enabled': config.get('enabled', True), 'min_amount': config.get('min_amount', 0.001), 'max_amount': config.get('max_amount', 1000000), 'fee_rate': config.get('fee_rate', 0.005), 'confirmation_blocks': config.get('confirmation_blocks', 12)}
    ...
    async def create_bridge_request(self, user_address: str, source_chain_id: int, target_chain_id: int, amount: Decimal | float | str, token_address: str | None=None, target_address: str | None=None, protocol: BridgeProtocol | None=None, security_level: BridgeSecurityLevel=BridgeSecurityLevel.MEDIUM, deadline_minutes: int=30) -> dict[str, Any]:
        ...
        try:
            if (source_chain_id, target_chain_id) not in self.allowed_transfers:
                ...
                raise ValueError(f'Cross-chain transfer from chain {source_chain_id} to {target_chain_id} is not permitted (chain isolation policy)')
            ...
            if transfer_key not in self.allowed_transfers:
                logger.warning('Chain isolation violation: Bridge request from chain %s to chain %s not in allowed_transfers whitelist. Rejecting cross-chain transfer for address %s', source_chain_id, target_chain_id, user_address)
                raise ValueError(f'Cross-chain transfer from chain {source_chain_id} to {target_chain_id} is not permitted (chain isolation policy)')
    ...
    async def get_bridge_request_status(self, bridge_request_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            if bridge_request.source_transaction_hash:
                source_tx = await self._get_transaction_details(bridge_request.source_chain_id, bridge_request.source_transaction_hash)
                transactions.append({'chain_id': bridge_request.source_chain_id, 'transaction_hash': bridge_request.source_transaction_hash, 'status': source_tx.get('status'), 'confirmations': await self._get_transaction_confirmations(bridge_request.source_chain_id, bridge_request.source_transaction_hash)})
            if bridge_request.target_transaction_hash:
                target_tx = await self._get_transaction_details(bridge_request.target_chain_id, bridge_request.target_transaction_hash)
                transactions.append({'chain_id': bridge_request.target_chain_id, 'transaction_hash': bridge_request.target_transaction_hash, 'status': target_tx.get('status'), 'confirmations': await self._get_transaction_confirmations(bridge_request.target_chain_id, bridge_request.target_transaction_hash)})
            progress = await self._calculate_bridge_progress(bridge_request)
            return {'bridge_request_id': bridge_request.id, 'user_address': bridge_request.user_address, 'source_chain_id': bridge_request.source_chain_id, 'target_chain_id': bridge_request.target_chain_id, 'amount': bridge_request.amount, 'token_address': bridge_request.token_address, 'target_address': bridge_request.target_address, 'protocol': bridge_request.protocol, 'status': bridge_request.status.value, 'progress': progress, 'transactions': transactions, 'bridge_fee': bridge_request.bridge_fee, 'network_fee': bridge_request.network_fee, 'total_fee': bridge_request.total_fee, 'deadline': bridge_request.deadline.isoformat(), 'created_at': bridge_request.created_at.isoformat(), 'updated_at': bridge_request.updated_at.isoformat(), 'completed_at': bridge_request.completed_at.isoformat() if bridge_request.completed_at else None}
        except Exception as e:
    ...
    async def cancel_bridge_request(self, bridge_request_id: str, reason: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {'bridge_request_id': bridge_request_id, 'status': BridgeRequestStatus.CANCELLED.value, 'reason': reason, 'cancelled_at': datetime.now(UTC).isoformat()}
        except Exception as e:
    ...
    async def get_liquidity_pools(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for chain_pair, pool in self.liquidity_pools.items():
                ...
                pool_info = {'source_chain_id': source_chain, 'target_chain_id': target_chain, 'total_liquidity': pool.get('total_liquidity', 0), 'utilization_rate': pool.get('utilization_rate', 0), 'apr': pool.get('apr', 0), 'fee_rate': pool.get('fee_rate', 0.005), 'last_updated': pool.get('last_updated', datetime.now(UTC).isoformat())}
    ...
    async def _initialize_liquidity_pool(self, chain_id: int, config: dict[str, Any]) -> None:
        ...
        try:
            ...
            self.liquidity_pools[chain_id, 1] = {'address': pool_address, 'total_liquidity': config.get('initial_liquidity', 1000000), 'utilization_rate': 0.0, 'apr': 0.05, 'fee_rate': 0.005, 'last_updated': datetime.now(UTC)}
