...
class DAOGovernanceService:
    ...
    async def create_proposal(self, request: ProposalCreate) -> DAOProposal:
        ...
        proposal = DAOProposal(proposer_address=request.proposer_address, title=request.title, description=request.description, proposal_type=request.proposal_type, target_region=request.target_region, execution_payload=request.execution_payload, start_time=start_time, end_time=end_time, status=ProposalState.ACTIVE)
    ...
    async def cast_vote(self, request: VoteCreate) -> Vote:
        ...
        existing_vote = self.session.execute(select(Vote).where(Vote.proposal_id == request.proposal_id, Vote.member_id == member.id)).first()
        if existing_vote:
        ...
        if proposal.target_region:
            ...
            weight = 1.0
        vote = Vote(proposal_id=proposal.id, member_id=member.id, support=request.support, weight=weight, tx_hash='0x_mock_vote_tx')
        if request.support:
        ...
        if proposal.for_votes > proposal.against_votes:
            ...
            if proposal.proposal_type == ProposalType.GRANT:
                ...
                if amount > 0 and recipient:
                    await self.allocate_treasury(AllocationCreate(proposal_id=proposal.id, amount=amount, recipient_address=recipient, purpose=f'Grant for proposal {proposal.title}'))
        else:
    ...
    async def allocate_treasury(self, request: AllocationCreate) -> TreasuryAllocation:
        ...
        allocation = TreasuryAllocation(proposal_id=request.proposal_id, amount=request.amount, token_symbol=request.token_symbol, recipient_address=request.recipient_address, purpose=request.purpose, tx_hash='0x_mock_treasury_tx')
