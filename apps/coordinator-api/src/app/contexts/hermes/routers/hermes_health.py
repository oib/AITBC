...
@router.post('/report')
async def report_health(health_check: HealthCheck, session: Annotated[Session, Depends(get_session)], current_user: str=Depends(require_admin_key())) -> dict:
...
@router.post('/error')
async def report_error(error_report: ErrorReport, session: Annotated[Session, Depends(get_session)], current_user: str=Depends(require_admin_key())) -> dict:
