...
async def register_resource(resource: Resource, session: Annotated[Session, Depends(get_session)], current_user: str=Depends(require_admin_key())) -> dict:
...
async def allocate_resource(allocation_request: ResourceAllocationRequest, session: Annotated[Session, Depends(get_session)], current_user: str=Depends(require_admin_key())) -> ResourceAllocationResponse:
...
async def release_resource(release_request: ResourceReleaseRequest, session: Annotated[Session, Depends(get_session)], current_user: str=Depends(require_admin_key())) -> ResourceReleaseResponse:
...
async def adjust_pricing(session: Annotated[Session, Depends(get_session)], current_user: str=Depends(require_admin_key())) -> PricingAdjustment | None:
...
async def get_resource_pools(session: Annotated[Session, Depends(get_session)], current_user: str=Depends(require_admin_key())) -> list[ResourcePool]:
...
async def get_allocations(session: Annotated[Session, Depends(get_session)], current_user: str=Depends(require_admin_key()), agent_id: str | None=None) -> list[dict]:
