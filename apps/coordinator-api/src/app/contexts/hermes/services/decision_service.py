...
class DecisionService:
    ...
    def propose_decision(self, proposal: DecisionProposal, session: Session) -> DecisionProposalResponse:
        ...
        decision = {'decision_id': decision_id, 'decision_type': proposal.decision_type, 'title': proposal.title, 'description': proposal.description, 'proposed_by': proposal.proposed_by, 'created_at': datetime.utcnow(), 'voting_deadline': proposal.voting_deadline, 'min_participation': proposal.min_participation, 'required_approval': proposal.required_approval, 'status': DecisionStatus.PENDING, 'metadata': proposal.metadata or {}}
        self.decisions[decision_id] = decision
        self.votes[decision_id] = []
        logger.info('Decision proposal created: %s - %s', decision_id, proposal.title)
        return DecisionProposalResponse(decision_id=decision_id, status=DecisionStatus.PENDING, created_at=decision['created_at'], voting_deadline=decision['voting_deadline'], message='Decision proposal created successfully')
    ...
    def submit_vote(self, vote: Vote, session: Session) -> VoteResponse:
        ...
        vote_record = {'vote_id': vote_id, 'agent_id': vote.agent_id, 'vote': vote.vote, 'weight': vote.weight, 'reason': vote.reason, 'voted_at': datetime.utcnow()}
        ...
        return VoteResponse(vote_id=vote_id, decision_id=vote.decision_id, status='success', message='Vote submitted successfully')
    ...
    def get_decision_result(self, decision_id: str, session: Session) -> DecisionResult | None:
        ...
        if now > deadline:
            ...
            concluded_at = now
        return DecisionResult(decision_id=decision_id, status=decision['status'], total_votes=total_votes, approve_votes=approve_votes, reject_votes=reject_votes, abstain_votes=abstain_votes, weighted_approve=weighted_approve, weighted_reject=weighted_reject, weighted_abstain=weighted_abstain, participation_rate=participation_rate, approval_rate=approval_rate, final_decision=final_decision, concluded_at=concluded_at)

    def list_decisions(self, session: Session, decision_type: DecisionType | None=None, status: DecisionStatus | None=None) -> list[DecisionResult]:
