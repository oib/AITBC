...
class HealthService:
    ...
    def report_health(self, health_check: HealthCheck, session: Session) -> str:
        ...
        logger.info('Health report: %s - %s - %s (%sms)', health_check.agent_id, health_check.service_name, health_check.status, health_check.response_time_ms)
    ...
    def report_error(self, error_report: ErrorReport, session: Session) -> str:
        ...
        logger.warning('Error reported: %s - %s - %s (%s)', error_report.agent_id, error_report.service_name, error_report.error_type, error_report.severity)
    ...
    def _determine_recovery_actions(self, health_check: HealthCheck) -> list[RecoveryAction]:
        ...
        if health_check.status == HealthStatus.UNHEALTHY:
            actions.append(RecoveryAction(action_type='restart_service', description=f'Restart service {health_check.service_name}', parameters={'service': health_check.service_name}))
        elif health_check.status == HealthStatus.DEGRADED:
            if health_check.response_time_ms > 1000:
                actions.append(RecoveryAction(action_type='optimize_resources', description='Optimize resource allocation', parameters={'target_response_time': 500}))
        return actions
    ...
    def _determine_error_recovery_actions(self, error_report: ErrorReport) -> list[RecoveryAction]:
        ...
        if error_report.error_type == ErrorType.NETWORK_ERROR:
            actions.append(RecoveryAction(action_type='retry_with_backoff', description='Retry operation with exponential backoff', parameters={'max_retries': 3, 'initial_delay': 1000}))
        elif error_report.error_type == ErrorType.TIMEOUT_ERROR:
            actions.append(RecoveryAction(action_type='increase_timeout', description='Increase timeout threshold', parameters={'timeout_multiplier': 2.0}))
        elif error_report.error_type == ErrorType.RESOURCE_ERROR:
            actions.append(RecoveryAction(action_type='allocate_more_resources', description='Allocate additional resources', parameters={'resource_type': 'memory', 'increase_percent': 50}))
        elif error_report.error_type == ErrorType.SERVICE_UNAVAILABLE:
            actions.append(RecoveryAction(action_type='restart_service', description='Restart unavailable service', parameters={'service': error_report.service_name}))
        elif error_report.error_type == ErrorType.DATABASE_ERROR:
            actions.append(RecoveryAction(action_type='reconnect_database', description='Reconnect to database', parameters={}))
        return actions
    ...
    def _execute_recovery_action(self, action: RecoveryAction, agent_id: str, session: Session) -> RecoveryResult:
        ...
        return RecoveryResult(action_id=action_id, agent_id=agent_id, success=success, message=message, timestamp=datetime.utcnow())
