...
class HealthService:
    ...
    def report_error(self, error_report: ErrorReport, session: Session) -> str:
        ...
        error_record = ErrorReportModel(id=str(uuid.uuid4()), agent_id=error_report.agent_id, service_name=error_report.service_name, error_type=error_report.error_type, severity=error_report.severity, error_message=error_report.error_message, timestamp=error_report.timestamp or datetime.utcnow(), context=json.dumps(error_report.context or {}))
    ...
    def get_health_status(self, session: Session, agent_id: str | None=None, service_name: str | None=None) -> list[HealthCheck]:
        ...
        return [HealthCheck(agent_id=r.agent_id, service_name=r.service_name, status=r.status, timestamp=r.timestamp, response_time_ms=r.response_time_ms, error_message=r.error_message, metadata=json.loads(r.meta_data) if r.meta_data else {}) for r in results]
    ...
    def get_recovery_history(self, session: Session, agent_id: str | None=None, limit: int=100) -> list[RecoveryResult]:
        ...
        return [RecoveryResult(action_id=str(r.action_id), agent_id=r.agent_id, success=r.success == 'true', message=r.message, timestamp=r.timestamp) for r in results]
    ...
    def _trigger_recovery(self, agent_id: str, service_name: str, session: Session) -> None:
        ...
        recovery_record = RecoveryResultModel(id=str(uuid.uuid4()), agent_id=agent_id, action_id=str(action_id), success='true' if success else 'false', message=message, timestamp=datetime.utcnow())
