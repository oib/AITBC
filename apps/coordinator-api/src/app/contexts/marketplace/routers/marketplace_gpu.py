...
def _gpu_to_dict(gpu: GPURegistry) -> dict[str, Any]:
    return {'id': gpu.id, 'miner_id': gpu.miner_id, 'model': gpu.model, 'memory_gb': gpu.memory_gb, 'cuda_version': gpu.cuda_version, 'region': gpu.region, 'price_per_hour': gpu.price_per_hour, 'status': gpu.status, 'capabilities': gpu.capabilities, 'created_at': gpu.created_at.isoformat() + 'Z', 'average_rating': gpu.average_rating, 'total_reviews': gpu.total_reviews}
...
async def register_gpu(request: dict[str, Any], session: Annotated[Session, Depends(get_session)]) -> dict[str, Any]:
    ...
    gpu_record = GPURegistry(id=gpu_id, miner_id=miner_id, model=gpu_specs.get('name', 'Unknown GPU'), memory_gb=gpu_specs.get('memory_gb', 0), cuda_version=cuda_version, region='default', price_per_hour=gpu_specs.get('price_per_hour', 0.05), status='available', capabilities=[], average_rating=0.0, total_reviews=0, created_at=datetime.now(UTC))
    session.add(gpu_record)
    session.commit()
    session.refresh(gpu_record)
    return {'gpu_id': gpu_id, 'status': 'registered', 'message': f"GPU {gpu_specs.get('name', 'Unknown GPU')} registered successfully", 'price_per_hour': gpu_specs.get('price_per_hour', 0.05)}

@router.get('/marketplace/gpu/list')
async def list_gpus(session: Annotated[Session, Depends(get_session)], available: bool | None=Query(default=None), price_max: float | None=Query(default=None), region: str | None=Query(default=None), model: str | None=Query(default=None), limit: int=Query(default=100, ge=1, le=500)) -> list[dict[str, Any]]:
...
async def get_gpu_details(gpu_id: str, session: Annotated[Session, Depends(get_session)]) -> dict[str, Any]:
    ...
    if gpu.status == 'booked':
        booking = session.execute(select(GPUBooking).where(GPUBooking.gpu_id == gpu_id, GPUBooking.status == 'active').limit(1)).first()
        if booking:
            result['current_booking'] = {'booking_id': booking.id, 'duration_hours': booking.duration_hours, 'total_cost': booking.total_cost, 'start_time': booking.start_time.isoformat() + 'Z', 'end_time': booking.end_time.isoformat() + 'Z' if booking.end_time else None}
    return result

@router.post('/marketplace/gpu/purchase')
async def buy_gpu(request: GPUBuyRequest, session: Annotated[Session, Depends(get_session)], engine: DynamicPricingEngine=Depends(get_pricing_engine)) -> dict[str, Any]:
    ...
    if gpu.status != 'available':
        raise HTTPException(status_code=http_status.HTTP_409_CONFLICT, detail=f'GPU {request.gpu_id} is not available for purchase')
    ...
    booking = GPUBooking(id=booking_id, gpu_id=request.gpu_id, client_id=request.buyer_id, job_id=f'purchase_{request.buyer_id[:8]}', duration_hours=request.duration_hours, total_cost=total_cost, start_time=start_time, end_time=end_time, status='active')
    gpu.status = 'booked'
    session.add(booking)
    session.add(gpu)
    session.commit()
    ...
    try:
        ...
        job_create = JobCreate(payload={'type': 'gpu_compute', 'gpu_id': request.gpu_id, 'task': 'general_compute', 'duration_hours': request.duration_hours}, constraints=Constraints(gpu=gpu.model, region=gpu.region, min_vram_gb=gpu.memory_gb if gpu.memory_gb else None, max_price=current_price * 1.1), ttl_seconds=int(request.duration_hours * 3600), payment_amount=total_cost, payment_currency='AITBC')
        ...
        try:
            ...
            payment_create = JobPaymentCreate(job_id=job.id, amount=total_cost, currency='AITBC', payment_method='aitbc_token' if request.payment_method == 'blockchain' else request.payment_method, escrow_timeout_seconds=int(request.duration_hours * 3600))
    ...
    return {'purchase_id': booking_id, 'gpu_id': request.gpu_id, 'buyer_id': request.buyer_id, 'job_id': job_id, 'payment_id': payment_id, 'duration_hours': request.duration_hours, 'total_cost': total_cost, 'price_per_hour': current_price, 'status': 'purchased', 'payment_method': request.payment_method, 'payment_status': payment_status, 'start_time': start_time.isoformat() + 'Z', 'end_time': end_time.isoformat() + 'Z'}
...
async def sell_gpu(request: GPUSellRequest, session: Annotated[Session, Depends(get_session)]) -> dict[str, Any]:
    ...
    session.commit()
    session.refresh(gpu)
    return {'gpu_id': request.gpu_id, 'seller_id': request.seller_id, 'listing_price': request.listing_price, 'status': 'listed', 'description': request.description, 'timestamp': datetime.now(UTC).isoformat() + 'Z'}

@router.post('/marketplace/gpu/{gpu_id}/book', status_code=http_status.HTTP_201_CREATED)
async def book_gpu(gpu_id: str, request: GPUBookRequest, session: Annotated[Session, Depends(get_session)], engine: DynamicPricingEngine=Depends(get_pricing_engine)) -> dict[str, Any]:
    ...
    if request.duration_hours <= 0:
        raise HTTPException(status_code=http_status.HTTP_400_BAD_REQUEST, detail='Booking duration must be greater than 0 hours')
    if request.duration_hours > 8760:
        raise HTTPException(status_code=http_status.HTTP_400_BAD_REQUEST, detail='Booking duration cannot exceed 8760 hours (1 year)')
    start_time = datetime.now(UTC)
    end_time = start_time + timedelta(hours=request.duration_hours)
    if end_time <= start_time:
        raise HTTPException(status_code=http_status.HTTP_400_BAD_REQUEST, detail='Booking end time must be in the future')
    try:
        dynamic_result = await engine.calculate_dynamic_price(resource_id=gpu_id, resource_type=ResourceType.GPU, base_price=gpu.price_per_hour, strategy=PricingStrategy.MARKET_BALANCE, region=gpu.region)
        current_price = dynamic_result.recommended_price
    except Exception:
        current_price = gpu.price_per_hour
    total_cost = request.duration_hours * current_price
    booking = GPUBooking(gpu_id=gpu_id, job_id=request.job_id, duration_hours=request.duration_hours, total_cost=total_cost, start_time=start_time, end_time=end_time, status='active')
    gpu.status = 'booked'
    session.add(booking)
    session.commit()
    session.refresh(booking)
...
async def confirm_gpu_booking(gpu_id: str, request: GPUConfirmRequest, session: Session=Depends(get_session)) -> dict[str, Any]:
    ...
    booking = session.execute(select(GPUBooking).where(GPUBooking.gpu_id == gpu_id, GPUBooking.status == 'active').limit(1)).scalar_one_or_none()
    if not booking:
    ...
    if request.client_id:
        ...
        session.commit()
        session.refresh(booking)
    return {'status': 'confirmed', 'gpu_id': gpu_id, 'booking_id': booking.id, 'client_id': booking.client_id, 'message': f'Booking confirmed for GPU {gpu_id}'}
...
async def submit_ollama_task(request: OllamaTaskRequest, session: Session=Depends(get_session)) -> dict[str, Any]:
    ...
    return {'task_id': task_id, 'status': 'submitted', 'submitted_at': submitted_at, 'gpu_id': request.gpu_id, 'model': request.model, 'prompt': request.prompt, 'parameters': request.parameters}
...
async def send_payment(request: PaymentRequest, session: Session=Depends(get_session)) -> dict[str, Any]:
    ...
    return {'tx_id': tx_id, 'status': 'processed', 'processed_at': processed_at, 'from': request.from_wallet, 'to': request.to_wallet, 'amount': request.amount, 'booking_id': request.booking_id, 'task_id': request.task_id}

@router.delete('/marketplace/gpu/{gpu_id}')
async def delete_gpu(gpu_id: str, session: Annotated[Session, Depends(get_session)], force: bool=Query(default=False, description='Force delete even if GPU is booked')) -> dict[str, Any]:
    """Delete (unregister) a GPU from the marketplace."""
    gpu = _get_gpu_or_404(session, gpu_id)
    if gpu.status == 'booked' and (not force):
        raise HTTPException(status_code=http_status.HTTP_409_CONFLICT, detail=f'GPU {gpu_id} is currently booked. Use force=true to delete anyway.')
    session.delete(gpu)
    session.commit()
...
async def add_gpu_review(gpu_id: str, request: GPUReviewRequest, session: Annotated[Session, Depends(get_session)]) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error('Failed to add review for GPU %s: %s', gpu_id, str(e), extra={'gpu_id': gpu_id, 'exc': str(e), 'error_type': type(e).__name__})
        try:
            session.rollback()
...
async def list_orders(session: Annotated[Session, Depends(get_session)], status: str | None=Query(default=None), limit: int=Query(default=100, ge=1, le=500)) -> list[dict[str, Any]]:
    ...
    for b in bookings:
        ...
        orders.append({'order_id': b.id, 'gpu_id': b.gpu_id, 'gpu_model': gpu.model if gpu else 'unknown', 'miner_id': gpu.miner_id if gpu else '', 'duration_hours': b.duration_hours, 'total_cost': b.total_cost, 'status': b.status, 'created_at': b.start_time.isoformat() + 'Z', 'job_id': b.job_id})
    return orders

@router.get('/marketplace/pricing/{model}')
async def get_pricing(model: str, session: Annotated[Session, Depends(get_session)], engine: DynamicPricingEngine=Depends(get_pricing_engine), collector: MarketDataCollector=Depends(get_market_collector)) -> dict[str, Any]:
...
async def bid_gpu(request: dict[str, Any], session: Session=Depends(get_session)) -> dict[str, Any]:
    ...
    bid_id = str(uuid4())
    return {'bid_id': bid_id, 'status': 'placed', 'gpu_id': request.get('gpu_id'), 'bid_amount': request.get('bid_amount'), 'duration_hours': request.get('duration_hours'), 'timestamp': datetime.now(UTC).isoformat() + 'Z'}