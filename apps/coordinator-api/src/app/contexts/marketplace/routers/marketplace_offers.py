...
async def sync_offers(session: Annotated[Session, Depends(get_session)], admin_key: str=Depends(require_admin_key())) -> dict[str, Any]:
    ...
    for miner in miners:
        ...
        if not existing:
            ...
            offer = MarketplaceOffer(provider=miner.id, capacity=miner.concurrency or 1, price=capabilities.get('pricing_per_hour', 0.5), gpu_model=capabilities.get('gpu', None), gpu_memory_gb=capabilities.get('gpu_memory_gb', None), gpu_count=capabilities.get('gpu_count', 1), cuda_version=capabilities.get('cuda_version', None), price_per_hour=capabilities.get('pricing_per_hour', 0.5), region=miner.region or None, attributes={'supported_models': capabilities.get('supported_models', [])})
            session.add(offer)
...
async def list_all_offers(session: Annotated[Session, Depends(get_session)]) -> list[dict[str, Any]]:
    ...
    try:
        ...
        for offer in offers:
            attrs = offer.attributes or {}
            offer_data = {'id': offer.id, 'provider': offer.provider, 'capacity': offer.capacity, 'price': offer.price, 'status': offer.status, 'created_at': offer.created_at.isoformat(), 'gpu_model': attrs.get('gpu_model', 'Unknown'), 'gpu_memory_gb': attrs.get('gpu_memory_gb', 0), 'cuda_version': attrs.get('cuda_version', 'Unknown'), 'supported_models': attrs.get('supported_models', []), 'region': attrs.get('region', 'unknown')}
