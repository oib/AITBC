...
class PluginManager:
    ...
    def __init__(self) -> None:
        ...
        self.plugin_hooks: dict[str, list[Callable]] = {'before_booking': [], 'after_booking': [], 'before_pricing': [], 'after_pricing': []}

    def register_plugin(self, plugin_name: str, plugin_version: str='1.0.0', plugin_type: str='extension', config: dict[str, Any] | None=None, description: str='', author: str='') -> Plugin:
        """Register a new plugin."""
        if plugin_name in self.plugins:
            logger.warning('Plugin %s already registered, updating', plugin_name)
        plugin = Plugin(plugin_name=plugin_name, plugin_version=plugin_version, plugin_type=plugin_type, config=config or {}, description=description, author=author)
