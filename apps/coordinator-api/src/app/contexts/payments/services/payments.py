...
class PaymentService:
    ...
    async def create_payment(self, job_id: str, payment_data: JobPaymentCreate) -> JobPayment:
        ...
        try:
            payment = JobPayment(job_id=job_id, amount=payment_data.amount, currency=payment_data.currency, payment_method=payment_data.payment_method, expires_at=datetime.now(UTC) + timedelta(seconds=payment_data.escrow_timeout_seconds))
    ...
    async def _create_token_escrow(self, payment: JobPayment) -> PaymentEscrow | None:
        ...
        try:
            ...
            response = client.post(f'{self.exchange_base_url}/api/v1/token/escrow/create', json={'amount': float(payment.amount), 'currency': payment.currency, 'job_id': payment.job_id, 'timeout_seconds': 3600})
            escrow_data = response
            payment.escrow_address = escrow_data.get('escrow_id')
            payment.status = 'escrowed'
            payment.escrowed_at = datetime.now(UTC)
            payment.updated_at = datetime.now(UTC)
            escrow = PaymentEscrow(payment_id=payment.id, amount=payment.amount, currency=payment.currency, address=escrow_data.get('escrow_id'), expires_at=datetime.now(UTC) + timedelta(hours=1))
            if escrow is not None:
                self.session.add(escrow)
            self.session.commit()
    ...
    async def _create_bitcoin_escrow(self, payment: JobPayment) -> PaymentEscrow | None:
        ...
        try:
            ...
            try:
                escrow_data = client.post(f'{self.wallet_base_url}/api/v1/escrow/create', json={'amount': float(payment.amount), 'currency': payment.currency, 'timeout_seconds': 3600})
                payment.escrow_address = escrow_data['address']
                payment.status = 'escrowed'
                payment.escrowed_at = datetime.now(UTC)
                payment.updated_at = datetime.now(UTC)
                escrow = PaymentEscrow(payment_id=payment.id, amount=payment.amount, currency=payment.currency, address=escrow_data['address'], expires_at=datetime.now(UTC) + timedelta(hours=1))
                if escrow is not None:
                    self.session.add(escrow)
                self.session.commit()
    ...
    async def release_payment(self, job_id: str, payment_id: str, reason: str | None=None) -> bool:
        ...
        try:
            client = AITBCHTTPClient(timeout=30.0)
            try:
                release_data = client.post(f'{self.wallet_base_url}/api/v1/escrow/release', json={'address': payment.escrow_address, 'reason': reason or 'Job completed successfully'})
    ...
    async def refund_payment(self, job_id: str, payment_id: str, reason: str) -> bool:
        ...
        try:
            client = AITBCHTTPClient(timeout=30.0)
            try:
                refund_data = client.post(f'{self.wallet_base_url}/api/v1/refund', json={'payment_id': payment_id, 'address': payment.refund_address, 'amount': float(payment.amount), 'reason': reason})
