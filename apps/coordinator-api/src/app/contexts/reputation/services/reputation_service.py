...
class TrustScoreCalculator:
    ...
    def __init__(self) -> None:
        self.weights = {TrustScoreCategory.PERFORMANCE: 0.35, TrustScoreCategory.RELIABILITY: 0.25, TrustScoreCategory.COMMUNITY: 0.2, TrustScoreCategory.SECURITY: 0.1, TrustScoreCategory.ECONOMIC: 0.1}
        self.decay_factors = {'daily': 0.95, 'weekly': 0.9, 'monthly': 0.8, 'yearly': 0.6}

    def calculate_performance_score(self, agent_id: str, session: Session, time_window: timedelta=timedelta(days=30)) -> float:
    ...
    def calculate_community_score(self, agent_id: str, session: Session, time_window: timedelta=timedelta(days=90)) -> float:
        ...
        cutoff_date = datetime.now(UTC) - time_window
        feedback_query = select(CommunityFeedback).where(and_(CommunityFeedback.agent_id == agent_id, CommunityFeedback.created_at >= cutoff_date, CommunityFeedback.moderation_status == 'approved'))
...
class ReputationService:
    ...
    async def update_trust_score(self, agent_id: str, event_type: str, impact_data: dict[str, Any]) -> AgentReputation:
        ...
        event = ReputationEvent(agent_id=agent_id, event_type=event_type, impact_score=new_trust_score - old_trust_score, trust_score_before=old_trust_score, trust_score_after=new_trust_score, reputation_level_before=old_reputation_level, reputation_level_after=new_reputation_level, event_data=impact_data, occurred_at=datetime.now(UTC), processed_at=datetime.now(UTC))
        ...
        history_entry = {'timestamp': datetime.now(UTC).isoformat(), 'event_type': event_type, 'trust_score_change': new_trust_score - old_trust_score, 'new_trust_score': new_trust_score, 'reputation_level': new_reputation_level.value}
    ...
    async def record_job_completion(self, agent_id: str, job_id: str, success: bool, response_time: float, earnings: float) -> AgentReputation:
        ...
        if reputation.average_response_time == 0:
            ...
            reputation.average_response_time = (reputation.average_response_time * reputation.jobs_completed + response_time) / (reputation.jobs_completed + 1)
        ...
        reputation.updated_at = datetime.now(UTC)
        reputation.last_activity = datetime.now(UTC)
        impact_data = {'job_id': job_id, 'success': success, 'response_time': response_time, 'earnings': earnings, 'total_jobs': total_jobs, 'success_rate': reputation.success_rate}
    ...
    async def _update_community_rating(self, agent_id: str) -> None:
        ...
        feedbacks = self.session.execute(select(CommunityFeedback).where(and_(CommunityFeedback.agent_id == agent_id, CommunityFeedback.moderation_status == 'approved'))).all()
        if not feedbacks:
            return
        total_weight = 0.0
        weighted_sum = 0.0
    ...
    async def get_leaderboard(self, category: str='trust_score', limit: int=50, region: str | None=None) -> list[dict[str, Any]]:
        ...
        for rank, reputation in enumerate(reputations, 1):
            leaderboard.append({'rank': rank, 'agent_id': reputation.agent_id, 'trust_score': reputation.trust_score, 'reputation_level': reputation.reputation_level.value, 'performance_rating': reputation.performance_rating, 'reliability_score': reputation.reliability_score, 'community_rating': reputation.community_rating, 'total_earnings': reputation.total_earnings, 'transaction_count': reputation.transaction_count, 'geographic_region': reputation.geographic_region, 'specialization_tags': reputation.specialization_tags})
        return leaderboard
