...
async def calculate_and_distribute_reward(request: Request, reward_request: RewardRequest, session: Session=Depends(get_session)) -> RewardResponse:
    ...
    try:
        ...
        result = await reward_engine.calculate_and_distribute_reward(agent_id=reward_request.agent_id, reward_type=reward_request.reward_type, base_amount=reward_request.base_amount, performance_metrics=reward_request.performance_metrics, reference_date=reference_date)
        return RewardResponse(calculation_id=result['calculation_id'], distribution_id=result['distribution_id'], reward_amount=result['reward_amount'], reward_type=result['reward_type'], tier_multiplier=result['tier_multiplier'], total_bonus=result['total_bonus'], status=result['status'])
    except Exception as e:
...
async def get_tier_progress(request: Request, agent_id: str, session: Session=Depends(get_session)) -> TierProgressResponse:
    ...
    try:
        ...
        requirements_met = {'minimum_trust_score': trust_score >= 400, 'minimum_performance': profile.performance_score >= 3.0, 'minimum_activity': profile.rewards_distributed >= 1, 'minimum_earnings': profile.total_earnings >= 0.1}
        tier_benefits = {'max_concurrent_jobs': 1, 'priority_boost': 1.0, 'fee_discount': 0.0, 'support_level': 'basic'}
        if current_tier == RewardTier.SILVER:
            tier_benefits.update({'max_concurrent_jobs': 2, 'priority_boost': 1.1, 'fee_discount': 5.0, 'support_level': 'priority'})
        elif current_tier == RewardTier.GOLD:
            tier_benefits.update({'max_concurrent_jobs': 3, 'priority_boost': 1.2, 'fee_discount': 10.0, 'support_level': 'priority'})
        elif current_tier == RewardTier.PLATINUM:
            tier_benefits.update({'max_concurrent_jobs': 5, 'priority_boost': 1.5, 'fee_discount': 15.0, 'support_level': 'premium'})
        elif current_tier == RewardTier.DIAMOND:
            tier_benefits.update({'max_concurrent_jobs': 10, 'priority_boost': 2.0, 'fee_discount': 20.0, 'support_level': 'premium'})
        return TierProgressResponse(agent_id=agent_id, current_tier=current_tier.value, next_tier=next_tier.value if next_tier else None, tier_progress=profile.tier_progress, trust_score=trust_score, requirements_met=requirements_met, benefits=tier_benefits)
    except HTTPException:
        raise
    except Exception as e:
...
async def get_reward_analytics(request: Request, period_type: str=Query(default='daily', description='Period type: daily, weekly, monthly'), start_date: str | None=Query(default=None, description='Start date (ISO format)'), end_date: str | None=Query(default=None, description='End date (ISO format)'), session: Session=Depends(get_session)) -> RewardAnalyticsResponse:
    ...
    try:
        ...
        analytics_data = await reward_engine.get_reward_analytics(period_type=period_type, start_date=start_dt, end_date=end_dt)
...
@rate_limit(rate=200, per=60)
async def get_reward_leaderboard(request: Request, tier: str | None=Query(default=None, description='Filter by tier'), period: str=Query(default='weekly', description='Period: daily, weekly, monthly'), limit: int=Query(default=50, ge=1, le=100, description='Number of results'), session: Session=Depends(get_session)) -> list[dict[str, Any]]:
...
@rate_limit(rate=200, per=60)
async def get_agent_milestones(request: Request, agent_id: str, include_completed: bool=Query(default=True, description='Include completed milestones'), session: Session=Depends(get_session)) -> list[MilestoneResponse]:
    ...
    try:
        ...
        return [MilestoneResponse(id=milestone.id, agent_id=milestone.agent_id, milestone_type=milestone.milestone_type, milestone_name=milestone.milestone_name, target_value=milestone.target_value, current_value=milestone.current_value, progress_percentage=milestone.progress_percentage, reward_amount=milestone.reward_amount, is_completed=milestone.is_completed, is_claimed=milestone.is_claimed, completed_at=milestone.completed_at.isoformat() if milestone.completed_at else None, claimed_at=milestone.claimed_at.isoformat() if milestone.claimed_at else None) for milestone in milestones]
    except Exception as e:
...
@rate_limit(rate=200, per=60)
async def get_reward_distributions(request: Request, agent_id: str, limit: int=Query(default=20, ge=1, le=100), status: str | None=Query(default=None, description='Filter by status'), session: Session=Depends(get_session)) -> list[dict[str, Any]]:
    ...
    try:
        ...
        return [{'id': distribution.id, 'reward_amount': distribution.reward_amount, 'reward_type': distribution.reward_type.value, 'status': distribution.status.value, 'distribution_method': distribution.distribution_method, 'transaction_id': distribution.transaction_id, 'transaction_status': distribution.transaction_status, 'created_at': distribution.created_at.isoformat(), 'processed_at': distribution.processed_at.isoformat() if distribution.processed_at else None, 'error_message': distribution.error_message} for distribution in distributions]
    except Exception as e:
