...
class EncryptedData:
    ...
    def __init__(self, ciphertext: bytes, encrypted_keys: dict[str, bytes], algorithm: str='AES-256-GCM+X25519', nonce: bytes | None=None, tag: bytes | None=None):
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {'ciphertext': base64.b64encode(self.ciphertext).decode(), 'encrypted_keys': {participant: base64.b64encode(key).decode() for participant, key in self.encrypted_keys.items()}, 'algorithm': self.algorithm, 'nonce': base64.b64encode(self.nonce).decode() if self.nonce else None, 'tag': base64.b64encode(self.tag).decode() if self.tag else None}

    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> 'EncryptedData':
        """Create from dictionary"""
        return cls(ciphertext=base64.b64decode(data['ciphertext']), encrypted_keys={participant: base64.b64decode(key) for participant, key in data['encrypted_keys'].items()}, algorithm=data['algorithm'], nonce=base64.b64decode(data['nonce']) if data.get('nonce') else None, tag=base64.b64decode(data['tag']) if data.get('tag') else None)
...
class EncryptionService:
    ...
    def encrypt(self, data: dict[str, Any], participants: list[str], include_audit: bool=True) -> EncryptedData:
        ...
        try:
            ...
            return EncryptedData(ciphertext=actual_ciphertext, encrypted_keys=encrypted_keys, algorithm=self.algorithm, nonce=nonce, tag=tag)
        except Exception as e:
    ...
    def _encrypt_dek(self, dek: bytes, public_key: X25519PublicKey) -> bytes:
        ...
        derived_key = HKDF(algorithm=hashes.SHA256(), length=32, salt=None, info=b'AITBC-DEK-Encryption', backend=self.backend).derive(shared_key)
