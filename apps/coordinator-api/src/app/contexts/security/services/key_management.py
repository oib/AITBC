...
class KeyManager:
    ...
    async def generate_key_pair(self, participant_id: str) -> KeyPair:
        ...
        try:
            ...
            key_pair = KeyPair(participant_id=participant_id, private_key=private_key.private_bytes_raw(), public_key=public_key.public_bytes_raw(), algorithm='X25519', created_at=datetime.now(UTC), version=1)
            await self.storage.store_key_pair(key_pair)
            self._key_cache[participant_id] = {'public_key': public_key, 'version': key_pair.version}
            logger.info('Generated key pair for participant: %s', participant_id)
            return key_pair
    ...
    async def rotate_keys(self, participant_id: str) -> KeyPair:
        ...
        try:
            ...
            self._key_cache[participant_id] = {'public_key': X25519PublicKey.from_public_bytes(new_key_pair.public_key), 'version': new_key_pair.version}
            rotation_log = KeyRotationLog(participant_id=participant_id, old_version=current_key.version, new_version=new_key_pair.version, rotated_at=datetime.now(UTC), reason='scheduled_rotation')
    ...
    async def create_audit_authorization(self, issuer: str, purpose: str, expires_in_hours: int=24) -> str:
        ...
        try:
            payload = {'issuer': issuer, 'subject': 'audit_access', 'purpose': purpose, 'created_at': datetime.now(UTC).isoformat(), 'expires_at': (datetime.now(UTC) + timedelta(hours=expires_in_hours)).isoformat(), 'signature': 'placeholder'}
    ...
    async def revoke_keys(self, participant_id: str, reason: str) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.info('Revoked keys for participant: %s', participant_id)
            return success
        except Exception as e:
            logger.error('Failed to revoke keys for %s: %s', participant_id, e)
            return False
...
class FileKeyStorage(KeyStorageBackend):
    ...
    async def store_key_pair(self, key_pair: KeyPair) -> bool:
        ...
        try:
            ...
            metadata = {'participant_id': key_pair.participant_id, 'public_key': base64.b64encode(key_pair.public_key).decode(), 'algorithm': key_pair.algorithm, 'created_at': key_pair.created_at.isoformat(), 'version': key_pair.version}
    ...
    def get_key_pair_sync(self, participant_id: str) -> KeyPair | None:
        ...
        try:
            ...
            with open(private_path, 'rb') as f:
                private_key = f.read()
            return KeyPair(participant_id=metadata['participant_id'], private_key=private_key, public_key=base64.b64decode(metadata['public_key']), algorithm=metadata['algorithm'], created_at=datetime.fromisoformat(metadata['created_at']), version=metadata['version'])
        except Exception as e:
    ...
    async def store_audit_key(self, key_pair: KeyPair) -> bool:
        ...
        try:
            with open(audit_priv_path, 'wb') as f:
                f.write(key_pair.private_key)
            metadata = {'participant_id': 'audit', 'public_key': base64.b64encode(key_pair.public_key).decode(), 'algorithm': key_pair.algorithm, 'created_at': key_pair.created_at.isoformat(), 'version': key_pair.version}
    ...
    async def log_rotation(self, rotation_log: KeyRotationLog) -> bool:
        ...
        try:
            ...
            logger.error('Failed to log rotation: %s', e)
            return False
...
class MockHSMStorage(KeyStorageBackend):
    ...
    async def store_key_pair(self, key_pair: KeyPair) -> bool:
        ...
        try:
            ...
            self.logger.info('Stored key pair for %s in mock HSM', key_pair.participant_id)
            return True
        except Exception as e:
            self.logger.error('Failed to store key pair in mock HSM: %s', e)
            return False
    ...
    async def store_audit_key(self, key_pair: KeyPair) -> bool:
        ...
        try:
            ...
            self.logger.error('Failed to store audit key in mock HSM: %s', e)
            return False
    ...
    async def revoke_keys(self, participant_id: str, reason: str) -> bool:
        ...
        try:
            if participant_id in self._keys:
                ...
                self.logger.info('Revoked keys for %s in mock HSM: %s', participant_id, reason)
                return True
            return False
        except Exception as e:
            self.logger.error('Failed to revoke keys in mock HSM: %s', e)
            return False
    ...
    async def log_rotation(self, rotation_log: KeyRotationLog) -> bool:
        ...
        try:
            ...
            self.logger.info('Logged rotation for %s in mock HSM', rotation_log.participant_id)
            return True
        except Exception as e:
            self.logger.error('Failed to log rotation in mock HSM: %s', e)
            return False
...
class HSMProviderInterface:
    ...
    async def connect_to_hsm(self) -> bool:
        ...
        try:
            ...
            self.logger.error('Failed to connect to mock HSM: %s', e)
            return False
    ...
    async def store_key_in_hsm(self, key_id: str, key_data: bytes) -> bool:
        ...
        try:
            ...
            self.logger.info('Stored key %s in mock HSM', key_id)
            return True
        except Exception as e:
            self.logger.error('Failed to store key in mock HSM: %s', e)
            return False
    ...
    async def retrieve_from_hsm(self, key_id: str) -> bytes | None:
        ...
        try:
            ...
            self.logger.error('Failed to retrieve key from mock HSM: %s', e)
            return None