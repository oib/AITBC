...
class QuotaEnforcementService:
    ...
    async def check_quota(self, resource_type: str, quantity: float, tenant_id: str | None=None) -> bool:
        ...
        if current_usage + quantity > quota.limit_value:
            self.logger.warning('Quota exceeded for tenant %s: %s %s/%s', tenant_id, resource_type, current_usage + quantity, quota.limit_value)
            raise QuotaExceededError(f'Quota exceeded for {resource_type}: {current_usage + quantity}/{quota.limit_value}')
        return True

    async def consume_quota(self, resource_type: str, quantity: float, resource_id: str | None=None, metadata: dict[str, Any] | None=None, tenant_id: str | None=None) -> UsageRecord:
        """Consume quota and record usage"""
        tenant_id = tenant_id or get_current_tenant_id()
        if not tenant_id:
            raise TenantError('No tenant context found')
        await self.check_quota(resource_type, quantity, tenant_id)
        usage_record = UsageRecord(tenant_id=tenant_id, resource_type=resource_type, resource_id=resource_id, quantity=quantity, unit=self._get_unit_for_resource(resource_type), unit_price=await self._get_unit_price(resource_type), total_cost=await self._calculate_cost(resource_type, quantity), currency='USD', usage_start=datetime.now(UTC), usage_end=datetime.now(UTC), metadata=metadata or {})
    ...
    async def get_quota_status(self, resource_type: str | None=None, tenant_id: str | None=None) -> dict[str, Any]:
        ...
        for quota in quotas:
            ...
            quota_status = {'limit': float(quota.limit_value), 'used': float(current_usage), 'remaining': float(quota.limit_value - current_usage), 'usage_percent': round(usage_percent, 2), 'period': quota.period_type, 'period_start': quota.period_start.isoformat(), 'period_end': quota.period_end.isoformat()}
    ...
    async def quota_reservation(self, resource_type: str, quantity: float, timeout: int=300, tenant_id: str | None=None):
        ...
        try:
            await self.check_quota(resource_type, quantity, tenant_id)
            if self.redis:
                reservation_data = {'tenant_id': tenant_id, 'resource_type': resource_type, 'quantity': quantity, 'created_at': datetime.now(UTC).isoformat()}
    ...
    async def reset_quota_period(self, tenant_id: str, resource_type: str) -> None:
        ...
        stmt = select(TenantQuota).where(and_(TenantQuota.tenant_id == tenant_id, TenantQuota.resource_type == resource_type, TenantQuota.is_active))
    ...
    async def get_quota_alerts(self, tenant_id: str | None=None) -> list[dict[str, Any]]:
        ...
        for resource_type, quota_status in status['quotas'].items():
            ...
            if usage_percent >= 100:
                alerts.append({'severity': 'critical', 'resource_type': resource_type, 'message': f'Quota exceeded for {resource_type}', 'usage_percent': usage_percent, 'used': quota_status['used'], 'limit': quota_status['limit']})
            elif usage_percent >= 90:
                alerts.append({'severity': 'warning', 'resource_type': resource_type, 'message': f'Quota almost exceeded for {resource_type}', 'usage_percent': usage_percent, 'used': quota_status['used'], 'limit': quota_status['limit']})
            elif usage_percent >= 80:
                alerts.append({'severity': 'info', 'resource_type': resource_type, 'message': f'Quota usage high for {resource_type}', 'usage_percent': usage_percent, 'used': quota_status['used'], 'limit': quota_status['limit']})
        return alerts
        ...
        if self.redis:
            ...
            if cached:
                ...
                if quota.period_end >= datetime.now(UTC):
                    return quota
        stmt = select(TenantQuota).where(and_(TenantQuota.tenant_id == tenant_id, TenantQuota.resource_type == resource_type, TenantQuota.is_active, TenantQuota.period_start <= datetime.now(UTC), TenantQuota.period_end >= datetime.now(UTC)))
        quota = self.db.execute(stmt).scalar_one_or_none()
        if quota and self.redis:
            quota_data = {'id': str(quota.id), 'tenant_id': str(quota.tenant_id), 'resource_type': quota.resource_type, 'limit_value': float(quota.limit_value), 'used_value': float(quota.used_value), 'period_start': quota.period_start.isoformat(), 'period_end': quota.period_end.isoformat()}
    ...
    async def _get_current_usage(self, tenant_id: str, resource_type: str) -> float:
        ...
        if self.redis:
            ...
            if cached:
                return float(cached)
        stmt = select(func.sum(UsageRecord.quantity)).where(and_(UsageRecord.tenant_id == tenant_id, UsageRecord.resource_type == resource_type, UsageRecord.usage_start >= func.date_trunc('month', func.current_date())))
    ...
    async def _update_quota_usage(self, tenant_id: str, resource_type: str, quantity: float) -> None:
        ...
        stmt = update(TenantQuota).where(and_(TenantQuota.tenant_id == tenant_id, TenantQuota.resource_type == resource_type, TenantQuota.is_active)).values(used_value=TenantQuota.used_value + quantity)
...
class QuotaMiddleware:
    ...
    def __init__(self, quota_service: QuotaEnforcementService):
        ...
        self.logger = __import__('logging').getLogger(f'aitbc.{self.__class__.__name__}')
        self.endpoint_costs = {'/api/v1/jobs': {'resource': 'compute_hours', 'cost': 0.1}, '/api/v1/models': {'resource': 'storage_gb', 'cost': 0.1}, '/api/v1/data': {'resource': 'storage_gb', 'cost': 0.05}, '/api/v1/analytics': {'resource': 'api_calls', 'cost': 1}}
