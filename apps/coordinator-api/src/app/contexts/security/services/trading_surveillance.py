...
class TradingSurveillance:
    ...
    async def _get_trading_data(self, symbol: str) -> dict[str, Any]:
        ...
        return {'symbol': symbol, 'current_volume': volume, 'current_price': price, 'volume_history': volumes, 'price_history': prices, 'timestamps': timestamps, 'user_distribution': user_volumes, 'trade_count': int(volume / 1000), 'order_cancellations': int(np.random.poisson(100)), 'total_orders': int(np.random.poisson(500))}
    ...
    async def _detect_wash_trading(self, symbol: str, data: dict[str, Any]) -> None:
        ...
        try:
            ...
            if max_user_share > self.thresholds['wash_trade_threshold']:
                ...
                alert = TradingAlert(alert_id=f'wash_trade_{symbol}_{int(datetime.now().timestamp())}', timestamp=datetime.now(), alert_level=AlertLevel.HIGH, manipulation_type=ManipulationType.WASH_TRADING, anomaly_type=AnomalyType.CONCENTRATED_TRADING, description=f'Potential wash trading detected in {symbol}', confidence=min(0.9, max_user_share), affected_symbols=[symbol], affected_users=[dominant_user], evidence={'user_share': max_user_share, 'user_distribution': user_distribution, 'total_volume': data['current_volume']}, risk_score=0.75)
                self.alerts.append(alert)
        ...
        try:
            ...
            if total_orders > 0:
                ...
                if cancellation_rate > self.thresholds['spoofing_threshold']:
                    alert = TradingAlert(alert_id=f'spoofing_{symbol}_{int(datetime.now().timestamp())}', timestamp=datetime.now(), alert_level=AlertLevel.MEDIUM, manipulation_type=ManipulationType.SPOOFING, anomaly_type=None, description=f'High order cancellation rate detected in {symbol}', confidence=min(0.8, cancellation_rate), affected_symbols=[symbol], affected_users=[], evidence={'cancellation_rate': cancellation_rate, 'total_orders': total_orders, 'cancellations': cancellations}, risk_score=0.6)
                    self.alerts.append(alert)
        ...
        try:
            ...
            if len(volumes) > 20:
                ...
                if volume_multiplier > self.thresholds['volume_spike_multiplier']:
                    alert = TradingAlert(alert_id=f'volume_spike_{symbol}_{int(datetime.now().timestamp())}', timestamp=datetime.now(), alert_level=AlertLevel.MEDIUM, manipulation_type=None, anomaly_type=AnomalyType.VOLUME_SPIKE, description=f'Unusual volume spike detected in {symbol}', confidence=min(0.8, volume_multiplier / 5), affected_symbols=[symbol], affected_users=[], evidence={'volume_multiplier': volume_multiplier, 'current_volume': current_volume, 'avg_volume': avg_volume, 'recent_avg': recent_avg}, risk_score=0.5)
                    self.alerts.append(alert)
            ...
            if len(prices) > 10:
                ...
                for i, change in enumerate(price_changes):
                    if abs(change) > self.thresholds['price_change_threshold']:
                        alert = TradingAlert(alert_id=f'price_anomaly_{symbol}_{int(datetime.now().timestamp())}_{i}', timestamp=datetime.now(), alert_level=AlertLevel.MEDIUM, manipulation_type=None, anomaly_type=AnomalyType.PRICE_ANOMALY, description=f'Unusual price movement detected in {symbol}', confidence=min(0.9, abs(change) / 0.2), affected_symbols=[symbol], affected_users=[], evidence={'price_change': change, 'price_before': prices[i], 'price_after': prices[i + 1] if i + 1 < len(prices) else None, 'timestamp_index': i}, risk_score=0.4)
                        self.alerts.append(alert)
    ...
    async def _detect_concentrated_trading(self, symbol: str, data: dict[str, Any]) -> None:
        ...
        try:
            ...
            if hhi > self.thresholds['concentration_threshold']:
                ...
                alert = TradingAlert(alert_id=f'concentrated_{symbol}_{int(datetime.now().timestamp())}', timestamp=datetime.now(), alert_level=AlertLevel.MEDIUM, manipulation_type=None, anomaly_type=AnomalyType.CONCENTRATED_TRADING, description=f'Concentrated trading detected in {symbol}', confidence=min(0.8, hhi), affected_symbols=[symbol], affected_users=[user for user, _ in top_users], evidence={'hhi': hhi, 'top_users': top_users, 'total_users': len(user_distribution)}, risk_score=0.5)
                self.alerts.append(alert)
    ...
    def resolve_alert(self, alert_id: str, resolution: str='resolved') -> bool:
        ...
        for alert in self.alerts:
            if alert.alert_id == alert_id:
                ...
                logger.info('✅ Alert %s marked as %s', alert_id, resolution)
                return True
        return False
...
def get_alerts(level: str | None=None) -> dict[str, Any]:
    ...
    return {'alerts': [{'alert_id': alert.alert_id, 'timestamp': alert.timestamp.isoformat(), 'level': alert.alert_level.value, 'manipulation_type': alert.manipulation_type.value if alert.manipulation_type else None, 'anomaly_type': alert.anomaly_type.value if alert.anomaly_type else None, 'description': alert.description, 'confidence': alert.confidence, 'risk_score': alert.risk_score, 'affected_symbols': alert.affected_symbols, 'affected_users': alert.affected_users} for alert in alerts], 'total': len(alerts)}
