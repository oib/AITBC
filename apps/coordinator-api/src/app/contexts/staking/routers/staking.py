...
async def get_stake(request: Request, stake_id: str, session: Session=Depends(get_session), staking_service: StakingService=Depends(get_staking_service), current_user: dict=Depends(get_current_user_optional)) -> StakeResponse:
    """Get stake details"""
    try:
...
@rate_limit(rate=200, per=60)
async def get_stakes(request: Request, filters: StakingFilterRequest=Depends(), session: Session=Depends(get_session), staking_service: StakingService=Depends(get_staking_service), current_user: dict=Depends(get_current_user_optional)) -> list[StakeResponse]:
...
@rate_limit(rate=20, per=60)
async def add_to_stake(request: Request, stake_id: str, stake_request: StakeUpdateRequest, background_tasks: BackgroundTasks, session: Session=Depends(get_session), staking_service: StakingService=Depends(get_staking_service), blockchain_service: BlockchainService=Depends(get_blockchain_service), current_user: dict=Depends(get_current_user_optional)) -> StakeResponse:
...
@rate_limit(rate=200, per=60)
async def get_stake_rewards(request: Request, stake_id: str, session: Session=Depends(get_session), staking_service: StakingService=Depends(get_staking_service), current_user: dict=Depends(get_current_user_optional)) -> dict[str, Any]:
    ...
    try:
        ...
        return {'stake_id': stake_id, 'accumulated_rewards': stake.accumulated_rewards, 'current_rewards': rewards, 'total_rewards': stake.accumulated_rewards + rewards, 'current_apy': stake.current_apy, 'last_reward_time': stake.last_reward_time}
    except HTTPException:
        raise
    except Exception as e:
...
@rate_limit(rate=200, per=60)
async def get_agent_metrics(request: Request, agent_wallet: str, session: Session=Depends(get_session), staking_service: StakingService=Depends(get_staking_service)) -> AgentMetricsResponse:
...
@rate_limit(rate=200, per=60)
async def get_staking_pool(request: Request, agent_wallet: str, session: Session=Depends(get_session), staking_service: StakingService=Depends(get_staking_service)) -> StakingPoolResponse:
...
@rate_limit(rate=200, per=60)
async def get_agent_apy(request: Request, agent_wallet: str, lock_period: int=Query(default=30, ge=1, le=365), session: Session=Depends(get_session), staking_service: StakingService=Depends(get_staking_service)) -> dict[str, Any]:
    """Get current APY for staking on an agent"""
    try:
        apy = await staking_service.calculate_apy(agent_wallet, lock_period)
        return {'agent_wallet': agent_wallet, 'lock_period': lock_period, 'current_apy': apy, 'base_apy': 5.0, 'tier_multiplier': apy / 5.0 if apy > 0 else 1.0}
    except Exception as e:
...
@rate_limit(rate=20, per=60)
async def update_agent_performance(request: Request, agent_wallet: str, performance_request: AgentPerformanceUpdateRequest, background_tasks: BackgroundTasks, session: Session=Depends(get_session), staking_service: StakingService=Depends(get_staking_service), blockchain_service: BlockchainService=Depends(get_blockchain_service), current_user: dict=Depends(get_current_user_optional)) -> dict[str, str]:
...
@rate_limit(rate=20, per=60)
async def distribute_agent_earnings(request: Request, agent_wallet: str, earnings_request: EarningsDistributionRequest, background_tasks: BackgroundTasks, session: Session=Depends(get_session), staking_service: StakingService=Depends(get_staking_service), blockchain_service: BlockchainService=Depends(get_blockchain_service), current_user: dict=Depends(get_current_user_optional)) -> dict[str, Any]:
...
@rate_limit(rate=200, per=60)
async def get_supported_agents(request: Request, page: int=Query(default=1, ge=1), limit: int=Query(default=50, ge=1, le=100), tier: PerformanceTier | None=None, session: Session=Depends(get_session), staking_service: StakingService=Depends(get_staking_service)) -> dict[str, Any]:
...
@rate_limit(rate=200, per=60)
async def get_staking_stats(request: Request, period: str=Query(default='daily', pattern='^(hourly|daily|weekly|monthly)$'), session: Session=Depends(get_session), staking_service: StakingService=Depends(get_staking_service)) -> StakingStatsResponse:
...
@rate_limit(rate=200, per=60)
async def get_staking_leaderboard(request: Request, period: str=Query(default='weekly', pattern='^(daily|weekly|monthly)$'), metric: str=Query(default='total_staked', pattern='^(total_staked|total_rewards|apy)$'), limit: int=Query(default=50, ge=1, le=100), session: Session=Depends(get_session), staking_service: StakingService=Depends(get_staking_service)) -> dict[str, Any]:
...
@rate_limit(rate=200, per=60)
async def get_my_staking_positions(request: Request, status: StakeStatus | None=None, agent_wallet: str | None=None, page: int=Query(default=1, ge=1), limit: int=Query(default=20, ge=1, le=100), session: Session=Depends(get_session), staking_service: StakingService=Depends(get_staking_service), current_user: dict=Depends(get_current_user_optional)) -> list[StakeResponse]:
...
@rate_limit(rate=200, per=60)
async def get_my_staking_rewards(request: Request, period: str=Query(default='monthly', pattern='^(daily|weekly|monthly)$'), session: Session=Depends(get_session), staking_service: StakingService=Depends(get_staking_service), current_user: dict=Depends(get_current_user_optional)) -> dict[str, Any]:
...
@rate_limit(rate=200, per=60)
async def get_risk_assessment(request: Request, agent_wallet: str, session: Session=Depends(get_session), staking_service: StakingService=Depends(get_staking_service)) -> dict[str, Any]:
