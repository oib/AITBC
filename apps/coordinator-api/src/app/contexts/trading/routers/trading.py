...
async def get_trade_request(request: Request, request_id: str, session: Session=Depends(get_session)) -> TradeRequestResponse:
...
@rate_limit(rate=200, per=60)
async def get_negotiation(request: Request, negotiation_id: str, session: Session=Depends(get_session)) -> NegotiationResponse:
    """Get negotiation details"""
    try:
        negotiation = session.execute(select(TradeNegotiation).where(TradeNegotiation.negotiation_id == negotiation_id)).first()
        if not negotiation:
...
@rate_limit(rate=200, per=60)
async def get_trade_match(request: Request, match_id: str, session: Session=Depends(get_session)) -> TradeMatchResponse:
...
@router.get('/requests')
@rate_limit(rate=200, per=60)
async def list_trade_requests(request: Request, agent_id: str | None=Query(default=None, description='Filter by agent ID'), trade_type: str | None=Query(default=None, description='Filter by trade type'), status: str | None=Query(default=None, description='Filter by status'), limit: int=Query(default=50, ge=1, le=100, description='Number of results'), session: Session=Depends(get_session)) -> list[TradeRequestResponse]:
...
@router.get('/matches')
@rate_limit(rate=200, per=60)
async def list_trade_matches(request: Request, agent_id: str | None=Query(default=None, description='Filter by agent ID'), min_score: float | None=Query(default=None, description='Minimum match score'), status: str | None=Query(default=None, description='Filter by status'), limit: int=Query(default=50, ge=1, le=100, description='Number of results'), session: Session=Depends(get_session)) -> list[TradeMatchResponse]:
...
@rate_limit(rate=200, per=60)
async def list_negotiations(request: Request, agent_id: str | None=Query(default=None, description='Filter by agent ID'), status: str | None=Query(default=None, description='Filter by status'), strategy: str | None=Query(default=None, description='Filter by strategy'), limit: int=Query(default=50, ge=1, le=100, description='Number of results'), session: Session=Depends(get_session)) -> list[NegotiationResponse]:
...
@rate_limit(rate=200, per=60)
async def get_trading_analytics(request: Request, period_type: str=Query(default='daily', description='Period type: daily, weekly, monthly'), start_date: str | None=Query(default=None, description='Start date (ISO format)'), end_date: str | None=Query(default=None, description='End date (ISO format)'), session: Session=Depends(get_session)) -> dict[str, Any]:
