...
try:
    ...
    logger.warning('PyCUDA not available or no CUDA-capable device detected: %s. GPU optimization will run in simulation mode.', e)
logger = get_logger(__name__)
...
class MarketplaceGPUOptimizer:
    ...
    def _initialize_gpu_devices(self) -> None:
        ...
        if self.simulation_mode:
            self.gpu_devices = [{'id': 0, 'name': 'Simulated RTX 4090', 'total_memory': 24 * 1024 * 1024 * 1024, 'free_memory': 24 * 1024 * 1024 * 1024, 'compute_capability': (8, 9), 'utilization': 0.0, 'temperature': 45.0, 'power_draw': 30.0, 'power_limit': 450.0, 'status': 'idle'}, {'id': 1, 'name': 'Simulated RTX 4090', 'total_memory': 24 * 1024 * 1024 * 1024, 'free_memory': 24 * 1024 * 1024 * 1024, 'compute_capability': (8, 9), 'utilization': 0.0, 'temperature': 42.0, 'power_draw': 28.0, 'power_limit': 450.0, 'status': 'idle'}]
            ...
            try:
                ...
                for i in range(num_devices):
                    ...
                    self.gpu_devices.append({'id': i, 'name': dev.name(), 'total_memory': total_mem, 'free_memory': free_mem, 'compute_capability': dev.compute_capability(), 'utilization': 0.0, 'temperature': 0.0, 'power_draw': 0.0, 'power_limit': 0.0, 'status': 'idle'})
                logger.info('Initialized %s real GPU devices', len(self.gpu_devices))
            except Exception as e:
                logger.error('Error initializing GPUs: %s', e)
                self.simulation_mode = True
                self._initialize_gpu_devices()
        for gpu in self.gpu_devices:
            self.gpu_memory_pools[gpu['id']] = {'allocated_blocks': [], 'free_blocks': [{'start': 0, 'size': gpu['total_memory']}], 'fragmentation': 0.0}
        ...
        with self.lock:
            ...
            if best_gpu_id == -1:
                if await self._attempt_memory_defragmentation():
                    ...
                    return {'success': False, 'reason': 'Insufficient GPU resources available even after optimization', 'queued': True, 'estimated_wait_ms': 5000}
            ...
            if not allocation['success']:
                return {'success': False, 'reason': 'Memory allocation failed due to fragmentation', 'queued': True}
            for i, gpu in enumerate(self.gpu_devices):
                if gpu['id'] == best_gpu_id:
                    ...
                    break
            self.active_jobs[job_id] = {'gpu_id': best_gpu_id, 'memory_allocated': required_memory, 'compute_allocated': required_compute, 'priority': priority, 'start_time': time.time(), 'status': 'running'}
            self._update_metrics()
            return {'success': True, 'job_id': job_id, 'gpu_id': best_gpu_id, 'allocation_plan': {'memory_blocks': allocation['blocks'], 'dynamic_batching': self.config['dynamic_batching_enabled'], 'power_state_enforced': self.config['active_power_state']}, 'estimated_completion_ms': int(required_compute * 100)}
    ...
    def _allocate_memory(self, gpu_id: int, size: int, job_id: str) -> dict[str, Any]:
        ...
        for i, block in enumerate(pool['free_blocks']):
            if block['size'] >= size:
                allocated_block = {'job_id': job_id, 'start': block['start'], 'size': size}
        ...
        if sum((b['size'] for b in pool['free_blocks'])) >= size:
            ...
            for i, block in enumerate(pool['free_blocks']):
                ...
                allocated_block = {'job_id': job_id, 'start': block['start'], 'size': take_size}
                allocated_blocks.append(allocated_block)
                pool['allocated_blocks'].append(allocated_block)
    ...
    async def _attempt_memory_defragmentation(self) -> bool:
        ...
        for gpu_id, pool in self.gpu_memory_pools.items():
            if pool['fragmentation'] > self.config['memory_fragmentation_threshold']:
                ...
                current_ptr = 0
                for block in pool['allocated_blocks']:
                    new_allocated.append({'job_id': block['job_id'], 'start': current_ptr, 'size': block['size']})
                    current_ptr += block['size']
                pool['allocated_blocks'] = new_allocated
                gpu = next((g for g in self.gpu_devices if g['id'] == gpu_id), None)
                if gpu:
                    pool['free_blocks'] = [{'start': total_allocated, 'size': gpu['total_memory'] - total_allocated}]
                pool['fragmentation'] = 0.0
    ...
    async def schedule_job(self, job_id: str, priority: int, memory_required: int, computation_complexity: float) -> bool:
        ...
        if best_gpu >= 0:
            ...
            if alloc_result['success']:
                ...
                return True
        logger.info('No GPU has %sMB free for job %s. Attempting preemption...', memory_required, job_id)
            ...
            for gpu_id, pool in self.gpu_memory_pools.items():
                if pool['total_memory'] - pool['allocated_memory'] >= memory_required:
                    ...
                    if alloc_result['success']:
                        ...
                        self.active_jobs[job_id] = job_data
                        return True
        logger.warning('Job %s remains queued. Insufficient resources even after preemption.', job_id)
        return False
    ...
    async def _preempt_low_priority_jobs(self, incoming_priority: int, required_memory: int) -> bool:
        ...
        if freed_memory >= required_memory:
            for job_id in jobs_to_preempt:
                logger.info('Preempting low priority job %s for higher priority request', job_id)
