...
def create_app() -> FastAPI:
    ...
    rate_limit_hits_total = Counter('rate_limit_hits_total', 'Total number of rate limit violations', ['endpoint', 'method', 'limit'], registry=rate_limit_registry)
    Histogram('rate_limit_response_time_seconds', 'Response time for rate limited requests', ['endpoint', 'method'], registry=rate_limit_registry)
    ...
    async def rate_limit_handler(request: Request, exc: RateLimitExceeded) -> JSONResponse:
        ...
        logger.warning('Rate limit exceeded: %s, Request ID: %s, Path: %s, Method: %s, Limit Detail: %s', exc, request_id, request.url.path, request.method, limit_detail)
        error_response = ErrorResponse(error={'code': 'RATE_LIMIT_EXCEEDED', 'message': 'Too many requests. Please try again later.', 'status': 429, 'details': [{'field': 'rate_limit', 'message': str(exc.detail), 'code': 'too_many_requests', 'retry_after': 60}]}, request_id=request_id)
    ...
    async def live_metrics() -> dict:
        return build_live_metrics_payload(cache_stats=cache_manager.get_stats(), dispatcher=alert_dispatcher, collector=metrics_collector)

    @app.exception_handler(Exception)
    async def general_exception_handler(request: Request, exc: Exception) -> JSONResponse:
        """Handle all unhandled exceptions with structured error responses."""
        request_id = request.headers.get('X-Request-ID')
        error_response = ErrorResponse(error={'code': 'INTERNAL_SERVER_ERROR', 'message': 'An unexpected error occurred', 'status': 500, 'details': [{'field': 'internal', 'message': str(exc), 'code': type(exc).__name__}]}, request_id=request_id)
    ...
    async def root_health() -> dict[str, str]:
        import sys
        return {'status': 'ok', 'env': settings.environment, 'python_version': f'{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}'}
    ...
    async def liveness() -> dict[str, str]:
        import sys
        return {'status': 'alive', 'python_version': f'{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}'}
    ...
    async def readiness() -> Response:
        try:
            ...
            for session in get_session():
                ...
                break
            return JSONResponse(status_code=200, content={'status': 'ready', 'database': 'connected', 'python_version': f'{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}'})
        except Exception as e:
