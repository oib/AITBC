...
class ParticipantKeyRepository:
    ...
    async def update_active(
        ...
        stmt = (
            ...
            .values(active=active, revoked_at=datetime.now(UTC) if not active else None, revoke_reason=reason)
        )
...
class AuditAuthorizationRepository:
    ...
    async def cleanup_expired(self, session: AsyncSession) -> int:
        ...
        stmt = update(AuditAuthorizationDB).where(AuditAuthorizationDB.expires_at < datetime.now(UTC)).values(active=False)
