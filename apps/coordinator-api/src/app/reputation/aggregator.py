...
class CrossChainReputationAggregator:
    ...
    async def collect_chain_reputation_data(self, chain_id: int) -> list[dict[str, Any]]:
        ...
        try:
            stmt = select(AgentReputation).where(AgentReputation.chain_id == chain_id if hasattr(AgentReputation, 'chain_id') else True)
            if not hasattr(AgentReputation, 'chain_id'):
                stmt = select(AgentReputation)
            reputations = self.session.exec(stmt).all()
            chain_data = []
            for reputation in reputations:
                chain_data.append({'agent_id': reputation.agent_id, 'trust_score': reputation.trust_score, 'reputation_level': reputation.reputation_level, 'total_transactions': getattr(reputation, 'transaction_count', 0), 'success_rate': getattr(reputation, 'success_rate', 0.0), 'dispute_count': getattr(reputation, 'dispute_count', 0), 'last_updated': reputation.updated_at, 'chain_id': getattr(reputation, 'chain_id', chain_id)})
            return chain_data
    ...
    async def detect_reputation_anomalies(self, agent_id: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if aggregation.consistency_score < 0.7:
                anomalies.append({'agent_id': agent_id, 'anomaly_type': 'low_consistency', 'detected_at': datetime.now(UTC), 'description': f'Low consistency score: {aggregation.consistency_score:.2f}', 'severity': 'high' if aggregation.consistency_score < 0.5 else 'medium', 'consistency_score': aggregation.consistency_score, 'score_variance': aggregation.score_variance, 'score_range': aggregation.score_range})
            if aggregation.score_variance > 0.25:
                anomalies.append({'agent_id': agent_id, 'anomaly_type': 'high_variance', 'detected_at': datetime.now(UTC), 'description': f'High score variance: {aggregation.score_variance:.2f}', 'severity': 'high' if aggregation.score_variance > 0.5 else 'medium', 'score_variance': aggregation.score_variance, 'score_range': aggregation.score_range, 'chain_scores': aggregation.chain_scores})
            expected_chains = await self._get_active_chain_ids()
            missing_chains = set(expected_chains) - set(aggregation.active_chains)
            if missing_chains:
                anomalies.append({'agent_id': agent_id, 'anomaly_type': 'missing_chain_data', 'detected_at': datetime.now(UTC), 'description': f'Missing data for chains: {list(missing_chains)}', 'severity': 'medium', 'missing_chains': list(missing_chains), 'active_chains': aggregation.active_chains})
            return anomalies
        except Exception as e:
            ...
            for update in updates:
                ...
                try:
                    ...
                    if reputation:
                        ...
                        event = ReputationEvent(agent_id=agent_id, event_type='batch_update', impact_score=new_score - reputation.trust_score / 1000.0, trust_score_before=reputation.trust_score, trust_score_after=reputation.trust_score, event_data=update, occurred_at=datetime.now(UTC))
                        self.session.add(event)
                        results[agent_id] = True
                    else:
                        reputation = AgentReputation(agent_id=agent_id, trust_score=new_score * 1000, reputation_level=self._determine_reputation_level(new_score), created_at=datetime.now(UTC), updated_at=datetime.now(UTC))
