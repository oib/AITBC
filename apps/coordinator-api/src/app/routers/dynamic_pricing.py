...
async def get_pricing_engine() -> DynamicPricingEngine:
    ...
    if pricing_engine is None:
        pricing_engine = DynamicPricingEngine({'min_price': 0.001, 'max_price': 1000.0, 'update_interval': 300, 'forecast_horizon': 72})
...
async def get_dynamic_price(request: Request, resource_type: str, resource_id: str, strategy: str | None=Query(default=None), region: str=Query(default='global'), engine: DynamicPricingEngine=Depends(get_pricing_engine)) -> DynamicPriceResponse:
    ...
    try:
        ...
        result = await engine.calculate_dynamic_price(resource_id=resource_id, resource_type=resource_enum, base_price=base_price, strategy=strategy_enum, region=region)
        return DynamicPriceResponse(resource_id=result.resource_id, resource_type=result.resource_type.value, current_price=result.current_price, recommended_price=result.recommended_price, price_trend=result.price_trend.value, confidence_score=result.confidence_score, factors_exposed=result.factors_exposed, reasoning=result.reasoning, next_update=result.next_update, strategy_used=result.strategy_used.value)
    except Exception as e:
        raise HTTPException(status_code=http_status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f'Failed to calculate dynamic price: {str(e)}')

@router.get('/forecast/{resource_type}/{resource_id}', response_model=PriceForecast)
@rate_limit(rate=200, per=60)
async def get_price_forecast(request: Request, resource_type: str, resource_id: str, hours: int=Query(default=24, ge=1, le=168), engine: DynamicPricingEngine=Depends(get_pricing_engine)) -> PriceForecast:
...
async def set_pricing_strategy(request: Request, provider_id: str, request_data: PricingStrategyRequest, engine: DynamicPricingEngine=Depends(get_pricing_engine)) -> PricingStrategyResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=http_status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f'Failed to set pricing strategy: {str(e)}')

@router.get('/strategy/{provider_id}', response_model=PricingStrategyResponse)
@rate_limit(rate=200, per=60)
async def get_pricing_strategy(request: Request, provider_id: str, engine: DynamicPricingEngine=Depends(get_pricing_engine)) -> PricingStrategyResponse:
    """Get current pricing strategy for a provider"""
    try:
        if provider_id not in engine.provider_strategies:
            raise HTTPException(status_code=http_status.HTTP_404_NOT_FOUND, detail=f'No strategy found for provider {provider_id}')
        strategy = engine.provider_strategies[provider_id]
        constraints = engine.price_constraints.get(provider_id)
        constraints_dict = None
        if constraints:
            constraints_dict = {'min_price': constraints.min_price, 'max_price': constraints.max_price, 'max_change_percent': constraints.max_change_percent, 'min_change_interval': constraints.min_change_interval, 'strategy_lock_period': constraints.strategy_lock_period}
        return PricingStrategyResponse(provider_id=provider_id, strategy=strategy.value, constraints=constraints_dict, set_at=datetime.now(UTC).isoformat(), status='active')
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=http_status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f'Failed to get pricing strategy: {str(e)}')
...
async def get_available_strategies(request: Request) -> list[dict[str, Any]]:
    ...
    try:
        ...
        for strategy_type, config in StrategyLibrary.get_all_strategies().items():
            strategies.append({'strategy': strategy_type.value, 'name': config.name, 'description': config.description, 'risk_tolerance': config.risk_tolerance.value, 'priority': config.priority.value, 'parameters': {'base_multiplier': config.parameters.base_multiplier, 'demand_sensitivity': config.parameters.demand_sensitivity, 'competition_sensitivity': config.parameters.competition_sensitivity, 'max_price_change_percent': config.parameters.max_price_change_percent}})
        return strategies
    except Exception as e:
        raise HTTPException(status_code=http_status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f'Failed to get available strategies: {str(e)}')

@router.get('/market-analysis', response_model=MarketAnalysisResponse)
@rate_limit(rate=200, per=60)
async def get_market_analysis(request: Request, region: str=Query(default='global'), resource_type: str=Query(default='gpu'), collector: MarketDataCollector=Depends(get_market_collector)) -> MarketAnalysisResponse:
    ...
    try:
        ...
        return MarketAnalysisResponse(region=region, resource_type=resource_type, current_conditions={'demand_level': market_data.demand_level, 'supply_level': market_data.supply_level, 'average_price': market_data.average_price, 'price_volatility': market_data.price_volatility, 'utilization_rate': market_data.utilization_rate, 'market_sentiment': market_data.market_sentiment}, trends={'demand_trend': demand_trend, 'supply_trend': supply_trend, 'price_trend': price_trend}, competitor_analysis={'average_competitor_price': sum(market_data.competitor_prices) / len(market_data.competitor_prices) if market_data.competitor_prices else 0, 'price_range': {'min': min(market_data.competitor_prices) if market_data.competitor_prices else 0, 'max': max(market_data.competitor_prices) if market_data.competitor_prices else 0}, 'competitor_count': len(market_data.competitor_prices)}, recommendations=recommendations, confidence_score=market_data.confidence_score, analysis_timestamp=market_data.timestamp.isoformat())
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=http_status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f'Failed to get market analysis: {str(e)}')

@router.get('/recommendations/{provider_id}', response_model=list[PricingRecommendation])
@rate_limit(rate=200, per=60)
async def get_pricing_recommendations(request: Request, provider_id: str, resource_type: str=Query(default='gpu'), region: str=Query(default='global'), engine: DynamicPricingEngine=Depends(get_pricing_engine), collector: MarketDataCollector=Depends(get_market_collector)) -> list[PricingRecommendation]:
    ...
    try:
        ...
        if market_data.demand_level > 0.8 and market_data.supply_level < 0.4:
            recommendations.append(PricingRecommendation(type='strategy_change', title='Switch to Profit Maximization', description='High demand and low supply conditions favor profit maximization strategy', impact='high', confidence=0.85, action='Set strategy to profit_maximization', expected_outcome='+15-25% revenue increase'))
        if market_data.price_volatility > 0.25:
            recommendations.append(PricingRecommendation(type='risk_management', title='Enable Price Stability Mode', description='High volatility detected - enable stability constraints', impact='medium', confidence=0.9, action='Set max_price_change_percent to 0.15', expected_outcome='Reduced price volatility by 60%'))
        if market_data.utilization_rate < 0.5:
            recommendations.append(PricingRecommendation(type='competitive_response', title='Aggressive Competitive Pricing', description='Low utilization suggests need for competitive pricing', impact='high', confidence=0.75, action='Set strategy to competitive_response', expected_outcome='+10-20% utilization increase'))
        if current_strategy == PricingStrategy.MARKET_BALANCE:
            recommendations.append(PricingRecommendation(type='optimization', title='Consider Dynamic Strategy', description='Market conditions favor more dynamic pricing approach', impact='medium', confidence=0.7, action='Evaluate demand_elasticity or competitive_response strategies', expected_outcome='Improved market responsiveness'))
        ...
        raise HTTPException(status_code=http_status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f'Failed to get pricing recommendations: {str(e)}')

@router.get('/history/{resource_id}', response_model=PriceHistoryResponse)
@rate_limit(rate=200, per=60)
async def get_price_history(request: Request, resource_id: str, period: str=Query(default='7d', regex='^(1d|7d|30d|90d)$'), engine: DynamicPricingEngine=Depends(get_pricing_engine)) -> PriceHistoryResponse:
    ...
    try:
        ...
        if resource_id not in engine.pricing_history:
            return PriceHistoryResponse(resource_id=resource_id, period=period, data_points=[], statistics={'average_price': 0, 'min_price': 0, 'max_price': 0, 'price_volatility': 0, 'total_changes': 0})
        ...
        return PriceHistoryResponse(resource_id=resource_id, period=period, data_points=[{'timestamp': point.timestamp.isoformat(), 'price': point.price, 'demand_level': point.demand_level, 'supply_level': point.supply_level, 'confidence': point.confidence, 'strategy_used': point.strategy_used} for point in filtered_history], statistics={'average_price': average_price, 'min_price': min_price, 'max_price': max_price, 'price_volatility': price_volatility, 'total_changes': total_changes})
    except Exception as e:
        raise HTTPException(status_code=http_status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f'Failed to get price history: {str(e)}')

@router.post('/bulk-update', response_model=BulkPricingUpdateResponse)
@rate_limit(rate=20, per=60)
async def bulk_pricing_update(request: Request, request_data: BulkPricingUpdateRequest, engine: DynamicPricingEngine=Depends(get_pricing_engine)) -> BulkPricingUpdateResponse:
    ...
    try:
        ...
        for update in request_data.updates:
            try:
                ...
                if success:
                    success_count += 1
                    results.append({'provider_id': update.provider_id, 'status': 'success', 'message': 'Strategy updated successfully'})
                else:
                    error_count += 1
                    results.append({'provider_id': update.provider_id, 'status': 'error', 'message': 'Failed to update strategy'})
            except Exception as e:
                error_count += 1
...
async def pricing_health_check(request: Request, engine: DynamicPricingEngine=Depends(get_pricing_engine), collector: MarketDataCollector=Depends(get_market_collector)) -> dict[str, Any]:
