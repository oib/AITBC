...
from ..contexts.trading.schemas.pricing import (
...
async def get_dynamic_price(
    ...
    try:
        try:
            ...
            raise HTTPException(
                status_code=http_status.HTTP_400_BAD_REQUEST, detail=f"Invalid resource type: {resource_type}"
            ) from None
        base_price = 0.05
        strategy_enum = None
        if strategy:
            try:
                ...
                raise HTTPException(
                    status_code=http_status.HTTP_400_BAD_REQUEST, detail=f"Invalid strategy: {strategy}"
                ) from None
...
async def bulk_pricing_update(
    request: Request,
    request_data: BulkPricingUpdateRequest,
    engine: Annotated[DynamicPricingEngine, Depends(get_pricing_engine)],
) -> BulkPricingUpdateResponse:
