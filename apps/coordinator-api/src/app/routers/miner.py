...
async def register(req: MinerRegister, request: Request, session: Annotated[Session, Depends(get_session)], miner_id: str=Depends(get_miner_id()), api_key: str=Depends(require_miner_key())) -> dict[str, Any]:
...
async def heartbeat(req: MinerHeartbeat, request: Request, session: Annotated[Session, Depends(get_session)], miner_id: str=Depends(get_miner_id()), api_key: str=Depends(require_miner_key())) -> dict[str, str]:
    try:
...
@rate_limit(rate=100, per=60)
async def poll(request: Request, req: PollRequest, session: Annotated[Session, Depends(get_session)], api_key: str=Depends(require_miner_key()), miner_id: str=Depends(get_miner_id())) -> AssignedJob | Response:
...
async def submit_result(request: Request, job_id: str, req: JobResultSubmit, session: Annotated[Session, Depends(get_session)], miner_id: str=Depends(get_miner_id())) -> dict[str, Any]:
    ...
    miner_service.release(miner_id, success=True, duration_ms=duration_ms, receipt_id=receipt['receipt_id'] if receipt else None)
    return {'status': 'ok', 'receipt': receipt}

@router.post('/miners/{job_id}/fail', summary='Submit job failure')
@rate_limit(rate=50, per=60)
async def submit_failure(request: Request, job_id: str, req: JobFailSubmit, session: Annotated[Session, Depends(get_session)], miner_id: str=Depends(get_miner_id())) -> dict[str, str]:
    try:
...
async def list_miner_jobs(request: Request, miner_id: str, session: Annotated[Session, Depends(get_session)], api_key: str=Depends(require_miner_key()), limit: int=20, offset: int=0, job_type: str | None=None, min_reward: float | None=None, job_status: str | None=None) -> dict[str, Any]:
    ...
    try:
        ...
        return {'jobs': [service.to_view(job) for job in jobs], 'total': len(jobs), 'limit': limit, 'offset': offset, 'miner_id': miner_id}
...
async def get_miner_earnings(request: Request, miner_id: str, session: Annotated[Session, Depends(get_session)], api_key: str=Depends(require_miner_key()), from_time: str | None=None, to_time: str | None=None) -> dict[str, Any]:
    ...
    try:
        ...
        return {'miner_id': miner_id, 'total_earnings': 0.0, 'pending_earnings': 0.0, 'completed_jobs': 0, 'currency': 'AITBC', 'error': str(e)}
...
async def update_miner_capabilities(request: Request, miner_id: str, req: MinerRegister, session: Annotated[Session, Depends(get_session)], api_key: str=Depends(require_miner_key())) -> dict[str, Any]:
    ...
    try:
        service = MinerService(session)
        record = service.register(miner_id, req)
        return {'miner_id': miner_id, 'status': 'updated', 'capabilities': req.capabilities, 'session_token': record.session_token}
    except KeyError:
...
async def complete_job(request: Request, miner_id: str, job_id: str, complete_req: CompleteJobRequest, session: Annotated[Session, Depends(get_session)], api_key: str=Depends(require_miner_key())) -> dict[str, Any]:
    ...
    try:
        ...
        logger.info('Job %s completed by miner %s', job_id, miner_id, extra={'job_id': job_id, 'miner_id': miner_id, 'output_size': len(str(complete_req.output))})
        return {'job_id': job_id, 'status': 'completed', 'state': job.state.value, 'completed_at': job.completed_at.isoformat() if job.completed_at else None, 'receipt_hash': complete_req.receipt.get('hash', '')[:16] if complete_req.receipt else None}
    except KeyError:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='job not found')
