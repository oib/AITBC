...
async def get_portfolio(request: Request, user_id: str | None = None) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if "error" in portfolio:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=portfolio["error"])
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get portfolio: {str(e)}"
        ) from e


@router.post("/", summary="Get portfolio for specific wallets")
async def get_portfolio_for_wallets(request: Request, req: PortfolioRequest) -> dict[str, Any]:
    """Get portfolio for specific wallet addresses"""
    try:
        service = get_portfolio_service()

        if not req.wallet_addresses:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="wallet_addresses required")

        portfolio = await service.get_portfolio(wallet_addresses=req.wallet_addresses)
...
async def get_wallet_breakdown(request: Request, address: str, chain_id: str | None = None) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if "error" in breakdown:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=breakdown["error"])
...
async def get_supported_chains(request: Request) -> dict[str, Any]:
    ...
    return {
        "chains": [
            {"chain_id": "ait-mainnet", "name": "AITBC Mainnet", "native_token": "AITBC"},
            {"chain_id": "ait-testnet", "name": "AITBC Testnet", "native_token": "tAITBC"},
        ]
    }
