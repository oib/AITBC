...
class AgentOrchestrator:
    ...
    async def _create_orchestration_plan(
        ...
        for stage_idx, stage_sub_tasks in enumerate(decomposition.execution_plan):
            stage_start = datetime.now(UTC) + timedelta(hours=stage_idx * 2)  # Estimate 2 hours per stage
            ...
            for sub_task_id in stage_sub_tasks:
                ...
                assignment = AgentAssignment(
                    sub_task_id=sub_task_id, agent_id="", assigned_at=datetime.now(UTC)  # Will be assigned during execution
                )
