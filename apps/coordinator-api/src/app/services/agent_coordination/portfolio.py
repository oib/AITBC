...
class AgentPortfolioManager:
    ...
    def __init__(self, session: Session, contract_service: ContractInteractionService, price_service: PriceService, risk_calculator: RiskCalculator, strategy_optimizer: StrategyOptimizer) -> None:
    ...
    async def create_portfolio(self, portfolio_data: PortfolioCreate, agent_address: str) -> PortfolioResponse:
        ...
        try:
            ...
            portfolio = AgentPortfolio(agent_address=agent_address, strategy_id=portfolio_data.strategy_id, initial_capital=portfolio_data.initial_capital, risk_tolerance=portfolio_data.risk_tolerance, is_active=True, created_at=datetime.now(UTC), last_rebalance=datetime.now(UTC))
    ...
    async def execute_trade(self, trade_request: TradeRequest, agent_address: str) -> TradeResponse:
        ...
        try:
            ...
            trade_result = await self.contract_service.execute_portfolio_trade(portfolio.contract_portfolio_id, trade_request.sell_token, trade_request.buy_token, trade_request.sell_amount, trade_request.min_buy_amount)
            trade = PortfolioTrade(portfolio_id=portfolio.id, sell_token=trade_request.sell_token, buy_token=trade_request.buy_token, sell_amount=trade_request.sell_amount, buy_amount=trade_result.buy_amount, price=trade_result.price, status=TradeStatus.EXECUTED, transaction_hash=trade_result.transaction_hash, executed_at=datetime.now(UTC))
    ...
    async def execute_rebalancing(self, rebalance_request: RebalanceRequest, agent_address: str) -> RebalanceResponse:
        ...
        try:
            ...
            return RebalanceResponse(success=True, message=f'Rebalanced with {len(executed_trades)} trades', trades_executed=len(executed_trades))
        except Exception as e:
    ...
    async def create_portfolio_strategy(self, strategy_data: StrategyCreate) -> StrategyResponse:
        ...
        try:
            ...
            strategy = PortfolioStrategy(name=strategy_data.name, strategy_type=strategy_data.strategy_type, target_allocations=strategy_data.target_allocations, max_drawdown=strategy_data.max_drawdown, rebalance_frequency=strategy_data.rebalance_frequency, is_active=True, created_at=datetime.now(UTC))
    ...
    async def _initialize_portfolio_assets(self, portfolio: AgentPortfolio, strategy: PortfolioStrategy) -> None:
        ...
        for token_symbol, allocation in strategy.target_allocations.items():
            if allocation > 0:
                asset = PortfolioAsset(portfolio_id=portfolio.id, token_symbol=token_symbol, target_allocation=allocation, current_allocation=0.0, balance=0, created_at=datetime.now(UTC))
                self.session.add(asset)

    async def _deploy_contract_portfolio(self, portfolio: AgentPortfolio, agent_address: str, strategy: PortfolioStrategy) -> str:
        ...
        try:
            ...
            portfolio_id = await self.contract_service.create_portfolio(agent_address, strategy.strategy_type.value, contract_allocations)
    ...
    async def _update_portfolio_assets(self, portfolio: AgentPortfolio, trade: PortfolioTrade) -> None:
        ...
        if buy_asset:
            ...
            new_asset = PortfolioAsset(portfolio_id=portfolio.id, token_symbol=trade.buy_token, target_allocation=0.0, current_allocation=0.0, balance=trade.buy_amount, created_at=datetime.now(UTC))
                ...
                if current_allocation > target_allocation:
                    ...
                    for other_asset in assets:
                        ...
                        if other_current < other_target:
                            trade = TradeRequest(sell_token=asset.token_symbol, buy_token=other_asset.token_symbol, sell_amount=sell_amount, min_buy_amount=0)
                            trades.append(trade)
                            break
        return trades
    ...
    async def _calculate_performance_metrics(self, portfolio: AgentPortfolio, period: str) -> dict:
        ...
        return {'total_return': total_return, 'current_value': current_value, 'initial_value': initial_value, 'total_trades': len(trades), 'last_updated': datetime.now(UTC).isoformat()}
