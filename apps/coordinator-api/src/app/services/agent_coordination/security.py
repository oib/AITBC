...
class AgentSecurityPolicy(SQLModel, table=True):
    ...
    allowed_verification_levels: list[VerificationLevel] = Field(default_factory=lambda: [VerificationLevel.BASIC], sa_column=Column(JSON))
...
class AgentAuditor:
    ...
    def _calculate_risk_score(self, event_type: AuditEventType, event_data: dict[str, Any], security_level: SecurityLevel) -> int:
        """Calculate risk score for audit event"""
        base_score = 0
        event_risk_scores = {AuditEventType.SECURITY_VIOLATION: 90, AuditEventType.SANDBOX_BREACH: 85, AuditEventType.ACCESS_DENIED: 70, AuditEventType.VERIFICATION_FAILED: 50, AuditEventType.EXECUTION_FAILED: 30, AuditEventType.STEP_FAILED: 20, AuditEventType.EXECUTION_CANCELLED: 15, AuditEventType.WORKFLOW_DELETED: 10, AuditEventType.WORKFLOW_CREATED: 5, AuditEventType.EXECUTION_STARTED: 3, AuditEventType.EXECUTION_COMPLETED: 1, AuditEventType.STEP_STARTED: 1, AuditEventType.STEP_COMPLETED: 1, AuditEventType.VERIFICATION_COMPLETED: 1}
        base_score += event_risk_scores.get(event_type, 0)
        security_multipliers = {SecurityLevel.PUBLIC: 1.0, SecurityLevel.INTERNAL: 1.2, SecurityLevel.CONFIDENTIAL: 1.5, SecurityLevel.RESTRICTED: 2.0}
...
class AgentTrustManager:
    ...
    async def update_trust_score(self, entity_type: str, entity_id: str, execution_success: bool, execution_time: float | None=None, security_violation: bool=False, policy_violation: bool=False) -> AgentTrustScore:
        ...
        trust_score_row = self.session.scalars(select(AgentTrustScore).where((AgentTrustScore.entity_type == entity_type) & (AgentTrustScore.entity_id == entity_id))).first()
        ...
        if security_violation:
            ...
            cast(list[Any], trust_score.violation_history).append({'timestamp': datetime.now(UTC).isoformat(), 'type': 'security_violation'})
        if policy_violation:
            trust_score.policy_violations += 1
            trust_score.last_violation = datetime.now(UTC)
            cast(list[Any], trust_score.violation_history).append({'timestamp': datetime.now(UTC).isoformat(), 'type': 'policy_violation'})
        ...
        trust_score.verification_success_rate = trust_score.successful_executions / trust_score.total_executions * 100 if trust_score.total_executions > 0 else 0
        if execution_time:
            if trust_score.average_execution_time is None:
                trust_score.average_execution_time = execution_time
            else:
                trust_score.average_execution_time = (trust_score.average_execution_time * (trust_score.total_executions - 1) + execution_time) / trust_score.total_executions
...
class AgentSandboxManager:
    ...
    async def create_sandbox_environment(self, execution_id: str, security_level: SecurityLevel=SecurityLevel.PUBLIC, workflow_requirements: dict[str, Any] | None=None) -> AgentSandboxConfig:
    ...
    async def monitor_sandbox(self, execution_id: str) -> dict[str, Any]:
        ...
        """
        sandbox = self.session.execute(select(AgentSandboxConfig).where(AgentSandboxConfig.id == f'sandbox_{execution_id}')).first()
        if not sandbox:
            raise ValueError(f'Sandbox not found for execution {execution_id}')
        monitoring_data = {'execution_id': execution_id, 'sandbox_type': sandbox.sandbox_type, 'security_level': sandbox.security_level, 'resource_usage': {'cpu_percent': 0.0, 'memory_mb': 0, 'disk_mb': 0}, 'security_events': [], 'command_count': 0, 'file_access_count': 0, 'network_access_count': 0, 'status': 'configured', 'note': 'Monitoring requires sandbox runtime integration'}
...
class AgentSecurityManager:
    ...
    async def create_security_policy(self, name: str, description: str, security_level: SecurityLevel, policy_rules: dict[str, Any]) -> AgentSecurityPolicy:
        """Create a new security policy"""
        policy = AgentSecurityPolicy(name=name, description=description, security_level=security_level, **policy_rules)
        self.session.add(policy)
        self.session.commit()
        self.session.refresh(policy)
        await self.auditor.log_event(AuditEventType.WORKFLOW_CREATED, user_id='system', security_level=SecurityLevel.INTERNAL, event_data={'policy_name': name, 'policy_id': policy.id}, new_state={'policy': policy.dict()})
        return policy
    ...
    async def validate_workflow_security(self, workflow: AIAgentWorkflow, user_id: str) -> dict[str, Any]:
        ...
        if security_sensitive_steps:
            ...
            validation_result['recommendations'].append('Consider using higher security level for workflows with sensitive operations')
        if workflow.max_execution_time > 3600:
            validation_result['warnings'].append(f'Long execution time ({workflow.max_execution_time}s) may require additional security measures')
        if not workflow.requires_verification:
            validation_result['violations'].append('Workflow does not require verification - this is not recommended for production use')
        ...
        await self.auditor.log_event(AuditEventType.WORKFLOW_CREATED, workflow_id=workflow.id, user_id=user_id, security_level=cast(SecurityLevel, validation_result['required_security_level']), event_data={'validation_result': validation_result})
