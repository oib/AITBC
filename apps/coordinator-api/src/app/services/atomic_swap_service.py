...
class AtomicSwapService:
    ...
    async def create_swap_order(self, request: SwapCreateRequest) -> AtomicSwapOrder:
        ...
        if request.source_timelock_hours <= request.target_timelock_hours:
            raise HTTPException(status_code=400, detail='Source timelock must be strictly greater than target timelock to ensure safety for initiator.')
    ...
    async def get_agent_swaps(self, agent_id: str) -> list[AtomicSwapOrder]:
        ...
        return list(self.session.scalars(select(AtomicSwapOrder).where((AtomicSwapOrder.initiator_agent_id == agent_id) | (AtomicSwapOrder.participant_agent_id == agent_id))).all())
