...
class BountyService:
    ...
    async def get_bounty_submissions(self, bounty_id: str) -> list[BountySubmission]:
        ...
        try:
            stmt = select(BountySubmission).where(BountySubmission.bounty_id == bounty_id).order_by(BountySubmission.submission_time.desc())  # type: ignore[attr-defined, arg-type]
    ...
    async def get_bounty_stats(self, period: str='monthly') -> BountyStats:
        ...
        try:
            ...
            tier_stmt = select(Bounty.tier, func.count(Bounty.bounty_id).label('count')).where(Bounty.creation_time >= start_date).group_by(Bounty.tier)  # type: ignore[arg-type, call-overload]
    ...
    async def search_bounties(self, query: str, page: int=1, limit: int=20) -> list[Bounty]:
        ...
        try:
            ...
            stmt = select(Bounty).where(or_(Bounty.title.ilike(search_pattern), Bounty.description.ilike(search_pattern))).order_by(Bounty.creation_time.desc())  # type: ignore[attr-defined]
            offset = (page - 1) * limit
