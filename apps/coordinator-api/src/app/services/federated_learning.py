...
class FederatedLearningService:
    ...
    async def create_session(self, request: FederatedSessionCreate) -> FederatedLearningSession:
        ...
        session = FederatedLearningSession(initiator_agent_id=request.initiator_agent_id, task_description=request.task_description, model_architecture_cid=request.model_architecture_cid, initial_weights_cid=request.initial_weights_cid, target_participants=request.target_participants, total_rounds=request.total_rounds, aggregation_strategy=request.aggregation_strategy, min_participants_per_round=request.min_participants_per_round, reward_pool_amount=request.reward_pool_amount, status=TrainingStatus.GATHERING_PARTICIPANTS)
    ...
    async def join_session(self, session_id: str, request: JoinSessionRequest) -> TrainingParticipant:
        ...
        existing = self.session.execute(select(TrainingParticipant).where(TrainingParticipant.session_id == session_id, TrainingParticipant.agent_id == request.agent_id)).first()
        if existing:
            raise HTTPException(status_code=400, detail='Agent already joined this session')
        mock_reputation = 95.0
        participant = TrainingParticipant(session_id=session_id, agent_id=request.agent_id, compute_power_committed=request.compute_power_committed, reputation_score_at_join=mock_reputation, status=ParticipantStatus.JOINED)
    ...
    async def _start_training(self, fl_session: FederatedLearningSession) -> None:
        ...
        round1 = TrainingRound(session_id=fl_session.id, round_number=1, status='active', starting_model_cid=fl_session.initial_weights_cid or fl_session.model_architecture_cid)
    ...
    async def submit_local_update(self, session_id: str, round_id: str, request: SubmitUpdateRequest) -> LocalModelUpdate:
        ...
        participant = self.session.execute(select(TrainingParticipant).where(TrainingParticipant.session_id == session_id, TrainingParticipant.agent_id == request.agent_id)).first()
        if not participant:
            raise HTTPException(status_code=403, detail='Agent is not a participant in this session')
        update = LocalModelUpdate(round_id=round_id, participant_agent_id=request.agent_id, weights_cid=request.weights_cid, zk_proof_hash=request.zk_proof_hash)
    ...
    async def _aggregate_round(self, fl_session: FederatedLearningSession, current_round: TrainingRound) -> None:
        ...
        if fl_session.current_round >= fl_session.total_rounds:
            ...
            fl_session.current_round += 1
            fl_session.status = TrainingStatus.TRAINING
            next_round = TrainingRound(session_id=fl_session.id, round_number=fl_session.current_round, status='active', starting_model_cid=new_global_cid)
