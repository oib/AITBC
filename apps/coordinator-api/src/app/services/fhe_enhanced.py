...
class BFVContext:
    ...
    def generate(cls, poly_modulus_degree: int=4096, plain_modulus: int=1032193) -> BFVContext:
        ...
        coeff_modulus = 2 ** 60
        return cls(poly_modulus_degree=poly_modulus_degree, plain_modulus=plain_modulus, coeff_modulus=coeff_modulus, public_key=public_key, secret_key=secret_key, scale=2 ** 40)
...
class EncryptedVector:
    ...
    def serialize(self) -> dict[str, Any]:
        ...
        return {'ciphertext': self.ciphertext.tobytes().hex(), 'shape': self.shape, 'dtype': self.dtype, 'context_id': self.context_id, 'scheme': 'bfv-simplified'}
...
class BFVProvider:
    ...
    def generate_context(self, scheme: str='bfv', poly_modulus_degree: int=4096, **kwargs: Any) -> dict[str, Any]:
        ...
        try:
            if scheme not in ['bfv', 'ckks', 'simplified']:
                scheme = 'bfv'
            context = BFVContext.generate(poly_modulus_degree=poly_modulus_degree, plain_modulus=kwargs.get('plain_modulus', 1032193))
            ...
            return {'context_id': context_id, 'scheme': scheme, 'poly_modulus_degree': poly_modulus_degree, 'plain_modulus': context.plain_modulus, 'coeff_modulus_bits': 60, 'scale': context.scale, 'public_key_hash': hash(context.public_key.tobytes()) % 10000, 'status': 'ready'}
    ...
    def add_cipher_cipher(self, encrypted_a: EncryptedVector, encrypted_b: EncryptedVector) -> EncryptedVector:
        ...
        return EncryptedVector(ciphertext=result_ciphertext, shape=encrypted_a.shape, dtype=encrypted_a.dtype, context_id=encrypted_a.context_id)
    ...
    def add_cipher_plain(self, encrypted: EncryptedVector, plain: np.ndarray) -> EncryptedVector:
        ...
        return EncryptedVector(ciphertext=result_ciphertext, shape=encrypted.shape, dtype=encrypted.dtype, context_id=encrypted.context_id)
    ...
    def get_context_info(self, context_id: str) -> dict[str, Any]:
        ...
        return {'context_id': context_id, 'poly_modulus_degree': context.poly_modulus_degree, 'plain_modulus': context.plain_modulus, 'scale': context.scale, 'available': True}
