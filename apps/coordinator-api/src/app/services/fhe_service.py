...
class MockFHEProvider(FHEProvider):
    ...
    def generate_context(self, scheme: str, **kwargs: Any) -> FHEContext:
        ...
        return FHEContext(scheme='mock', poly_modulus_degree=kwargs.get('poly_modulus_degree', 8192), coeff_modulus=kwargs.get('coeff_modulus', [60, 40, 60]), scale=kwargs.get('scale', 2 ** 40), public_key=b'mock_public_key', private_key=b'mock_private_key', provider_specific={'mock': True})
...
class TenSEALProvider(FHEProvider):
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            logger.warning('TenSEAL not available: %s', e)
    ...
    def generate_context(self, scheme: str, **kwargs: Any) -> FHEContext:
        ...
        if scheme.lower() == 'ckks':
            context = self.ts.context(self.ts.SCHEME_TYPE.CKKS, poly_modulus_degree=kwargs.get('poly_modulus_degree', 8192), coeff_mod_bit_sizes=kwargs.get('coeff_mod_bit_sizes', [60, 40, 40, 60]))
            context.global_scale = kwargs.get('scale', 2 ** 40)
            context.generate_galois_keys()
        elif scheme.lower() == 'bfv':
            context = self.ts.context(self.ts.SCHEME_TYPE.BFV, poly_modulus_degree=kwargs.get('poly_modulus_degree', 8192), coeff_mod_bit_sizes=kwargs.get('coeff_mod_bit_sizes', [60, 40, 60]))
        else:
            raise ValueError(f'Unsupported scheme: {scheme}')
        return FHEContext(scheme=scheme, poly_modulus_degree=kwargs.get('poly_modulus_degree', 8192), coeff_modulus=kwargs.get('coeff_mod_bit_sizes', [60, 40, 60]), scale=kwargs.get('scale', 2 ** 40), public_key=context.serialize(save_secret_key=True), private_key=context.serialize(save_secret_key=True), provider_specific={'is_public': context.is_public})
    ...
    def encrypted_inference(self, model: dict[str, Any], encrypted_input: EncryptedData) -> EncryptedData:
        ...
        if weights is not None and biases is not None:
            ...
            return EncryptedData(ciphertext=result.serialize(), context=encrypted_input.context, shape=(len(biases_array),), dtype='float32')
        else:
            raise ValueError('Model must contain weights and biases')
...
class ConcreteMLProvider(FHEProvider):
    ...
    def __init__(self) -> None:
        ...
        if sys.version_info >= (3, 13):
            logger.warning('Concrete ML requires Python <3.13. Current version: %s', sys.version.split()[0])
        else:
            try:
                ...
                logger.warning('Concrete ML not available: %s', e)

    def generate_context(self, scheme: str, **kwargs: Any) -> FHEContext:
        """Generate Concrete ML context"""
        if not self.available:
            raise RuntimeError('Concrete ML provider is not available')
        return FHEContext(scheme='concrete', poly_modulus_degree=kwargs.get('poly_modulus_degree', 1024), coeff_modulus=[kwargs.get('coeff_modulus', 15)], scale=1.0, public_key=b'concrete_context_placeholder', private_key=None, provider_specific={'p': kwargs.get('p', 15), 'compilation_required': True})
    ...
    def encrypted_inference(self, model: dict[str, Any], encrypted_input: EncryptedData) -> EncryptedData:
        ...
        if not self.available:
            raise RuntimeError('Concrete ML provider is not available')
        logger.warning('Concrete ML inference requires circuit compilation. Use the compile_and_execute method for full functionality.')
...
class FHEService:
    ...
    def encrypted_inference(self, model: dict[str, Any], encrypted_input: EncryptedData, provider: str | None=None) -> EncryptedData:
        """Perform inference on encrypted data"""
        fhe_provider = self.get_provider(provider)
