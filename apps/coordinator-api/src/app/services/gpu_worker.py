...
class OllamaClient:
    ...
    async def generate(self, model: str, prompt: str, options: dict | None=None) -> dict[str, Any]:
        ...
        try:
            ...
            return {'success': True, 'output': result.get('response', ''), 'model': model, 'prompt_length': len(prompt), 'tokens_generated': result.get('eval_count', 0), 'execution_time_ms': execution_time, 'done': result.get('done', False)}
...
class GPUWorker:
    ...
    def __init__(self, worker_id: str, ollama_url: str='http://localhost:11434', max_concurrent: int=2, coordinator_url: str='http://localhost:8203'):
    ...
    async def initialize(self) -> bool:
        ...
        self._capabilities = GPUCapabilities(gpu_available=ollama_healthy, models=models, max_concurrency=self.max_concurrent, memory_gb=memory_gb, compute_units=compute_units, architecture='cuda' if ollama_healthy else 'cpu', edge_optimized=False)
    ...
    async def _poll_and_execute(self, api_key: str) -> None:
        ...
        try:
            response = await self._http_client.post(f'{self.coordinator_url}/miners/{self.worker_id}/poll', headers={'X-Miner-ID': self.worker_id, 'X-API-Key': api_key}, params={'max_wait_seconds': 5})
    ...
    async def _execute_job(self, job: dict[str, Any]) -> JobExecutionResult:
        ...
        try:
            ...
            if model not in (self._capabilities.models if self._capabilities else []):
                return JobExecutionResult(success=False, output={}, execution_time_ms=0, gpu_utilization=0.0, receipt={}, error=f'Model {model} not available')
            if self._capabilities and self._capabilities.gpu_available:
                ...
                inference_result = {'success': True, 'output': f'[Mock output for {model}] Generated text based on: {prompt[:50]}...', 'model': model, 'prompt_length': len(prompt), 'tokens_generated': max_tokens, 'execution_time_ms': 100, 'done': True}
            execution_time = int((time.time() - start_time) * 1000)
            ...
            try:
                import subprocess
                result = subprocess.run(['nvidia-smi', '--query-gpu=utilization.gpu', '--format=csv,noheader,nounits'], capture_output=True, text=True, timeout=5)
                if result.returncode == 0 and result.stdout.strip():
                    gpu_utilization = float(result.stdout.strip()) / 100.0
            except (FileNotFoundError, subprocess.TimeoutExpired, ValueError):
                gpu_utilization = min(0.9, max(0.1, execution_time / 5000.0))
            return JobExecutionResult(success=True, output=inference_result, execution_time_ms=execution_time, gpu_utilization=gpu_utilization, receipt=receipt)
        except Exception as e:
            execution_time = int((time.time() - start_time) * 1000)
            return JobExecutionResult(success=False, output={}, execution_time_ms=execution_time, gpu_utilization=0.0, receipt={}, error=str(e))

    async def _submit_result(self, job_id: str, result: JobExecutionResult, api_key: str) -> None:
        """Submit job result to coordinator"""
        try:
            response = await self._http_client.post(f'{self.coordinator_url}/miners/{self.worker_id}/jobs/{job_id}/complete', headers={'X-Miner-ID': self.worker_id, 'X-API-Key': api_key}, json={'output': result.output, 'receipt': result.receipt})
            if response.status_code == 200:
    ...
    def _generate_receipt(self, job_id: str, inference_result: dict[str, Any], execution_time_ms: int) -> dict[str, Any]:
        ...
        verification_data = {'job_id': job_id, 'worker_id': self.worker_id, 'model': inference_result.get('model'), 'tokens_generated': inference_result.get('tokens_generated'), 'execution_time_ms': execution_time_ms, 'timestamp': timestamp}
        hash_value = hashlib.sha256(json.dumps(verification_data, sort_keys=True).encode()).hexdigest()
        return {'hash': hash_value, 'worker_id': self.worker_id, 'timestamp': timestamp, 'verification_data': verification_data, 'proof_type': 'gpu_inference'}
