...
class IPFSAdapterService:
    ...
    def __init__(self, session: Session, contract_service: ContractInteractionService, ipfs_gateway_url: str='http://127.0.0.1:5001/api/v0', pinning_service_token: str | None=None):
    ...
    async def store_memory(self, request: MemoryNodeCreate, raw_data: bytes, zk_proof_hash: str | None=None) -> AgentMemoryNode:
        """
        Upload raw memory data (e.g. serialized vector DB or JSON knowledge graph) to IPFS
        and create a tracking record.
        """
        node = AgentMemoryNode(agent_id=request.agent_id, memory_type=request.memory_type, is_encrypted=request.is_encrypted, metadata=request.metadata, tags=request.tags, size_bytes=len(raw_data), status=StorageStatus.PENDING, zk_proof_hash=zk_proof_hash)
    ...
    async def get_memory_nodes(self, agent_id: str, memory_type: MemoryType | None=None, tags: list[str] | None=None) -> list[AgentMemoryNode]:
