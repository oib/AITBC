...
class IPFSStorageService:
    ...
    async def upload_memory(self, agent_id: str, memory_data: Any, memory_type: str='experience', tags: list[str] | None=None, compress: bool=True, pin: bool=False) -> IPFSUploadResult:
        ...
        try:
            ...
            if should_pin:
                ...
                pinned = False
            metadata = MemoryMetadata(agent_id=agent_id, memory_type=memory_type, timestamp=start_time, version=1, tags=tags, compression_ratio=compression_ratio, integrity_hash=integrity_hash)
            await self._store_metadata(cid, metadata)
            upload_result = IPFSUploadResult(cid=cid, size=original_size, compressed_size=len(upload_data), upload_time=start_time, pinned=pinned)
    ...
    async def batch_upload_memories(self, agent_id: str, memories: list[tuple[Any, str, list[str]]], batch_size: int=10) -> list[IPFSUploadResult]:
