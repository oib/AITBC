...
class MarketplaceMonitor:
    ...
    def __init__(self) -> None:
        ...
        self.alert_thresholds = {'api_latency_p95_ms': 500.0, 'api_error_rate_pct': 5.0, 'gpu_utilization_pct': 90.0, 'matching_time_ms': 100.0, 'miner_uptime_pct': 95.0, 'miner_response_time_ms': 1000.0, 'job_completion_rate_pct': 90.0, 'capacity_availability_pct': 80.0}
    ...
    def _evaluate_alerts(self) -> None:
        ...
        if avg_error_rate > self.alert_thresholds['api_error_rate_pct']:
            current_alerts.append({'id': f'alert_error_{int(time.time())}', 'severity': 'critical', 'metric': 'error_rate', 'value': avg_error_rate, 'threshold': self.alert_thresholds['api_error_rate_pct'], 'message': f'High API Error Rate: {avg_error_rate:.2f}%', 'timestamp': datetime.now(UTC).isoformat()})
        ...
        if avg_matching > self.alert_thresholds['matching_time_ms']:
            current_alerts.append({'id': f'alert_matching_{int(time.time())}', 'severity': 'medium', 'metric': 'matching_time', 'value': avg_matching, 'threshold': self.alert_thresholds['matching_time_ms'], 'message': f'Slow Order Matching: {avg_matching:.2f}ms', 'timestamp': datetime.now(UTC).isoformat()})
        avg_uptime = self.miner_uptime_pct.get_average(window_seconds=60)
        if avg_uptime < self.alert_thresholds['miner_uptime_pct']:
            current_alerts.append({'id': f'alert_miner_uptime_{int(time.time())}', 'severity': 'high' if avg_uptime < self.alert_thresholds['miner_uptime_pct'] * 0.9 else 'medium', 'metric': 'miner_uptime', 'value': avg_uptime, 'threshold': self.alert_thresholds['miner_uptime_pct'], 'message': f'Low Miner Uptime: {avg_uptime:.2f}%', 'timestamp': datetime.now(UTC).isoformat()})
        ...
        if p95_response > self.alert_thresholds['miner_response_time_ms']:
            current_alerts.append({'id': f'alert_miner_response_{int(time.time())}', 'severity': 'high' if p95_response > self.alert_thresholds['miner_response_time_ms'] * 2 else 'medium', 'metric': 'miner_response_time', 'value': p95_response, 'threshold': self.alert_thresholds['miner_response_time_ms'], 'message': f'High Miner Response Time (p95): {p95_response:.2f}ms', 'timestamp': datetime.now(UTC).isoformat()})
        ...
        if avg_completion < self.alert_thresholds['job_completion_rate_pct']:
            current_alerts.append({'id': f'alert_job_completion_{int(time.time())}', 'severity': 'critical', 'metric': 'job_completion_rate', 'value': avg_completion, 'threshold': self.alert_thresholds['job_completion_rate_pct'], 'message': f'Low Job Completion Rate: {avg_completion:.2f}%', 'timestamp': datetime.now(UTC).isoformat()})
        ...
        if avg_capacity < self.alert_thresholds['capacity_availability_pct']:
            current_alerts.append({'id': f'alert_capacity_{int(time.time())}', 'severity': 'high', 'metric': 'capacity_availability', 'value': avg_capacity, 'threshold': self.alert_thresholds['capacity_availability_pct'], 'message': f'Low Capacity Availability: {avg_capacity:.2f}%', 'timestamp': datetime.now(UTC).isoformat()})
