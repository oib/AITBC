...
class ScalingPolicy:
    ...
    def __init__(self, min_nodes: int=2, max_nodes: int=100, target_utilization: float=0.75, scale_up_threshold: float=0.85, scale_down_threshold: float=0.4, cooldown_period_sec: int=300, predictive_scaling: bool=True):
...
class ResourceScaler:
    ...
    async def evaluate_scaling(self, current_utilization: float, queue_depth: int) -> dict[str, Any] | None:
        ...
        if action and target_nodes != self.current_nodes:
            ...
            record = {'timestamp': datetime.now(UTC).isoformat(), 'action': action, 'nodes_changed': diff, 'new_total': target_nodes, 'reason': reason, 'metrics_at_time': {'utilization': current_utilization, 'queue_depth': queue_depth, 'predicted_utilization': predicted_utilization}}
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        return {'status': 'running' if self.is_running else 'stopped', 'current_nodes': {'total': self.current_nodes, 'cpu_nodes': self.active_cpu_nodes, 'gpu_nodes': self.active_gpu_nodes}, 'policy': {'min_nodes': self.policy.min_nodes, 'max_nodes': self.policy.max_nodes, 'target_utilization': self.policy.target_utilization}, 'last_action': self.scaling_history[-1] if self.scaling_history else None, 'prediction': {'next_hour_utilization_estimate': round(self._predict_demand(1), 3)}}
