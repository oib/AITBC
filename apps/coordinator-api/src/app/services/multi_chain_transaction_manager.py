...
class MultiChainTransactionManager:
    ...
    def __init__(self, session: Session):
        ...
        self.routing_config: dict[str, Any] = {'default_strategy': RoutingStrategy.BALANCED, 'max_retries': 3, 'retry_delay': 5, 'confirmation_threshold': 6, 'gas_price_multiplier': 1.1, 'max_pending_per_chain': 100}
        self.metrics: dict[str, Any] = {'total_transactions': 0, 'successful_transactions': 0, 'failed_transactions': 0, 'average_processing_time': 0.0, 'chain_performance': defaultdict(dict)}
    ...
    async def initialize(self, chain_configs: dict[int, dict[str, Any]]) -> None:
        ...
        try:
            for chain_id, config in chain_configs.items():
                adapter = WalletAdapterFactory.create_adapter(chain_id=chain_id, rpc_url=config['rpc_url'], security_level=SecurityLevel(config.get('security_level', 'medium')))
                self.wallet_adapters[chain_id] = adapter
                self.metrics['chain_performance'][chain_id] = {'total_transactions': 0, 'success_rate': 0.0, 'average_gas_price': 0.0, 'average_confirmation_time': 0.0, 'last_updated': datetime.now(UTC)}
    ...
    async def submit_transaction(self, user_id: str, chain_id: int, transaction_type: MultiChainTransactionType, from_address: str, to_address: str, amount: Decimal | float | str, token_address: str | None=None, data: dict[str, Any] | None=None, priority: TransactionPriority=TransactionPriority.MEDIUM, routing_strategy: RoutingStrategy | None=None, gas_limit: int | None=None, gas_price: int | None=None, max_fee_per_gas: int | None=None, deadline_minutes: int=30, metadata: dict[str, Any] | None=None) -> dict[str, Any]:
        ...
        try:
            ...
            transaction = MultiChainTransaction(id=transaction_id, user_id=user_id, chain_id=chain_id, transaction_type=transaction_type, from_address=from_address, to_address=to_address, amount=float(amount), token_address=token_address, data=data or {}, priority=priority, routing_strategy=routing_strategy or self.routing_config['default_strategy'], gas_limit=gas_limit, gas_price=gas_price, max_fee_per_gas=max_fee_per_gas, status=TransactionStatus.QUEUED, deadline=datetime.now(UTC) + timedelta(minutes=deadline_minutes), meta_data=metadata or {}, retry_count=0, submit_attempts=0)
            self.session.add(transaction)
            self.session.commit()
            self.session.refresh(transaction)
            logger.info('Submitted transaction %s for user %s', transaction_id, user_id)
            return {'transaction_id': transaction_id, 'status': TransactionStatus.QUEUED.value, 'priority': priority.value, 'estimated_processing_time': await self._estimate_processing_time(transaction), 'deadline': transaction.deadline.isoformat(), 'submitted_at': transaction.created_at.isoformat()}
        except Exception as e:
    ...
    async def cancel_transaction(self, transaction_id: str, reason: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {'transaction_id': transaction_id, 'status': TransactionStatus.CANCELLED.value, 'reason': reason, 'cancelled_at': transaction.cancelled_at.isoformat()}
        except Exception as e:
            logger.error('Error cancelling transaction: %s', e)
            self.session.rollback()
            raise

    async def get_transaction_history(self, user_id: str | None=None, chain_id: int | None=None, transaction_type: MultiChainTransactionType | None=None, status: TransactionStatus | None=None, priority: TransactionPriority | None=None, limit: int=100, offset: int=0, from_date: datetime | None=None, to_date: datetime | None=None) -> list[dict[str, Any]]:
    ...
    async def optimize_transaction_routing(self, transaction_type: MultiChainTransactionType, amount: float, from_chain: int, to_chain: int | None=None, urgency: TransactionPriority=TransactionPriority.MEDIUM) -> dict[str, Any]:
        ...
        try:
            ...
            for chain_id in self.wallet_adapters.keys():
                ...
                routing_options.append({'chain_id': chain_id, 'score': score, 'estimated_gas_price': chain_metrics.get('average_gas_price', 0), 'estimated_confirmation_time': chain_metrics.get('average_confirmation_time', 0), 'success_rate': chain_metrics.get('success_rate', 0), 'queue_length': 0})
            routing_options.sort(key=lambda x: x['score'], reverse=True)
            best_option = routing_options[0] if routing_options else None
            return {'recommended_chain': best_option['chain_id'] if best_option else None, 'routing_options': routing_options, 'optimization_factors': {'gas_price_weight': 0.3, 'confirmation_time_weight': 0.3, 'success_rate_weight': 0.2, 'queue_length_weight': 0.2}, 'generated_at': datetime.now(UTC).isoformat()}
        except Exception as e:
    ...
    async def _calculate_transaction_progress(self, transaction: MultiChainTransaction) -> float:
        ...
        status_progress = {TransactionStatus.QUEUED: 0.0, TransactionStatus.PENDING: 10.0, TransactionStatus.PROCESSING: 30.0, TransactionStatus.SUBMITTED: 50.0, TransactionStatus.CONFIRMED: 80.0, TransactionStatus.COMPLETED: 100.0, TransactionStatus.FAILED: 100.0, TransactionStatus.CANCELLED: 100.0, TransactionStatus.EXPIRED: 100.0}
    ...
    async def _estimate_processing_time(self, transaction: MultiChainTransaction) -> float:
        ...
        priority_time = {TransactionPriority.CRITICAL: 30.0, TransactionPriority.URGENT: 60.0, TransactionPriority.HIGH: 120.0, TransactionPriority.MEDIUM: 300.0, TransactionPriority.LOW: 600.0}
    ...
    def _get_min_reputation_for_transaction(self, transaction_type: MultiChainTransactionType, priority: TransactionPriority) -> float:
        ...
        priority_multiplier = {TransactionPriority.CRITICAL: 1.5, TransactionPriority.URGENT: 1.3, TransactionPriority.HIGH: 1.1, TransactionPriority.MEDIUM: 1.0, TransactionPriority.LOW: 0.8}
        ...
        try:
            ...
            if status == TransactionStatus.COMPLETED.value:
                ...
                if transaction['confirmations'] > 0:
                    confirmation_progress = min(transaction['confirmations'] / self.routing_config['confirmation_threshold'] * 30, 30.0)
