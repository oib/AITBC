...
class Payment:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "escrow": {"id": self.escrow_id, "released": self.escrow_released} if self.escrow_id else None,
            "transaction": {"hash": self.tx_hash, "confirmations": self.block_confirmation} if self.tx_hash else None,
        }
...
class PaymentsService:
    ...
    def get_payment_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "by_status": {"completed": completed, "pending": pending, "escrowed": escrowed, "refunded": refunded},
            "escrow_holdings": sum(e["amount"] for e in self._escrows.values() if e["status"] == "held"),
        }
