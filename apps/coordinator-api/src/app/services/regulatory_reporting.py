...
class RegulatoryReporter:
    ...
    async def generate_sar_report(self, activities: list[SuspiciousActivity]) -> RegulatoryReport:
        ...
        try:
            ...
            logger.error('❌ SAR report generation failed: %s', e)
            raise
    ...
    async def generate_ctr_report(self, transactions: list[dict[str, Any]]) -> RegulatoryReport:
        ...
        try:
            ...
            logger.error('❌ CTR report generation failed: %s', e)
            raise
    ...
    async def submit_report(self, report_id: str) -> bool:
        ...
        try:
            ...
            if report.status != ReportStatus.DRAFT:
                logger.warning('⚠️  Report %s already submitted', report_id)
                return False
            await asyncio.sleep(2)
            report.status = ReportStatus.SUBMITTED
            report.submitted_at = datetime.now()
            logger.info('✅ Report %s submitted to %s', report_id, report.regulatory_body.value)
            return True
        except Exception as e:
    ...
    def list_reports(self, report_type: ReportType | None=None, status: ReportStatus | None=None) -> list[dict[str, Any]]:
        ...
        return [{'report_id': r.report_id, 'report_type': r.report_type.value, 'regulatory_body': r.regulatory_body.value, 'status': r.status.value, 'generated_at': r.generated_at.isoformat()} for r in sorted(filtered_reports, key=lambda x: x.generated_at, reverse=True)]
    ...
    def _analyze_timing_patterns(self, activities: list[SuspiciousActivity]) -> dict[str, Any]:
        ...
        return {'time_span': time_span, 'activity_density': activity_density, 'peak_hours': 'Off-hours activity detected' if activity_density > 10 else 'Normal activity pattern'}
...
async def generate_sar(activities: list[dict[str, Any]]) -> dict[str, Any]:
    """Generate SAR report"""
    suspicious_activities = [SuspiciousActivity(activity_id=activity['id'], timestamp=datetime.fromisoformat(activity['timestamp']), user_id=activity['user_id'], activity_type=activity['type'], description=activity['description'], amount=activity['amount'], currency=activity['currency'], risk_score=activity['risk_score'], indicators=activity['indicators'], evidence=activity.get('evidence', {})) for activity in activities]
    report = await regulatory_reporter.generate_sar_report(suspicious_activities)
    return {'report_id': report.report_id, 'report_type': report.report_type.value, 'status': report.status.value, 'generated_at': report.generated_at.isoformat()}
...
async def generate_compliance_summary(period_start: str, period_end: str) -> dict[str, Any]:
    ...
    return {'report_id': report.report_id, 'report_type': report.report_type.value, 'status': report.status.value, 'generated_at': report.generated_at.isoformat(), 'overall_score': report.content.get('executive_summary', {}).get('overall_compliance_score', 0)}
