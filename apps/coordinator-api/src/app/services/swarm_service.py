...
class SwarmNode:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {'node_id': self.node_id, 'address': self.address, 'capabilities': self.capabilities, 'status': self.status.value, 'resources': {'cpu_cores': self.cpu_cores, 'memory_gb': self.memory_gb, 'gpu_count': self.gpu_count}, 'metrics': {'tasks_completed': self.tasks_completed, 'tasks_failed': self.tasks_failed, 'load_percentage': self.load_percentage}, 'last_heartbeat': self.last_heartbeat.isoformat()}
...
class SwarmCluster:
    ...
    def to_dict(self, node_service: Any) -> dict[str, Any]:
        return {'cluster_id': self.cluster_id, 'name': self.name, 'description': self.description, 'created_at': self.created_at.isoformat(), 'node_count': len(self.nodes), 'task_count': len(self.tasks), 'nodes': [node_service.get_node(n).to_dict() if node_service.get_node(n) else {'node_id': n} for n in self.nodes], 'status': 'active' if self.nodes else 'empty'}
...
class SwarmService:
    ...
    def register_node(self, node_id: str, address: str, capabilities: list[str], cpu_cores: int=4, memory_gb: int=16, gpu_count: int=0) -> SwarmNode:
        """
        ...
        Returns:
            Registered node
        """
        node = SwarmNode(node_id=node_id, address=address, capabilities=capabilities, status=NodeStatus.online, last_heartbeat=datetime.now(UTC), cpu_cores=cpu_cores, memory_gb=memory_gb, gpu_count=gpu_count)
    ...
    def submit_task(self, task_type: str, payload: dict[str, Any], required_capabilities: list[str] | None=None, priority: int=1) -> SwarmTask:
        """
        ...
        task = SwarmTask(task_id=task_id, task_type=task_type, payload=payload, status=TaskStatus.pending, max_retries=self.MAX_RETRIES)
    ...
    def report_task_status(self, task_id: str, node_id: str, status: str, result: dict[str, Any] | None=None, error: str | None=None) -> bool:
        """
    ...
    def create_cluster(self, name: str, description: str='', node_ids: list[str] | None=None) -> SwarmCluster:
        ...
        cluster = SwarmCluster(cluster_id=cluster_id, name=name, description=description, created_at=datetime.now(UTC), nodes=set(node_ids) if node_ids else set())
