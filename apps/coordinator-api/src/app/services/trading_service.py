...
class P2PTradingProtocol:
    ...
    async def create_trade_request(
        ...
        **kwargs,
        ...
        trade_request = TradeRequest(
            ...
            expires_at=kwargs.get("expires_at", datetime.now(timezone.utc) + timedelta(days=7)),
        )
    ...
    async def find_matches(self, request_id: str) -> list[dict[str, Any]]:
        ...
        for match in matches:
            trade_match = TradeMatch(
                ...
                expires_at=datetime.now(timezone.utc) + timedelta(hours=self.matching_engine.match_expiry_hours),
            )
        ...
        trade_request.updated_at = datetime.now(timezone.utc)
        self.session.commit()
    ...
    async def initiate_negotiation(
        ...
        negotiation = TradeNegotiation(
            ...
            expires_at=datetime.now(timezone.utc) + timedelta(hours=self.negotiation_system.max_negotiation_hours),
        )
        ...
        trade_match.last_interaction = datetime.now(timezone.utc)
        self.session.commit()
        ...
        mock_sellers = [
            {
                ...
                "timing": {"start_time": datetime.now(timezone.utc), "duration_hours": 8},
            ...
            },
            {
                ...
                "timing": {"start_time": datetime.now(timezone.utc), "duration_hours": 6},
                "regions": ["us-east"],
    ...
    async def get_trading_summary(self, agent_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "recent_activity": {
                "requests_last_30d": len([r for r in requests if r.created_at >= datetime.now(timezone.utc) - timedelta(days=30)]),
                "matches_last_30d": len([m for m in matches if m.created_at >= datetime.now(timezone.utc) - timedelta(days=30)]),
                "agreements_last_30d": len([a for a in agreements if a.created_at >= datetime.now(timezone.utc) - timedelta(days=30)]),
            },
        }
