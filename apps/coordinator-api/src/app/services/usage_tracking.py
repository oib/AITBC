...
class UsageTrackingService:
    ...
    async def get_usage_summary(
        ...
        )  # type: ignore[call-overload, arg-type, operator]
        if resource_type:
    ...
    async def _adjust_quota(self, event: BillingEvent) -> None:
        ...
        self.db.commit()
        self.logger.info(
            "Adjusted quota: tenant=%s, resource=%s, %s -> %s", event.tenant_id, event.resource_type, old_limit, new_limit
        )
...
class BillingScheduler:
    ...
    async def _reset_daily_quotas(self) -> None:
        ...
        now = datetime.now(UTC)
        stmt = select(TenantQuota).where(
            and_(TenantQuota.period_type == "daily", TenantQuota.is_active, TenantQuota.period_end <= now)
        )  # type: ignore[arg-type, operator]
        ...
        for tenant in tenants:
            try:
                await self.usage_service.generate_invoice(
                    tenant_id=str(tenant.id), period_start=last_month_start, period_end=last_month_end
                )
                generated += 1
            except Exception as e:
