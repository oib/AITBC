...
class ZKProofService:
    ...
    async def generate_receipt_proof(self, receipt: Receipt, job_result: JobResult, privacy_level: str='basic') -> dict[str, Any] | None:
        ...
        try:
            ...
            return {'proof': proof_data['proof'], 'public_signals': proof_data['publicSignals'], 'privacy_level': privacy_level, 'circuit_hash': await self._get_circuit_hash()}
        except Exception as e:
            logger.error('Failed to generate ZK proof: %s', e)
            return None

    async def generate_proof(self, circuit_name: str, inputs: dict[str, Any], private_inputs: dict[str, Any] | None=None) -> dict[str, Any] | None:
        ...
        try:
            ...
            proof_data = await self._generate_proof_generic(inputs, private_inputs, circuit_paths['wasm_path'], circuit_paths['zkey_path'], circuit_paths['vkey_path'])
            return {'proof_id': f'{circuit_name}_{asyncio.get_event_loop().time()}', 'proof': proof_data['proof'], 'public_signals': proof_data['publicSignals'], 'verification_key': proof_data.get('verificationKey'), 'circuit_type': circuit_name, 'optimization_level': 'phase3_optimized' if 'modular' in circuit_name else 'baseline'}
        except Exception as e:
            logger.error('Failed to generate %s proof: %s', circuit_name, e)
            return None

    async def verify_proof(self, proof: dict[str, Any], public_signals: list[str], verification_key: dict[str, Any] | None=None, test_mode: bool=False) -> dict[str, Any]:
    ...
    async def _prepare_inputs(self, receipt: Receipt, job_result: JobResult, privacy_level: str) -> dict[str, Any]:
        ...
        if privacy_level == 'basic':
            return {'data': [str(receipt.receiptId), str(receipt.miner), str(getattr(job_result, 'output_hash', '')), str((receipt.payload or {}).get('rate', 0))], 'hash': await self._hash_receipt(receipt)}
        elif privacy_level == 'enhanced':
            payload = receipt.payload or {}
            return {'settlementAmount': payload.get('settlement_amount', 0), 'timestamp': receipt.issuedAt.isoformat(), 'receipt': self._serialize_receipt(receipt), 'computationResult': getattr(job_result, 'output_hash', ''), 'pricingRate': payload.get('rate', 0), 'minerReward': payload.get('miner_reward', 0), 'coordinatorFee': payload.get('coordinator_fee', 0)}
        else:
    ...
    async def _hash_receipt(self, receipt: Receipt) -> str:
        ...
        import hashlib
        payload = receipt.payload or {}
        receipt_data = {'receipt_id': receipt.receiptId, 'miner': receipt.miner, 'timestamp': receipt.issuedAt.isoformat(), 'pricing': payload.get('pricing', {})}
    ...
    def _serialize_receipt(self, receipt: Receipt) -> list[str]:
        ...
        payload = receipt.payload or {}
        return [str(receipt.receiptId)[:32], str(receipt.miner)[:32], str(receipt.issuedAt)[:32], str(payload.get('settlement_amount', 0))[:32], str(payload.get('miner_reward', 0))[:32], str(payload.get('coordinator_fee', 0))[:32], '0', '0']
    ...
    async def _generate_proof_generic(self, public_inputs: dict[str, Any], private_inputs: dict[str, Any] | None, wasm_path: Path, zkey_path: Path, vkey_path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.js', delete=False) as f:
                ...
                script_file = f.name
            try:
                result = await asyncio.create_subprocess_exec('node', script_file, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
