...
class AlertDispatcher:
    ...
    def dispatch(self, alerts: dict[str, dict[str, Any]]) -> dict[str, Any]:
        ...
        for name, alert in triggered.items():
            ...
            try:
                ...
                self._last_sent[name] = datetime.now(UTC)
    ...
    def _is_suppressed(self, name: str) -> bool:
        ...
        if last_sent is None:
            return False
        return datetime.now(UTC) - last_sent < timedelta(seconds=self.cooldown_seconds)
    ...
    def _record_alert(
        ...
        record = {
            "id": f"metrics_alert_{name}_{int(datetime.now(UTC).timestamp() * 1000)}",
            "deployment_id": None,
