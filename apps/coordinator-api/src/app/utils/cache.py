...
class CacheManager:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {**self._stats, 'total_entries': len(self._cache), 'hit_rate_percent': round(hit_rate, 2), 'total_requests': total_requests, 'max_size': self.max_size, 'max_memory_mb': self.max_memory_mb}
    ...
    def _check_memory_limit(self) -> None:
        ...
        if cache_size_mb > self.max_memory_mb:
            logger.warning('Cache memory limit exceeded (%sMB > %sMB), evicting entries', cache_size_mb, self.max_memory_mb)
...
def cached(ttl_seconds: int=300, key_prefix: str='') -> Callable[[Any], Any]:
    ...
    return decorator
CACHE_CONFIGS = {'marketplace_stats': {'ttl_seconds': 300, 'key_prefix': 'marketplace_'}, 'job_list': {'ttl_seconds': 60, 'key_prefix': 'jobs_'}, 'miner_list': {'ttl_seconds': 120, 'key_prefix': 'miners_'}, 'user_balance': {'ttl_seconds': 30, 'key_prefix': 'balance_'}, 'exchange_rates': {'ttl_seconds': 600, 'key_prefix': 'rates_'}}
...
async def cleanup_expired_cache() -> None:
    ...
    while True:
        try:
            ...
            if removed_count > 0:
                logger.info('Background cleanup removed %s expired entries', removed_count)
...
class CacheWarmer:
    ...
    async def warm_exchange_rates(self) -> None:
        ...
        try:
            ...
            logger.error('Failed to warm exchange rates cache: %s', e)
