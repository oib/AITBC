...
def get_cache_health() -> dict:
    ...
    if total_requests == 0:
        ...
        if hit_rate >= 80:
            ...
            health_status = 'poor'
    return {'health_status': health_status, 'hit_rate_percent': hit_rate, 'total_entries': stats['total_entries'], 'total_requests': total_requests, 'memory_usage_mb': round(len(str(cache_manager._cache)) / 1024 / 1024, 2), 'last_cleanup': stats.get('last_cleanup', 'never')}
    ...
    while True:
        try:
            ...
            if removed_count > 0:
                ...
                logger.info('Cache cleanup completed: %s entries removed, hit rate: %s%, entries: %s', removed_count, health['hit_rate_percent'], health['total_entries'])
...
class CacheWarmer:
    ...
    async def _warm_admin_stats(self) -> None:
        ...
        try:
            ...
            active_jobs = self.session.exec(select(func.count()).select_from(Job).where(column('state').in_(['QUEUED', 'RUNNING']))).one()
            miner_service.list_records()
            stats = {'total_jobs': int(total_jobs or 0), 'active_jobs': int(active_jobs or 0), 'online_miners': miner_service.online_count(), 'avg_miner_job_duration_ms': 0}
            from ..utils.cache import cache_manager
