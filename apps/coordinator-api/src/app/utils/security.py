...
class InputValidator:
    ...
    SQL_INJECTION_PATTERNS = ['(\\bunion\\b.*\\bselect\\b)', '(\\bselect\\b.*\\bfrom\\b)', '(\\binsert\\b.*\\binto\\b)', '(\\bdelete\\b.*\\bfrom\\b)', '(\\bdrop\\b.*\\btable\\b)', '(\\bexec\\b|\\bexecute\\b)', '(--|;|\\/\\*|\\*\\/)', '(\\bor\\b.*=.*\\bor\\b)', '(\\band\\b.*=.*\\band\\b)']
    XSS_PATTERNS = ['<script[^>]*>.*?</script>', 'javascript:', 'on\\w+\\s*=', '<iframe[^>]*>.*?</iframe>', '<object[^>]*>.*?</object>', '<embed[^>]*>.*?</embed>']
    ...
    def validate_string(cls, value: str, max_length: int=1000) -> str:
        ...
        if len(value) > max_length:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f'String exceeds maximum length of {max_length}')
    ...
    def validate_numeric_range(cls, value: float, min_val: float, max_val: float) -> float:
        ...
        if value < min_val or value > max_val:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f'Value must be between {min_val} and {max_val}')
        return value
