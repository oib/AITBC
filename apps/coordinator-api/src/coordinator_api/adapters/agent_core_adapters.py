...
class AgentSecurityManagerAdapter(ISecurityManager):
    ...
    async def audit_event(self, event_type: str, details: dict[str, Any]) -> None:
        # Delegate to app-specific implementation (deprecated - use log_audit instead)
        if hasattr(self._manager, "log_audit"):
            await self._manager.log_audit(event_type, details)
        elif hasattr(self._manager, "audit_event"):
