...
from coordinator_api.contexts.agent_coordination.domain.agent import AgentExecution, AgentStepExecution, VerificationLevel
                ...
                if instances_to_remove > 0:
                    ...
                    for inst_to_remove in instances_to_remove_list:
                        ...
                        scaling_result["scaled_instances"].append(
                            {"instance_id": inst_to_remove.instance_id, "status": "removed"}
                        )
            else:
            ...
            for instance in current_instances:
                try:
                    if getattr(config, "previous_version", None):
                        ...
                        previous_config.agent_version = getattr(
                            config, "previous_version", getattr(config, "agent_version", "")
                        )
                            ...
                            metrics_data.update(
                                {
                                    ...
                                    "average_response_time": agent_metrics.get(
