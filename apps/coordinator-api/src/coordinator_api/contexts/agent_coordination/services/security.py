...
from coordinator_api.contexts.agent_coordination.domain.agent import AIAgentWorkflow, VerificationLevel
...
class AgentTrustManager:
    ...
    async def update_trust_score(
        ...
        if security_violation:
            ...
            cast(list[Any], trust_score.violation_history).append(
                {"timestamp": datetime.now(UTC).isoformat(), "type": "security_violation"}
            )
        if policy_violation:
            trust_score.policy_violations += 1
            trust_score.last_violation = datetime.now(UTC)
            cast(list[Any], trust_score.violation_history).append(
                {"timestamp": datetime.now(UTC).isoformat(), "type": "policy_violation"}
            )
    ...
    def _get_sandbox_config(self, security_level: SecurityLevel) -> dict[str, Any]:
        ...
        configs = {
            ...
            SecurityLevel.INTERNAL: {
                ...
                "allowed_ports": [80, 443, 8000, 8001, 8002, 8003, 8010, 8203, 8012, 8013, 8014, 8015, 8016],
            ...
            SecurityLevel.RESTRICTED: {
                ...
                "blocked_domains": ["malicious.com", "*.suspicious.net", "*.evil.org"],
                "allowed_ports": [80, 443, 8000, 8001, 8002, 8003, 8010, 8203, 8012, 8013, 8014, 8015, 8016, 22, 25],
