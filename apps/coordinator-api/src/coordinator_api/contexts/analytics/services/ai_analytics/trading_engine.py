...
class AITradingEngine:
            ...
            for strategy_name, strategy in self.strategies.items():
                if strategy.is_trained:
                    try:
                        ...
                        logger.info(
                            "📈 %s signal: %s (confidence: %s)", strategy_name, signal.signal_type.value, signal.confidence
                        )
                    except Exception as e:
    ...
    def get_performance_metrics(self) -> dict[str, float]:
        ...
        return {
            ...
            "avg_confidence": float(np.mean([s.confidence for s in recent_signals])),
            "avg_risk_score": float(np.mean([s.risk_score for s in recent_signals])),
