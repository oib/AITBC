"""
Global Market Domain Models
Domain models for global market operations, multi-region support, and cross-chain integration
"""
...
class MarketRegion(SQLModel, table=True):
    """Global market region configuration"""
...
class GlobalMarketConfig(SQLModel, table=True):
    """Global market configuration settings"""
...
class GlobalMarketOffer(SQLModel, table=True):
    """Global market offer with multi-region support"""

    __tablename__ = "global_marketplace_offers"
    __table_args__ = {"extend_existing": True}

    id: str = Field(default_factory=lambda: f"offer_{uuid4().hex[:8]}", primary_key=True)
    original_offer_id: str = Field(index=True)  # Reference to original market offer
    ...
    # Global status
    global_status: MarketStatus = Field(default=MarketStatus.ACTIVE)
    region_statuses: dict[str, MarketStatus] = Field(default_factory=dict, sa_column=Column(JSON))
...
class GlobalMarketTransaction(SQLModel, table=True):
    """Global market transaction with cross-chain support"""
...
class GlobalMarketAnalytics(SQLModel, table=True):
    """Global market analytics and metrics"""
...
class GlobalMarketGovernance(SQLModel, table=True):
    """Global market governance and rules"""
...
class GlobalMarketOfferRequest(SQLModel):
    """Request model for creating global market offers"""
...
class GlobalMarketTransactionRequest(SQLModel):
    """Request model for creating global market transactions"""
...
class GlobalMarketAnalyticsRequest(SQLModel):
    """Request model for global market analytics"""
...
# Response Models
class GlobalMarketOfferResponse(SQLModel):
    """Response model for global market offers"""
...
class GlobalMarketTransactionResponse(SQLModel):
    """Response model for global market transactions"""
...
class GlobalMarketAnalyticsResponse(SQLModel):
    """Response model for global market analytics"""
