"""
Global Market Integration API Router
REST API endpoints for integrated global market with cross-chain capabilities
"""
...
from ..domain.global_market import GlobalMarketOffer
from ..services.global_market_integration import GlobalMarketIntegrationService, IntegrationStatus

logger = get_logger(__name__)

router = APIRouter(prefix="/global-market-integration", tags=["Global Market Integration"])


def get_integration_service(session: Annotated[Session, Depends(get_session)]) -> GlobalMarketIntegrationService:
    return GlobalMarketIntegrationService(session)
...
async def create_cross_chain_market_offer(
    agent_id: str,
    service_type: str,
    ...
    integration_service: Annotated[GlobalMarketIntegrationService, Depends(get_integration_service)],
    ...
    """Create a cross-chain enabled market offer"""
    try:
        identity = await identity_manager.get_agent_identity_summary(agent_id)
        if not identity:
            raise HTTPException(status_code=404, detail="Agent identity not found")
        offer = await integration_service.create_cross_chain_market_offer(
            agent_id=agent_id,
...
async def get_integrated_market_offers(
    session: Annotated[Session, Depends(get_session)],
    integration_service: Annotated[GlobalMarketIntegrationService, Depends(get_integration_service)],
    ...
    """Get integrated market offers with cross-chain capabilities"""
    try:
        offers = await integration_service.get_integrated_market_offers(
            region=region,
            service_type=service_type,
            chain_id=chain_id,
...
async def optimize_offer_pricing(
    ...
    """Optimize pricing for a global market offer"""
    try:
...
async def execute_cross_chain_transaction(
    ...
    """Execute a cross-chain market transaction"""
    try:
...
async def get_cross_chain_transactions(
    ...
    """Get cross-chain market transactions"""
    try:
        transactions = await integration_service.market_service.get_global_transactions(
...
@router.get("/analytics/market-integration", response_model=dict[str, Any])
async def get_market_integration_analytics(
    session: Annotated[Session, Depends(get_session)],
    integration_service: Annotated[GlobalMarketIntegrationService, Depends(get_integration_service)],
) -> dict[str, Any]:
    """Get market integration status and metrics"""
    try:
        ...
        raise HTTPException(status_code=500, detail="Error getting market integration analytics") from None
...
async def get_integration_status(
    ...
    """Get global market integration status"""
    try:
        services_status = {
...
async def get_integration_config(
    ...
    """Get global market integration configuration"""
    try:
        config = integration_service.integration_config
...
async def update_integration_config(
    ...
    """Update global market integration configuration"""
    try:
...
async def get_integration_health(
    ...
    """Get global market integration health status"""
    try:
        health_status: dict[str, Any] = {"overall_status": "healthy", "services": {}, "metrics": {}, "issues": []}
        try:
            await integration_service.market_service.get_global_offers(limit=1)
            health_status["services"]["market_service"] = "healthy"
        except Exception:
            health_status["services"]["market_service"] = "unhealthy"
            health_status["issues"].append("Market service error")
        try:
...
async def run_integration_diagnostics(
    ...
    try:
        ...
        if diagnostic_type == "full" or diagnostic_type == "services":
            ...
            try:
                await integration_service.market_service.get_global_offers(limit=1)
                diagnostics["results"]["services"]["market_service"] = {"status": "healthy", "offers_accessible": True}
            except Exception:
                diagnostics["results"]["services"]["market_service"] = {"status": "unhealthy", "error": "Service error"}
            try:
                regions = await integration_service.region_manager._get_active_regions()  # type: ignore[attr-defined]
