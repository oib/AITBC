"\nGPU market endpoints backed by persistent SQLModel tables.\n"
...
from aitbc.market.energy_oracle import EVMEnergyOracle, EnergyOracleError
...
from ...trading.services.trading_market.dynamic_pricing import (
...
@router.post("/market/gpu/register")
async def register_gpu(
    request: dict[str, Any],
    session: Annotated[Session, Depends(get_session)],
    user: MinerDep,
) -> dict[str, Any]:
    """Register a GPU in the market."""
...
async def quote_gpu(
    ...
    POST /market/gpu/purchase using the same job_id and energy_quote.
    """
    ...
    if settlement_route == SettlementRoute.EVM:
        from aitbc.market.energy_pricing import EVM_UNITS_PER_AIT
    ...
    from aitbc.market.energy_pricing import evaluate_quote
...
@router.post("/market/gpu/purchase")
async def buy_gpu(
    ...
    """Buy GPU compute from market with blockchain payment and AI job scheduling."""
    gpu = _get_gpu_or_404(session, request.gpu_id)
    if gpu.status != "available":
        raise HTTPException(
...
@router.post("/market/gpu/{gpu_id}/book", status_code=http_status.HTTP_201_CREATED)
async def book_gpu(
    gpu_id: str,
...
@router.post("/market/gpu/{gpu_id}/release")
...
@router.post("/market/gpu/{gpu_id}/confirm")
...
@router.delete("/market/gpu/{gpu_id}")
async def delete_gpu(
    ...
    """Delete (unregister) a GPU from the market."""
    gpu = _get_gpu_or_404(session, gpu_id)
...
@router.get("/market/gpu/{gpu_id}/reviews")
...
@router.post("/market/gpu/{gpu_id}/reviews", status_code=http_status.HTTP_201_CREATED)
...
@router.get("/market/pricing/{model}")
...
@router.post("/market/native-energy/profile", status_code=http_status.HTTP_201_CREATED)
...
@router.post("/market/native-energy/rate")
...
@router.get("/market/native-energy/profile/{resource_id}")
async def get_native_energy_profile(resource_id: str, session: Annotated[Session, Depends(get_session)]) -> dict[str, Any]:
...
@router.get("/market/native-energy/rate")
...
@router.get("/market/native-energy/floor")
