...
from ..domain.gpu_market import GPUBooking, GPURegistry
...
class MarketAnalytics:
    ...
    def analyze_trends(self, hours: int = 24) -> dict[str, Any]:
        ...
        try:
            ...
            recent_bookings = (
                self.session.execute(select(GPUBooking).where(GPUBooking.created_at >= cutoff_time)).scalars().all()
            )
    ...
    def generate_forecasts(self, hours_ahead: int = 24) -> dict[str, Any]:
        ...
        try:
            ...
            bookings_per_hour = (
                current_metrics.get("active_bookings", 0) / 24 if current_metrics.get("active_bookings", 0) > 0 else 0
            )
    ...
    def track_event(self, event_type: str, resource_id: str, metadata: dict[str, Any] | None = None) -> bool:
        """Track market events for analytics."""
        try:
