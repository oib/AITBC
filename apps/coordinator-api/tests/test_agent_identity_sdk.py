    ...
    async def test_create_identity_success(self, client, mock_session):
        ...
        with patch.object(client, "session", mock_session):
            ...
            result = await client.create_identity(
                owner_address="0x123...", chains=[1, 137], display_name="Test Agent", description="Test description"
            )
    ...
    async def test_verify_identity_success(self, client, mock_session):
        ...
        with patch.object(client, "session", mock_session):
            ...
            result = await client.verify_identity(
                agent_id="agent_456", chain_id=1, verifier_address="0x789...", proof_hash="abc123", proof_data={"test": "data"}
            )
...
class TestConvenienceFunctions:
    ...
    async def test_create_identity_with_wallets_success(self):
        ...
        client.create_identity.return_value = AsyncMock(
            identity_id="identity_123",
            agent_id="agent_456",
            wallet_results=[{"chain_id": 1, "success": True}, {"chain_id": 137, "success": True}],
        )

        # Call function
        result = await create_identity_with_wallets(
            client=client, owner_address="0x123...", chains=[1, 137], display_name="Test Agent"
        )
    ...
    async def test_verify_identity_on_all_chains_success(self):
        ...
        client.verify_identity.return_value = AsyncMock(verification_id="verify_123", verified=True)

        # Call function
        results = await verify_identity_on_all_chains(
            client=client, agent_id="agent_456", verifier_address="0x789...", proof_data_template={"test": "data"}
        )
