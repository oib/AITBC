...
class TestFHERouter:
    ...
    def test_encrypt(self, client: TestClient):
        ...
        encrypt_data = {"public_key": public_key, "plaintext": [1, 2, 3, 4, 5], "scheme": "BFV"}
    ...
    def test_encrypt_batch(self, client: TestClient):
        ...
        batch_data = {"public_key": public_key, "plaintexts": [[1, 2], [3, 4], [5, 6]]}
    ...
    def test_decrypt(self, client: TestClient):
        ...
        encrypt_response = client.post(
            "/fhe/encrypt", json={"public_key": public_key, "plaintext": [42, 100], "scheme": "BFV"}
        )
        ciphertext = encrypt_response.json()["ciphertext"]

        # Decrypt
        decrypt_data = {"secret_key": secret_key, "ciphertext": ciphertext}
    ...
    def test_add_encrypted(self, client: TestClient):
        ...
        # Encrypt two values
        ct1 = client.post("/fhe/encrypt", json={"public_key": public_key, "plaintext": [10], "scheme": "BFV"}).json()[
            "ciphertext"
        ]

        ct2 = client.post("/fhe/encrypt", json={"public_key": public_key, "plaintext": [20], "scheme": "BFV"}).json()[
            "ciphertext"
        ]

        # Add them
        add_data = {"ciphertext_a": ct1, "ciphertext_b": ct2, "public_key": public_key}
    ...
    def test_multiply_encrypted(self, client: TestClient):
        ...
        ct1 = client.post("/fhe/encrypt", json={"public_key": public_key, "plaintext": [5], "scheme": "BFV"}).json()[
            "ciphertext"
        ]

        ct2 = client.post("/fhe/encrypt", json={"public_key": public_key, "plaintext": [7], "scheme": "BFV"}).json()[
            "ciphertext"
        ]

        multiply_data = {"ciphertext_a": ct1, "ciphertext_b": ct2, "public_key": public_key}
...
class TestFHEIntegration:
    ...
    def test_full_fhe_workflow(self, client: TestClient):
        ...
        ct1 = client.post("/fhe/encrypt", json={"public_key": public_key, "plaintext": [15], "scheme": "BFV"}).json()[
            "ciphertext"
        ]

        ct2 = client.post("/fhe/encrypt", json={"public_key": public_key, "plaintext": [25], "scheme": "BFV"}).json()[
            "ciphertext"
        ]

        # 3. Add them homomorphically
        sum_ct = client.post(
            "/fhe/operations/add", json={"ciphertext_a": ct1, "ciphertext_b": ct2, "public_key": public_key}
        ).json()["result_ciphertext"]

        # 4. Decrypt result
        result = client.post("/fhe/decrypt", json={"secret_key": secret_key, "ciphertext": sum_ct}).json()
