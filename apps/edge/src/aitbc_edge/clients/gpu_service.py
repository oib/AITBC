...
class GPUServiceClient:
    ...
    async def scan_gpus(self, miner_id: str) -> dict[str, Any]:
        ...
        response = await self.client.post(f"{self.base_url}/v1/market/edge-gpu/scan/{miner_id}")
    ...
    async def get_gpu_profiles(
        ...
        response = await self.client.get(f"{self.base_url}/v1/market/edge-gpu/profiles", params=params)
        response.raise_for_status()
        return cast(list[dict[str, Any]], response.json())

    async def get_gpu_metrics(self, gpu_id: str, limit: int = 100) -> list[dict[str, Any]]:
        """Get GPU metrics via GPU service"""
        response = await self.client.get(f"{self.base_url}/v1/market/edge-gpu/metrics/{gpu_id}", params={"limit": limit})
        response.raise_for_status()
        return cast(list[dict[str, Any]], response.json())
