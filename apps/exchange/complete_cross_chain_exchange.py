...
SUPPORTED_CHAINS = {'ait-devnet': {'name': 'AITBC Development Network', 'status': 'active', 'blockchain_url': 'http://localhost:8007', 'token_symbol': 'AITBC-DEV', 'bridge_contract': '0x1234567890123456789012345678901234567890'}, 'ait-testnet': {'name': 'AITBC Test Network', 'status': 'inactive', 'blockchain_url': None, 'token_symbol': 'AITBC-TEST', 'bridge_contract': '0x0987654321098765432109876543210987654321'}}
...
def init_database():
    ...
    try:
        ...
        for chain_id, chain_info in SUPPORTED_CHAINS.items():
            cursor.execute('\n                INSERT OR REPLACE INTO chains \n                (chain_id, name, status, blockchain_url, token_symbol, bridge_contract)\n                VALUES (?, ?, ?, ?, ?, ?)\n            ', (chain_id, chain_info['name'], chain_info['status'], chain_info['blockchain_url'], chain_info['token_symbol'], chain_info.get('bridge_contract')))
...
async def execute_cross_chain_swap(swap_request: CrossChainSwapRequest) -> dict[str, Any]:
    ...
    try:
        ...
        rate = get_cross_chain_rate(swap_request.from_chain, swap_request.to_chain, swap_request.from_token, swap_request.to_token)
        if not rate:
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('\n            INSERT INTO cross_chain_swaps \n            (swap_id, from_chain, to_chain, from_token, to_token, amount, min_amount, \n             expected_amount, user_address, bridge_fee, slippage)\n            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)\n        ', (swap_id, swap_request.from_chain, swap_request.to_chain, swap_request.from_token, swap_request.to_token, swap_request.amount, swap_request.min_amount, expected_amount, swap_request.user_address, bridge_fee, swap_request.slippage_tolerance))
        conn.commit()
        conn.close()
...
async def health_check():
    ...
    chain_status = {}
    for chain_id, chain_info in SUPPORTED_CHAINS.items():
        chain_status[chain_id] = {'name': chain_info['name'], 'status': chain_info['status'], 'blockchain_url': chain_info['blockchain_url'], 'connected': False, 'bridge_contract': chain_info.get('bridge_contract')}
        if chain_info['status'] == 'active' and chain_info['blockchain_url']:
            try:
                ...
                pass
    return {'status': 'ok', 'service': 'complete-cross-chain-exchange', 'version': '3.0.0', 'supported_chains': list(SUPPORTED_CHAINS.keys()), 'chain_status': chain_status, 'cross_chain': True, 'features': ['trading', 'swaps', 'bridging', 'liquidity_pools'], 'timestamp': datetime.now().isoformat()}
...
async def get_chains():
    ...
    chains = []
    for chain_id, chain_info in SUPPORTED_CHAINS.items():
        chains.append({'chain_id': chain_id, 'name': chain_info['name'], 'status': chain_info['status'], 'blockchain_url': chain_info['blockchain_url'], 'token_symbol': chain_info['token_symbol'], 'bridge_contract': chain_info.get('bridge_contract')})
    return {'chains': chains, 'total_chains': len(chains), 'active_chains': len([c for c in chains if c['status'] == 'active'])}
...
async def create_bridge_transaction(bridge_request: BridgeRequest):
    ...
    try:
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('\n            INSERT INTO bridge_transactions \n            (bridge_id, source_chain, target_chain, token, amount, recipient_address, bridge_fee)\n            VALUES (?, ?, ?, ?, ?, ?, ?)\n        ', (bridge_id, bridge_request.source_chain, bridge_request.target_chain, bridge_request.token, bridge_request.amount, bridge_request.recipient_address, bridge_fee))
        conn.commit()
        conn.close()
        asyncio.create_task(process_bridge_transaction(bridge_id))
        return {'success': True, 'bridge_id': bridge_id, 'source_chain': bridge_request.source_chain, 'target_chain': bridge_request.target_chain, 'token': bridge_request.token, 'amount': bridge_request.amount, 'bridge_fee': bridge_fee, 'status': 'pending'}
    except Exception as e:
...
async def get_cross_chain_stats():
    ...
    try:
        ...
        conn.close()
        return {'swap_stats': swap_stats, 'bridge_stats': bridge_stats, 'total_volume': total_volume, 'supported_chains': list(SUPPORTED_CHAINS.keys()), 'timestamp': datetime.now().isoformat()}
    except Exception as e:
