...
async def execute_cross_chain_swap(swap_request: CrossChainSwapRequest) -> dict[str, Any]:
    ...
    try:
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('\n            INSERT INTO cross_chain_swaps \n            (swap_id, from_chain, to_chain, from_token, to_token, amount, min_amount, expected_amount, user_address)\n            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)\n        ', (swap_id, swap_request.from_chain, swap_request.to_chain, swap_request.from_token, swap_request.to_token, swap_request.amount, swap_request.min_amount, expected_amount, swap_request.user_address))
        conn.commit()
        conn.close()
        asyncio.create_task(process_cross_chain_swap(swap_id))
        return {'success': True, 'swap_id': swap_id, 'from_chain': swap_request.from_chain, 'to_chain': swap_request.to_chain, 'amount': swap_request.amount, 'expected_amount': expected_amount, 'rate': rate, 'status': 'pending'}
    except Exception as e:
...
async def process_cross_chain_swap(swap_id: str):
    ...
    try:
        ...
        if not from_tx_hash:
            ...
            conn.commit()
            return
        to_tx_hash = await transfer_to_target_chain(swap['to_chain'], swap['to_token'], swap['expected_amount'], swap['user_address'])
        if not to_tx_hash:
...
async def create_bridge_transaction(bridge_request: BridgeRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        return {'success': True, 'bridge_id': bridge_id, 'source_chain': bridge_request.source_chain, 'target_chain': bridge_request.target_chain, 'amount': bridge_request.amount, 'bridge_fee': bridge_fee, 'status': 'pending'}
    except Exception as e:
...
async def process_bridge_transaction(bridge_id: str):
    ...
    try:
        ...
        conn.commit()
        source_tx_hash = await lock_funds_on_chain(bridge['source_chain'], bridge['token'], bridge['amount'], bridge['recipient_address'])
        if source_tx_hash:
            target_tx_hash = await transfer_to_target_chain(bridge['target_chain'], bridge['token'], bridge['amount'], bridge['recipient_address'])
            if target_tx_hash:
