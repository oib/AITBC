...
SUPPORTED_CHAINS = {'ait-devnet': {'name': 'AITBC Development Network', 'status': 'active', 'blockchain_url': 'http://localhost:8007', 'token_symbol': 'AITBC-DEV'}, 'ait-testnet': {'name': 'AITBC Test Network', 'status': 'inactive', 'blockchain_url': None, 'token_symbol': 'AITBC-TEST'}}
...
async def health_check():
    ...
    chain_status = {}
    for chain_id, chain_info in SUPPORTED_CHAINS.items():
        chain_status[chain_id] = {'name': chain_info['name'], 'status': chain_info['status'], 'blockchain_url': chain_info['blockchain_url'], 'connected': False}
        if chain_info['status'] == 'active' and chain_info['blockchain_url']:
            try:
                ...
                pass
    return {'status': 'ok', 'service': 'multi-chain-exchange', 'version': '2.0.0', 'supported_chains': list(SUPPORTED_CHAINS.keys()), 'chain_status': chain_status, 'multi_chain': True, 'timestamp': datetime.now().isoformat()}
...
async def get_chains():
    ...
    chains = []
    for chain_id, chain_info in SUPPORTED_CHAINS.items():
        chains.append({'chain_id': chain_id, 'name': chain_info['name'], 'status': chain_info['status'], 'blockchain_url': chain_info['blockchain_url'], 'token_symbol': chain_info['token_symbol']})
...
async def create_order(order: OrderRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        conn.commit()
        conn.close()
        return {'success': True, 'order_id': order_id, 'chain_id': order.chain_id, 'status': 'created', 'message': f"Order created on {chain_info['name']}"}
    except Exception as e:
...
async def submit_order_to_blockchain(order_id: int, chain_id: str):
    ...
    try:
        ...
        if order:
            order_data = {'type': 'order', 'order_type': order['order_type'], 'amount': order['amount'], 'price': order['price'], 'user_address': order['user_address']}
...
async def get_chain_orderbook(chain_id: str):
    ...
    try:
        ...
        conn.close()
        return {'chain_id': chain_id, 'buy_orders': buy_orders, 'sell_orders': sell_orders, 'spread': sell_orders[0]['price'] - buy_orders[0]['price'] if buy_orders and sell_orders else None}
    except Exception as e:
...
async def create_trade(trade: MultiChainTradeRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        conn.commit()
        conn.close()
        return {'success': True, 'trade_id': trade_id, 'chain_id': trade.chain_id, 'status': 'created', 'message': f"Trade created on {chain_info['name']}"}
    except Exception as e:
...
async def submit_trade_to_blockchain(trade_id: int, chain_id: str):
    ...
    try:
        ...
        if trade:
            trade_data = {'type': 'trade', 'buy_order_id': trade['buy_order_id'], 'sell_order_id': trade['sell_order_id'], 'amount': trade['amount'], 'price': trade['price']}
...
async def get_chain_stats(chain_id: str):
    ...
    try:
        ...
        conn.close()
        return {'chain_id': chain_id, 'chain_name': SUPPORTED_CHAINS[chain_id]['name'], 'orders': order_stats, 'trades': trade_stats, 'timestamp': datetime.now().isoformat()}
    except Exception as e:
