...
class RealExchangeManager:
    ...
    async def connect_exchange(self, exchange_name: str, credentials: ExchangeCredentials) -> bool:
        ...
        try:
            ...
            if exchange_name == 'binance':
                exchange = ccxt.binance({'apiKey': credentials.api_key, 'secret': credentials.secret, 'sandbox': credentials.sandbox, 'enableRateLimit': True})
            elif exchange_name == 'coinbasepro':
                exchange = ccxt.coinbasepro({'apiKey': credentials.api_key, 'secret': credentials.secret, 'passphrase': credentials.passphrase, 'sandbox': credentials.sandbox, 'enableRateLimit': True})
            elif exchange_name == 'kraken':
                exchange = ccxt.kraken({'apiKey': credentials.api_key, 'secret': credentials.secret, 'sandbox': credentials.sandbox, 'enableRateLimit': True})
            await self._test_connection(exchange, exchange_name)
            self.exchanges[exchange_name] = exchange
            self.credentials[exchange_name] = credentials
            self.health_status[exchange_name] = ExchangeHealth(status=ExchangeStatus.CONNECTED, latency_ms=0.0, last_check=datetime.now(UTC))
            logger.info('✅ Connected to %s', exchange_name)
            return True
        except Exception as e:
            logger.error('❌ Failed to connect to %s: %s', exchange_name, str(e))
            self.health_status[exchange_name] = ExchangeHealth(status=ExchangeStatus.ERROR, latency_ms=0.0, last_check=datetime.now(UTC), error_message=str(e))
            return False
    ...
    async def disconnect_exchange(self, exchange_name: str) -> bool:
        ...
        try:
            if exchange_name in self.exchanges:
                ...
                self.health_status[exchange_name] = ExchangeHealth(status=ExchangeStatus.DISCONNECTED, latency_ms=0.0, last_check=datetime.now())
    ...
    async def check_exchange_health(self, exchange_name: str) -> ExchangeHealth:
        ...
        if exchange_name not in self.exchanges:
            return ExchangeHealth(status=ExchangeStatus.DISCONNECTED, latency_ms=0.0, last_check=datetime.now(), error_message='Not connected')
        try:
            start_time = time.time()
            ...
            health = ExchangeHealth(status=ExchangeStatus.ERROR, latency_ms=0.0, last_check=datetime.now(), error_message=str(e))
            self.health_status[exchange_name] = health
            return health
    ...
    async def place_order(self, order_request: OrderRequest) -> dict[str, Any]:
        ...
        try:
            ...
            order_params = {'symbol': order_request.symbol, 'type': order_request.type, 'side': order_request.side.value, 'amount': order_request.amount}
...
async def connect_to_exchange(exchange_name: str, api_key: str, secret: str, sandbox: bool=True, passphrase: str=None) -> bool:
