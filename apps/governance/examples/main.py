...
def check_suspicious_patterns(transaction: TransactionMonitoring) -> list[str]:
    ...
    recent_transactions = [
        t for t in user_transactions if datetime.fromisoformat(t["monitored_at"]) > datetime.now(UTC) - timedelta(hours=1)
    ]
...
def get_recent_activity() -> list[dict]:
    ...
    recent_kyc = [
        r
        for r in kyc_records.values()
        if r.get("approved_at") and datetime.fromisoformat(r["approved_at"]) > datetime.now(UTC) - timedelta(hours=24)
    ]
    for kyc in recent_kyc[:5]:
        activities.append(
            {"type": "kyc_approved", "description": f"KYC approved for {kyc['name']}", "timestamp": kyc["approved_at"]}
        )
    recent_reports = [
        r
        for r in compliance_reports.values()
        if datetime.fromisoformat(r["created_at"]) > datetime.now(UTC) - timedelta(hours=24)
    ]
    for report in recent_reports[:5]:
        activities.append(
            {"type": "compliance_report", "description": f"Report: {report['description']}", "timestamp": report["created_at"]}
        )
