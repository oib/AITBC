...
app = FastAPI(title='AITBC Governance Service', description='Manages governance operations', version='0.1.0', lifespan=lifespan)
...
async def get_profiles(role: str | None=None, user_id: str | None=None, svc: GovernanceService=Depends(get_governance_service)):
...
async def get_proposals(status: str | None=None, category: str | None=None, proposer_id: str | None=None, svc: GovernanceService=Depends(get_governance_service)):
...
async def get_votes(proposal_id: str | None=None, voter_id: str | None=None, svc: GovernanceService=Depends(get_governance_service)):
    """Get votes"""
...
async def execute_proposal(proposal_id: str, executor_id: str, svc: GovernanceService=Depends(get_governance_service)):
    ...
    try:
        execution_result = {'proposal_id': proposal_id, 'executor_id': executor_id, 'status': 'executed', 'executed_at': svc.get_current_timestamp(), 'tx_hash': None}
...
async def get_governance_params():
    ...
    params = {'voting_period': 604800, 'execution_delay': 86400, 'quorum_threshold': 0.5, 'approval_threshold': 0.6, 'min_proposal_deposit': 1000, 'max_proposals_per_period': 10, 'emergency_quorum_threshold': 0.8, 'voting_power_calculation': 'token_weighted', 'proposal_categories': ['parameter_change', 'spending', 'contract_upgrade', 'emergency', 'other'], 'last_updated': '2026-06-02'}
    return params

@app.get('/v1/governance/voting-power/{address}')
async def get_voting_power(address: str, proposal_id: str | None=None, svc: GovernanceService=Depends(get_governance_service)):
...
async def get_transactions(transaction_type: str | None=None, action: str | None=None, status: str | None=None, island_id: str | None=None, session: AsyncSession=Depends(get_session_dep)):
    ...
    try:
        ...
        if action == 'propose' or not action:
            ...
            transactions.extend([{'id': p.proposal_id, 'action': 'propose', 'title': p.title, 'status': p.status, 'created_at': p.created_at.isoformat() if p.created_at else None} for p in proposals])
        if action == 'vote' or not action:
            result = await session.execute(select(Vote))
            votes = result.scalars().all()
            transactions.extend([{'id': v.vote_id, 'action': 'vote', 'proposal_id': v.proposal_id, 'vote_type': v.vote_type, 'created_at': v.created_at.isoformat() if v.created_at else None} for v in votes])
        if status:
...
async def stake_tokens(staker_address: str, amount: int, lock_period_days: int, svc: GovernanceService=Depends(get_governance_service)):
    """Stake tokens for enhanced voting power"""
    try:
        stake = await svc.stake_tokens(staker_address, amount, lock_period_days)
        return {'stake_id': stake.stake_id, 'staker_address': staker_address, 'amount_staked': amount, 'lock_period_days': lock_period_days, 'unstakes_at': stake.unstakes_at.isoformat() if stake.unstakes_at else None, 'voting_power': await svc.calculate_voting_power(staker_address)}
    except Exception as e:
...
async def delegate_voting_power(delegator_address: str, delegate_address: str, amount: int, svc: GovernanceService=Depends(get_governance_service)):
    """Delegate voting power to another address"""
    try:
        delegation = await svc.delegate_voting_power(delegator_address, delegate_address, amount)
        return {'delegation_id': delegation.delegation_id, 'delegator_address': delegator_address, 'delegate_address': delegate_address, 'voting_power': amount, 'created_at': delegation.created_at.isoformat() if delegation.created_at else None}
    except Exception as e:
...
async def execute_proposal_v2(proposal_id: str, svc: GovernanceService=Depends(get_governance_service)):
    ...
    try:
        ...
        if not proposal:
            return ({'error': 'Proposal not found'}, 404)
        return {'proposal_id': proposal_id, 'status': proposal.status, 'executed_at': proposal.executed_at.isoformat() if proposal.executed_at else None}
