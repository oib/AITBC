...
class EdgeGPUService:
    ...
    async def list_profiles(self, architecture: GPUArchitecture | None=None, edge_optimized: bool | None=None, min_memory_gb: int | None=None) -> list[ConsumerGPUProfile]:
    ...
    async def list_metrics(self, gpu_id: str, limit: int=100) -> list[EdgeGPUMetrics]:
        ...
        try:
            stmt = select(EdgeGPUMetrics).where(EdgeGPUMetrics.gpu_id == gpu_id).order_by(EdgeGPUMetrics.timestamp.desc()).limit(limit)
            result = self.session.execute(stmt)
            return list(result.scalars().all())
        except Exception as e:
    ...
    def _discover_gpus_via_nvidia_smi(self) -> list[dict[str, Any]]:
        ...
        try:
            result = subprocess.run(['nvidia-smi', '--query-gpu=index,name,memory.total,driver_version', '--format=csv,noheader,nounits'], capture_output=True, text=True, timeout=10)
            ...
            for line in result.stdout.strip().split('\n'):
                ...
                if len(parts) >= 3:
                    gpus.append({'index': int(parts[0]), 'name': parts[1], 'memory_mb': int(parts[2]), 'driver_version': parts[3] if len(parts) > 3 else 'unknown'})
            return gpus
    ...
    async def discover_and_register_edge_gpus(self, miner_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            if not discovered_gpus:
                ...
                gpu_list = [{'id': gpu.id, 'model': gpu.model, 'memory_gb': gpu.memory_gb, 'region': gpu.region, 'status': gpu.status, 'capabilities': gpu.capabilities} for gpu in existing_gpus]
                return {'miner_id': miner_id, 'gpus': gpu_list, 'registered': len(existing_gpus), 'edge_optimized': 0, 'discovery_method': 'database_fallback'}
            registered_count = 0
            gpu_list = []
            for gpu_info in discovered_gpus:
                ...
                if existing:
                    ...
                    gpu_list.append({'id': existing.id, 'model': existing.model, 'memory_gb': existing.memory_gb, 'region': existing.region, 'status': existing.status, 'capabilities': existing.capabilities})
                else:
                    new_gpu = GPURegistry(id=gpu_id, miner_id=miner_id, model=gpu_info['name'], memory_gb=gpu_info['memory_mb'] // 1024, cuda_version=gpu_info['driver_version'], status='available', capabilities=['edge', 'inference'])
                    self.session.add(new_gpu)
                    gpu_list.append({'id': new_gpu.id, 'model': new_gpu.model, 'memory_gb': new_gpu.memory_gb, 'region': new_gpu.region, 'status': new_gpu.status, 'capabilities': new_gpu.capabilities})
                    registered_count += 1
            self.session.commit()
            return {'miner_id': miner_id, 'gpus': gpu_list, 'registered': registered_count, 'edge_optimized': len(gpu_list), 'discovery_method': 'nvidia_smi'}
        except Exception as e:
            self.session.rollback()
