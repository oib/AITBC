...
class RequestCoinsHandler(BaseHandler):
    ...
    async def handle(self, message: dict[str, Any]) -> dict[str, Any]:
        ...
        approval_request = {'id': request_id, 'sender': sender, 'recipient': self.agent_id, 'amount': amount, 'wallet_address': wallet_address, 'created_at': datetime.utcnow()}
        ...
        with get_db_session() as session:
            try:
                coin_request = CoinRequest(id=request_id, sender=sender, recipient=self.agent_id, amount=amount, wallet_address=wallet_address, status=CoinRequestStatus.APPROVED if approval_decision['approved'] else CoinRequestStatus.PENDING, approval_mode=self.approval_mode, approved_by=self.strategy.__class__.__name__, approved_at=datetime.utcnow() if approval_decision['approved'] else None, rejection_reason=approval_decision['reason'] if not approval_decision['approved'] else None, created_at=datetime.utcnow(), expires_at=datetime.utcnow() + timedelta(days=30), signed_transaction=signed_transaction, audit_log=f"Mode: {self.approval_mode}, Decision: {approval_decision['approved']}, Reason: {approval_decision['reason']}")
        ...
        if response.get('status') == 'success':
            return {'status': 'coin_request_processed', 'request_id': request_id, 'approved': approval_decision['approved'], 'reason': approval_decision['reason']}
        else:
