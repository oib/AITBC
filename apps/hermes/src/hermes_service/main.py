...
async def remote_execute_coin_request(
    ...
    if balance < total_required:
        raise HTTPException(
            status_code=400,
            detail=f"Insufficient genesis balance: {balance} < {total_required}"
        )
    ...
    logger.info(f"Remote execution of {req.request_id}: {req.amount} AIT to {req.wallet_address} — tx {tx_hash}")
