"""
Plugin Market Frontend Service for AITBC
Provides web interface and market functionality for plugins
"""
...
app = FastAPI(title="AITBC Plugin Market", description="Plugin market frontend and community features", version="1.0.0")
...
async def plugins_page(request: Request):
    ...
    return templates.TemplateResponse(
        "plugins.html",
        {"request": request, "plugins": get_all_plugins(), "categories": get_categories(), "tags": get_all_tags()},
    )
...
async def developers_page(request: Request):
    ...
    return templates.TemplateResponse(
        "developers.html",
        {"request": request, "verified_developers": get_verified_developers(), "developer_stats": get_developer_stats()},
    )
...
async def submit_plugin_page(request: Request):
    ...
    return templates.TemplateResponse(
        "submit.html", {"request": request, "categories": get_categories(), "guidelines": get_submission_guidelines()}
    )


@app.get("/api/v1/market/featured")
async def get_featured_plugins_api():
    """Get featured plugins for market"""
    return {"featured_plugins": get_featured_plugins(), "generated_at": datetime.now(UTC).isoformat()}


@app.get("/api/v1/market/popular")
...
async def get_market_stats_api():
    """Get market statistics"""
    return {"stats": get_market_stats(), "generated_at": datetime.now(UTC).isoformat()}


@app.post("/api/v1/reviews")
async def create_review(review: MarketReview):
...
async def get_developer_revenue(developer_id: str):
    ...
    developer_revenue = revenue_sharing.get(
        developer_id,
        {"total_revenue": 0.0, "plugin_revenue": {}, "monthly_revenue": {}, "last_updated": datetime.now(UTC).isoformat()},
    )
...
def get_plugin_details(plugin_id: str) -> dict | None:
    ...
    plugins = {
        "ai_trading_bot": {
            ...
            "license": "MIT",
            "screenshots": ["/static/screenshots/trading-bot-1.png", "/static/screenshots/trading-bot-2.png"],
...
async def update_market_analytics():
    ...
    while True:
        ...
        logger.info("Market analytics updated")


@app.on_event("startup")
async def startup_event():
    logger.info("Starting AITBC Plugin Market")
    asyncio.create_task(update_market_analytics())


@app.on_event("shutdown")
async def shutdown_event():
    logger.info("Shutting down AITBC Plugin Market")
