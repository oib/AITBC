...
`OfferStatus` in `aitbc.market.offer_fsm` is the vocabulary the state machine validates
...
from aitbc.market import OfferStatus

# Where each spelling comes from, so that removing one is an informed decision:
#
#   available  MarketOffer's default, MatchingService, the /offers?status= filter
...
#   booked     MarketService.book_offer writes it; also coordinator-api and apps/gpu
