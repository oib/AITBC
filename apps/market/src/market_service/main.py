"""
Market Service main application
...
configure_logging(level="INFO", service_name="market", to_file=True)
...
app = FastAPI(
    title="AITBC Market Service",
    description="Manages hardware+software bundle market operations",
    version="0.1.0",
    lifespan=lifespan,
)
...
# `enable_market_rate_limiting` as on at 100% rollout since 2026-05-24.
#
...
async def health() -> HealthResponse:
    ...
    return HealthResponse(**create_simple_health_response("market-service"))
...
async def live() -> dict[str, str]:
    ...
    return {"status": "alive", "service": "market-service"}


@app.get("/v1/market/status")
async def market_status() -> dict[str, str]:
    """Get market status"""
    return {"status": "operational", "service": "market-service", "message": "Market service is running"}
...
async def get_market_service(session: Annotated[AsyncSession, Depends(get_session)]) -> MarketService:
...
require_market_api_key = APIKeyAuthenticator(
    expected_key=settings.api_key,
    auth_enabled=settings.auth_enabled,
    success_role="market_admin",
)
...
async def get_offers(
    ...
    """Get market offers (v0.6.6: optional chain_id filter)"""
    try:
        logger.info(
            "GET /v1/market/offers called with filters: status=%s, region=%s, gpu_model=%s, chain_id=%s",
            status,
            region,
            gpu_model,
            chain_id,
        ...
        logger.info("GET /v1/market/offers returned %s offers", len(result))
        return result
        ...
        logger.error("Error in GET /v1/market/offers: %s: %s", type(e).__name__, str(e))
        raise


@app.get("/v1/market/offers/{offer_id}")
async def get_offer(offer_id: str, svc: Annotated[MarketService, Depends(get_market_service)]) -> Any:
    """Get a specific market offer"""
    try:
        logger.info("GET /v1/market/offers/%s called", offer_id)
        ...
        if not result:
            ...
            logger.info("GET /v1/market/offers/%s: not found", offer_id)
            return JSONResponse(status_code=404, content={"error": "Offer not found"})
        logger.info("GET /v1/market/offers/%s returned an offer", offer_id)
        return result
    except Exception as e:
        logger.error("Error in GET /v1/market/offers/%s: %s: %s", offer_id, type(e).__name__, str(e))
        raise
...
async def _create_escrow_bg(job_id: str, buyer: str, provider: str, amount: Decimal) -> None:
    ...
    try:
        headers = {"X-API-Key": settings.blockchain_rpc_api_key} if settings.blockchain_rpc_api_key else {}
...
@app.post("/v1/market/offers/{offer_id}/book")
...
@app.post("/v1/market/bids/{bid_id}/complete")
...
class MatchRequest(BaseModel):
    """Request model for market matching (v0.6.6)."""
...
@app.get("/v1/market")
async def get_market_overview(svc: Annotated[MarketService, Depends(get_market_service)]) -> Any:
    """Get hardware+software bundle market overview"""
    logger.info("GET /v1/market called - market overview")
...
@app.get("/v1/market/offers/{offer_id}/history")
async def get_offer_history(offer_id: str, svc: Annotated[MarketService, Depends(get_market_service)]) -> Any:
    """Get offer history (migrated from Coordinator API)"""
    logger.info("GET /v1/market/offers/%s/history called", offer_id)
    offer = await svc.get_offer(offer_id)
    if not offer:
        return JSONResponse(status_code=404, content={"error": "Offer not found"})
    history = {
...
@app.get("/v1/market/performance")
async def get_market_performance(
    svc: Annotated[MarketService, Depends(get_market_service)],
    # Required by the same accident as /analytics above, and feeding the same service default.
    period: str = "daily",
) -> Any:
    """Get market performance metrics (migrated from Coordinator API)"""
    logger.info("GET /v1/market/performance called with period=%s", period)
...
@app.post("/v1/market/dynamic-pricing")
async def calculate_dynamic_pricing(
    ...
    logger.info("POST /v1/market/dynamic-pricing called for offer %s", offer_id)
    offer = await svc.get_offer(offer_id)
    if not offer:
        return JSONResponse(status_code=404, content={"error": "Offer not found"})
...
@app.get("/v1/market/plugins")
async def get_plugins(plugin_type: str | None = None, status: str = "approved") -> Any:
    """Get market plugins"""
    logger.info("GET /v1/market/plugins called with type=%s, status=%s", plugin_type, status)
    return {
        "plugins": [
            {
...
@app.post("/v1/market/plugins")
async def register_plugin(plugin_data: dict[str, Any], svc: Annotated[MarketService, Depends(get_market_service)]) -> Any:
    """Register a new plugin"""
    try:
        logger.info("POST /v1/market/plugins called with data keys: %s", plugin_data.keys())
        result = await svc.register_plugin(plugin_data)
        logger.info("POST /v1/market/plugins registered plugin with id: %s", result["id"])
        return result
    except Exception as e:
        logger.error("Error in POST /v1/market/plugins: %s: %s", type(e).__name__, str(e))
        raise


@app.get("/v1/market/offer")
async def list_software_offers(
    ...
    try:
        ...
        logger.info("GET /v1/market/offer returned %s offers", len(result))
        return {"offers": result, "total": len(result)}
    except Exception as e:
        logger.error("Error in GET /v1/market/offer: %s: %s", type(e).__name__, str(e))
        raise


@app.get("/v1/market/offer/{plugin_id}")
async def get_software_offer(plugin_id: str, svc: Annotated[MarketService, Depends(get_market_service)]) -> Any:
    """Get a specific market offer"""
    try:
        logger.info("GET /v1/market/offer/%s called", plugin_id)
        result = await svc.get_software_service(plugin_id)
        if not result:
            return JSONResponse(status_code=404, content={"error": "Offer not found"})
        logger.info("GET /v1/market/offer/%s returned offer", plugin_id)
        return result
    except Exception as e:
        logger.error("Error in GET /v1/market/offer/%s: %s: %s", plugin_id, type(e).__name__, str(e))
        raise


@app.post("/v1/market/offer")
async def register_offer(service_data: dict[str, Any], svc: Annotated[MarketService, Depends(get_market_service)]) -> Any:
    """Register or update a market offer"""
    try:
        logger.info("POST /v1/market/offer called with data keys: %s", service_data.keys())
...
async def unregister_offer(plugin_id: str, svc: Annotated[MarketService, Depends(get_market_service)]) -> Any:
    ...
    try:
        ...
        logger.error("Error in DELETE /v1/market/offer/%s: %s: %s", plugin_id, type(e).__name__, str(e))
        raise


@app.post("/v1/market/ipfs/rental-token")
async def register_ipfs_rental_token(
    token_data: dict[str, Any], svc: Annotated[MarketService, Depends(get_market_service)]
) -> Any:
...
@app.get("/v1/market/ipfs/rental/{access_key}")
...
# ---------------------------------------------------------------------------
# Market jobs (v0.25.7 first-class IPFS and future software jobs)
# ---------------------------------------------------------------------------


@app.post("/v1/market/jobs")
async def create_market_job(job_data: dict[str, Any], svc: Annotated[MarketService, Depends(get_market_service)]) -> Any:
    """Create a market job and payment record."""
    try:
        logger.info("POST /v1/market/jobs called with data keys: %s", job_data.keys())
        result = await svc.create_market_job(job_data)
        logger.info("POST /v1/market/jobs created job: %s", result.get("job_id"))
        return result
    except ValueError as e:
        logger.info("Rejecting market job creation: %s", e)
        return JSONResponse(status_code=400, content={"error": str(e)})
    except Exception as e:
        logger.error("Error in POST /v1/market/jobs: %s: %s", type(e).__name__, str(e))
        raise


@app.get("/v1/market/jobs")
async def list_market_jobs(
    ...
    """List market jobs."""
    try:
        logger.info("GET /v1/market/jobs called")
        return await svc.list_market_jobs(
        ...
        logger.error("Error in GET /v1/market/jobs: %s: %s", type(e).__name__, str(e))
        raise


@app.get("/v1/market/jobs/usage")
async def get_market_job_usage(
    buyer_address: str,
    offer_id: str,
    svc: Annotated[MarketService, Depends(get_market_service)],
) -> Any:
    ...
    try:
        logger.info("GET /v1/market/jobs/usage called")
        return {
            "buyer_address": buyer_address,
            "offer_id": offer_id,
            "used_bytes": await svc.get_market_job_usage(buyer_address, offer_id),
        }
    except Exception as e:
        logger.error("Error in GET /v1/market/jobs/usage: %s: %s", type(e).__name__, str(e))
        raise


@app.get("/v1/market/jobs/{job_id}")
async def get_market_job(job_id: str, svc: Annotated[MarketService, Depends(get_market_service)]) -> Any:
    """Get a market job by ID."""
    try:
        logger.info("GET /v1/market/jobs/%s called", job_id)
        result = await svc.get_market_job(job_id)
        if not result:
            return JSONResponse(status_code=404, content={"error": f"Job not found: {job_id}"})
        return result
    except Exception as e:
        logger.error("Error in GET /v1/market/jobs/%s: %s: %s", job_id, type(e).__name__, str(e))
        raise


@app.post("/v1/market/jobs/{job_id}/cancel")
async def cancel_market_job(
    job_id: str,
    svc: Annotated[MarketService, Depends(get_market_service)],
    cancel_data: dict[str, Any] | None = None,
) -> Any:
    """Cancel a market job and request a refund."""
    try:
        cancel_data = cancel_data or {}
        logger.info("POST /v1/market/jobs/%s/cancel called", job_id)
        result = await svc.cancel_market_job(job_id, reason=cancel_data.get("reason", ""))
        return result
    except ValueError as e:
        return JSONResponse(status_code=404, content={"error": str(e)})
    except Exception as e:
        logger.error("Error in POST /v1/market/jobs/%s/cancel: %s: %s", job_id, type(e).__name__, str(e))
        raise


@app.post("/v1/market/jobs/{job_id}/pin-confirm")
async def confirm_market_job_pin(
    ...
    try:
        ...
        logger.info("POST /v1/market/jobs/%s/pin-confirm called", job_id)
        result = await svc.confirm_market_job_pin(
            job_id,
        ...
        logger.error("Error in POST /v1/market/jobs/%s/pin-confirm: %s: %s", job_id, type(e).__name__, str(e))
        raise


@app.post("/v1/market/jobs/{job_id}/release")
async def release_market_job_payment(
    job_id: str,
    release_data: dict[str, Any] | None = None,
    svc: Annotated[MarketService, Depends(get_market_service)] = None,  # type: ignore[assignment]
) -> Any:
    """Release the escrow payment for a market job."""
    try:
        logger.info("POST /v1/market/jobs/%s/release called", job_id)
        result = await svc.release_market_job_payment(job_id, release_data=release_data or {})
        return result
    except ValueError as e:
        return JSONResponse(status_code=400, content={"error": str(e)})
    except Exception as e:
        logger.error("Error in POST /v1/market/jobs/%s/release: %s: %s", job_id, type(e).__name__, str(e))
        raise


@app.post("/v1/market/jobs/{job_id}/refund")
async def refund_market_job_payment(
    job_id: str,
    svc: Annotated[MarketService, Depends(get_market_service)],
    refund_data: dict[str, Any] | None = None,
) -> Any:
    """Refund the escrow payment for a market job."""
    try:
        refund_data = refund_data or {}
        logger.info("POST /v1/market/jobs/%s/refund called", job_id)
        result = await svc.refund_market_job_payment(job_id, reason=refund_data.get("reason", ""), refund_data=refund_data)
        return result
    except ValueError as e:
        return JSONResponse(status_code=400, content={"error": str(e)})
    except Exception as e:
        logger.error("Error in POST /v1/market/jobs/%s/refund: %s: %s", job_id, type(e).__name__, str(e))
        raise
...
async def get_market_job_access(
    ...
    try:
        ...
        logger.error("Error in GET /v1/market/jobs/%s/access: %s: %s", job_id, type(e).__name__, str(e))
        raise


@app.get("/v1/market/access/{access_key}")
async def get_market_access_token(
    access_key: str,
    access_secret: str,
    svc: Annotated[MarketService, Depends(get_market_service)],
) -> Any:
    ...
    try:
        logger.info("GET /v1/market/access/%s called", access_key)
        result = await svc.get_market_job_access_token(access_key, access_secret)
        if not result:
            return JSONResponse(status_code=404, content={"error": "Invalid, expired, or unknown access token"})
        return result
    except Exception as e:
        logger.error("Error in GET /v1/market/access/%s: %s: %s", access_key, type(e).__name__, str(e))
        raise


@app.get("/v1/market/offer/{plugin_id}/health")
async def get_software_offer_health(plugin_id: str, svc: Annotated[MarketService, Depends(get_market_service)]) -> Any:
    """Check the health of a software offer."""
    try:
        logger.info("GET /v1/market/offer/%s/health called", plugin_id)
        return await svc.check_software_offer_health(plugin_id)
    except ValueError as e:
        return JSONResponse(status_code=404, content={"error": str(e)})
    except Exception as e:
        logger.error("Error in GET /v1/market/offer/%s/health: %s: %s", plugin_id, type(e).__name__, str(e))
        raise


@app.post("/v1/knowledge-graph")
async def create_graph(graph_data: dict[str, Any], svc: Annotated[MarketService, Depends(get_market_service)]) -> Any:
...
async def add_node(
    graph_id: str, node_data: dict[str, Any], svc: Annotated[MarketService, Depends(get_market_service)]
) -> Any:
...
async def add_edge(
    graph_id: str, edge_data: dict[str, Any], svc: Annotated[MarketService, Depends(get_market_service)]
) -> Any:
...
async def query_graph(graph_id: str, svc: Annotated[MarketService, Depends(get_market_service)]) -> Any:
...
@app.post("/v1/market/offer/{service_id}/rate")
async def rate_service(
    service_id: str, rating_data: RatingRequest, svc: Annotated[MarketService, Depends(get_market_service)]
) -> Any:
    """Rate a market service offer"""
    try:
        logger.info("POST /v1/market/offer/%s/rate called with rating=%s", service_id, rating_data.rating)
        ...
        logger.error("Error in POST /v1/market/offer/%s/rate: %s: %s", service_id, type(e).__name__, str(e))
        raise


@app.get("/v1/market/offer/{service_id}/ratings")
async def get_service_ratings(
    ...
    """Get ratings for a market service offer"""
    try:
        logger.info("GET /v1/market/offer/%s/ratings called", service_id)
        ...
        logger.error("Error in GET /v1/market/offer/%s/ratings: %s: %s", service_id, type(e).__name__, str(e))
        raise


@app.get("/v1/market/offer-by-id/{offer_id}")
async def get_offer_by_id(offer_id: str, svc: Annotated[MarketService, Depends(get_market_service)]) -> Any:
    """Get a market service offer by offer_id (blockchain offer ID)"""
    try:
        logger.info("GET /v1/market/offer-by-id/%s called", offer_id)
        service = await svc.get_service_by_offer_id(offer_id)
        if not service:
            return JSONResponse(status_code=404, content={"error": "Service not found"})
        return service
    except Exception as e:
        logger.error("Error in GET /v1/market/offer-by-id/%s: %s: %s", offer_id, type(e).__name__, str(e))
        raise


@app.get("/v1/market/ratings/unsynced")
async def get_unsynced_ratings(
    ...
    try:
        logger.info("GET /v1/market/ratings/unsynced called")
        ratings = await svc.get_unsynced_ratings(limit)
        return {"ratings": ratings, "count": len(ratings)}
    except Exception as e:
        logger.error("Error in GET /v1/market/ratings/unsynced: %s: %s", type(e).__name__, str(e))
        raise


@app.post("/v1/market/ratings/sync")
async def sync_ratings(ratings: list[dict[str, Any]], svc: Annotated[MarketService, Depends(get_market_service)]) -> Any:
    """Sync ratings from remote node"""
    try:
        logger.info("POST /v1/market/ratings/sync called with %s ratings", len(ratings))
        result = await svc.sync_ratings_from_remote(ratings)
        return {"status": "success", **result}
    except Exception as e:
        logger.error("Error in POST /v1/market/ratings/sync: %s: %s", type(e).__name__, str(e))
        raise


@app.post("/v1/market/ratings/mark-synced")
async def mark_ratings_synced(rating_ids: list[str], svc: Annotated[MarketService, Depends(get_market_service)]) -> Any:
    """Mark ratings as synced to remote nodes"""
    try:
        logger.info("POST /v1/market/ratings/mark-synced called with %s IDs", len(rating_ids))
        count = await svc.mark_ratings_synced(rating_ids)
        return {"status": "success", "marked_synced": count}
    except Exception as e:
        logger.error("Error in POST /v1/market/ratings/mark-synced: %s: %s", type(e).__name__, str(e))
        raise
...
async def submit_transaction(
    ...
    if transaction_type != "market":
        return JSONResponse(status_code=400, content={"error": "Invalid transaction type for market service"})
    try:
        if action == "offer":
            offer = MarketOffer(**transaction_data)
            session.add(offer)
        else:
            return JSONResponse(
...
async def get_transactions(
    ...
    try:
        ...
        if action == "offer" or not action:
            result = await session.execute(select(MarketOffer))
...
class ParameterChangeRequest(BaseModel):
    ...
    target_service: str = Field(default="market", description="Service type (must be 'market')")
...
# Allowed parameters that governance can change on the market service
_MARKET_GOVERNANCE_PARAMETERS: dict[str, type] = {
...
@app.post("/v1/market/parameters/apply")
async def apply_market_parameter(
    request: ParameterChangeRequest,
    authenticated: Annotated[dict[str, Any], Depends(require_market_api_key)],
) -> dict[str, Any]:
    """Apply a governance-approved parameter change to the market service (v0.7.4 §B4).
    ...
    """
    if request.target_service != "market":
        return JSONResponse(  # type: ignore[return-value]
            status_code=400, content={"error": f"target_service must be 'market', got '{request.target_service}'"}
        )

    param_type = _MARKET_GOVERNANCE_PARAMETERS.get(request.parameter_name)
    if param_type is None:
        return JSONResponse(  # type: ignore[return-value]
            status_code=400,
            content={
                "error": f"Unknown parameter: {request.parameter_name}",
                "allowed": sorted(_MARKET_GOVERNANCE_PARAMETERS),
            },
        )
...
class EdgeAdvertiseRequest(BaseModel):
    """Request body for edge node advertisement (POST /v1/market/edge-advertise)."""
...
@app.post("/v1/market/edge-advertise")
async def edge_advertise(
    request: EdgeAdvertiseRequest, session: Annotated[AsyncSession, Depends(get_session)]
) -> dict[str, Any]:
    """Register or update an edge node's GPU capabilities in the market (v0.6.6).
...
@app.get("/v1/market/edge-advertise")
...
@app.get("/v1/market/edge/{node_id}/health")
async def get_edge_health(node_id: str, session: Annotated[AsyncSession, Depends(get_session)]) -> dict[str, Any]:
    ...
    basic capability info from the market database.
    """
    from sqlalchemy import select
