...
async def record_plugin_usage(usage: PluginUsage):
    ...
    usage_record = {
        "usage_id": f"usage_{int(datetime.now(UTC).timestamp())}",
...
async def record_plugin_performance(performance: PluginPerformance):
    ...
    performance_record = {
        "performance_id": f"perf_{int(datetime.now(UTC).timestamp())}",
    ...
    logger.info(
        "Performance recorded: %s - CPU: %s%, Memory: %s%",
...
async def record_plugin_rating(rating: PluginRating):
    ...
    rating_record = {
        "rating_id": f"rating_{int(datetime.now(UTC).timestamp())}",
...
async def get_plugin_ratings(plugin_id: str):
    ...
    return {
        ...
        "recent_ratings": rating_records[-10:],
        "generated_at": datetime.now(UTC).isoformat(),
    }
...
def calculate_usage_statistics(usage_records: list[dict]) -> dict[str, Any]:
    ...
    unique_users = len({r["user_id"] for r in usage_records})
    action_counts = {}
...
def get_overview_statistics() -> dict[str, Any]:
    ...
    active_plugins = 0
    for _plugin_id, usage_records in plugin_usage_data.items():
        recent_usage = [r for r in usage_records if datetime.fromisoformat(r["timestamp"]) > cutoff_date]
        if recent_usage:
            active_plugins += 1
    return {
...
def get_trending_plugins(limit: int = 10) -> list[dict]:
    ...
    for plugin_id, usage_records in plugin_usage_data.items():
        recent_usage = [r for r in usage_records if datetime.fromisoformat(r["timestamp"]) > cutoff_date]
        if recent_usage:
            score = len(recent_usage) + len({r["user_id"] for r in recent_usage})
            plugin_scores.append(
                {
                    "plugin_id": plugin_id,
                    "trend_score": score,
                    "recent_usage": len(recent_usage),
                    "unique_users": len({r["user_id"] for r in recent_usage}),
                }
            )
...
def get_rating_summary() -> dict[str, Any]:
    ...
    for plugin_id, rating_records in plugin_ratings.items():
        if rating_records:
            ...
            all_ratings.append(
                {"plugin_id": plugin_id, "average_rating": round(avg_rating, 2), "total_ratings": len(rating_records)}
            )
    all_ratings.sort(key=lambda x: x["average_rating"], reverse=True)
    return {
        "total_plugins": len(all_ratings),
        "top_rated": all_ratings[:10],
        "average_rating_all": round(sum(r["average_rating"] for r in all_ratings) / len(all_ratings), 2)
        if all_ratings
        else 0.0,
    }
