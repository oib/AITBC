...
app = FastAPI(title='AITBC Marketplace Service', description='Manages hardware+software bundle marketplace operations', version='0.1.0', lifespan=lifespan)
...
async def ready() -> Any:
    ...
    try:
        ...
        return JSONResponse(status_code=503, content={'status': 'not_ready', 'service': 'marketplace-service', 'error': str(e)})
...
async def get_offers(status: str | None=None, region: str | None=None, gpu_model: str | None=None, svc: MarketplaceService=Depends(get_marketplace_service)) -> Any:
    """Get marketplace offers"""
    try:
        logger.info('GET /v1/marketplace/offers called with filters: status=%s, region=%s, gpu_model=%s', status, region, gpu_model)
...
async def get_offer(offer_id: str, svc: MarketplaceService=Depends(get_marketplace_service)) -> Any:
...
async def _create_escrow_bg(job_id: str, buyer: str, provider: str, amount: float) -> None:
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            resp = await client.post(f'{BLOCKCHAIN_RPC_URL}/rpc/escrow/create', json={'job_id': job_id, 'buyer': buyer, 'provider': provider, 'amount': amount})
...
async def get_analytics(period_type: str='daily', svc: MarketplaceService=Depends(get_marketplace_service)) -> Any:
    """Get marketplace analytics"""
    return await svc.get_analytics(period_type=period_type)

@app.get('/v1/marketplace')
async def get_marketplace_overview(svc: MarketplaceService=Depends(get_marketplace_service)) -> Any:
...
async def get_offer_history(offer_id: str, svc: MarketplaceService=Depends(get_marketplace_service)) -> Any:
    ...
    history = {'offer_id': offer_id, 'created_at': offer.created_at, 'price_history': [{'price': offer.price_per_hour, 'timestamp': offer.created_at, 'reason': 'initial_listing'}], 'booking_count': 0, 'total_revenue': 0, 'last_booked': None}
    return history

@app.post('/v1/marketplace/offers/{offer_id}/cancel')
async def cancel_offer(offer_id: str, reason: str | None=None, svc: MarketplaceService=Depends(get_marketplace_service)) -> Any:
    ...
    cancelled_offer = {'offer_id': offer_id, 'status': 'cancelled', 'cancelled_at': svc.get_current_timestamp(), 'reason': reason or 'user_requested'}
    logger.info('Cancelled offer %s', offer_id)
    return cancelled_offer

@app.get('/v1/marketplace/performance')
async def get_marketplace_performance(period: str='daily', svc: MarketplaceService=Depends(get_marketplace_service)) -> Any:
    ...
    performance = {'period': period, 'total_volume': analytics.get('total_volume', 0), 'total_trades': analytics.get('total_trades', 0), 'average_price': analytics.get('average_price', 0), 'price_volatility': analytics.get('price_volatility', 0), 'liquidity_score': analytics.get('liquidity_score', 0), 'active_providers': analytics.get('active_providers', 0), 'utilization_rate': analytics.get('utilization_rate', 0), 'fill_rate': analytics.get('fill_rate', 0)}
    return performance

@app.post('/v1/marketplace/dynamic-pricing')
async def calculate_dynamic_pricing(offer_id: str, current_demand: int, current_supply: int, svc: MarketplaceService=Depends(get_marketplace_service)) -> Any:
    ...
    return {'offer_id': offer_id, 'base_price': base_price, 'suggested_price': suggested_price, 'price_multiplier': price_multiplier, 'supply_demand_ratio': supply_demand_ratio, 'current_demand': current_demand, 'current_supply': current_supply, 'reason': 'dynamic_pricing_calculation'}
...
async def list_software_offers(service_type: str | None=None, status: str | None=None, svc: MarketplaceService=Depends(get_marketplace_service)) -> Any:
...
async def get_software_offer(plugin_id: str, svc: MarketplaceService=Depends(get_marketplace_service)) -> Any:
    """Get a specific marketplace offer"""
    try:
...
async def unregister_offer(plugin_id: str, svc: MarketplaceService=Depends(get_marketplace_service)) -> Any:
...
async def query_graph(graph_id: str, svc: MarketplaceService=Depends(get_marketplace_service)) -> Any:
...
async def rate_service(service_id: str, rating_data: RatingRequest, svc: MarketplaceService=Depends(get_marketplace_service)) -> Any:
    ...
    try:
        ...
        rating = await svc.add_service_rating(service_id=service_id, rating=rating_data.rating, reviewer_id=rating_data.reviewer_id, comment=rating_data.comment)
        return {'status': 'success', 'rating': {'id': rating.id, 'service_id': rating.service_id, 'rating': rating.rating, 'reviewer_id': rating.reviewer_id, 'comment': rating.comment, 'created_at': rating.created_at.isoformat() if rating.created_at else None}}
...
async def get_service_ratings(service_id: str, limit: int=50, offset: int=0, svc: MarketplaceService=Depends(get_marketplace_service)) -> Any:
    ...
    try:
        ...
        service_info = {'avg_rating': service.get('avg_rating', 0.0) if service else 0.0, 'rating_count': service.get('rating_count', 0) if service else 0} if service else {'avg_rating': 0.0, 'rating_count': 0}
        return {'service_id': service_id, 'service_info': service_info, 'ratings': ratings, 'count': len(ratings), 'limit': limit, 'offset': offset}
    except Exception as e:
...
async def get_offer_by_id(offer_id: str, svc: MarketplaceService=Depends(get_marketplace_service)) -> Any:
...
async def get_unsynced_ratings(limit: int=100, svc: MarketplaceService=Depends(get_marketplace_service)) -> Any:
...
async def mark_ratings_synced(rating_ids: list[str], svc: MarketplaceService=Depends(get_marketplace_service)) -> Any:
