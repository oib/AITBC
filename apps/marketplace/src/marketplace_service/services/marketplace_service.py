...
class MarketplaceService:
    ...
    async def list_offers(self, status: str | None=None, region: str | None=None, gpu_model: str | None=None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for row in result:
                ...
                if offer:
                    offers_list.append({'id': offer.id, 'provider': offer.provider, 'capacity': offer.capacity, 'price': offer.price, 'sla': offer.sla, 'status': offer.status, 'created_at': offer.created_at.isoformat() if offer.created_at else None, 'attributes': offer.attributes, 'gpu_model': offer.gpu_model, 'gpu_memory_gb': offer.gpu_memory_gb, 'gpu_count': offer.gpu_count, 'cuda_version': offer.cuda_version, 'price_per_hour': offer.price_per_hour, 'region': offer.region})
    ...
    async def get_software_service(self, plugin_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            if not service:
                return None
            return {'plugin_id': service.plugin_id, 'service_type': service.service_type, 'model': service.model, 'price': service.price, 'price_unit': service.price_unit, 'offer_id': service.offer_id, 'endpoint': service.endpoint, 'public_endpoint': service.public_endpoint, 'health_url': service.health_url, 'provider_address': service.provider_address, 'node_id': service.node_id, 'gpu_name': service.gpu_name, 'gpu_device': service.gpu_device, 'gpu_uuid': service.gpu_uuid, 'gpu_offer_id': service.gpu_offer_id, 'description': service.description, 'status': service.status, 'registered_at': service.registered_at.isoformat() if service.registered_at else None, 'updated_at': service.updated_at.isoformat() if service.updated_at else None, 'avg_rating': service.avg_rating, 'rating_count': service.rating_count}
        except Exception as e:
    ...
    async def register_software_service(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {'plugin_id': existing.plugin_id, 'service_type': existing.service_type, 'model': existing.model, 'status': existing.status}
        except Exception as e:
    ...
    async def add_edge(self, edge_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {'id': edge.id, 'graph_id': edge.graph_id, 'source_node_id': edge.source_node_id, 'target_node_id': edge.target_node_id}
        except Exception as e:
    ...
    async def add_service_rating(self, service_id: str, rating: float, reviewer_id: str, comment: str='', source_node: str='local') -> ServiceRating:
        ...
        try:
            ...
            service_rating = ServiceRating(service_id=service_id, rating=rating, reviewer_id=reviewer_id, comment=comment, source_node=source_node)
    ...
    async def get_service_ratings(self, service_id: str, limit: int=50, offset: int=0) -> list[dict[str, Any]]:
        ...
        try:
            ...
            return [{'id': r.id, 'service_id': r.service_id, 'rating': r.rating, 'reviewer_id': r.reviewer_id, 'comment': r.comment, 'created_at': r.created_at.isoformat() if r.created_at else None, 'source_node': r.source_node} for r in ratings]
        except Exception as e:
    ...
    async def sync_ratings_from_remote(self, remote_ratings: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        try:
            ...
            for remote_rating in remote_ratings:
                stmt = select(ServiceRating).where(ServiceRating.service_id == remote_rating['service_id'], ServiceRating.reviewer_id == remote_rating['reviewer_id'])
                result = await self.session.execute(stmt)
                existing = result.scalar_one_or_none()
                if existing:
                    ...
                    new_rating = ServiceRating(id=remote_rating['id'], service_id=remote_rating['service_id'], rating=remote_rating['rating'], reviewer_id=remote_rating['reviewer_id'], comment=remote_rating['comment'], created_at=datetime.fromisoformat(remote_rating['created_at']), synced_at=datetime.utcnow(), source_node=remote_rating.get('source_node', 'remote'))
    ...
    async def _update_service_rating(self, service_id: str) -> None:
        ...
        try:
            ...
            if service:
                ...
                await self.session.commit()
                logger.info('Updated service %s rating: avg=%s, count=%s', service_id, service.avg_rating, service.rating_count)
        except Exception as e:
