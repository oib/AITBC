...
MAX_RETRIES = 10
RETRY_DELAY = 30
coordinator_client = AITBCHTTPClient(base_url=COORDINATOR_URL, headers={'X-Api-Key': AUTH_TOKEN, 'Content-Type': 'application/json'}, timeout=30)
...
class FlushHandler(logging.StreamHandler):
    ...
    def emit(self, record):
        super().emit(record)
        self.flush()
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s', handlers=[FlushHandler(sys.stdout), logging.FileHandler(LOG_PATH)])
logger = get_logger(__name__)
sys.stdout.reconfigure(line_buffering=True)
sys.stderr.reconfigure(line_buffering=True)
ARCH_MAP = {'4090': 'ada_lovelace', '4080': 'ada_lovelace', '4070': 'ada_lovelace', '4060': 'ada_lovelace', '3090': 'ampere', '3080': 'ampere', '3070': 'ampere', '3060': 'ampere', '2080': 'turing', '2070': 'turing', '2060': 'turing', '1080': 'pascal', '1070': 'pascal', '1060': 'pascal'}
...
def build_gpu_capabilities() -> dict:
    ...
    return {'gpus': [{'name': model, 'memory_mb': memory_total, 'architecture': arch, 'consumer_grade': True, 'edge_optimized': edge_optimized}] if gpu_info else [], 'cuda': cuda_version, 'models': models if ollama_available else [], 'price': 0.01, 'region': 'localhost', 'platform': 'CUDA' if gpu_info else 'CPU', 'supported_tasks': ['inference', 'training', 'stable-diffusion', 'llama'], 'max_concurrent_jobs': 1}
...
def send_heartbeat():
    ...
    if gpu_info:
        heartbeat_data = {'status': 'active', 'current_jobs': 0, 'last_seen': datetime.now(UTC).isoformat(), 'gpu_utilization': gpu_info['utilization'], 'memory_used': gpu_info['memory_used'], 'memory_total': gpu_info['memory_total'], 'architecture': arch, 'edge_optimized': arch in {'ada_lovelace', 'ampere', 'turing'}, 'network_latency_ms': latency_ms}
    else:
        heartbeat_data = {'status': 'active', 'current_jobs': 0, 'last_seen': datetime.now(UTC).isoformat(), 'gpu_utilization': 0, 'memory_used': 0, 'memory_total': 0, 'architecture': 'unknown', 'edge_optimized': False, 'network_latency_ms': latency_ms}
    headers = {'X-Api-Key': AUTH_TOKEN, 'X-Miner-ID': MINER_ID, 'Content-Type': 'application/json'}
    try:
    ...
    try:
        ...
        if job_type == 'inference':
            ...
            if ollama_response:
                ...
                submit_result(job_id, {'result': {'status': 'completed', 'output': output, 'model': model, 'tokens_processed': result.get('eval_count', 0), 'execution_time': execution_time, 'gpu_used': True}, 'metrics': {'gpu_utilization': gpu_after['utilization'] if gpu_after else 0, 'memory_used': gpu_after['memory_used'] if gpu_after else 0, 'memory_peak': max(gpu_after['memory_used'] if gpu_after else 0, 2048)}})
