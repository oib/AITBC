...
def test_check_ollama_success(mock_get):
    ...
    mock_get.return_value = Mock(
        status_code=200, json=lambda: {"models": [{"name": "llama3.2:latest"}, {"name": "mistral:latest"}]}
    )
...
def test_register_miner_success(mock_build, mock_post):
    ...
    mock_post.return_value = Mock(status_code=200, json=lambda: {"session_token": "test-token-123"})
...
def test_execute_job_inference_success(mock_gpu, mock_post, mock_submit):
    ...
    mock_post.return_value = Mock(status_code=200, json=lambda: {"response": "Test output", "eval_count": 100})
