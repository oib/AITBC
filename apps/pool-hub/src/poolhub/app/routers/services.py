...
async def create_or_update_service_config(
    ...
    if service_type not in [s.value for s in ServiceType]:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid service type: {service_type}")
...
async def get_service_template(service_type: str) -> dict[str, Any]:
    ...
    templates = {
        "whisper": {
            ...
            "pricing": {"per_minute": 0.001, "min_charge": 0.01},
        ...
        "stable_diffusion": {
            ...
            "pricing": {"per_image": 0.01, "per_step": 0.001},
        ...
        "llm_inference": {
            ...
            "pricing": {"per_1k_tokens": 0.001, "min_charge": 0.01},
        ...
        "ffmpeg": {
            ...
            "pricing": {"per_minute": 0.005, "per_gb": 0.01},
        ...
        "blender": {
            ...
            "pricing": {"per_frame": 0.01, "per_hour": 0.5},
    ...
    if service_type not in templates:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Unknown service type: {service_type}")
