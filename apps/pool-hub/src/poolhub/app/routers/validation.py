...
async def get_compatible_services(miner: Miner = Depends(get_miner_from_token)) -> list[dict[str, Any]]:
    ...
    return [
        {"service_id": service_id, "compatibility_score": score, "grade": _get_grade_from_score(score)}
...
async def validate_multiple_services(
    ...
    for validation in validations:
        ...
        if not service_id:
            results.append({"service_id": service_id, "valid": False, "errors": ["Missing service_id"]})
            continue
