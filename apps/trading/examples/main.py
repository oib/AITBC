...
app = FastAPI(title='AITBC Trading Engine', description='High-performance order matching and trade execution', version='1.0.0', lifespan=lifespan)
...
@app.get('/')
async def root():
    return {'service': 'AITBC Trading Engine', 'status': 'running', 'timestamp': datetime.now(UTC).isoformat(), 'version': '1.0.0'}

@app.get('/health')
async def health_check():
    return {'status': 'healthy', 'active_order_books': len(order_books), 'total_orders': len(orders), 'total_trades': len(trades), 'uptime': 'running'}
...
async def submit_order(order: Order):
    ...
    if symbol not in order_books:
        order_books[symbol] = {'bids': defaultdict(list), 'asks': defaultdict(list), 'last_price': None, 'volume_24h': 0.0, 'high_24h': None, 'low_24h': None, 'created_at': datetime.now(UTC).isoformat()}
    order_data = {'order_id': order.order_id, 'symbol': order.symbol, 'side': order.side, 'type': order.type, 'quantity': order.quantity, 'remaining_quantity': order.quantity, 'price': order.price, 'user_id': order.user_id, 'timestamp': order.timestamp.isoformat(), 'status': 'open', 'filled_quantity': 0.0, 'average_price': None}
    orders[order.order_id] = order_data
    trades_executed = await process_order(order_data)
    logger.info('Order submitted: %s - %s %s %s', order.order_id, order.side, order.quantity, order.symbol)
    return {'order_id': order.order_id, 'status': order_data['status'], 'filled_quantity': order_data['filled_quantity'], 'remaining_quantity': order_data['remaining_quantity'], 'trades_executed': len(trades_executed), 'average_price': order_data['average_price']}
...
async def list_orders():
    """List all orders"""
    return {'orders': list(orders.values()), 'total_orders': len(orders), 'open_orders': len([o for o in orders.values() if o['status'] == 'open']), 'filled_orders': len([o for o in orders.values() if o['status'] == 'filled'])}
...
async def cancel_order(order_id: str):
    ...
    return {'order_id': order_id, 'status': 'cancelled', 'cancelled_at': order['cancelled_at']}
...
async def get_market_data():
    ...
    for symbol, book in order_books.items():
        trades_24h = [t for t in trades.values() if t['symbol'] == symbol and datetime.fromisoformat(t['timestamp']) > datetime.now(UTC) - timedelta(hours=24)]
        market_summary[symbol] = {'last_price': book['last_price'], 'volume_24h': book['volume_24h'], 'high_24h': book['high_24h'], 'low_24h': book['low_24h'], 'trades_count_24h': len(trades_24h), 'bid_price': max(book['bids'].keys()) if book['bids'] else None, 'ask_price': min(book['asks'].keys()) if book['asks'] else None}
    return {'market_data': market_summary, 'total_symbols': len(market_summary), 'generated_at': datetime.now(UTC).isoformat()}
...
async def get_engine_stats():
    ...
    return {'engine_stats': {'total_orders': total_orders, 'total_trades': total_trades, 'total_volume': total_volume, 'orders_by_status': dict(orders_by_status), 'trades_by_symbol': dict(trades_by_symbol), 'active_order_books': len(order_books), 'uptime': 'running'}, 'generated_at': datetime.now(UTC).isoformat()}
...
async def execute_trade(order1: dict, order2: dict, price: float) -> dict | None:
    """Execute a trade between two orders"""
    trade_quantity = min(order1['remaining_quantity'], order2['remaining_quantity'])
    if trade_quantity <= 0:
        return None
    trade_id = f'trade_{int(datetime.now(UTC).timestamp())}_{len(trades)}'
    trade = {'trade_id': trade_id, 'symbol': order1['symbol'], 'buy_order_id': order1['order_id'] if order1['side'] == 'buy' else order2['order_id'], 'sell_order_id': order2['order_id'] if order2['side'] == 'sell' else order1['order_id'], 'quantity': trade_quantity, 'price': price, 'timestamp': datetime.now(UTC).isoformat()}
    ...
    for order in [order1, order2]:
        if order['remaining_quantity'] <= 0:
            if order['side'] == 'buy' and price_key in book['bids']:
                ...
                if not book['asks'][price_key]:
                    del book['asks'][price_key]
    logger.info('Trade executed: %s - %s @ %s', trade_id, trade_quantity, price)
    return trade
...
async def simulate_market_activity():
    ...
    while True:
        ...
        if len(order_books) > 0:
            import random
            for symbol in list(order_books.keys())[:3]:
                if random.random() < 0.3:
                    side = random.choice(['buy', 'sell'])
                    quantity = random.uniform(10, 1000)
                    order_id = f'sim_order_{int(datetime.now(UTC).timestamp())}'
                    order = Order(order_id=order_id, symbol=symbol, side=side, type='market', quantity=quantity, user_id='sim_user', timestamp=datetime.now(UTC))
                    order_data = {'order_id': order.order_id, 'symbol': order.symbol, 'side': order.side, 'type': order.type, 'quantity': order.quantity, 'remaining_quantity': order.quantity, 'price': order.price, 'user_id': order.user_id, 'timestamp': order.timestamp.isoformat(), 'status': 'open', 'filled_quantity': 0.0, 'average_price': None}
