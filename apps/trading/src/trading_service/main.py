...
async def get_requests(status: str | None=None, buyer_agent_id: str | None=None, trade_type: str | None=None, svc: TradingService=Depends(get_trading_service)):
    """Get trade requests"""
...
async def get_matches(status: str | None=None, buyer_agent_id: str | None=None, seller_agent_id: str | None=None, svc: TradingService=Depends(get_trading_service)):
    """Get trade matches"""
...
async def get_agreements(status: str | None=None, buyer_agent_id: str | None=None, seller_agent_id: str | None=None, svc: TradingService=Depends(get_trading_service)):
...
async def get_transactions(transaction_type: str | None=None, action: str | None=None, status: str | None=None, island_id: str | None=None, session: AsyncSession=Depends(get_session_dep)):
    ...
    try:
        ...
        if action == 'request' or not action:
            ...
            transactions.extend([{'request_id': r.request_id, 'action': 'request', 'buyer_agent_id': r.buyer_agent_id, 'trade_type': r.trade_type, 'status': r.status, 'island_id': r.island_id, 'created_at': r.created_at.isoformat() if r.created_at else None} for r in requests])
        if action == 'match' or not action:
            result = await session.execute(select(TradeMatch))
            matches = result.scalars().all()
            transactions.extend([{'match_id': m.match_id, 'action': 'match', 'request_id': m.request_id, 'seller_agent_id': m.seller_agent_id, 'status': m.status, 'island_id': m.island_id, 'created_at': m.created_at.isoformat() if m.created_at else None} for m in matches])
        if action == 'agreement' or not action:
            result = await session.execute(select(TradeAgreement))
            agreements = result.scalars().all()
            transactions.extend([{'agreement_id': a.agreement_id, 'action': 'agreement', 'match_id': a.match_id, 'status': a.status, 'island_id': a.island_id, 'created_at': a.created_at.isoformat() if a.created_at else None} for a in agreements])
        if status:
...
async def get_transaction_explorer(tx_hash: str, chain_id: str | None=None, session: AsyncSession=Depends(get_session_dep)):
...
async def create_exchange_payment(payment_request: ExchangePaymentRequest, background_tasks: BackgroundTasks) -> dict[str, Any]:
...
async def get_exchange_rates() -> dict[str, Any]:
    ...
    return {'btc_to_aitbc': BITCOIN_CONFIG['exchange_rate'], 'aitbc_to_btc': 1.0 / BITCOIN_CONFIG['exchange_rate'], 'fee_percent': 0.5}
...
async def get_market_stats() -> dict[str, Any]:
    ...
    return {'price': base_price, 'price_change_24h': price_change_percent, 'daily_volume': daily_volume, 'daily_volume_btc': daily_volume / BITCOIN_CONFIG['exchange_rate'], 'total_payments': len([p for p in payments.values() if p['status'] == 'confirmed']), 'pending_payments': len([p for p in payments.values() if p['status'] == 'pending'])}
