...
async def create_chain(request: dict[str, Any]):
    ...
    request.get("coordinator_api_key")
...
async def list_chain_wallets(chain_id: str):
    ...
    return JSONResponse({"chain_id": chain_id, "wallets": wallet_list, "total": len(wallet_list)})
...
async def unlock_chain_wallet(chain_id: str, wallet_id: str):
    ...
    return JSONResponse({"wallet_id": wallet_id, "chain_id": chain_id, "address": wallet["address"], "unlocked": True})
...
if __name__ == "__main__":
    ...
    uvicorn.run(wallet_app, host="0.0.0.0", port=8003, log_level="critical", access_log=False)  # nosec B104
