...
class ChainAwareWalletService:
    ...
    def create_wallet(
        ...
        try:
            ...
            success = self.multichain_ledger.create_wallet(
                chain_id, wallet_id, keystore_record.public_key, metadata=keystore_record.metadata
            )
            if not success:
                try:
            ...
            self.multichain_ledger.record_event(
                ...
                wallet_id,
                "created",
                {"public_key": keystore_record.public_key, "chain_id": chain_id, "metadata": metadata or {}},
            )
    ...
    def sign_message(
        self, chain_id: str, wallet_id: str, password: str, message: bytes, ip_address: str | None = None
    ) -> str | None:
        ...
        try:
            ...
            if signature:
                self.multichain_ledger.record_event(
                    ...
                    wallet_id,
                    "signed",
                    {"message_length": len(message), "ip_address": ip_address, "chain_id": chain_id},
                )
    ...
    def get_wallet_events(
        self, chain_id: str, wallet_id: str, event_type: str | None = None, limit: int = 100
    ) -> list[dict[str, Any]]:
    ...
    def migrate_wallet_between_chains(
        self, source_chain_id: str, target_chain_id: str, wallet_id: str, password: str, new_password: str | None = None
    ) -> bool:
            ...
            try:
                ...
                if target_wallet:
                    self.multichain_ledger.record_event(
                        ...
                        wallet_id,
                        "migrated_from",
                        {"target_chain": target_chain_id, "migration_timestamp": datetime.now().isoformat()},
                    )
                    self.multichain_ledger.record_event(
                        target_chain_id,
                        wallet_id,
                        "migrated_to",
                        {"source_chain": source_chain_id, "migration_timestamp": datetime.now().isoformat()},
                    )
