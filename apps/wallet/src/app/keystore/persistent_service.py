...
class PersistentKeystoreService:
    ...
    def _get_wallet_unlocked(self, wallet_id: str) -> WalletRecord | None:
    ...
    def _register_account_on_chain(self, address: str) -> dict[str, Any]:
        ...
        try:
            ...
            response = httpx.post(f"{rpc_url}/rpc/register-account", json={"address": address}, timeout=10.0)
        ...
        with self._lock:
            ...
            try:
                conn.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                    (wallet_id, public_key_hex, salt, nonce, ciphertext, json.dumps(metadata_map), now, now),
                )
    ...
    def unlock_wallet(self, wallet_id: str, password: str, ip_address: str | None = None) -> bytes:
    ...
    def sign_message(self, wallet_id: str, password: str, message: bytes, ip_address: str | None = None) -> bytes:
    ...
    def _submit_transaction_to_chain(self, tx_data: dict[str, Any], signature: str) -> dict[str, Any]:
        ...
        try:
            ...
            response = httpx.post(f"{rpc_url}/rpc/transaction", json=request_data, timeout=30.0)
            response.raise_for_status()
    ...
    def _log_access(self, wallet_id: str, action: str, success: bool, ip_address: str | None = None) -> None:
    ...
    def get_access_log(self, wallet_id: str, limit: int = 50) -> list[dict[str, Any]]:
        ...
        with self._lock:
            ...
            try:
                ...
                return [
                    {"action": row[0], "timestamp": row[1], "success": bool(row[2]), "ip_address": row[3]}
                    for row in cursor.fetchall()
                ]
            finally:
                conn.close()
