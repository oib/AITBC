...
async def _import_genesis_wallet_from_env():
    ...
    try:
        async with httpx.AsyncClient(timeout=5) as client:
            ...
            payload = {'wallet_id': 'genesis', 'chain_id': chain_id, 'password': password, 'secret_key': secret_b64, 'metadata': {'address': address, 'original_address': address, 'chain_id': chain_id}}
                ...
                for wallet_file in wallet_files:
                    try:
                        ...
                        if not private_key_hex:
                            continue
                        secret_b64 = base64.b64encode(bytes.fromhex(private_key_hex)).decode()
                        payload = {'wallet_id': wallet_id, 'chain_id': chain_id, 'password': password, 'secret_key': secret_b64, 'metadata': {'address': address, 'imported_from': str(wallet_file), 'original_address': address}}
                        r = await client.post(f'{daemon_url}/v1/wallets', json=payload)
                        if r.status_code in (200, 201):
                            imported += 1
