...
| `energy` | Energy-floor pricing, operator signing, and provider tariff management | `floor`, `operator`, `provider`, `suggest` |
...
| `network` | Peer connectivity and network operations | `force-sync`, `gossip`, `heartbeat`, `lease-status`, `peers`, `set-sync-source`, `status`, `subscribe`, `subscribers`, `test` |
...
| `wallet` | Manage your wallets and transactions | `address`, `backup`, `balance`, `bridge`, `create`, `delete`, `earn`, `export`, `import-wallet`, `info`, `liquidity-claim`, `liquidity-stake`, `liquidity-unstake`, `list`, `multisig-create`, `multisig-propose`, `multisig-sign`, `request-payment`, `restore`, `rewards`, `send`, `spend`, `stake`, `staking-info`, `stats`, `switch`, `transactions`, `unstake` |
