...
def receive(ctx, receiver_id, limit, format):
    ...
    try:
        ...
        queued = comm.message_queue.get(receiver_id, [])[:limit]

        if not queued:
            output(f"No queued messages for {receiver_id}", ctx.obj.get("output_format", "table"))
            return
        ...
        message_data = [
            {
                ...
                "Chain": m.chain_id,
                "Target Chain": m.target_chain_id or "Same",
                "Priority": m.priority,
                "Payload": json.dumps(m.payload) if m.payload else "",
                "Timestamp": m.timestamp.strftime("%Y-%m-%d %H:%M:%S"),
            }
            for m in queued
        ]

        output(message_data, ctx.obj.get("output_format", format), title=f"Messages for {receiver_id}")
    except Exception as e:
        abort(ctx, f"Error receiving messages: {str(e)}", from_exception=e)
