...
@click.pass_context
def escrow_create(ctx, job_id: str, buyer: str, provider: str, amount: str):
    """Create on-chain escrow for a marketplace job"""
    from ...utils.money import wallet_amount

    try:
        config = get_config()
        amount_decimal = wallet_amount(amount)
        contract_id = _escrow_create(job_id, buyer, provider, amount_decimal, config)
        if contract_id:
            success(f"Escrow created for job {job_id}: contract_id={contract_id}")
            output(
                {"job_id": job_id, "contract_id": contract_id, "buyer": buyer, "provider": provider, "amount": str(amount_decimal)},
                ctx.obj.get("output_format", "table"),
            )
        else:
            error(f"Failed to create escrow for job {job_id}")
    except Exception as e:
