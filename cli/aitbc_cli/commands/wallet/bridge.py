...
def deposit(ctx, amount: Decimal, wallet_name: str | None, eth_rpc_url: str | None, password: str | None):
    ...
    bridge_eth_address = deposit_info.get("bridge_address", "")
    eth_amount = Decimal(deposit_info.get("eth_amount", 0))
    hex_data = deposit_info.get("hex_data", "")
    estimated_ait = deposit_info.get("estimated_ait", 0)
    ...
    rpc_url = eth_rpc_url or os.getenv("ETH_RPC_URL", config.eth_rpc_url or "https://eth.llamarpc.com")
    try:
