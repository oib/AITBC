...
class GlobalChainMarket:
    ...
    async def create_listing(
        ...
        """Create a new chain listing in the market"""
        try:
            ...
            for listing in self.listings.values():
                if listing.chain_id == chain_id and listing.status == MarketStatus.ACTIVE:
                    return None
    ...
    async def purchase_chain(self, listing_id: str, buyer_id: str, payment_method: str) -> str | None:
        """Purchase a chain from the market"""
        try:
            listing = self.listings.get(listing_id)
            if not listing or listing.status != MarketStatus.ACTIVE:
                return None
    ...
    async def get_user_transactions(self, user_id: str, role: str = "both") -> list[MarketTransaction]:
    ...
    async def get_market_overview(self) -> dict[str, Any]:
        ...
        try:
            ...
            overview = {
                "market_metrics": asdict(self.market_metrics),
            ...
            logger.error("Error getting market overview: %s", e)
            return {}

    async def _create_escrow_contract(self, transaction: MarketTransaction):
    ...
    async def _release_escrow(self, transaction: MarketTransaction):
        ...
        try:
            ...
            if escrow_contract:
                ...
                market_fee = transaction.price * Decimal(str(self.thresholds["market_fee_percentage"]))
                seller_amount = transaction.price - escrow_fee - market_fee
    ...
    async def _update_market_metrics(self):
        ...
        try:
            ...
            active_listings = len([listing for listing in self.listings.values() if listing.status == MarketStatus.ACTIVE])
