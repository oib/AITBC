...
| `DISABLED_COMMANDS_CLEANUP.md` | Historical analysis of previously disabled commands (agent-msg, marketplace_cmd, marketplace_advanced) — all have since been re-enabled or merged. Kept for historical context. |
...
- The `DISABLED_COMMANDS_CLEANUP.md` timeline (Week 1-3 actions) is historical. The commands it discusses (`agent-msg`, `marketplace_cmd`) are now registered in `aitbc_cli/core/main.py` and visible via `aitbc --help`.
