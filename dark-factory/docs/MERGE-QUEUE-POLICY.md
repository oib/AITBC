...
## Scope and Governance Exception

This policy is a **bounded, POPM-approved exception** to
[ADR-A-0005](../../adrs/agentic/ADR-A-0005-mandatory-prs.md) ("the merge button is
the human gate"), recorded 2026-07-07. The exception is scoped to:

- **GitHub-hosted consumer projects only** — the governance/boilerplate repo itself
  does **not** use the queue; it follows the `CONTRIBUTING.md` rebase-first,
  "Rebase and merge", human-merged flow.
- **Story PRs into an epic integration branch only**
  (`epic/AITBC-XX-{description}`), consistent with
  [ADR-A-0014](../../adrs/agentic/ADR-A-0014-workflow-v3-per-epic-merge-gate.md).

The **epic→`main` PR keeps its human merge gate**
([ADR-A-0004](../../adrs/agentic/ADR-A-0004-human-approval-boundaries.md)): it is
never auto-enqueued and never squash-merged by an agent. Where the sections below
reference `main` as the queue target, read it as the queue's target
branch — the epic integration branch under an orchestrated Workflow v3 epic. The
one path an agent may auto-squash-merge into is that integration branch, not `main`.
...
```bash
gh pr create --title "type(scope): description [AITBC-XXX]" --body "..."
```
...
- PR titles MUST follow: `type(scope): description [AITBC-XXX]`
