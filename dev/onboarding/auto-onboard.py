        ...
        try:
            ...
            if agent_type == 'compute_provider':
                ...
                agent = ComputeProvider.register(
                    agent_name=f"auto-provider-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}",
                    capabilities={
                ...
                agent = ComputeConsumer.create(
                    agent_name=f"auto-consumer-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}",
                    capabilities={
                        "compute_type": "inference",
                ...
                agent = PlatformBuilder.create(
                    agent_name=f"auto-builder-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}",
                    capabilities={
                ...
                agent = SwarmCoordinator.create(
                    agent_name=f"auto-coordinator-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}",
                    capabilities={
    ...
    async def generate_onboarding_report(self, agent):
        ...
        report = {
            'onboarding': {
                ...
                'duration_minutes': (datetime.now(timezone.utc) - self.session['start_time']).total_seconds() / 60,
                'status': 'success',
