...
3. **Apply substitutions** -- Replaces `{{PLACEHOLDER}}` tokens in upstream
   files with your manifest's identity values (e.g., `AITBC`
   becomes `REN`).
...
Upstream repo                    Your fork
  ...
  commands/                        commands/
    start-work.md   ---substitute-->  start-work.md (AITBC -> SCA)
```
...
### Upstream host (GitHub or Bitbucket)

Release discovery (`get_latest_release`, `list_releases`, `get_upstream_sha`) and
tarball fetch switch on the upstream git host. Set it in `.harness-sync.json`:

```json
{
  "upstream_repo": "workspace/repo",
...
| --- | --- | --- | --- |
| `upstream_host` | `github`, `bitbucket` | `github` | `bitbucket` discovers releases via host-neutral `git ls-remote --tags` (the annotated release tag is the release record — no API token needed) and fetches tarballs from `bitbucket.org/<repo>/get/<ref>.tar.gz`. |
| `upstream_git_url` | full clone URL | derived from `upstream_repo` + `upstream_host` | Override when your remote URL is not the default `https://<host>/<repo>.git`. |

Both may also be set via the `UPSTREAM_HOST` / `UPSTREAM_GIT_URL` environment
variables. The canonical boilerplate upstream is hosted on Bitbucket; a GitHub
mirror or fork sets `upstream_host: github` (the default).
...
2. Keys are sorted by length descending (longest-match-first) to prevent
   partial matches. For example, `https://github.com/oib/AITBC` is replaced before
   `oib`.
...
[ERROR] Preflight failed: 2 issue(s) found
  ...
  commands/start-work.md:12: unreplaced token AITBC
