...
# Check stakes
SELECT * FROM stake WHERE chain_id = 'ait-localnet' AND address = '<wallet_address>';

# Check agent identities
SELECT * FROM agent_identity WHERE chain_id = 'ait-localnet' AND agent_id = '<agent_id>';

# Check governance proposals
SELECT * FROM governance_proposal WHERE chain_id = 'ait-localnet' AND proposal_id = 'prop_test_001';

# Check governance votes
SELECT * FROM governance_vote WHERE chain_id = 'ait-localnet' AND proposal_id = 'prop_test_001';

# Check GPU registrations
SELECT * FROM gpu_registration WHERE chain_id = 'ait-localnet' AND gpu_id = 'GPU-ba5c6553-6396-ab66-5706-17e6de30a93a';

# Check GPU allocations
SELECT * FROM gpu_allocation WHERE chain_id = 'ait-localnet' AND gpu_id = 'GPU-ba5c6553-6396-ab66-5706-17e6de30a93a';
```
...
curl -X POST http://hub.example.net:8202/rpc/staking/stake \
  ...
  -d '{"address": "<wallet_address>", "amount": 1000000000000000000, "lock_days": 30, "chain_id": "ait-localnet", "signature": "<sig>", "nonce": <n>, "timestamp": <unix>}'
```
...
curl -X POST http://hub.example.net:8202/rpc/gpu/register \
  -H "Content-Type: application/json" \
  -H "X-API-Key: $BLOCKCHAIN_RPC_API_KEY" \
  -d '{"gpu_id": "GPU-test", "miner_id": "miner-001", "model": "RTX 4090", "memory_gb": 24, "price_per_hour": 0.5, "registered_by": "<wallet_address>", "chain_id": "ait-localnet"}'

# GPU query via the blockchain RPC
curl -X GET "http://hub.example.net:8202/rpc/gpu/info/GPU-test?chain_id=ait-localnet"

# GPU list via the blockchain RPC
curl -X GET "http://hub.example.net:8202/rpc/gpus?chain_id=ait-localnet"
```
