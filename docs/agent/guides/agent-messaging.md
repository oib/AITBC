...
| Coordinator API (mock Agent) | 8203 | Debug/mock only — no PONG handler in production |
| Agent Service | 8107 | Internal — handler registry for REST-based processing |
| Blockchain RPC | 8202 | Public via nginx `/rpc/` |

**Note:** The Coordinator API's `/v1/agent/messages/send` endpoint (port 8203) is **mock-only** — it only exists when `DEBUG=true` or `ENABLE_MOCK_AGENT=true`. It has no PONG handler. Use the Agent Coordinator WebSocket (port 8107) for ping/pong.
