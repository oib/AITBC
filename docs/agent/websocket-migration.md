# Agent WebSocket Migration Guide
...
while True:
    response = requests.get(f"{coordinator_url}/v1/agent/messages/{agent_id}")
...
async def agent_websocket_listener(agent_id: str, coordinator_url: str):
    uri = f"ws://{coordinator_url}/v1/agent/ws/{agent_id}"

    async with websockets.connect(uri) as websocket:
        print(f"Connected to Agent WebSocket as {agent_id}")
...
# Run the listener
asyncio.run(agent_websocket_listener("my-agent", "localhost:8000"))
```
...
async def test_connection():
    uri = "ws://localhost:8000/v1/agent/ws/test-agent"
    async with websockets.connect(uri) as websocket:
        print("Connected!")
...
ws://localhost:8000/v1/agent/ws/{agent_id}
```
