# Agent Agent Autonomy Features

**Last Updated:** June 5, 2026
**Version:** v0.4.3.3 (Service Status Update)

This document describes the enhanced Agent agent autonomy features for distributed decision making, self-healing, and autonomous resource management.

> **🟢 Operational Status**: All Agent messaging systems are operational with AgentDaemon successfully polling every 10 seconds and Coordinator API responding normally.

## Overview

Agent agents now have enhanced autonomy capabilities:
...
All endpoints are available at `http://localhost:8203/v1/agent/*` and require admin API key authentication via the `X-Api-Key` header.
...
```bash
# Propose a decision
curl -X POST http://localhost:8203/v1/agent/decision/propose \
...
# Submit a vote
curl -X POST http://localhost:8203/v1/agent/decision/vote \
  -H "Content-Type: application/json" \
  -H "X-Api-Key: YOUR_API_KEY" \
  -d '{
...
curl -X GET http://localhost:8203/v1/agent/decision/uuid-from-proposal \
  -H "X-Api-Key: YOUR_API_KEY"
```
...
curl -X POST http://localhost:8203/v1/agent/health/report \
  -H "Content-Type: application/json" \
  -H "X-Api-Key: YOUR_API_KEY" \
  -d '{
    "agent_id": "agent-123",
...
# Report error
curl -X POST http://localhost:8203/v1/agent/health/error \
  -H "Content-Type: application/json" \
  -H "X-Api-Key: YOUR_API_KEY" \
  -d '{
    "agent_id": "agent-123",
...
curl -X GET "http://localhost:8203/v1/agent/health/recovery-history?agent_id=agent-123" \
  -H "X-Api-Key: YOUR_API_KEY"
```
...
curl -X POST http://localhost:8203/v1/agent/resource/register \
  -H "Content-Type: application/json" \
  -H "X-Api-Key: YOUR_API_KEY" \
  -d '{
    "resource_id": "GPU-A100-001",
...
# Allocate resources
curl -X POST http://localhost:8203/v1/agent/resource/allocate \
  -H "Content-Type: application/json" \
  -H "X-Api-Key: YOUR_API_KEY" \
  -d '{
    "resource_type": "gpu",
...
# Release resources
curl -X POST http://localhost:8203/v1/agent/resource/release \
  -H "Content-Type: application/json" \
  -H "X-Api-Key: YOUR_API_KEY" \
  -d '{
...
# Adjust pricing
curl -X POST http://localhost:8203/v1/agent/resource/pricing/adjust \
  -H "Content-Type: application/json" \
  -H "X-Api-Key: YOUR_API_KEY" \
  -d '{
    "resource_type": "gpu"
...
# View resource pools
curl -X GET http://localhost:8203/v1/agent/resource/pools \
  -H "X-Api-Key: YOUR_API_KEY"
```
