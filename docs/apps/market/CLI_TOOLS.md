# AITBC CLI Market Tools

## Overview

The `aitbc` CLI (0.10.18) exposes the market through the top-level **`aitbc market`** group: GPU/software offers published by shop miners and backed by the coordinator, paid job execution with metered on-chain escrow, IPFS hosting rentals, protected fixed-duration GPU rentals, ratings, and the ETH↔AIT exchange bridge.

> **There is no `aitbc market` command group.** The old global chain-listings group has been removed from the CLI; `aitbc market` is the only market surface. The related `aitbc operations …` group still exists but is deprecated and hidden from `aitbc --help`.
...
## Market Command Group (`aitbc market`)
...
```bash
# List all market offers (optionally filtered / sorted)
aitbc market list
...
It was a stub until 2026-09-15, printing "GPU provider query via P2P network to be implemented" and pointing at `aitbc gpu list-gpus`, which lists locally registered GPUs rather than market sellers.
...
Escrow is created automatically when a paid market job is funded (`market run`, `market host`, `market gpu buy`); these commands inspect or settle it manually.
...
market run / host / gpu buy
      │
      ├─→ market tx + POST /rpc/escrow/create
      │          │
      │          ▼
...
# Sync ratings to/from a remote market node (URL via --remote-url or AITBC_MARKET_URL)
aitbc market sync-ratings --remote-url https://<remote-host>
```

## ETH↔AIT Exchange (`aitbc market exchange`)

Bridge operations for market payments:
...
- `aitbc http call market <path>` — raw market service (port 8102) API for anything the CLI does not wrap.
...
- Market analytics, personal spending/earnings reports, regional filters, and price-trend reports.
...
- The `aitbc market` chain-listings group (`list`/`search`/`buy`/`complete`) — removed from the CLI entirely; the `GlobalChainMarket` core module remains in `cli/aitbc_cli/core/market.py` with no command surface.
