# Market Web - AITBC Documentation

> **Important:** This document describes the Market Web architecture. For authoritative port configuration, see [Service Ports Reference](../reference/SERVICE_PORTS.md).

> **Status: the app described here was never built.** No
> `apps/market-web` directory exists; there is no Vite/React frontend.
> The live market UI is the static site in `website/` (vanilla
> HTML/JS — `market.html`, `explorer.html`, `shop-dashboard.html`,
...
The Market Web is the primary interface for clients to submit AI compute jobs and for miners to offer their services. It provides a real-time trading platform with comprehensive job management and analytics.

### Key Features

- Real-time job market with offer/bid functionality
...
VITE_EXPLORER_URL=http://localhost:8100
