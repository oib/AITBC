...
# apply-workflow.sh
# This script integrates the AITBC SAFe-Agentic-Workflow template into your project.
...
# --- Main Script ---
log_info "Starting AITBC SAFe-Agentic-Workflow integration..."
...
TICKET_PREFIX_ESC=$(printf '%s' "$TICKET_PREFIX" | sed 's/[\\&|]/\\&/g')
PRIMARY_DEV_BRANCH_ESC=$(printf '%s' "$PRIMARY_DEV_BRANCH" | sed 's/[\\&|]/\\&/g')
PROJECT_GIT_URL_ESC=$(printf '%s' "$PROJECT_GIT_URL" | sed 's/[\\&|]/\\&/g')
PROJECT_NAME_ESC=$(printf '%s' "$PROJECT_NAME" | sed 's/[\\&|]/\\&/g')
TICKET_URL_PREFIX_ESC=$(printf '%s' "$TICKET_URL_PREFIX" | sed 's/[\\&|]/\\&/g')

find "$TARGET_DIR" -type f \( -name "*.md" -o -name "*.yml" -o -name "*.sh" -o -name "*.json" \) -print0 | while IFS= read -r -d $'' file;
do
  sed -i.tmp \
    -e "s|__TICKET_PREFIX__|$TICKET_PREFIX_ESC|g" \
    -e "s|__PRIMARY_DEV_BRANCH__|$PRIMARY_DEV_BRANCH_ESC|g" \
    -e "s|__PROJECT_GIT_URL__|$PROJECT_GIT_URL_ESC|g" \
    -e "s|__PROJECT_NAME__|$PROJECT_NAME_ESC|g" \
    -e "s|__TICKET_URL_PREFIX__|$TICKET_URL_PREFIX_ESC|g" \
    "$file"
  rm -f "${file}.tmp"
done
...
log_success "AITBC SAFe-Agentic-Workflow integration complete!"
log_info "Next steps:"
