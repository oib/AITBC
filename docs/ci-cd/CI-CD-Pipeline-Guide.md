...
```bash
# Payment features
/app/api/payments/ @payments-team oib
/lib/stripe/ @payments-team oib

# Authentication
/app/api/auth/ @auth-team oib
/middleware.ts @auth-team oib
...
- AITBC-1X: Payments team
- AITBC-2X: Auth team
- AITBC-3X: Frontend team
...
git checkout -b AITBC-15-stripe-checkout-integration

# Make changes and commit with SAFe format
git commit -m "feat(payments): add Stripe checkout flow [AITBC-15]"
```
...
git push --force-with-lease origin AITBC-15-stripe-checkout-integration
```
...
yarn lint:fix
git add .
git commit -m "style: fix linting issues [AITBC-15]"
```
...
```bash
# Rename branch
git branch -m AITBC-15-correct-format
git push origin -u AITBC-15-correct-format
