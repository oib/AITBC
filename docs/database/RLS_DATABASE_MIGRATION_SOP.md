...
**Impact**: Production incident AITBC-221/AITBC-222 (Sept 2025) - RLS policies not applied, 4+ hour emergency debugging
...
**Impact**: Production incident AITBC-315 (Oct 2025) - Tables created WITHOUT RLS protection
...
-- 4. Create RLS policies for aitbc (superuser)
...
- [ ] **Security Testing**
  ...
  psql -U {{LINEAR_WORKSPACE}}_app_user -d aitbc < scripts/test-new-table-rls.sql
  ```
...
# - Update "Recent Changes" section with AITBC-XXX reference

# 3. Commit both schema and docs together
git add prisma/migrations/ docs/database/DATA_DICTIONARY.md
git commit -m "feat(db): add new table with documentation [AITBC-XXX]"
```
...
psql -U {{LINEAR_WORKSPACE}}_app_user -d aitbc << 'EOF'
...
This SOP is **mandatory** for all database changes in the AITBC application. Non-compliance may result in:
