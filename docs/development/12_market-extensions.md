# Building Market Extensions in AITBC

This tutorial shows how to extend the AITBC market with custom features, plugins, and integrations.

> **⚠️ DEPRECATION NOTICE (v0.4.7)**: Auction functionality has been deprecated. GPU-only market auctions are no longer supported. The market now focuses on hardware+software bundles with fixed pricing. See the "Auction Extensions" section below for details.

## Overview

The AITBC market is designed to be extensible. You can add:
...
- Node.js 16+
- AITBC market source code
- Understanding of React/TypeScript
- API development experience

## Step 1: Create a Custom Auction Type ~~(DEPRECATED)~~

> **⚠️ DEPRECATED (v0.4.7)**: This section describes auction functionality that has been removed. The AITBC market no longer supports GPU auctions. The system now uses hardware+software bundles with fixed pricing. If auction functionality is needed in the future, it should be designed fresh for the software service market model.

The following code is provided for reference only and will not work with the current market architecture.

Let's create a Dutch auction extension:

```typescript
// src/extensions/DutchAuction.ts
import { Auction, Bid, MarketPlugin } from '../types';
...
export class DutchAuction implements MarketPlugin {
...
import { MarketRegistry } from '../core/MarketRegistry';
...
```python
# apps/coordinator-api/src/coordinator_api/routers/market_extensions.py
from fastapi import APIRouter, Depends, HTTPException
from pydantic import BaseModel
from typing import Dict, Any, List
import asyncio

router = APIRouter(prefix="/market/extensions", tags=["market-extensions"])
...
async def create_dutch_auction(request: DutchAuctionRequest) -> Dict[str, str]:
    ...
    auction_id = await market_service.create_extension_auction(
        type="dutch",
        config=request.dict()
    )
...
async def get_dutch_auction_state(auction_id: str) -> DutchAuctionState:
    ...
    auction = await market_service.get_auction(auction_id)
...
async def start_price_decrement(auction_id: str):
    ...
    while True:
        ...
        await market_service.update_auction_price(auction_id, new_price)

        if new_price <= auction.config["reserve_price"]:
            await market_service.close_auction(auction_id)
            break
```
...
class SlackIntegration:
    ...
    async def notify_new_offer(self, offer: Dict[str, Any]) -> None:
        """Send notification about new market offer to Slack."""
        message = {
                    ...
                    "elements": [
                        {
                            ...
                            "url": f"https://hub.example.net/market/offer/{offer['id']}"
                        }
                    ]
                }
            ]
...
**For current market extensions**, focus on testing:
...
# script, and no separate market_extensions.py file. Add the router
...
2. **Backward Compatibility** - Ensure extensions work with existing market
...
This tutorial covered creating market extensions including service categories, advanced search filters, external system integrations, and custom pricing models. Auction functionality has been deprecated in v0.4.7 as the market now focuses on hardware+software bundles with fixed pricing.
...
- Plugin system for market hooks

If auction functionality is needed in the future for software service bundles, it should be designed fresh for the new market microservice architecture.

For more examples and community contributions, visit the market extensions repository.
