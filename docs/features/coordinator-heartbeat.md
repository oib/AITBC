...
- `apps/coordinator-api/src/coordinator_api/contexts/agent_coordination/routers/agent_messaging.py` — Request to register agent *(the coordinator-api `agent_messaging.py` router is a stub — real agent messaging lives on agent-coordinator at `/api/v1/agent/messages/*`)*
...
- `POST /agents/{agent_id}/heartbeat` (`heartbeat` in `apps/coordinator-api/src/coordinator_api/contexts/agent_coordination/routers/agent_messaging.py`) *(the coordinator-api `agent_messaging.py` router is a stub — real agent messaging lives on agent-coordinator at `/api/v1/agent/messages/*`)*
- `GET /health` (`agent_health` in `apps/coordinator-api/src/coordinator_api/contexts/agent_coordination/routers/agent_messaging.py`) *(the coordinator-api `agent_messaging.py` router is a stub — real agent messaging lives on agent-coordinator at `/api/v1/agent/messages/*`)*
...
- **Prerequisites**: Requires [v0.6.5](../releases/v0.6/v0.6.5_change.log) (Agent Coordination — task assignment uses agent coordination), [v0.6.3](../releases/v0.6/v0.6.3_change.log) (Multi...
