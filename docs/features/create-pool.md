...
- `Coordinator API` exposes `POST /v1/governance-enhanced/staking/pools` (operation `create_staking_pool_v1_governance_enhanced_staking_pools_post`) — Create Staking Pool *(mounted but the handler returns 501 — 'disabled until on-chain staking is implemented')*
