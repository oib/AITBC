...
- `Openapi` exposes `POST /v1/global-marketplace-integration/offers/{offer_id}/optimize-pricing` (operation `optimize_offer_pricing_v1_global_marketplace_integration_offers__offer_id__optimize_pricing_post`) — Optimize Offer Pricing *(router not mounted — 404s today)*
...
- `POST /offers/{offer_id}/optimize-pricing` (`optimize_offer_pricing` in `apps/coordinator-api/src/coordinator_api/contexts/marketplace/routers/global_marketplace_integration.py`) *(router defined but not mounted in `main.py` — 404s today)*
