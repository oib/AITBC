...
- `aitbc/market/offer_fsm.py` — from **future** import annotations import logging from enum import StrEnum logger = logging.getLogge...
...
- `Market` exposes `GET /v1/market/offers/{offer_id}` (operation `get_offer_v1_market_offers__offer_id__get`) — Get Offer
- `Market` exposes `GET /v1/market/offers/{offer_id}/history` (operation `get_offer_history_v1_market_offers__offer_id__history_get`) — Get Offer History
...
- `GET /offers/{offer_id}/cross-chain-details` (`get_cross_chain_offer_details` in `apps/coordinator-api/src/coordinator_api/contexts/market/routers/global_market_integration.py`) *(router defined but not mounted in `main.py` — 404s today)*
- `GET /v1/market/offer/{service_id}/ratings` (`get_service_ratings_v1_market_offer__service_id__ratings_get`) on `Market`
- `GET /v1/market/offers/{offer_id}` (`get_offer_v1_market_offers__offer_id__get`) on `Market`
- `GET /v1/market/offers/{offer_id}/history` (`get_offer_history_v1_market_offers__offer_id__history_get`) on `Market`
