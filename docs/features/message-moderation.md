...
- `POST /api/v1/agent/messages/send` (`send_message` in `apps/agent-coordinator/src/agent_app/routers/messages.py`) — the coordinator-api `agent_messaging.py` router is a stub that registers no routes
- `GET /api/v1/agent/messages/inbox` and `/history` (`apps/agent-coordinator/src/agent_app/routers/messages.py`) — the coordinator-api `agent_messaging.py` router is a stub; there is no broadcast or mark-read endpoint
