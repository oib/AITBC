...
The AITBC project uses a SAFe Agile Release Train (ART) agent team powered by Claude Code.
This guide explains how to work with the 17 specialized AI agents that support development workflows.

## Quick Start

### For Human Developers

1. **Understanding the Team**: We have 17 specialized agents, each with specific expertise
...
4. **HITL Model**: oib (POPM) has final approval on all deliverables

### For AI Agents

1. **Branch/commit/PR conventions** - invoke the `safe-workflow` skill (loads on demand); `CONTRIBUTING.md` is the reference, not a mandatory read
...
├── aitbc-228-planning.md           # Planning doc example
├── spec-aitbc-135-posthog-analytics-safe.md  # Implementation spec example
...
# 3. Review documentation (branch/commit/PR conventions: invoke the safe-workflow skill on demand)
...
- [SAFe ART Agent Team: Evidence-Based Delivery Model](https://oib.atlassian.net/wiki/spaces/WA/pages/365264898) - Complete agent team structure
- [Coding Standards & Pattern Discovery](https://oib.atlassian.net/wiki/spaces/WA/pages/365494275) - Standards and discovery protocol
...
2. **Branch/commit/PR conventions**: invoke the `safe-workflow` skill (loads on demand); `CONTRIBUTING.md` is the reference, not a mandatory read
