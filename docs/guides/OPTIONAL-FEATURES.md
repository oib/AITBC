...
- [ ] Delete `harness/claude/hooks/pre-bash-rls-validation.sh`
- [ ] Delete `agent_providers/claude_code/hooks/pre-bash-rls-validation.sh`
- [ ] Delete `.claude/hooks/pre-bash-rls-validation.sh`
- [ ] Remove the `pre-bash-rls-validation.sh` hook entry (PreToolUse / `Bash`)
      from `.claude/settings.template.json` and `.claude/hooks-config.json`
      (and their `harness/claude/` sources)
...
ls harness/claude/hooks/pre-bash-rls-validation.sh 2>/dev/null \
...
4. Start a session: `./dark-factory/scripts/factory-start.sh feature AITBC-123`
