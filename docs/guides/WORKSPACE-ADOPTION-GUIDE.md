...
### Step 1: Run the Adoption Analyzer

Before touching the target repository, run the read-only adoption analyzer
from a boilerplate checkout against the target path:

```bash
bash scripts/adopt-analyze.sh /path/to/your-project --out adoption-report.md
```

`scripts/adopt-analyze.sh` is **strictly read-only** — it never writes
inside the target repo path (only to `--out`, defaulting to
`adoption-report.md` in the current directory) and never executes any
migration step. It produces `adoption-report.md` with:

- **Detected stack** — package.json/pyproject/requirements/go.mod/Gemfile/
  pom/gradle, Dockerfile, docker-compose, with versions where cheap
- **Existing CI** — `.github/workflows`, `bitbucket-pipelines.yml`,
  `.gitlab-ci.yml`
- **Tracker references** — first Jira/Linear/GitHub-Issues URL found in
  README/docs
- **Harness conflicts** — pre-existing `.claude/`, `.agents/`, `.gemini/`,
  `.cursor/`, `AGENTS.md`, `CLAUDE.md`, or custom git hooks that will
  conflict with boilerplate files of the same name
- **Capability mapping suggestion** (blueprint §19) — detected candidate
  provider per capability, or "unresolved"
- **Migration plan skeleton** — the staged-PR list (harness files, CI,
  tracker adapter, docs) with per-stage conflict notes, explicitly marked
  as **requiring human approval before execution (ADR-A-0004)**
...
### Step 2: Add the Template as a Remote
...
ls .claude/agents/ | wc -l  # 17 agents
ls .claude/skills/ | wc -l  # 21 skills
...
```
oib/
├── AITBC/             # Main application repo (PRIMARY)
│   ├── .claude/                  # Full harness
│   ├── dark-factory/             # Agent teams run here
│   └── ...
│
└── AITBC-packages/    # Shared packages repo (SECONDARY)
...
cp -r ../AITBC/.claude/ .claude/
cp -r ../AITBC/.gemini/ .gemini/
cp -r ../AITBC/.codex/ .codex/
cp -r ../AITBC/.cursor/ .cursor/
cp -r ../AITBC/.agents/ .agents/
cp ../AITBC/CLAUDE.md .
cp ../AITBC/AGENTS.md .

# Adjust project-specific values
sed -i "s/AITBC/AITBC-packages/g" CLAUDE.md
```
...
2. **Multiple sessions** — Run separate dark factory sessions for each repo:
   ...
   cd AITBC-packages
   ./dark-factory/scripts/factory-start.sh story AITBC-11
   ```
...
```bash
FACTORY_PROJECT_DIR="/home/{{REMOTE_USER}}/AITBC"
FACTORY_MAIN_BRANCH="main"
FACTORY_TICKET_PREFIX="AITBC"
...
cd /home/{{REMOTE_USER}}
git clone git@github.com:oib/AITBC-packages.git

# Start a separate factory session pointing to packages
cd AITBC-packages
# Temporarily override FACTORY_PROJECT_DIR
FACTORY_PROJECT_DIR="$(pwd)" ./dark-factory/scripts/factory-start.sh story AITBC-11
```
