...
class NemotronCloudClient:
    ...
    def discover_offers(self) -> Dict[str, Any]:
        """Discover available market offers with retry logic"""
        for attempt in range(self.max_retries):
            try:
                response = self.session.get(f"{self.base_url}/api/v1/market/offer", timeout=10)
...
class NemotronCloudClient:
    ...
    def discover_offers(self):
        """Discover available market offers"""
        response = requests.get(f"{self.base_url}/api/v1/market/offer")
        return response.json()
...
class NemotronAgent:
    ...
    async def handle_message(self, message):
        ...
        if self.requires_llm_reasoning(message):
            response = await self.client.run_offer(  # AgentClient/run_market_offer do not exist;
