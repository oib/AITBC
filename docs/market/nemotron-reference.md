...
1. **API Gateway Issues**
   ...
   curl -s https://shop.example.net/api/v1/market/offer | jq '.offers[0].plugin_id'
...
3. **Offer Not Available**
   ...
   curl -s https://shop.example.net/api/v1/market/offer/ollama-nemotron-3-super-cloud | jq '.status'
...
6. **CLI Limitations**
   ...
   ```bash
   # aitbc market run queries blockchain, not market service
   # Use API Gateway calls instead:
   curl -s https://shop.example.net/api/v1/market/offer
   ```
...
7. **Service Startup Issues (Fixed 2026-06-05)**
   ...
   # Fixed: Removed deprecated MarketBidRequest imports (v0.4.7 deprecation)
   ...
   # If Market Service has database errors:
   systemctl status aitbc-market.service
   ...
   # "Cannot import name 'MarketBidRequest'" → Removed in v0.4.7 deprecation
   ...
   # "Unit aitbc-market.service not found" → Symlink recreated
   ```

### Error Messages

- **"Offer not found"**: Check offer ID and market status
...
- **"Cannot import name 'MarketBidRequest'"**: Fixed - removed deprecated imports
...
- **Market**: Check `<node2>` status and network connectivity
