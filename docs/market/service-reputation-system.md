...
The AITBC Service Reputation System enables customers to rate and review software services offered through the market. Ratings are automatically aggregated to calculate average scores and counts, and can be synchronized across multiple nodes to ensure consistency across the distributed network.
...
The reputation system supports cross-node rating synchronization between the hub's local market and the `<node2>` software service registry.

**Sync Flow:**

1. Rating created on hub with `source_node="local"` and `synced_at=NULL`
2. CLI command fetches unsynced ratings via `/v1/market/ratings/unsynced`
3. Ratings pushed to remote via `/v1/market/ratings/sync`
4. Remote applies conflict resolution (most recent `created_at` wins)
5. Hub marks ratings as synced via `/v1/market/ratings/mark-synced`
...
Ratings are displayed in the market listings:
...
2. Verify ratings are unsynced: `curl http://localhost:8102/v1/market/ratings/unsynced`
3. Check sync logs: `journalctl -u aitbc-market -f`

### Average Rating Not Updating

1. Verify rating was submitted successfully
2. Check database for rating record
3. Restart market service: `systemctl restart aitbc-market`
