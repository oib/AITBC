...
1. Install the 17 SAFe multi-agent agents
...
The SAFe multi-agent methodology uses **17 specialized AI agents** that act like team members. Each agent has:
...
```bash
git clone https://github.com/oib/AITBC
cd AITBC
```
...
```bash
# Verify the pattern library and agents are present
find patterns_library -name '*.md' | wc -l   # available patterns
ls .claude/agents/*.md | wc -l                # should be 11
```

Expected output: a non-zero pattern count and `17` agent files. The session
start banner (available commands + reminders) is emitted automatically by the
`SessionStart` hook wired in `.claude/settings.template.json`.
...
Task({
  ...
  prompt:
    "Create comprehensive spec for AITBC-123 user profile feature. Include pattern discovery, acceptance criteria, and testing strategy. Reference existing patterns from patterns_library/.",
});
```
...
```
@tdm I need to implement AITBC-123 (user profile feature).
...
# 3. Check pattern library is present
find patterns_library -name '*.md' | wc -l
# Should output a non-zero count
...
- **AGENTS.md**: Quick reference for all 17 agents
...
- **Confluence Cheat Sheet**: https://oib.atlassian.net/wiki/spaces/WA/pages/366411778

---

## Success!

**Congratulations!** 🎉 You've successfully set up the SAFe multi-agent 17-agent system.
