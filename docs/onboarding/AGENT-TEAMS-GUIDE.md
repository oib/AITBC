...
```
I need to implement AITBC-42 (User Profile Management).
The spec is at specs/AITBC-42-user-profile-spec.md.
...
```
I need a thorough review of the changes on branch AITBC-99-payment-flow.
...
Each of the 17 agent roles can be spawned as a teammate. Use the agent prompt files in `.claude/agents/` as the system prompt for each teammate. Below are the recommended configurations.
...
- For Stories where the implementer can handle the PR (RTE role collapsed per AITBC-499), a team of 2 (implementer + QAS) is often sufficient.
...
if [ "$TEAMMATE_ROLE" = "qas" ]; then
  ...
  if [ ! -f "test-results/AITBC-latest.json" ]; then
...
./dark-factory/scripts/factory-start.sh feature AITBC-42
...
Dark Factory enforces merge queue policy as a hard requirement. Agents create PRs via `gh pr create` and enqueue them via `gh pr merge --auto --squash`. This is a **bounded exception** to [ADR-A-0005](../../adrs/agentic/ADR-A-0005-mandatory-prs.md) (POPM-approved 2026-07-07), scoped to GitHub-hosted consumer projects and story PRs into an epic integration branch only — consistent with [ADR-A-0014](../../adrs/agentic/ADR-A-0014-workflow-v3-per-epic-merge-gate.md). The epic→main PR keeps its human merge gate ([ADR-A-0004](../../adrs/agentic/ADR-A-0004-human-approval-boundaries.md)); agents never squash- or auto-merge it. This governance/boilerplate repo itself follows the `CONTRIBUTING.md` rebase-merge + human-gate flow, not the queue. See [Merge Queue Policy](../../dark-factory/docs/MERGE-QUEUE-POLICY.md).
