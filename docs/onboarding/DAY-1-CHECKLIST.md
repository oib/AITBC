...
1. Visit: https://gitingest.com/oib/AITBC
...
- [ ] Can see 17 agent files (bsa.md, system-architect.md, etc.)
...
2. Create new issue
3. Title: `AITBC-1: Add Hello World endpoint for agent workflow validation`
...
- [ ] Ticket has proper format (AITBC-1)
...
```
I'm implementing Linear ticket AITBC-1: Add Hello World endpoint.
...
```bash
git checkout -b AITBC-1-hello-world-endpoint
git add .
git commit -m "feat(api): add Hello World endpoint [AITBC-1]
...
Closes AITBC-1"
git push origin AITBC-1-hello-world-endpoint
```
...
# Using GitHub CLI
gh pr create \
  --title "feat(api): add Hello World endpoint [AITBC-1]" \
  --body "Implements AITBC-1
...
## Linear Ticket
https://linear.app/your-team/issue/AITBC-1"
```
...
- [ ] 17 agents installed and accessible
...
2. **Explore Agent Roles**:
   - [ ] Read all 17 agent prompts in `.claude/agents/`
...
```bash
FACTORY_PROJECT_DIR="/path/to/AITBC"
...
./dark-factory/scripts/factory-start.sh story AITBC-TEST
