...
- **Operational-hygiene gap addressed:** the current `VALIDATOR_SET` keypair was written into `/etc/aitbc/node.env`/`blockchain.env` by hand during same-day incident recovery and still exists nowhere in git history. Per-node backups of `/etc/aitbc` (including `node.env` and the rotated validator keys) are now stored under `/var/lib/aitbc/secrets/etc-aitbc-<timestamp>.tar.gz` with `0600` permissions on hub, aitbc1, and aitbc3. If the live key is lost, the backup on any of the three nodes can be used to restore it.
