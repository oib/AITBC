...
- **Operational-hygiene gap, flagged not fixed:** the current `VALIDATOR_SET` keypair was written into `/etc/aitbc/node.env`/`blockchain.env` by hand during same-day incident recovery (see below) and exists nowhere in git history. If it is lost without a separate backup, hub cannot resume solo block production without another manual intervention. This is an explicit ops-hygiene TODO, not something this pass fixed.
