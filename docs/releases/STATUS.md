...
| G8 — doc debt | **CLOSED** | The `--show-deprecated` visibility gate is removed: `cli/aitbc_cli/core/validated_group.py` and `surface_policy.py` are deleted, `main.py` no longer filters the help surface, and canonical top-level groups are visible. `aitbc market` and `aitbc governance` are canonical; `aitbc operations` (including `operations governance`) is hidden and marked deprecated but still invocable, while the legacy `aitbc marketplace` group has been removed entirely. `test_cli_surface.py` covers the visibility and deprecation behavior. |
