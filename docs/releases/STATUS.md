...
| Bridge — multi-sig/Merkle enforcement | **ENABLED, VALIDATORS REGISTERED** | `BRIDGE_RELEASE_ENABLED=true` is live on `hub.aitbc`, `aitbc3`, and `aitbc1`. `aitbc bridge security-status` reports `release_enabled: true`, `multisig_enabled: true`, `require_merkle_proof: true`, `validators_configured: 2`, `validator_count: 2`, and `threshold: 2` on all three nodes. Two bridge validators are registered: the hub proposer and a fresh bridge-only key. No bridge transfers are pending after the 2026-08-27 reset; `bridge/health` shows `pending_transfer_count: 0` on all three nodes. The bridge admin key remains the exposed `PROPOSER_KEY`; T1 key rotation is still recommended when authorized. |
