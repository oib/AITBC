...
**Release Theme**: Monolithic router/module decomposition and cross-chain
settlement config wiring.

**Goal**: Split the largest monolithic routers and modules into focused,
per-domain units while preserving public APIs, wire settlement block times to
per-chain config, and keep all tests passing.

> **Scope**: Decompose `sync.py`/`bridge.py` in the blockchain node,
> `main.py` in the trading service, and `developer_platform.py` in
> coordinator-api; wire cross-chain settlement block times to
> `settings.chain_configs`; add a production DEBUG guard.
...
cd /opt/aitbc && ./venv/bin/python -m mypy --show-error-codes \
  ...
  apps/blockchain-node/src/aitbc_chain/cross_chain/settlement.py \
  apps/blockchain-node/src/aitbc_chain/contracts/htlc_contract.py
...
- **Agent B** wired cross-chain settlement block times to per-chain config in `apps/blockchain-node/src/aitbc_chain/cross_chain/settlement.py` and `apps/blockchain-node/src/aitbc_chain/contracts/htlc_contract.py`.
...
- Settlement block times are read from `settings.chain_configs` keyed by `chain_id`; `block_time_seconds` is parsed via `aitbc.utils.chain_config.ChainConfigParser`. Missing configs fall back to `settings.block_time_seconds`.
