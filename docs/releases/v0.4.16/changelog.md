...
- **[Cache Consolidation](CACHE_CONSOLIDATION.md)** - Unified cache architecture, pluggable backends, deprecation warnings
- **[HTTP Client Consolidation](HTTP_CLIENT_CONSOLIDATION.md)** - Unified HTTP client architecture, pluggable backends
- **[Monolithic Files Breakdown](MONOLITHIC_FILES_BREAKDOWN.md)** - Split 7 monolithic files into 47 focused modules
- **[Security Fixes](SECURITY_FIXES.md)** - Removed hardcoded secrets, encrypted wallet keys, replaced unsafe pickle, locked down CORS
- **[Operational Fixes](OPERATIONAL_FIXES.md)** - Systemd fixes, backup script authentication, logging improvements, keystore permissions
- **[Migration Guide](MIGRATION_GUIDE.md)** - Import migration instructions, deprecation timeline
