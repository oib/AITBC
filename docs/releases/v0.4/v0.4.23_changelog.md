...
- **[Architecture Refactoring](v0.4.23_ARCHITECTURE_REFACTORING.md)** - Split monolithic aitbc/**init**.py into submodules (logging, config)
- **[Logging Standardization](v0.4.23_LOGGING_STANDARDIZATION.md)** - Migrate all 12+ services to aitbc_logging with INFO level
- **[Observability Enhancement](v0.4.23_OBSERVABILITY_ENHANCEMENT.md)** - Add X-Request-ID propagation for distributed tracing
- **[CI/CD Improvements](v0.4.23_CI_CD_IMPROVEMENTS.md)** - Integration test matrix, coverage gates (50% → 70% → 85%)
- **Wrapper Script Templating** - Generate service wrappers from Jinja2 template
- **[Security Hardening](v0.4.23_SECURITY_HARDENING.md)** - Remove hardcoded ports, configurable via env vars
- **[Documentation Validation](v0.4.23_DOCUMENTATION_VALIDATION.md)** - Validate MASTER_INDEX.md links, publish OpenAPI specs
- **[B008 Lint Refactor](v0.4.23_B008_LINT_REFACTOR.md)** - Fix 1,105 B008 violations via LibCST transformer
- **[OpenTelemetry Collector](v0.4.23_OPENTELEMETRY_COLLECTOR.md)** - Deploy OTLP gRPC (4317), HTTP (4318), health (13133), Prometheus (8889)
- **[OpenAPI Spec Generation](v0.4.23_OPENAPI_SPEC_GENERATION.md)** - Generate OpenAPI specs for all 4 services
- **[Integration Test DB Setup](v0.4.23_INTEGRATION_TEST_DB_SETUP.md)** - SQLite in-memory fixtures for database-backed tests
