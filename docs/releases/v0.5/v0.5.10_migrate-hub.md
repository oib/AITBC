...
**Node**: `hub.example.net` (PoA authority)
...
- **[Overview](./v0.5.10_overview.md)** - Migration overview and documentation structure
- **[Pre-flight Checks](./v0.5.10_pre-flight-checks.md)** - Pre-migration verification (P1-P7)
- **[Migration Steps](./v0.5.10_migration-steps.md)** - Step-by-step hub migration procedure (Step 1-11)
...
- [Step 5. Verify migration results](./v0.5.10_migration-steps.md#step-5-verify-migration-results)
- [Step 6. Verify genesis.json was scaled](./v0.5.10_migration-steps.md#step-6-verify-genesisjson-was-scaled)
- [Step 7. Flush Redis cache](./v0.5.10_migration-steps.md#step-7-flush-redis-cache)
- [Step 8. Restart services](./v0.5.10_migration-steps.md#step-8-restart-services)
- [Step 9. Post-migration verification](./v0.5.10_migration-steps.md#step-9-post-migration-verification)
- [Step 10. Test a transaction](./v0.5.10_migration-steps.md#step-10-test-a-transaction-optional-but-recommended)
- [Step 11. Announce migration complete](./v0.5.10_migration-steps.md#step-11-announce-migration-complete)
...
# sed -i 's|default_peer_rpc_url=http://127.0.0.1:8202|default_peer_rpc_url=https://hub.example.net|' /etc/aitbc/blockchain.env

# Expected: default_peer_rpc_url=https://hub.example.net
```
...
HUB_HEIGHT=$(curl -s https://hub.example.net/rpc/head | python3 -c "import sys,json; print(json.load(sys.stdin).get('height',0))")
...
HUB_ROOT=$(curl -s "https://hub.example.net/rpc/state/snapshot?chain_id=ait-hub.aitbc.bubuit.net" | python3 -c "import sys,json; print(json.load(sys.stdin).get('state_root',''))")
...
Check `default_peer_rpc_url` in `/etc/aitbc/blockchain.env`. If it points to `http://127.0.0.1:8202`, the node is syncing from itself. Fix it to `https://hub.example.net`, then wipe chain.db and restart.
...
Verify the hub is reachable: `curl -s https://hub.example.net/rpc/head | python3 -m json.tool`
...
4. **Verify sync completed:**
   ...
   curl -s "https://hub.example.net/rpc/state/snapshot?chain_id=ait-hub.aitbc.bubuit.net" | python3 -c "import sys,json; print(json.load(sys.stdin).get('state_root'))"
   # Must match
   ```
...
**Hub migration** (performed on `hub.example.net`):
...
**Follower migration** (performed on `<node2>` / shop node):

- Initial attempt with just Redis flush + restart failed — node synced from itself (`default_peer_rpc_url=http://127.0.0.1:8202`)
- Fixed `default_peer_rpc_url` to `https://hub.example.net` in `/etc/aitbc/blockchain.env`
...
| Node | File | Change |
|------|------|--------|
| `<node2>` (follower) | `/etc/aitbc/blockchain.env` | `default_peer_rpc_url`: `http://127.0.0.1:8202` → `https://hub.example.net` |
...
| Follower (`<node2>`) | `/var/lib/aitbc/data/ait-hub.aitbc.bubuit.net/chain.db.pre-fork.*` | `20260623_161722` |
