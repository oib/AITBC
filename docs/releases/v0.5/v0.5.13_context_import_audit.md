...
The list below originally read "14 models" but contained 17 names — and 5 of those were **false orphans** (they *are* imported, via patterns this audit's grep missed: 4-dot relative `....domain.X` and name-imports `from ..domain import X` through `app/domain/__init__.py`). These were audited in full in <ref_file file="/opt/aitbc/docs/releases/v0.5/v0.5.13_orphan_models_audit.md" />.
...
**Recommendation**: See <ref_file file="/opt/aitbc/docs/releases/v0.5/v0.5.13_orphan_models_audit.md" /> for migration destinations and the dead-code deletion plan for `pricing_models.py`.
