...
| **CLI — chain sync-status** | `cli/aitbc_cli/commands/chain.py` | ❌ Does NOT exist | Add `chain sync-status` subcommand |
| **CLI — island health** | `cli/aitbc_cli/commands/node.py` | ❌ Does NOT exist | Add `node island health` subcommand |
| **CLI — island list** | `cli/aitbc_cli/commands/node.py:605-625` | EXISTS but is a STUB with hardcoded data | Replace stub with real island manager query |
...
|-------|--------|-------|-------|
| **Agent A** | Shared core (`aitbc/`) | 3 items | `aitbc/sync/`, `aitbc/network/`, `tests/unit/` |
...
| A3 | Unit tests for A1-A2 + verify mypy/ruff/pytest clean | High | `tests/unit/test_sync_source_resolver.py`, `tests/unit/test_island_registry.py` | ⬜ |
...
| # | Task | Priority | Files | Status |
|---|------|----------|-------|--------|
| B1 | Add config: `chain_sync_sources`, `island_registry`, `gossip_backends`, `island_tasks_enabled` | 🔴 P0 | `config.py` | ⬜ |
| B2 | Wire up `SyncSourceResolver` in `main.py` — resolve hub URL per chain | 🔴 P0 | `main.py` | ⬜ |
| B3 | Multi-hub subscription in `main.py` — one SubscriptionClient per (chain_id, hub_url) pair | 🔴 P0 | `main.py` | ⬜ |
| B4 | Enable island manager background tasks + auto-join islands from config | High | `main.py` | ⬜ |
| B5 | Add `chain sync-status` CLI command — per-chain sync status | Medium | `cli/aitbc_cli/commands/chain.py` | ⬜ |
| B6 | Add `node island health` CLI command + fix `node island list` stub | Medium | `cli/aitbc_cli/commands/node.py` | ⬜ |
...
Modify the subscription client setup (currently lines 324-334):
...
1. Add `health` subcommand to the island group in `cli/aitbc_cli/commands/node.py`:
   ```python
   @island.command()
...
2. Fix `list_islands` (line 605-625) — replace hardcoded stub with real query:
...
| `cli/aitbc_cli/commands/chain.py` | Agent B | B5: sync-status command |
| `cli/aitbc_cli/commands/node.py` | Agent B | B6: island health + list fix |
...
Phase 2 (Agent B — after A1-A2 are merged):
  B1 config fields                    (independent of A)
...
**B1, B5, B6 can start immediately** (no Agent A dependency). B2-B4 depend on A1-A2. B7-B8 depend on B2-B4.
