# Market Offers and Price Discovery
...
breadcrumb: Home > Scenarios > Market Offers and Price Discovery

---

> **Renamed.** This file was *Market Bidding* and drove the `aitbc market`
> group (`list`/`search`/`buy`/`complete`) against a chain-for-sale market. That
...
- **Feature Documentation**: [Market CLI Tools](../apps/market/CLI_TOOLS.md)

---

## Scenario Overview

This scenario covers the buyer's half of the AITBC market: how offers reach the
...
- A funded wallet (market prices are quoted in **AIT**, not ETH)
- Network reach to the hub, which fronts the market service

### Setup Required

- A configured `hub_discovery_url`; `aitbc market list` queries `https://<hub>/v1/market/offer`
...
falls back to `N.N (K reviews)` from the market service otherwise.
...
> Until 2026-09-15 this view also listed settled jobs. `GPU_MARKET` carries
...
| <miner-id>          | <miner-id>          | gpu_market                       |        1 |        1 | unrated         |
| <miner-id>          | <miner-id>          | gpu_market                       |        5 |        5 | unrated         |
+---------------------+---------------------+---------------------------------------+----------+----------+-----------------+
Total: 3 provider(s), 11 offer(s)
```

The `GPU` and `Endpoint` columns are omitted above for width; the command prints them.
`Rating` prefers the coordinator trust score and falls back to the market star
...
`aitbc gpu list`, which lists locally registered GPUs rather than market sellers.
...
- Explain why the market has asks and caps but no bids
...
- Removed: `cli/aitbc_cli/commands/market_cmd.py` and the `aitbc market` group
- Vestigial: `cli/aitbc_cli/core/market.py` (`GlobalChainMarket`, `ChainType`) — still present, wired to nothing
- [Whisper offer walkthrough](../apps/market/HOWTO_WHISPER_OFFER.md)
- [Next Scenario: GPU Listing](./09_gpu_listing.md)
