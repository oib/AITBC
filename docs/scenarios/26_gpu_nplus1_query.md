...
breadcrumb: Home > Scenarios > GPU Market N+1 Query Fix
...
Listing market orders used to `session.get()` each GPU (N+1). B14 batch-fetches with `WHERE id IN (...)`. Operators hit that code by listing GPU hardware and market offers through `aitbc gpu` and `aitbc market`.
...
- Shop GPU service (8101) and hub market reachable via CLI config
...
### Step 2: Market offers (the list path customers actually use)
...
**Expected output:** offer rows (or an empty table). This is the live shop→hub offer path, not `curl /v1/market/orders`.
...
**Expected output:** a `GPU_MARKET` submission, then the offer visible in `list`. File-ownership issues with island credentials are documented in scenario 34.
...
- Inventory GPUs and market offers entirely through `aitbc`
...
import inspect
from coordinator_api.contexts.market.routers.market_gpu import list_orders
