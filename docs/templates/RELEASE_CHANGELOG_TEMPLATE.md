...
**Source note (ABS-96).** The boilerplate repository authors shipped-harness changes under
`harness/claude/` (the source, mirrored into the live `.claude/` copy). This template's `path:`
fields stay `.claude/…` regardless — they describe the file as a consuming fork receives and syncs
it, not where this repository edited it.
