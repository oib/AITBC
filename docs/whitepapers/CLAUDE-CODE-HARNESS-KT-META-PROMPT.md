# Knowledge Transfer Meta-Prompt: Harness Modernization AITBC-444

## For the ARCHitect of AITBC

**Purpose**: Enable extraction and generalization of AITBC harness improvements for the public repository
**Source Epic**: AITBC-444 (Harness & Skills Modernization)
**Related Tickets**: AITBC-454 (SOP Integration), AITBC-455, AITBC-456
**Date**: 2025-12-20
...
The AITBC team completed a comprehensive harness modernization that:
...
> **Note**: An earlier revision of this guide referenced a "Main Whitepaper"
> (`CLAUDE-CODE-HARNESS-MODERNIZATION-*.md`) as document 1. That whitepaper was
> never part of this repository; the sources below are the canonical set.

### 1. Agent Perspective Addendum (The "Why")
...
### 2. Quick Reference (The "How to Use")
...
- Replace `AITBC-XXX` pattern with `AITBC-XXX`
...
### What's AITBC-Specific (Must Replace)

| Component        | AITBC Value                    | Replace With                       |
| ---------------- | ----------------------------- | ---------------------------------- |
| Ticket prefix    | `AITBC-`                        | `AITBC-` or parameterize |
| Branch pattern   | `AITBC-{number}-{desc}`         | `{{PREFIX}}-{number}-{desc}`         |
| Team name        | `AITBC`                        | `AITBC`                   |
| Slack channels   | `#github-feed`                | `{{SLACK_CHANNEL}}`                  |
| Linear workspace | AITBC workspace                | Generic Linear setup               |
| Docker registry  | `ghcr.io/oib/{{LINEAR_WORKSPACE}}-app` | `{{REGISTRY}}/{{PROJECT}}`             |
| Database         | `aitbc`, `aitbc`       | `{{PROJECT}}_dev`, `{{PROJECT}}_user`  |
...
1. **Create SOP templates** based on AITBC SOPs
...
### Commits on AITBC-454 Branch

```text
fa4e942 docs(harness): integrate SOPs into whitepaper and AGENTS.md [AITBC-454]
c1f19b7 docs(harness): add SAFe Role Expansion Vision to Phase 3 roadmap [AITBC-454]
4d68d72 feat(harness): upgrade SAFe agents to Opus 4.5, add skills integration [AITBC-454]
9b3beec docs(harness): update whitepaper with Phase 0-2 completion [AITBC-454]
```
...
```text
AITBC Repository Structure (Source)
├── .claude/
...
The public repo is "a practical and artistic expression" of AITBC's work.
...
[AITBC](https://github.com/oib/AITBC) repository.
