...
> **Date-scope note.** These whitepapers are historical records. Agent, skill, and command counts (e.g. "11 agents", "18 skills") reflect the harness at each document's time of writing and are intentionally NOT updated in place. For the current authoritative roster and counts, see `AGENTS.md` and the live `.claude/` set.

## Documents

### 1. Claude Code Harness Modernization (Main Whitepaper)

**File**: `CLAUDE-CODE-HARNESS-MODERNIZATION-AITBC-444.md`
...
These whitepapers document the harness developed during production use on the AITBC project and generalized for adoption by other SAFe/Agentic teams.
