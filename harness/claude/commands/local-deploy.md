...
| ------------------------- | ----------- | ---------------------------- | ---------------------------------------------------------------------------- |
| **Development** (default) | (none)      | `AITBC-dev-app`     | Active development with hot-reload (STANDARD port 3000)                      |
| **Staging**               | `--staging` | `AITBC-staging-app` | Production-like, self-contained, survives git operations (port 3001)         |
| **Both**                  | `--both`    | Both containers              | Run dev and staging simultaneously (AITBC-400/AITBC-401) |
...
docker ps --filter name=AITBC-staging-app --format '{{.Names}}\t{{.Status}}'

# Check for dev container (AITBC-400: updated container name)
docker ps --filter name=AITBC-dev-app --format '{{.Names}}\t{{.Status}}'
```

Get current commit SHA:

```bash
# Try staging first, then dev (AITBC-400: updated container names)
docker inspect AITBC-staging-app 2>/dev/null | grep 'org.opencontainers.image.revision' | cut -d'"' -f4 || \
docker inspect AITBC-dev-app 2>/dev/null | grep 'org.opencontainers.image.revision' | cut -d'"' -f4
```
...
```bash
# For dev mode (STANDARD port 3000, AITBC-401)
docker ps --filter name=AITBC-dev --format 'table {{.Names}}\t{{.Status}}'

# For staging mode (port 3001)
docker ps --filter name=AITBC-staging --format 'table {{.Names}}\t{{.Status}}'
```
...
```bash
# Staging
docker inspect AITBC-staging-app | grep 'org.opencontainers.image.revision' | cut -d'"' -f4

# Dev (AITBC-400: updated container name)
docker inspect AITBC-dev-app | grep 'org.opencontainers.image.revision' | cut -d'"' -f4
```
...
```bash
# Staging (AITBC-400: use container name)
docker logs AITBC-staging-app --tail 50

# Dev (AITBC-400: use container name or script)
docker logs AITBC-dev-app --tail 50
# Or use script
...
Current:  e9722d4 - style(docs): apply markdown linting fixes [AITBC-347]
Status:   Up 7 hours (healthy)

Target:   3a49b85 - feat(ci): add Slack notifications [AITBC-350]
...
Running:  3a49b85 - feat(ci): add Slack notifications [AITBC-350]
Status:   Healthy
...
## Running Both Modes Simultaneously (AITBC-400/AITBC-401)
...
```bash
docker ps --filter name=AITBC --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'
```
