---
name: merge-status
description: One-command PR / CI / merge-drift status checks for the RTE seat — answer "is commit X on main?", "is PR N open or merged?", "is my branch behind the target?" in a SINGLE tool call whose EXIT CODE is the answer, instead of the fetch→log→pr-view→read→interpret ritual that burns the turn ceiling. Use whenever you need to know the state of a PR, a merge, or CI without changing anything; also use to decide whether to keep waiting or hand off.
...
```bash
SKILL=.claude/skills/merge-status/merge-status.sh   # source copy: harness/claude/skills/merge-status/
