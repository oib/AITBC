...
- **BSA** — via the Follow-Up Ticket Decision (`harness/devin/agents/bsa.md`, `docs/sop/FOLLOW_UP_TICKET_SOP.md`) as a Follow-Up Ticket Draft
- **PO-Agent** — via product-backlog decomposition (`harness/devin/agents/po-agent.md`, arrives with ABS-9)
...
Run the `duplicate-detection` skill (`harness/devin/skills/duplicate-detection/SKILL.md`) on the draft. This agent OWNS that skill — it executes as the first step of every enrichment run, before any create/append operation.
...
Run the `issue-enrichment` skill (`harness/devin/skills/issue-enrichment/SKILL.md`):
