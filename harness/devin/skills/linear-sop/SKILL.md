---
name: linear-sop
description: Linear ticket management best practices. Use when creating issues, updating
  status, or attaching evidence. Provides evidence templates for dev/staging/done
  phases.
triggers:
- user
- model
...
mcp__linear-mcp__get_issue({ id: "AITBC-459" })
...
Tickets referenced in commit messages (e.g., `[AITBC-123]`) automatically move to **Done** when the PR merges. Child stories not referenced in any commit message must be manually closed after merge.
...
- Branch name contains `AITBC-XXX`
- PR title contains `[AITBC-XXX]`
