...
description: "Run, launch, drive, or smoke-test this repo's orchestrator (scripts/orchestrator.sh)\
  \ safely in a sandbox \u2014 mock tracker, stub spawn seam, isolated state. Use\
  \ when asked to run the orchestrator, verify an orchestrator change actually works,\
  \ reproduce a dispatch/handoff behavior, or drive a ticket through the status machine\
  \ without touching Jira or a live run."
triggers:
- user
- model
---
...
```bash
harness/devin/skills/run-boilerplate/driver.sh smoke
```
...
```bash
eval "$(harness/devin/skills/run-boilerplate/driver.sh sandbox)"
...
mkdir -p work/scratch && printf 'release\n' > work/scratch/demo-1-reason.md
scripts/mock-tracker.sh transition DEMO-1 "Ready for Development" --actor human-operator --reason-file work/scratch/demo-1-reason.md
...
harness/devin/skills/run-boilerplate/driver.sh clean    # remove sandboxes when done
```
