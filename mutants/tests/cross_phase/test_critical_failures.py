...
class TestEconomicCalculationsDuringValidatorChurn:
    ...
    async def test_concurrent_stake_unstake_operations(self, economic_system_with_churn):
        ...
        # Run concurrently
        await asyncio.gather(stake_operation(), unstake_operation(), stake_operation(), return_exceptions=True)
...
class TestSystemUnderHighLoad:
    ...
    async def test_consensus_under_transaction_flood(self, loaded_system):
        ...
        transactions = []
        for i in range(1000):
            tx = Mock(tx_id=f"tx_{i}", from_address="0xclient", to_address="0xagent", amount=Decimal("1.0"))
        ...
        async def process_tx(tx):
            try:
                await asyncio.wait_for(poa.validate_transaction_async(tx), timeout=1.0)
                return True
            except TimeoutError:
                return False
            except Exception:
    ...
    async def test_network_under_peer_flood(self, loaded_system):
        ...
        for i in range(peer_attempts):
            try:
                result = await asyncio.wait_for(discovery.attempt_peer_connection("127.0.0.1", 8001 + i), timeout=0.1)
                if result:
...
class TestByzantineFaultTolerance:
    ...
    async def test_consensus_with_byzantine_majority(self, byzantine_setup):
        ...
        result = await poa.attempt_consensus(block_hash="test_block", round=1)
