...
This is the single source of truth. ``aitbc.exceptions`` re-exports these same
class objects, so ``aitbc.exceptions.NetworkError is aitbc_errors.NetworkError``
and existing ``except`` handlers keep working.

This package has no dependencies and must keep it that way -- see README.
"""
