...
-- 5. Create RLS policy for superuser (aitbc)
...
DATABASE_URL="postgresql://aitbc:{{DB_PASSWORD}}@localhost:5432/aitbc" \
npx prisma migrate dev --name add_user_preferences_with_rls
...
- 2025-10-03: Created table with RLS policies [AITBC-300]
```
...
# Test user isolation
psql -U {{LINEAR_WORKSPACE}}_app_user -d aitbc \
