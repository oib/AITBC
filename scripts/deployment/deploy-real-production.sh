...
StandardOutput=journal
StandardError=journal
SyslogIdentifier=aitbc-agent-ai

# AI security
NoNewPrivileges=true
ProtectSystem=strict
ProtectHome=true
ReadWritePaths=/opt/aitbc/production/data/agent /opt/aitbc/production/logs/agent
...
# Test agent AI
echo "Testing agent AI..."
python production/services/agent_ai.py > /tmp/agent_test.log 2>&1
if [ $? -eq 0 ]; then
    echo "✅ agent AI test passed"
    head -10 /tmp/agent_test.log
else
    echo "❌ agent AI test failed"
