...
class hermesDAODeployment:
    ...
    async def register_initial_agents(self):
        ...
        for config in agent_configs:
            # Deploy agent wallet
            agent_wallet = await self.deploy_agent_wallet(config["address"], config["role"])
            ...
            tx_hash = self.w3.eth.send_transaction(
                {
                    ...
                    "data": self.dao_contract.functions.registerAgentWallet(
                        agent_wallet.address, config["role"]
                    ).encode_transaction_data(),
                    "gas": 200000,
                    "gasPrice": self.w3.eth.gas_price,
                    "nonce": self.w3.eth.get_transaction_count(self.address),
        ...
        tx_hash = self.w3.eth.send_transaction(
            {
                "from": self.address,
                "data": contract.constructor(
                    owner_address, role, self.dao_contract.address, self.governance_token
                ).encode_transaction_data(),
                "gas": 2000000,
                "gasPrice": self.w3.eth.gas_price,
                "nonce": self.w3.eth.get_transaction_count(self.address),
...
async def main():
    ...
    print("🎉 Deployment complete! Check hermes_dao_deployment.json for details")
