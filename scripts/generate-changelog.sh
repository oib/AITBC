...
# -----------------------------------------------------------------------------
# Category helpers (bash 3.2 compatible -- no associative arrays)
# -----------------------------------------------------------------------------
# The category set is fixed (see classify_file above): NEW_FILE, METHODOLOGY,
# UPDATED_FILE, CONFIG, BREAKING. macOS ships bash 3.2, which has no
# `declare -A`, so category -> label and category -> count lookups use a
# `case` statement and a space-delimited membership string instead of
# associative arrays.

# Display label for a category. Usage: category_label "NEW_FILE" -> "New Files"
category_label() {
    case "$1" in
        NEW_FILE)     echo "New Files" ;;
        METHODOLOGY)  echo "Methodology Changes" ;;
        UPDATED_FILE) echo "Updated Files" ;;
        CONFIG)       echo "Configuration Changes" ;;
        BREAKING)     echo "Breaking Changes" ;;
        *)            echo "$1" ;;
    esac
}

# Build a space-delimited string of the distinct categories present, e.g.
# " NEW_FILE METHODOLOGY " (leading/trailing space so membership checks can
# always match on " $cat " without worrying about start/end-of-string anchors).
# Usage: present="$(build_present_categories "${categories[@]}")"
build_present_categories() {
    local present=" "
    local cat
    for cat in "$@"; do
        case "$present" in
...
# Membership check against the string built by build_present_categories.
# Usage: if category_is_present "NEW_FILE" "$present"; then ...
category_is_present() {
    local cat="$1"
    local present="$2"
    case "$present" in
...
# Count occurrences of a category in the categories array.
# Usage: count=$(category_count "NEW_FILE" "${categories[@]}")
category_count() {
    local cat="$1"
    shift
    local c=0
...
EOF
    ...
    present_categories="$(build_present_categories "${categories[@]}")"
...
EOF
    ...
    for cat in $category_order; do
        category_is_present "$cat" "$present_categories" || continue
...
emit_markdown() {
    ...
    # Category display names come from category_label() (see helpers above)
    ...
    for cat in $category_order; do
        category_is_present "$cat" "$present_categories" || continue
        echo "| $(category_label "$cat") | $(category_count "$cat" "${categories[@]}") |"
    done
    echo ""
    ...
    if category_is_present "BREAKING" "$present_categories"; then
        echo "---"
        echo ""
