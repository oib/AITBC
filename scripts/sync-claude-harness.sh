...
UPSTREAM_REPO="oib/AITBC"
UPSTREAM_BRANCH="main"
# Upstream git host: "github" (default) or "bitbucket". The canonical boilerplate
# upstream is on Bitbucket; a GitHub mirror/fork sets "github". Release discovery
# and fetch switch on this (ABS-145). Override via .harness-sync.json ("upstream_host")
# or the UPSTREAM_HOST env var.
UPSTREAM_HOST="${UPSTREAM_HOST:-github}"
# Optional explicit clone URL for the host-neutral `git ls-remote` release-discovery
# path (needs no API token). Derived from UPSTREAM_REPO + UPSTREAM_HOST when empty.
...
UPSTREAM_PATH=".claude"  # Legacy default; overridden by SYNC_SCOPE when manifest has sync_scope.
                         # ABS-96: the canonical upstream source is now the "harness/claude"
                         # namespace ("harness/.claude" in pre-v2.23.0 tags); ".claude"
                         # remains a valid (legacy) sync domain and is byte-identical to
                         # the harness source at any release tag.

# Multi-domain sync scope (v1.1+)
# Allowed domains — hardcoded allowlist for v2.10.0
# ABS-96: "harness/claude" is the SHIPPED-harness product namespace (source of
# truth going forward). "harness/.claude" stays listed for pre-v2.23.0 tags and
# for consumers whose manifests still declare the dotted namespace. ".claude"
# stays listed for legacy consumers whose manifests declare
# sync_scope: [".claude", ...] — post-move the committed .claude/ at a tag is
# byte-identical to the harness source (and post-Phase-2b it equals
# generate(harness source)@tag, keeping ABS-95 tag-freshness correct).
ALLOWED_DOMAINS=("harness/claude" "harness/.claude" ".claude" ".gemini" ".codex" ".cursor" ".agents" "dark-factory")
...
load_config() {
    if [ -f "$SYNC_CONFIG" ]; then
        UPSTREAM_REPO=$(json_get "$SYNC_CONFIG" "upstream_repo" "oib/AITBC")
        UPSTREAM_BRANCH=$(json_get "$SYNC_CONFIG" "upstream_branch" "main")
        UPSTREAM_HOST=$(json_get "$SYNC_CONFIG" "upstream_host" "$UPSTREAM_HOST")
        UPSTREAM_GIT_URL=$(json_get "$SYNC_CONFIG" "upstream_git_url" "$UPSTREAM_GIT_URL")
    fi
}
...
#     to prevent partial matches (e.g., https://github.com/oib/AITBC before oib)
...
# Output: lines like "rel_path:42: unreplaced token AITBC"
...
# Resolve the upstream clone URL for the host-neutral `git ls-remote` path.
_upstream_clone_url() {
    if [ -n "$UPSTREAM_GIT_URL" ]; then
        echo "$UPSTREAM_GIT_URL"
    elif [ "$UPSTREAM_HOST" = "bitbucket" ]; then
        echo "https://bitbucket.org/$UPSTREAM_REPO.git"
    else
        echo "https://github.com/$UPSTREAM_REPO.git"
    fi
}
...
get_upstream_sha() {
    ...
    if [ "$UPSTREAM_HOST" = "bitbucket" ]; then
        # Host-neutral: no API token needed
        git ls-remote "$(_upstream_clone_url)" "$ref" 2>/dev/null | awk 'NR==1{print $1}'
        return
    fi
    local response
...
get_latest_release() {
    if [ "$UPSTREAM_HOST" = "bitbucket" ]; then
        # Bitbucket: the annotated release tag IS the release record (see release.md).
        # Discover the newest semver tag via host-neutral git ls-remote (no API token).
        git ls-remote --tags --refs "$(_upstream_clone_url)" 2>/dev/null \
            | sed 's#.*refs/tags/##' \
            | node -e "
                let d=''; process.stdin.on('data',c=>d+=c);
                process.stdin.on('end',()=>{
                    const tags=d.split('\n').map(t=>t.trim()).filter(t=>/^v?[0-9]+\.[0-9]+\.[0-9]+\$/.test(t));
                    tags.sort((a,b)=>{const pa=a.replace(/^v/,'').split('.').map(Number);const pb=b.replace(/^v/,'').split('.').map(Number);return pa[0]-pb[0]||pa[1]-pb[1]||pa[2]-pb[2];});
                    console.log(tags.length?tags[tags.length-1]:'');
                });
            "
        return
    fi
...
fetch_upstream() {
    ...
    if [ "$UPSTREAM_HOST" = "bitbucket" ]; then
        tarball_url="https://bitbucket.org/$UPSTREAM_REPO/get/$ref.tar.gz"
    fi
...
# Honor the migration driver's unioned project_owned_exceptions list (ABS-264).
# The migration driver (scripts/migrate-project.sh) exports MIGRATE_EXCEPTIONS —
# newline-separated, repo-root-relative pathspecs = SOURCE ownership.yaml ∪
# TARGET .agentic/upgrade/ownership.local.yaml — when it delegates the .claude
# domain to this sync. A path a consuming project declared there must be SKIPPED
# here too; otherwise the fork budget grades a `.claude/**` exception the
# delegated sync silently overwrites (the report/classifier divergence ABS-259
# otherwise eliminates). Prefix match mirrors the driver's is_exception: an exact
# file OR any path under a directory pathspec.
is_migration_exception() {
    local rel="$1" qualified ex
    [ -n "${MIGRATE_EXCEPTIONS:-}" ] || return 1
    # The sync passes DOMAIN-relative (v1.0 manifest) or root-relative (v1.1)
    # paths; the ownership map is repo-root-relative. Qualify with the current
    # domain unless the path already carries it.
    case "$rel" in
        "${CURRENT_DOMAIN:-}"/*) qualified="$rel" ;;
        *) qualified="${CURRENT_DOMAIN:+${CURRENT_DOMAIN}/}$rel" ;;
    esac
...
is_excluded() {
    ...
    # Migration driver's unioned exception list (ABS-264) — honored regardless of
    # manifest presence, so a declared .claude/** fork is preserved by the sync
    # exactly as the fork budget graded it.
    is_migration_exception "$file" && return 0
...
do_rollback() {
    ...
    # Timestamps are ISO-8601 UTC (YYYY-MM-DDTHH:MM:SSZ), so plain string
    # comparison sorts correctly — avoids GNU-only `date -d` (BSD date has
    # no -d flag, which silently made every timestamp compare as epoch 0).
    local latest_timestamp=""
    while IFS= read -r ts_dir; do
        local ts_name
        ts_name=$(basename "$ts_dir")
        if [[ -z "$latest_timestamp" || "$ts_name" > "$latest_timestamp" ]]; then
...
# Look up the array index of a key in the `dir_rename_keys` parallel array
# (bash-3.2-safe stand-in for an associative array; bash 3.2 has no
# `declare -A` / `local -n`, so do_diff tracks directory-rename counts with
# two indexed arrays, dir_rename_keys[] and dir_rename_vals[], instead).
# Sets DIR_RENAME_IDX to the index if found, or -1 if not found.
# Usage: dir_rename_find_index "src|dst"   (caller's array must be named dir_rename_keys)
dir_rename_find_index() {
    local key="$1"
    local i
    DIR_RENAME_IDX=-1
    for i in "${!dir_rename_keys[@]}"; do
        if [ "${dir_rename_keys[$i]}" = "$key" ]; then
            DIR_RENAME_IDX=$i
            return 0
        fi
    done
...
#
# Runs in a subshell `( ... )` rather than the caller's shell so `set -o
# pipefail` (needed below) is scoped to this function's execution only and
# never leaks out. A RETURN trap was considered instead, but it stays
# installed globally after the function returns and its guard would read a
# `local` variable that no longer exists post-return — a latent footgun for
# any future second call site. do_diff only prints output and returns a
# status (verified: nothing after its sole call site at `do_diff "${2:-}";
# exit $?` depends on variables it sets), so a subshell is safe here and
# needs no manual pipefail restoration at all.
do_diff() (
    # Fail loudly: with plain `set -e`, a failing command in the middle of a
    # pipeline (e.g. `node ... | while read ...`) is masked because only the
    # pipeline's last stage's exit status counts. Turn on pipefail for the
    # duration of this function so an internal failure aborts instead of
    # silently falling through to a "clean" (but wrong) summary line.
    set -o pipefail
    ...
    # bash 3.2 (macOS default) has no associative arrays, so we use two
    # parallel indexed arrays (key -> count) with a linear-scan helper
    # instead of `declare -A`. Domain count is small, so this is cheap.
    ...
    while IFS= read -r -d '' file; do
        ...
        if [ "$rtype" = "directory" ]; then
            ...
            if [ -n "$dir_key" ]; then
                if dir_rename_find_index "$dir_key"; then
                    dir_rename_vals[$DIR_RENAME_IDX]=$(( dir_rename_vals[DIR_RENAME_IDX] + 1 ))
                else
    ...
    if [ ${#dir_rename_keys[@]} -gt 0 ]; then
        echo ""
        local dir_rename_idx
        for dir_rename_idx in "${!dir_rename_keys[@]}"; do
            dir_key="${dir_rename_keys[$dir_rename_idx]}"
            local src_dir="${dir_key%%|*}"
            local dst_dir="${dir_key##*|}"
            local count="${dir_rename_vals[$dir_rename_idx]}"
