#!/bin/bash
# agent Agent Coordination Demo Script
...
echo "agent Agent Coordination Demo"
...
if ! command -v agent &> /dev/null; then
    log_error "agent not found. Please install agent 2026.3.24+"
    exit 1
fi

log_info "agent version: $(agent --version)"
...
agent agent --agent FollowerAgent --session-id $SESSION_ID \
    --message "Response to Coordinator: Ready for distributed task participation" \
    --thinking medium

agent agent --agent AIResourceAgent --session-id $SESSION_ID \
...
agent agent --agent MultiModalAgent --session-id $SESSION_ID \
...
log_info "  .windsurf/workflows/agent_MASTER_INDEX.md"
