...
def deploy_to_container(container_name, genesis_config):
    ...
    subprocess.run(
        ["ssh", container_name, "sudo systemctl stop aitbc-blockchain-node.service aitbc-blockchain-rpc.service"], check=False
    ...
    subprocess.run(["ssh", container_name, "sudo rm -f /var/lib/aitbc/data/chain.db"], check=False)
    ...
    subprocess.run(
        ["ssh", container_name, "sudo systemctl start aitbc-blockchain-node.service aitbc-blockchain-rpc.service"], check=True
    )
    ...
    result = subprocess.run(["ssh", container_name, "curl -s http://localhost:8005/rpc/head"], capture_output=True, text=True)
...
def enable_new_services(container_name):
    ...
    for service in services:
        try:
            subprocess.run(
                ["ssh", container_name, f"sudo systemctl enable {service} && sudo systemctl start {service}"], check=True
            )
...
def verify_features(container_name):
    ...
    result = subprocess.run(
        ["ssh", container_name, 'curl -s http://localhost:8005/rpc/head | jq ".height"'], capture_output=True, text=True
    )
    ...
    result = subprocess.run(
        ["ssh", container_name, 'curl -s -o /dev/null -w "%{http_code}" http://localhost:8016'], capture_output=True, text=True
    )
