...
# Check if already setup as follower
if [ -f "/etc/aitbc/.env" ] && grep -q "enable_block_production=false" /etc/aitbc/.env; then
    if systemctl is-active --quiet aitbc-blockchain-node; then
        echo "⚠️  System appears to be already configured as follower node."
        echo "   - enable_block_production=false in /etc/aitbc/.env"
        echo "   - aitbc-blockchain-node is active"
        echo ""
        read -p "Force re-setup? This will reconfigure follower node. [y/N] " -n 1 -r
        echo
        if [[ ! $REPLY =~ ^[Yy]$ ]]; then
            echo "Skipping follower node setup. System already configured."
            exit 0
        fi
    fi
fi
