...
# AITBC Market Scenario Test
# Complete market workflow: software offer, job execution, escrow, payment
...
echo "=== 🛒 AITBC MARKET SCENARIO TEST ==="
echo "Timestamp: $(date)"
echo ""
...
echo "🎯 MARKET WORKFLOW SCENARIO (Updated for v0.4.x)"
echo "Testing complete market functionality with new CLI"
echo ""
...
echo "Checking market offers..."
aitbc market list

echo ""
echo -e "${GREEN}✅ Market list retrieved${NC}"
...
# Query market list to find the offer by transaction hash
echo "Querying market list to find offer ID..."
sleep 2  # Wait for blockchain to include the transaction
MARKET_LIST=$(aitbc market list 2>&1)
echo "$MARKET_LIST"

# Extract offer_id from market list (parse entire JSON array with capital keys)
...
if [ -z "$WHISPER_TX_HASH" ]; then
    ...
    # Query market list to find the offer by transaction hash
    echo "Querying market list to find Whisper offer ID..."
    sleep 2  # Wait for blockchain to include the transaction
...
echo -e "${GREEN}🎉 MARKET SCENARIO: SUCCESSFUL${NC}"
