---
name: agent-autonomy-testing
description: Test and diagnose Agent agent autonomy features — distributed decisions, self-healing, resource management on AITBC coordinator.
---
...
- `agent_recovery_results` table stays empty (0 rows)
...
- Effect: `GET /v1/agent/resource/pools` returns 500
...
# Decision vote
curl -s -X POST $BASE/v1/agent/decision/vote \
  -H "Content-Type: application/json" -H "X-Api-Key: $KEY" \
...
curl -s -X POST $BASE/v1/agent/health/report \
  -H "Content-Type: application/json" -H "X-Api-Key: $KEY" \
...
curl -s -X POST $BASE/v1/agent/health/error \
  -H "Content-Type: application/json" -H "X-Api-Key: $KEY" \
...
cur = conn.cursor()
for t in ['agent_decisions','agent_votes','agent_health_checks',
          'agent_error_reports','agent_recovery_results',
          'agent_resources','agent_resource_allocations','agent_pricing_adjustments']:
