...
- [ ] **ADR acceptance closeout (only if this story authors/carries an ADR)**: on human
      acceptance, the ADR **file frontmatter** (`status: accepted`, `accepted_by`,
      `accepted_date`) and its `adrs/agentic/README.md` index row (`— **Accepted**`) are
      flipped in the **same acceptance PR** — not a loose follow-up. Acceptance stays
      human-only (ADR-A-0004); `scripts/adr-acceptance-drift.sh` must report no drift. See
      [`docs/sop/ADR_AUTHORING_GUIDE.md`](../docs/sop/ADR_AUTHORING_GUIDE.md) → "ADR Acceptance Closeout".
