...
class MockVisionProcessor:
    ...
    async def _extract_text(self, image_data: bytes) -> dict[str, Any]:
        ...
        text_data = {
            ...
            "confidence": 0.91,
            "language": "en",
            "text_regions": [{"text": "STOP", "bbox": [50, 100, 150, 150]}, {"text": "MAIN ST", "bbox": [200, 100, 350, 150]}],
        }
    ...
    async def process_multi_modal(self, inputs: dict[str, Any]) -> dict[str, Any]:
        ...
        if "image" in inputs:
            results["vision"] = await self.vision_processor.process_image(
                inputs["image"], inputs.get("vision_processing_type", "general")
            )
...
class TestMultiModalIntegration:
    ...
    async def test_full_multi_modal_processing(self):
        ...
        inputs = {"image": self.sample_image, "text": self.sample_text, "sensor_data": self.sample_sensor_data}
    ...
    async def test_cross_modal_insights(self):
        ...
        inputs = {"image": self.sample_image, "text": self.sample_text, "vision_processing_type": "object_detection"}
...
class TestContextIntegration:
    ...
    async def test_basic_context_integration(self):
        ...
        context_data = {"location": "intersection", "time_of_day": "daytime", "weather": "clear"}
    ...
    async def test_location_context(self):
        ...
        context_data = {"location": "intersection", "traffic_flow": "moderate"}
    ...
    async def test_time_context(self):
        ...
        vision_result = await self.vision_processor.process_image(self.sample_image)
        context_data = {"time_of_day": "night", "lighting_conditions": "low"}
    ...
    async def test_context_history_tracking(self):
        ...
        for i in range(3):
            vision_result = await self.vision_processor.process_image(self.sample_image)
            context_data = {"location": f"location_{i}", "timestamp": datetime.now(UTC).isoformat()}
...
class TestVisionIntegration:
    ...
    async def test_end_to_end_vision_pipeline(self):
        ...
        context_data = {"location": "urban_intersection", "time": "daytime", "purpose": "traffic_monitoring"}
