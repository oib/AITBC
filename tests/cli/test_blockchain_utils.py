...
class TestGetChainInfo:
    ...
    def test_get_chain_info_success(self, mock_client_class):
        ...
        mock_client.get.side_effect = [
            {"supported_chains": ["ait-mainnet", "ait-testchain.local"], "proposer_id": "node1"},
        ...
        assert result["supported_chains"] == "ait-mainnet, ait-testchain.local"
        assert result["height"] == 1000
        assert result["hash"] == "0xabc123"
        assert result["tx_count"] == 50

    @patch("aitbc_cli.utils.blockchain.AITBCHTTPClient")
    def test_get_chain_info_empty_chains(self, mock_client_class, monkeypatch):
        ...
        assert result["supported_chains"] == ""
