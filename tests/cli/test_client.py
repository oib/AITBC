...
class TestClientCommands:
    ...
    def test_submit_job_success(self, mock_client_class, runner, mock_config):
        ...
        result = runner.invoke(
            client,
            ["submit", "--type", "inference", "--prompt", "Test prompt", "--model", "test_model"],
    ...
    def test_submit_job_from_file(self, mock_client_class, runner, mock_config, tmp_path):
        ...
        job_data = {"type": "training", "model": "gpt-3", "dataset": "test_data"}
        ...
        # Run command
        result = runner.invoke(client, ["submit", "--file", str(job_file)], obj={"config": mock_config, "output": "json"})
    ...
    def test_status_success(self, mock_client_class, runner, mock_config):
        ...
        mock_response.json.return_value = {"job_id": "test_job_123", "state": "completed", "result": "Test result"}
        mock_client.get.return_value = mock_response

        # Run command
        result = runner.invoke(client, ["status", "test_job_123"], obj={"config": mock_config, "output": "json"})
        ...
        # Verify API call
        mock_client.get.assert_called_once_with("http://test:8000/v1/jobs/test_job_123", headers={"X-Api-Key": "test_key"})
    ...
    def test_cancel_job_success(self, mock_client_class, runner, mock_config):
        ...
        # Run command
        result = runner.invoke(client, ["cancel", "test_job_123"], obj={"config": mock_config, "output": "json"})
        ...
        mock_client.post.assert_called_once_with(
            "http://test:8000/v1/jobs/test_job_123/cancel", headers={"X-Api-Key": "test_key"}
        )
    ...
    def test_blocks_success(self, mock_client_class, runner, mock_config):
        ...
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"items": [{"height": 100, "hash": "0x123"}, {"height": 101, "hash": "0x456"}]}
        mock_client.get.return_value = mock_response

        # Run command
        result = runner.invoke(client, ["blocks", "--limit", "2"], obj={"config": mock_config, "output": "json"})
    ...
    def test_history_with_filters(self, mock_client_class, runner, mock_config):
        ...
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"jobs": [{"id": "job1", "status": "completed"}, {"id": "job2", "status": "failed"}]}
        mock_client.get.return_value = mock_response

        # Run command with filters
        result = runner.invoke(
            client,
            ["history", "--status", "completed", "--type", "inference", "--limit", "10"],
            obj={"config": mock_config, "output": "json"},
        )
    ...
    def test_submit_missing_required_args(self, runner, mock_config):
        ...
        result = runner.invoke(client, ["submit"], obj={"config": mock_config, "output": "json"})
    ...
    def test_pay_command_success(self, mock_client_class, runner, mock_config):
        ...
        result = runner.invoke(
            client,
            ["pay", "job_123", "10.0", "--currency", "AITBC", "--method", "aitbc_token"],
            obj={"config": mock_config, "output": "json"},
        )
    ...
    def test_pay_command_failure(self, mock_client_class, runner, mock_config):
        ...
        result = runner.invoke(client, ["pay", "job_123", "10.0"], obj={"config": mock_config, "output": "json"})
    ...
    def test_payment_status_success(self, mock_client_class, runner, mock_config):
        ...
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"job_id": "job_123", "payment_id": "pay_abc", "status": "escrowed", "amount": 10.0}
        mock_client.get.return_value = mock_response

        result = runner.invoke(client, ["payment-status", "job_123"], obj={"config": mock_config, "output": "json"})
    ...
    def test_payment_status_not_found(self, mock_client_class, runner, mock_config):
        ...
        result = runner.invoke(client, ["payment-status", "job_999"], obj={"config": mock_config, "output": "json"})
    ...
    def test_payment_receipt_success(self, mock_client_class, runner, mock_config):
        ...
        result = runner.invoke(client, ["payment-receipt", "pay_abc"], obj={"config": mock_config, "output": "json"})
    ...
    def test_refund_success(self, mock_client_class, runner, mock_config):
        ...
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"status": "refunded", "payment_id": "pay_abc"}
        mock_client.post.return_value = mock_response

        result = runner.invoke(
            client,
            ["refund", "job_123", "pay_abc", "--reason", "Job timed out"],
            obj={"config": mock_config, "output": "json"},
        )
    ...
    def test_refund_failure(self, mock_client_class, runner, mock_config):
        ...
        result = runner.invoke(
            client, ["refund", "job_123", "pay_abc", "--reason", "Changed mind"], obj={"config": mock_config, "output": "json"}
        )
