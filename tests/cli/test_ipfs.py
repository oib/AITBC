...
def test_upload_uses_daemon_when_available(runner, tmp_path):
    ...
    with patch("aitbc_cli.commands.ipfs._daemon_available", return_value=True), patch(
        "aitbc_cli.commands.ipfs.requests.post"
    ) as mock_post:
...
def test_download_falls_back_when_daemon_down(runner, tmp_path):
    ...
    with patch("aitbc_cli.commands.ipfs._daemon_available", return_value=False), patch(
        "aitbc_cli.commands.ipfs.IPFS_DIR", ipfs_dir
    ):
