...
class TestGetChainInfo:
    ...
    def test_get_chain_info_success(self, mock_client):
        ...
        mock_http_client.get.side_effect = [
            {"supported_chains": ["ait-devnet", "ait-testchain.local"], "proposer_id": "node1"},
    ...
    @patch("aitbc_cli.utils.blockchain.AITBCHTTPClient")
    def test_get_chain_info_no_chains(self, mock_client, monkeypatch):
        ...
        assert result is not None
        assert result["chain_id"] == ""  # Fallback to empty string (no default)
...
class TestGetNetworkStatus:
    ...
    def test_get_network_status_success(self, mock_client):
        ...
        mock_http_client = Mock()
        mock_http_client.get.return_value = {"height": 1000, "hash": "0xabc123", "timestamp": "2024-01-01T00:00:00"}
        mock_client.return_value = mock_http_client
