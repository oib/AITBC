...
class TestWalletCommands:
    ...
    def mock_wallet_http(self):
        ...
        with patch('aitbc_cli.commands.wallet.get_wallet_client') as mock_get_client, \
            ...
            def mock_get(path):
                ...
                if '/transactions' in path:
                    return {"transactions": [{"transaction_id": "tx1", "tx_hash": "0xabc", "sender": "aitbc1sender", "recipient": "aitbc1test", "value": 50.0, "fee": 1.0, "status": "confirmed", "created_at": "2024-01-01T00:00:00"}]}
                if '/staking-info' in path:
                    return {"total_staked": sum(s["amount"] for s in state["stakes"]), "active_stakes": len(state["stakes"]), "stakes": state["stakes"]}
                if '/rewards' in path:
                    return {
                        "staking_active_amount": sum(s["amount"] for s in state["stakes"]),
                        "liquidity_active_amount": sum(s["amount"] for s in state["liquidity"]),
                        "total_staked": sum(s["amount"] for s in state["stakes"]) + sum(s["amount"] for s in state["liquidity"])
                    }
                if '/account/' in path:
                    return {"balance": 1000.0, "nonce": 0}
                if '/rpc/staking/' in path:
                    return {"total_staked": sum(s["amount"] for s in state["stakes"]), "active_stake_count": len(state["stakes"]), "active_stakes": state["stakes"]}
                return {}

            def mock_post(path, json=None):
                data = json or {}
                if '/stake' in path and '/unstake' not in path:
                    stake = {"stake_id": 123, "amount": data.get("amount", 50.0) / 10**18, "remaining_balance": state["balance"] - data.get("amount", 50.0) / 10**18, "locked_until": "2024-12-31"}
                    state["balance"] = stake["remaining_balance"]
                    state["stakes"].append(stake)
                    return stake
                if '/unstake' in path:
                    return {"stake_id": data.get("stake_id", "stake_123"), "principal": 50.0, "rewards": 5.0, "total_returned": 55.0}
                if '/liquidity-stake' in path:
                    liq = {"stake_id": "liq_123", "new_balance": state["balance"] - data.get("amount", 40.0), "amount": data.get("amount", 40.0), "tier": data.get("tier", "bronze"), "apy": 3.0}
                    state["balance"] = liq["new_balance"]
                    state["liquidity"].append(liq)
                    return liq
                if '/liquidity-unstake' in path:
                    return {"stake_id": data.get("stake_id", "liq_123"), "principal": 50.0, "rewards": 2.5, "total_returned": 52.5}
