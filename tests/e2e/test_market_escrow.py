"""
End-to-end tests for the market escrow flow.

This suite exercises the full lifecycle:
  1. Register a market software-service offer.
  2. Discover the offer through the market API.
...
class TestMarketEscrowFlow:
    """Market offer -> purchase -> escrow -> execution -> release."""

    @pytest.mark.asyncio
    async def test_market_escrow_flow(
        self,
    ...
    async def _register_offer(
        ...
        resp = await client.post("/v1/market/offer", json=service)
        ...
        resp = await client.get(f"/v1/market/offer/{plugin_id}")
    ...
    async def _create_job_against_offer(
        ...
        """Create a job bound to a market offer and return the job + quoted price."""
        payload = {
            "offer_id": offer_id,
            "offer_quantity": "1",
            "payload": {
    ...
    async def _submit_result(self, client: httpx.AsyncClient, job_id: str) -> None:
        result = {
            "output": "E2E market escrow test result",
