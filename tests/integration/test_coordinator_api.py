...
# The three `client` fixtures below set DATABASE_URL through `monkeypatch`, not by assigning
# to `os.environ`. They used to assign, and nothing undid it, so the temp path outlived the
# fixture and every suite collected afterwards inherited it. `apps/trading` reads DATABASE_URL
# at import time to build an *async* engine, and a plain `sqlite://` URL leaves it holding
# pysqlite: 58 failures across four trading files, not one of which fails on its own. Same
# shape as the leak V23-73 fixed, one layer down -- so the URL names the async driver too.
