...
# Note: uses wc -l + head -n instead of GNU-only `head -n -3` for BSD/bash 3.2
# portability (macOS ships BSD head, which rejects negative line counts).
make_sourceable() {
    local src="$1"
    local dst="$2"
    local segment total
    segment=$(mktemp)
    sed -n '1,/^case "\${1:-}"/p' "$src" > "$segment"
    total=$(wc -l < "$segment")
    head -n "$((total - 3))" "$segment" > "$dst"
    rm -f "$segment"
}
...
result=$(
    ...
    # Verify results
    # trim with tr -d to strip BSD wc's leading-space padding (matches existing
    # `wc -l | tr -d ' '` convention used elsewhere in this repo)
