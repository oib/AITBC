...
class TestPerformanceLoggingMiddleware:
    ...
    async def test_dispatch_logs_performance_metrics(self):
        ...
        with patch("aitbc.middleware.performance.logger") as mock_logger:
            ...
            # A fast 2xx is logged at debug, not info: this middleware runs on every
            # request of every service, and the levels were split so the routine case stays
            # out of the journal. `info` is no longer used here at all -- 4xx and 5xx and
            # anything over the slow-request threshold go to `warning`.
            mock_logger.debug.assert_called_once()
            mock_logger.info.assert_not_called()
            mock_logger.warning.assert_not_called()
            assert "Request performance" in mock_logger.debug.call_args[0][0]
...
class TestRequestIDMiddleware:
    ...
    async def test_dispatch_logs_request_info(self):
        ...
        with patch("aitbc.middleware.request_id.logger") as mock_logger:
            await middleware.dispatch(request, call_next)
            # Start is info, completion is debug for a 2xx on a non-quiet path. It used to
            # be info for both, which is where the `>= 2` came from.
