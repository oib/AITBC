...
class TestValidateAddress:
    """`validate_address` answers True/False; `validate_address_strict` raises.

    These three were written when there was one function and it raised. The split came
    later and this file never ran, so nothing pointed it out — `tests/` holds 61 modules
    that no `testpaths` entry collects (V23-71). The address literal also predates EIP-55
    being enforced: its checksum is wrong, so the "valid address" case was asserting True
    of something the validator now rejects. Corrected to the checksummed form of the same
    address rather than swapped for a different one.
    """

    def test_valid_address(self):
        assert validate_address("0x5e2D7C7A4F8E9B1C3d5A2e8F4c6b8a0D2e4f6A8C") is True

    def test_bad_checksum_is_not_valid(self):
        assert validate_address("0x5E2D7C7A4F8E9B1c3D5A2E8F4C6B8A0D2E4F6A8C") is False
    ...
    def test_invalid_address(self):
        ...
        with pytest.raises(ValidationError):
            validate_address_strict("invalid")
