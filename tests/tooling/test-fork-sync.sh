...
# Claude Harness - AITBC

This is the AITBC harness directory.
Project: AITBC | Prefix: AITBC | Branch: main
UPSTREAM

    cat > "$upstream_dir/AGENT_OUTPUT_GUIDE.md" <<'UPSTREAM'
# AITBC Agent Output Guide
...
assert_file_contains "$PROJ/.claude/agents/qas.md" "RenderTrust" \
    "AITBC replaced with RenderTrust in qas.md"
assert_file_not_contains "$PROJ/.claude/agents/qas.md" "AITBC" \
    "no unreplaced AITBC tokens in qas.md"

if [ -f "$PROJ/.claude/commands/start-work.md" ]; then
    assert_file_contains "$PROJ/.claude/commands/start-work.md" "REN" \
        "AITBC replaced with REN in start-work.md"
fi
...
if [ -f "$PROJ/.claude/agents/qas.md" ]; then
    assert_file_contains "$PROJ/.claude/agents/qas.md" "ScaleForge" \
        "AITBC replaced with ScaleForge in qas.md"
    assert_file_not_contains "$PROJ/.claude/agents/qas.md" "AITBC" \
        "no unreplaced AITBC tokens in qas.md (keryk-ai)"
fi
...
# =============================================================================
# SECTION 5: Upstream host neutrality (ABS-145)
# =============================================================================
# The canonical boilerplate upstream is on Bitbucket. Release discovery must
# switch on `upstream_host` and use the host-neutral `git ls-remote --tags`
# path for Bitbucket (no API token required).

echo -e "\n${CYAN}=== Section 5: Upstream Host Neutrality (ABS-145) ===${NC}\n"

# --- Test 5.1: status reflects configured Bitbucket host ---
echo -e "${CYAN}--- Test 5.1: upstream_host: bitbucket surfaced in status ---${NC}"

PROJ=$(setup_fork_project "rendertrust" "bb-host-status")
UPSTREAM=$(setup_mock_upstream "$PROJ")
STUBBED=$(create_stubbed_script "$PROJ" "$PROJ/_mock_upstream")

# Point the sync config at a Bitbucket upstream
cat > "$PROJ/.harness-sync.json" <<'EOF'
{
  "upstream_repo": "workspace/boilerplate",
  "upstream_branch": "main",
  "upstream_host": "bitbucket",
  "last_synced_commit": "abc1234",
  "last_synced_version": "v2.6.0",
  "last_synced_at": "2026-03-01T00:00:00Z",
  "sync_history": []
...
output=$("$STUBBED" status 2>&1) || true
assert_contains "$output" "Host:          bitbucket" "status shows Host: bitbucket when configured"

# --- Test 5.2: status defaults to github when host unset ---
echo -e "\n${CYAN}--- Test 5.2: host defaults to github when unset ---${NC}"

PROJ=$(setup_fork_project "rendertrust" "gh-host-default")
UPSTREAM=$(setup_mock_upstream "$PROJ")
STUBBED=$(create_stubbed_script "$PROJ" "$PROJ/_mock_upstream")
# setup_fork_project writes a config WITHOUT upstream_host -> default github

output=$("$STUBBED" status 2>&1) || true
assert_contains "$output" "Host:          github" "status defaults to Host: github when unset"

# --- Test 5.3: Bitbucket release discovery via host-neutral git ls-remote ---
echo -e "\n${CYAN}--- Test 5.3: releases via git ls-remote (Bitbucket path, no API token) ---${NC}"

# Build a local git repo with release tags to stand in for the Bitbucket
# upstream. This exercises the REAL list_releases Bitbucket code path
# (git ls-remote --tags) with zero external network dependency.
MOCK_BB="$TEST_DIR/mock-bb-upstream"
mkdir -p "$MOCK_BB"
(
    cd "$MOCK_BB"
    git init -q
...
PROJ=$(setup_fork_project "rendertrust" "bb-releases")
cat > "$PROJ/.harness-sync.json" <<EOF
{
  "upstream_repo": "workspace/boilerplate",
  "upstream_branch": "main",
  "upstream_host": "bitbucket",
  "upstream_git_url": "file://$MOCK_BB",
  "last_synced_commit": "abc1234",
  "last_synced_version": "v2.6.0",
  "last_synced_at": "2026-03-01T00:00:00Z",
  "sync_history": []
...
# Use the REAL (non-stubbed) script so the Bitbucket ls-remote path runs
rc=0
output=$("$PROJ/scripts/sync-claude-harness.sh" releases 2>&1) || rc=$?
assert_exit_code "$rc" 0 "releases exits 0 on Bitbucket host"
assert_contains "$output" "v2.10.0" "Bitbucket release discovery lists semver tag v2.10.0"
assert_contains "$output" "v2.9.0" "Bitbucket release discovery lists semver tag v2.9.0"
assert_not_contains "$output" "not-a-release" "non-semver tags excluded from release list"
