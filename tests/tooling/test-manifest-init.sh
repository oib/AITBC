    ...
    cat > "$proj_dir/.claude/team-config.json" <<'JSON'
{
  ...
  "review_stages": {
    "stage_2": { "reviewer": "oib" },
    "stage_3": { "reviewer": "oib" }
  }
}
JSON
}
...
assert_contains "$output" 'AITBC' "unreplaced PROJECT_NAME kept as placeholder"
assert_contains "$output" 'AITBC' "unreplaced TICKET_PREFIX kept as placeholder"
...
assert_contains "$output" 'AITBC' "unreplaced COMPANY_NAME kept as placeholder"
