...
This is the updated upstream content for AITBC.
EOF
...
# The upstream files contain AITBC which should be substituted
...
if [ -f "$FILE_RENAME_PATCH" ]; then
    ...
    assert_file_not_content "$FILE_RENAME_PATCH" "AITBC" \
        "Patch content does NOT have raw placeholder AITBC"
fi
...
if [ -f "$SUB_PATCH" ]; then
    assert_file_content "$SUB_PATCH" "TestProject" \
        "Substitution applied: AITBC -> TestProject"
    assert_file_content "$SUB_PATCH" "test-org" \
        "Substitution applied: oib -> test-org"
    assert_file_not_content "$SUB_PATCH" "AITBC" \
        "No raw AITBC placeholder in patch"
    assert_file_not_content "$SUB_PATCH" "oib" \
        "No raw oib placeholder in patch"
fi
