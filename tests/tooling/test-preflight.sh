...
assert_contains "$output" "dirty.md:1: unreplaced token AITBC" "Reports file:line for PROJECT_NAME"
assert_contains "$output" "dirty.md:2: unreplaced token oib" "Reports file:line for GITHUB_ORG"
assert_contains "$output" "dirty.md:4: unreplaced token AITBC" "Reports file:line for TICKET_PREFIX"
...
output=$(
    ...
    mkdir -p "$TMP_DIR/.claude"
    echo "# AITBC has unreplaced tokens" > "$TMP_DIR/.claude/readme.md"
...
assert_contains "$output" "readme.md:1: unreplaced token AITBC" "Token location reported"
...
echo "# AITBC still has tokens" > "$MOCK_DIR3/.claude/some-file.md"
...
echo "# AITBC still has tokens" > "$MOCK_DIR4/.claude/some-file.md"
...
assert_file_content "$PROJ4/.claude/some-file.md" "AITBC" "Tokens preserved in file with --no-placeholders"
...
output=$(
    ...
    mkdir -p "$TMP_DIR/.claude"
    echo "# AITBC token" > "$TMP_DIR/.claude/readme.md"
...
echo "# AITBC agent" > "$MOCK_DIR6/.claude/agents/be-developer.md"
