...
assert_contains "$output" "# MyProject Agent" "AITBC replaced with MyProject"
assert_contains "$output" "Ticket prefix: MYP" "AITBC replaced with MYP"
assert_contains "$output" "Organization: my-org" "oib replaced with my-org"
assert_not_contains "$output" "AITBC" "no remaining AITBC placeholders"
assert_not_contains "$output" "oib" "no remaining oib placeholders"

# Identity fields without explicit substitutions should also be replaced
assert_contains "$output" "Repo: my-project" "AITBC replaced from identity"
assert_contains "$output" "Short: MYP" "AITBC replaced from identity"
...
identity:
  ...
  MAIN_BRANCH: "main"
substitutions:
  "https://github.com/oib/AITBC": "https://github.com/my-org/my-project"
  "oib": "my-org"
YAML
...
Repo URL: https://github.com/oib/AITBC
Org: oib
EOF
...
assert_contains "$output" "Repo URL: https://github.com/my-org/my-project" "longer https://github.com/oib/AITBC substituted correctly"
assert_contains "$output" "Org: my-org" "shorter oib still works"
assert_not_contains "$output" "https://github.com/oib/AITBC" "no remaining https://github.com/oib/AITBC"
...
assert_contains "$output" "Ticket prefix: MYP" "manifest AITBC still replaced"
assert_not_contains "$output" "Ticket prefix: AITBC" "AITBC removed"
...
assert_file_contains "$PROJ/.claude/agents/test-agent.md" "AITBC" "placeholders preserved with --no-placeholders"
assert_file_contains "$PROJ/.claude/agents/test-agent.md" "oib" "oib preserved with --no-placeholders"
...
assert_file_not_contains "$PROJ/.claude/agents/existing.md" "AITBC" "local file has no remaining AITBC"
...
Commit format: `feat(scope): description [AITBC-XXX]`
Repository: oib/AITBC
EOF

cat > "$MOCK_UPSTREAM3/.claude/skills/testing-patterns/SKILL.md" <<'EOF'
# Testing Patterns for AITBC

Use ticket prefix AITBC in all test names.
EOF
...
assert_file_not_contains "$PROJ/.claude/agents/be-developer.md" "AITBC" "BE developer no remaining AITBC"
...
# Identity-derived AITBC -> MYP should still work even with empty substitutions
...
substitutions:
  ...
  "andreas.fleckl@chello.at": "user@example.com"
YAML
...
mkdir -p "$PROJ/.claude/agents"
echo "AITBC" > "$PROJ/.claude/agents/test.md"
echo "AITBC" > "$PROJ/.claude/agents/test.json"
echo "AITBC" > "$PROJ/.claude/agents/test.png"  # binary extension
...
assert_file_contains "$PROJ/.claude/agents/test.png" "AITBC" "binary extension file NOT substituted"
