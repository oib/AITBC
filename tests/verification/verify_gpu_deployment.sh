...
# `|| true` because `systemctl status` exits non-zero for a stopped or failed unit, and a
# stopped unit is the single most useful thing this script can report. Under `set -e` it
# would instead abort here, before the port and registration checks below -- turning the
# diagnostic into a script that only runs when there is nothing to diagnose.
ssh aitbc 'sudo systemctl status aitbc-gpu-registry.service --no-pager --lines=3' || true
ssh aitbc 'sudo systemctl status aitbc-gpu-miner.service --no-pager --lines=3' || true
