...
STORY_CHAIN = ["Design", "Implement", "Code Review", "Security Review",
               "Test Prep", "In Test", "Design Test", "Story Acceptance",
               "Merging", "Docs", "Done"]
CONDITIONAL = {"Design": "design", "Security Review": "security",
               "Test Prep": "data", "Design Test": "design"}
STORY_SPAWN = {"Design": "ui-ux-design", "Code Review": "system-architect",
               "Security Review": "security-engineer",
               "Test Prep": "data-provisioning-eng", "In Test": "qas",
               "Design Test": "qas-design", "Story Acceptance": "po-agent",
               "Merging": "rte", "Docs": "tech-writer"}
STORY_OWNED = set(STORY_SPAWN) | {"Implement"}
EPIC_SPAWN = {"PO Triage": "po-agent", "Grooming": "bsa",
              "Enrichment": "issue-enrichment",
              "Ticket Review": "qas",           # DoR gate (spec §3.10)
              "Architecture Review": "system-architect",
              "Epic Integration": "rte", "Epic Done": "self-improvement"}
REWORK_LIMIT = 3
FOLLOWUP_BUDGET = 5
CRASH_LIMIT = 3
...
class Engine:
    def __init__(self, join=True, skip=True, rework_guard=True,
                 quiescence=True, empty_guard=True, crash_guard=True,
                 dor_gate=True):
    ...
    def sweep(self):  # follow-up watcher + crash recovery + JOIN re-check
        for (src, eid, acb) in self.pending_followups:
    ...
    def _join_check(self, epic):
        if not self.join or epic.status != "Stories In Flight":
            return
        if self.quiescence and any(eid == epic.id for (_, eid, _) in
                                   self.pending_followups):
    ...
    def _story(self, t, to):
        ...
        if isinstance(act, tuple) and act[0] in ("followups", "followups-acb"):
            for i in range(act[1]):
                self.pending_followups.append((t.id, t.parent,
                                               act[0] == "followups-acb"))
        if to == "Merging":
    ...
    def _epic(self, e, to):
        if to in ("Stories In Flight", "Ready for Epic Acceptance",
                  "Needs PO Decision"):
            return
        if to == "Blocked":
            self._blocked(e)
        ...
        if to == "PO Triage":
            ...
            for flags in e.pending_stories:
                e.children.append(
                    self._mk_story(e, f"{e.id}-S{len(e.children)+1}", flags))
            e.pending_stories = []
            self.transition(e, "Ticket Review" if self.dor_gate
...
def S6(**kw):
    ...
    g.sweep(); g.sweep()                                          # no TDM re-spawn
...
def S13(**kw):
    ...
    for must in ("ui-ux-design", "security-engineer", "data-provisioning-eng",
                 "qas", "qas-design"):
